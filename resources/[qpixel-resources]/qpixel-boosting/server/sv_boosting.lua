...
RegisterServerEvent('qpixel-boosting:removeGNE')
AddEventHandler('qpixel-boosting:removeGNE', function(amount, cid)
    exports.oxmysql:execute('SELECT gne FROM boosting_users WHERE identifier = ?', {cid}, function(result)
...
RegisterServerEvent('qpixel-boosting:pay_gne')
AddEventHandler('qpixel-boosting:pay_gne', function()
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
    local gnes = GetGNE(cid)
...
RegisterServerEvent('qpixel-boosting:giveGNE')
AddEventHandler('qpixel-boosting:giveGNE', function(amount)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
