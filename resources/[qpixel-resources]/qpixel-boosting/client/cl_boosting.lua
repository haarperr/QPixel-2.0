...
function FinishPolice() 
  if isVin then
    TriggerEvent("qpixel-hud:boosting_value", 0)
...
function DoHackingStagesCOP()
  if hacks.number1 == true then
    ...
    if HackingIsCompleted() then
      FinishPolice() 
      TriggerEvent("qpixel-hud:boosting_value", 2)
    end
  elseif hacks.number2 == true then
    ...
    if HackingIsCompleted() then
      FinishPolice() 
      TriggerEvent("qpixel-hud:boosting_value", 3)
    end
  elseif hacks.number3 == true then
    ...
    if HackingIsCompleted() then
      FinishPolice() 
      TriggerEvent("qpixel-hud:boosting_value", 4)
    end
  elseif hacks.number4 == true then
    ...
    if HackingIsCompleted() then
      FinishPolice() 
      TriggerEvent("qpixel-hud:boosting_value", 5)
    end
  elseif hacks.number5 == true then
    ...
    if HackingIsCompleted() then
      FinishPolice() 
      TriggerEvent("qpixel-hud:boosting_value", 6)
    end
  elseif hacks.number6 == true then
    ...
    if HackingIsCompleted() then
      FinishPolice() 
      TriggerEvent("qpixel-hud:boosting_value", 7)
    end
  elseif hacks.number7 == true then
    ...
    if HackingIsCompleted() then
      FinishPolice() 
      TriggerEvent("qpixel-hud:boosting_value", 8)
    end
  elseif hacks.number8 == true then
    ...
    if HackingIsCompleted() then
      FinishPolice() 
      TriggerEvent("qpixel-hud:boosting_value", 9)
    end
  elseif hacks.number9 == true then
    ...
    if HackingIsCompleted() then
      FinishPolice() 
      TriggerEvent("qpixel-hud:boosting_value", 10)
    end
            ...
            Citizen.CreateThread(function()
                while notEmailed do
                  ...
                  if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), setCoords) >= 100.0 then
                    ...
                    TriggerServerEvent('qpixel-boosting:pay_gne')
                    notEmailed = false
                  end
                end
              end)
  ...
  if targetVehicle == 0 then
    if vehicleHandle ~= nil and vehicleHandle ~= 0 and IsEntityAVehicle(vehicleHandle) then
        ...
        if CustomMinigame() == true then
            ...
            TriggerEvent("qpixel-hud:boosting_value", 1)
            print("lockpickdone-start hud")
            if(GetVehicleNumberPlateText(vehicleHandle) == placa)then
            if isVin then
                exports['qpixel-dispatch']:dispatchadd('10-99', 'Tracker Device Tampering - Grand Theft Auto', '227')
...
RegisterNetEvent("qpixel-boosting:open:boostingApp")
AddEventHandler('qpixel-boosting:open:boostingApp', function()
  TriggerServerEvent("boosting:getLevel")
  TriggerEvent("boosting:qbSpawn")
  TriggerEvent("open:boostingApp")
end)

RegisterNUICallback("qpixel-boosting:closeBoostingApp", function(data, cb)
  DoTimer(false)
...
RegisterNUICallback("qpixel-boosting:startContract", function(data, cb) 
  if not AlredyContract then
    if data.IsVin then
      ...
      if units + tonumber(data.props.ExtraVin) <= tonumber(gne) then
        if exports['qpixel-inventory']:hasEnoughOfItem('trackerdisabler', 1) then
...
RegisterNUICallback("qpixel-boosting:transferContract", function(data, cb) 
  sendAppEvent("RemoveContractInfo", {IdToRemove = data.props.id})
  TriggerServerEvent("boosting:TransferContract", data)
  cb({})
end)

RegisterNUICallback("qpixel-boosting:updateTime", function(data, cb) 
...
RegisterNUICallback("qpixel-boosting:DeclineContract", function(data, cb) 
  sendAppEvent("RemoveContractInfo", {IdToRemove = data.props.id})
...
RegisterNUICallback("qpixel-boosting:cancelContract", function(data, cb) 
  TriggerEvent("qpixel-hud:boosting_value", 0)
  sendAppEvent("RemoveContractInfo", {IdToRemove = data.props.id})
  sendAppEvent('SetDisableButtons', {enableButtons = false})
  TriggerServerEvent("boosting:CancelContract", data)
...
RegisterNUICallback("qpixel-boosting:JoinQueue", function(data, cb) 
...
RegisterNUICallback("qpixel-boosting:BennysItem", function(data, cb) 
  if pendingDelivery == false then
    if data.ToPay <= tonumber(gne) then
      TriggerServerEvent('qpixel-boosting:removeGNE', data.ToPay, exports['isPed']:isPed('cid'))
...
AddEventHandler("aspect-boosting:client:UseComputer" , function()
  if exports['qpixel-boosting']:pCanUseLaptop() then
    CanScratchVehicle = false
    RemoveBlip(dropZoneblip)
    FreezeEntityPosition(PlayerPedId(),true)
    TriggerEvent("animation:PlayAnimation", 'type')
    local finished = exports['qpixel-taskbar']:taskBar(1500, 'Opening Laptop')
    if (finished == 100) then
      Citizen.Wait(250)
      TriggerEvent("animation:PlayAnimation", 'type')
      local finished = exports['qpixel-taskbar']:taskBar(7000, 'Connection to network...')
      if (finished == 100) then
        Citizen.Wait(250)
        TriggerEvent("animation:PlayAnimation", 'type')
        local finished = exports['qpixel-taskbar']:taskBar(13000, "Wiping Online Paperwork...")
        if finished == 100 then
...
AddEventHandler('aspect-boosting:finish_scratch', function()
  ...
  TriggerEvent('animation:PlayAnimation', 'kneel')
    ...
    local finished = exports['qpixel-taskbar']:taskBar(10000, 'Scratching Vin')
    if (finished == 100) then
