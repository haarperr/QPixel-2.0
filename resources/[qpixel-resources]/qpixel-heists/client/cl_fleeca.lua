...
RegisterInterfaceCallback('qpixel-fleeca:email_1', function(data, cb)
...
RegisterInterfaceCallback('qpixel-fleeca:email_2', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterInterfaceCallback('qpixel-fleeca:email_3', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterInterfaceCallback('qpixel-fleeca:email_4', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterInterfaceCallback('qpixel-fleeca:email_5', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterInterfaceCallback('qpixel-fleeca:email_6', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterNetEvent('qpixel-robbery:usb')
AddEventHandler('qpixel-robbery:usb', function()
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    local Police = RPC.execute("qpixel-police:getActiveUnits")
            ...
            if closestBank ~= nil then
                ...
                if dist < 1.5 then				
                    if not Config.SmallBanks[closestBank]["isOpened"] then 
                        if Police >= 4 then
                            TriggerServerEvent('qpixel-heists:fleecaBankLog')
                            TriggerServerEvent("qpixel-heists:availabilty")
                            exports["qpixel-dispatch"]:dispatchadd('10-90', "Fleeca Bank Robbery", "108")
...
RegisterNetEvent('qpixel-robbery:client:setBankState')
AddEventHandler('qpixel-robbery:client:setBankState', function(bankId, state)
...
RegisterNetEvent('qpixel-robbery:client:enableAllBankSecurity')
AddEventHandler('qpixel-robbery:client:enableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = true
    end
end)

RegisterNetEvent('qpixel-robbery:client:disableAllBankSecurity')
AddEventHandler('qpixel-robbery:client:disableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = false
    end
end)

RegisterNetEvent('qpixel-robbery:client:BankSecurity')
AddEventHandler('qpixel-robbery:client:BankSecurity', function(key, status)
...
RegisterNetEvent('qpixel-robbery:client:setLockerState')
AddEventHandler('qpixel-robbery:client:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "pacific" then	
        print('[QUEER]')
    else
...
RegisterNetEvent('qpixel-robbery:client:ResetFleecaLockers')
AddEventHandler('qpixel-robbery:client:ResetFleecaLockers', function(BankId)
...
AddEventHandler("fleeca:loot", function()
    local card = exports["qpixel-taskbar"]:taskBar(40000,"Taking cash...")
    if card == 100 then
...
RegisterNetEvent("qpixel-fleeca:grab")
AddEventHandler("qpixel-fleeca:grab", function()
