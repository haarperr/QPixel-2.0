...
RegisterNetEvent('qpixel-heists:start_loop')
AddEventHandler('qpixel-heists:start_loop', function()
    pickup = true
    TriggerEvent("qpixel-heists:pick_cash")
    Wait(180000)
    Wait(additionalWait)
    pickup = false
end)

RegisterNetEvent('qpixel-heists:pick_cash')
AddEventHandler('qpixel-heists:pick_cash', function()
        ...
        if aDist < 2.0 and not LootedTruck then
            ...
            if IsDisabledControlJustReleased(0, 38) then
                if not LootedTruck then
                    ...
                    TriggerEvent('qpixel-hud:show_money')
                    local BankTruckBarLoot = exports['qpixel-taskbar']:taskBar(60000, 'Looting the truck')
                    if BankTruckBarLoot == 100 then
                        FreezeEntityPosition(PlayerPedId(), false)
                        TriggerEvent('qpixel-heists:banktruck_loot') 
                        TriggerEvent('qpixel-hud:hide_money') 
                        LootedTruck = true
                        Citizen.Wait(900000) 
                        LootedTruck = false     
                    else
...
RegisterNetEvent('qpixel-heists:banktruck_loot')
AddEventHandler('qpixel-heists:banktruck_loot', function()
...
RegisterNetEvent('qpixel-heists:spawn_peds')
AddEventHandler('qpixel-heists:spawn_peds', function(veh)
...
RegisterNetEvent('qpixel-heists:banktruck:cl')
AddEventHandler('qpixel-heists:banktruck:cl', function()
