...
if store_safe_1 == false then
    ...
    local ply = PlayerId()
    if police >= 2 then
        if exports["qpixel-inventory"]:hasEnoughOfItem("safecrackingkit", 1) then
            if store_crack_1 == true then
                TriggerEvent("qpixel-dispatch:storerobbery2")
                exports['qpixel-thermite']:OpenThermiteGame(function(success)
                    if success then
                        exports["qpixel-dispatch"]:dispatchadd('10-31A', "Store Robbery", "59")
...
RegisterNetEvent("qpixel-heists:safe:rob", function()
    local policecount = RPC.execute("qpixel-police:getActiveUnits")
    ...
    if isRobbed then
     ...
     if policecount >= 2 then
        if GetEntityHealth(obj) < 800 then
            local finished = exports["qpixel-ui"]:taskBarSkill(25000, math.random(10, 20))
            if (finished == 100) then
                TriggerEvent("qpixel-dispatch:storerobbery2")
                local finished = exports["qpixel-ui"]:taskBarSkill(30000, math.random(10, 20))
                if (finished == 100) then
                    TriggerEvent("qpixel-dispatch:storerobbery2")
                    ClearPedTasks(PlayerPedId())
