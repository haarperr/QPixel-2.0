RegisterNetEvent('qpixel-heists:bay_city:first_door')
AddEventHandler('qpixel-heists:bay_city:first_door', function()
    ...
    if pd >= 4 then
        if distance < 1.5 then
            if exports["qpixel-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                firstDoorBC()
                TriggerServerEvent('qpixel-heists:bayCityLog')
                exports["qpixel-dispatch"]:dispatchadd('10-90', "Bay City Bank", "500")
                ...
                exports['qpixel-thermite']:OpenThermiteGame(function(success)
                    if success then
                        ...
                        TriggerServerEvent('qpixel-doors:change-lock-state', 563, false) 
                        Citizen.Wait(3000)
...
RegisterNetEvent('qpixel-heists:bay_city:second_door')
AddEventHandler('qpixel-heists:bay_city:second_door', function()
    local pd = RPC.execute("qpixel-police:getActiveUnits")
    if pd >= 4 then
        exports["qpixel-dispatch"]:dispatchadd('10-90', "Bay City Bank", "500")
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['qpixel-taskbar']:taskBar(15000, 'Opening Door.')
        if finished == 100 then
            TriggerEvent('aspect:createTrolleys')
            TriggerServerEvent('pStopSecondDoor')
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerServerEvent('qpixel-doors:change-lock-state', 564, false) 
            -- Create Trolleys.
        end
    end
end)

RegisterNetEvent('qpixel-heists:bay_city:second_door:sv')
AddEventHandler('qpixel-heists:bay_city:second_door:sv', function()
    TriggerServerEvent('qpixel-heists:second_door:cl')
end)

exports["qpixel-polytarget"]:AddCircleZone("qpixel_baycity_unlock", vector3(-1309.13, -815.79, 17.7), 0.45, {
    useZ = true
})

exports["qpixel-interact"]:AddPeekEntryByPolyTarget("qpixel_baycity_unlock", {{
    event = "qpixel-heists:bay_city:second_door:sv",
...
AddEventHandler("qpixel-inventory:itemUsed", function(item)
    if item == "thermitecharge" then
        TriggerServerEvent('qpixel-heists:first_door:cl')
    end
end)