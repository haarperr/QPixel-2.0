...
RegisterNetEvent('qpixel-bobcat:firstdoor')
AddEventHandler('qpixel-bobcat:firstdoor', function()
    local player = GetEntityCoords(PlayerPedId())
    local distance = #(vector3(882.208, -2258.1, 30.541439) - player)
    local pd = RPC.execute("qpixel-police:getActiveUnits")
    if pd >= 5 then
        if distance < 1.5 then
            if exports["qpixel-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                frontdoor()
                TriggerServerEvent('qpixel-heists:bobcatLog')
                TriggerEvent("inventory:removeItem", "thermitecharge", 1)
                exports['qpixel-thermite']:OpenThermiteGame(function(success)
                    if success then
                        TriggerServerEvent('qpixel-doors:change-lock-state', 272, false) 
                        TriggerServerEvent("qpixel-doors:change-lock-state", 273, false)
                        exports["qpixel-dispatch"]:dispatchadd('10-90', "Bobcat Security Robbery", "500")
...
RegisterNetEvent('qpixel-bobcat:seconddoor')
AddEventHandler('qpixel-bobcat:seconddoor', function()
local player = GetEntityCoords(PlayerPedId())
        ...
        if exports["qpixel-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                ...
                exports['qpixel-thermite']:OpenThermiteGame(function(success)
                    if success then
                        ...
                        --Door IDS
                        TriggerServerEvent("qpixel-doors:change-lock-state", 274, false)
                        exports["qpixel-dispatch"]:dispatchadd('10-90', "Bobcat Security Robbery", "500")
                        Citizen.Wait(3000)
                    else
                        Citizen.Wait(2000)
...
RegisterNetEvent('qpixel-heists:bobcat_armed_door')
AddEventHandler('qpixel-heists:bobcat_armed_door', function()
    ...
    if pd >= 5 then
        if Dist < 1.5 then
            ...
            TriggerEvent('qpixel-heists:bobcat_explosion_ped')
            FreezeEntityPosition(PlayerPedId(), true)
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerServerEvent("qpixel-doors:change-lock-state", 276, false)
            TriggerServerEvent("qpixel-doors:change-lock-state", 277, false)
        end
    else
        TriggerEvent('DoLongHudText', 'Not enough cops on duty', 2)
    end
...
RegisterNetEvent("bobcat:success:panel", function()
    TriggerEvent('DoLongHudText', "Success!", 1)
    TriggerServerEvent("qpixel-doors:change-lock-state", 275, false)
    TriggerServerEvent("qpixel-doors:change-lock-state", 276, false)
    exports["qpixel-dispatch"]:dispatchadd('10-90', "Bobcat Security Robbery", "500")
end)
...
RegisterNetEvent('qpixel-bobcat:bubbles')
...
RegisterNetEvent('blow_door', function()
    TriggerServerEvent('qpixel-heists:bobcatLog')
    TriggerServerEvent('qpixel-bobcat:blow_door')
end)

RegisterNetEvent('qpixel-heists:bobcat_explosion_ped')
AddEventHandler('qpixel-heists:bobcat_explosion_ped', function()
...
RegisterNetEvent('qpixel-heists:explosion_ped_walk') -- PED WALK
AddEventHandler('qpixel-heists:explosion_ped_walk', function()
    exports["qpixel-dispatch"]:dispatchadd('10-90', "Bobcat Security Robbery", "500")
...
AddEventHandler('lumo:client:requestblast', function()
	TriggerServerEvent("qpixel-bobcat:request_door")
end)
...
AddEventHandler('lumo:pedbomb', function()
    ...
    TriggerServerEvent("qpixel-bobcat:bubbles")
    -- TriggerEvent("lumo:propcreate")
    Wait(1500)
end)


RegisterNetEvent('qpixel-bobcat:search_crate_1unchecked') -- PED WALK
AddEventHandler('qpixel-bobcat:search_crate_1unchecked', function()
    TriggerServerEvent("qpixel-bobcat:search_crate_1")
end)

RegisterNetEvent('qpixel-bobcat:search_crate_2unchecked') -- PED WALK
AddEventHandler('qpixel-bobcat:search_crate_2unchecked', function()
    TriggerServerEvent("qpixel-bobcat:search_crate_2")
end)

RegisterNetEvent('qpixel-bobcat:search_crate_3unchecked') -- PED WALK
AddEventHandler('qpixel-bobcat:search_crate_3unchecked', function()
    TriggerServerEvent("qpixel-bobcat:search_crate_3")
end)

RegisterNetEvent('qpixel-bobcat:search_crate_1') 
AddEventHandler('qpixel-bobcat:search_crate_1', function()
    FreezeEntityPosition(GetPlayerPed(-1),true)
    TriggerEvent("doAnim","search")
    exports["qpixel-dispatch"]:dispatchadd('10-90', "Bobcat Security Robbery", "500")
    local finished = exports["qpixel-taskbar"]:taskBar(30000,"Looting")
    if finished == 100 then
        TriggerEvent('qpixel-heists:bobcat_loot')
...
RegisterNetEvent('qpixel-bobcat:search_crate_2') 
...
RegisterNetEvent('qpixel-bobcat:search_crate_3')
...
RegisterNetEvent('qpixel-heists:bobcat_loot')
AddEventHandler('qpixel-heists:bobcat_loot', function()
...
AddEventHandler("qpixel-inventory:itemUsed", function(item)
    if item == "thermitecharge" then
        TriggerEvent('qpixel-bobcat:firstdoor')
        TriggerEvent('qpixel-bobcat:seconddoor') 
    end
end)

AddEventHandler("qpixel-inventory:itemUsed", function(item)
    if item == "bobcatsecuritycard" then
        TriggerEvent('qpixel-heists:bobcat_armed_door')
    end
end)
...
exports["qpixel-polytarget"]:AddCircleZone("qpixel_bobcat_loot_1",  vector3(886.61, -2287.32, 29.92), 1.0, {
    useZ = true,
})

exports["qpixel-polytarget"]:AddCircleZone("qpixel_bobcat_loot_3",  vector3(881.4, -2282.62, 30.42), 1.0, {
    useZ = true,
})

exports["qpixel-polytarget"]:AddCircleZone("qpixel_bobcat_loot_2",  vector3(882.49, -2286.17, 30.37), 1.0, {
    useZ = true,
})

exports["qpixel-interact"]:AddPeekEntryByPolyTarget('qpixel_bobcat_loot_1', {{
    id = "qpixel_bobcat_loot_1",
    event = "qpixel-bobcat:search_crate_1unchecked",
    icon = "circle",
    label = "Loot!",
    paramers = {},
}}, { distance = { radius = 3.5 }  })

exports["qpixel-interact"]:AddPeekEntryByPolyTarget('qpixel_bobcat_loot_2', {{
    id = "qpixel_bobcat_loot_2",
    event = "qpixel-bobcat:search_crate_2unchecked",
    icon = "circle",
    label = "Loot!",
    paramers = {},
}}, { distance = { radius = 3.5 }  })

exports["qpixel-interact"]:AddPeekEntryByPolyTarget('qpixel_bobcat_loot_3', {{
    id = "qpixel_bobcat_loot_3",
    event = "qpixel-bobcat:search_crate_3unchecked",
    icon = "circle",
    label = "Loot!",
    paramers = {},
}}, { distance = { radius = 3.5 }  })
