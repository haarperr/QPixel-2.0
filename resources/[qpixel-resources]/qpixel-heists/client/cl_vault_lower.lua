...
RegisterNetEvent('qpixel-vault:startlowervault')
AddEventHandler('qpixel-vault:startlowervault', function()
    ...
    if pd >= 4 then
        if distance < 1.5 then
            if exports["qpixel-inventory"]:hasEnoughOfItem("heistlaptop4", 1) then
                ...
                -- if exports['qpixel-blackout']:Blackout() then
                    StartLowerVaultKid()
                    TriggerEvent('qpixel-dispatch:bankrobbery')
                    exports['qpixel-hacking']:hacking(
                        function()
                            ClearPedTasksImmediately(PlayerPedId())
                            TriggerEvent('qpixel-heists:lower_vault_done')
                            DeleteObject(bag)
                            DeleteObject(laptop)
...
RegisterNetEvent('qpixel-heists:lower_vault_done')
AddEventHandler('qpixel-heists:lower_vault_done', function()
    TriggerServerEvent('qpixel-doors:change-lock-state', 278, false) 
    TriggerServerEvent("qpixel-doors:change-lock-state", 279, false)
end)
...
function CreatenzylozCrates()
  ...
  AddEventHandler('qpixel-vault:startlowervault', function()
    ...
    if distance < 1.5 then
        exports['qpixel-vault']:OpenVaultGame(function(success)
    if success then
        TriggerServerEvent("qpixel-doors:change-lock-state", 325, false)
        print("280")  
    else
...
RegisterNetEvent('qpixel-vault:closevaultlower')
AddEventHandler('qpixel-vault:closevaultlower', function()
...
RegisterNetEvent('qpixel-vault:blockedvaultlower')
AddEventHandler('qpixel-vault:blockedvaultlower', function()
    RequestIpl("np_int_placement_ch_interior_6_dlc_casino_vaultmilo")
...
RegisterNetEvent('qpixel-vault:openvaultlower')
AddEventHandler('qpixel-vault:openvaultlower', function()
    RequestIpl("np_int_placement_ch_interior_6_dlc_casino_vaultmilo")
...
RegisterNetEvent('qpixel-vault:openvaultdoor_ITSLOWER')
AddEventHandler('qpixel-vault:openvaultdoor_ITSLOWER', function()
    TriggerEvent('qpixel-vault:openvaultlower')
end)


RegisterNetEvent('qpixel-vaultrob:lower:vaultdoor')
AddEventHandler('qpixel-vaultrob:lower:vaultdoor', function(s, args)
    local ped = PlayerPedId()
    ...
    if #(PLAYER_COORDS - vaultdoorlowerlocation) < 20.0 and not vaultdoor then
        ...
        TriggerEvent('qpixel-vault:openvaultdoor_ITSLOWER')
        CreatenzylozCrates()
        Wait(1800000)
        TriggerEvent("qpixel-vault:LOWERVAULT_REFRESH")
    end
end)

RegisterNetEvent("qpixel-vault:LOWERVAULT_REFRESH")
AddEventHandler("vault:LOWERVAULT_REFRESH", function()
    ...
    if vaultdoor then
        vaultdoor = false
        TriggerEvent('qpixel-vault:blockedvaultlower')
        Citizen.Wait(900000)
        TriggerEvent('qpixel-vault:closevaultlower')
    end
...
RegisterNetEvent("qpixel-vaultrob:lower:fourpincontrol")
AddEventHandler("qpixel-vaultrob:lower:fourpincontrol", function()
...
RegisterNetEvent('qpixel-vaultrob:lower:firstpindoor')
AddEventHandler('qpixel-vaultrob:lower:firstpindoor', function()
    if not firstpindoor  then
        exports['qpixel-vault']:OpenVaultGame(function(success)
            if success then
                firstpindoor = true
                TriggerEvent("qpixel-vaultrob:lower:fourpincontrol")
            else
                firstpindoor = false
            end
        end, 5, 5)
...
RegisterNetEvent('qpixel-vaultrob:lower:secpindoor')
AddEventHandler('qpixel-vaultrob:lower:secpindoor', function()
...
RegisterNetEvent('qpixel-vaultrob:lower:thirdpindoor')
AddEventHandler('qpixel-vaultrob:lower:thirdpindoor', function()
...
RegisterNetEvent('qpixel-vaultrob:lower:fourthpindoor')
AddEventHandler('qpixel-vaultrob:lower:fourthpindoor', function()
