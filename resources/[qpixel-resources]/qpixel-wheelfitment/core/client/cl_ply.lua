DecorRegister("qpixel-wheelfitment_applied", 2)
DecorRegister("qpixel-wheelfitment_w_width", 1)
DecorRegister("qpixel-wheelfitment_w_fl", 1)
DecorRegister("qpixel-wheelfitment_w_fr", 1)
DecorRegister("qpixel-wheelfitment_w_rl", 1)
DecorRegister("qpixel-wheelfitment_w_rr", 1)
--[[
cl_ply.lua
]] -- #[Local Variables]#--
...
function SyncWheelFitment()
    if isPlyWhitelisted then
        ...
        if didPlyAdjustFitments then
            if not DecorExistOn(plyVeh, "qpixel-wheelfitment_applied") then
                DecorSetBool(plyVeh, "qpixel-wheelfitment_applied", true)
            end

            DecorSetFloat(plyVeh, "qpixel-wheelfitment_w_width", roundNum(GetVehicleWheelWidth(plyVeh), 2))
            DecorSetFloat(plyVeh, "qpixel-wheelfitment_w_fl", roundNum(GetVehicleWheelXOffset(plyVeh, 0), 2))
            DecorSetFloat(plyVeh, "qpixel-wheelfitment_w_fr", roundNum(GetVehicleWheelXOffset(plyVeh, 1), 2))
            DecorSetFloat(plyVeh, "qpixel-wheelfitment_w_rl", roundNum(GetVehicleWheelXOffset(plyVeh, 2), 2))
            DecorSetFloat(plyVeh, "qpixel-wheelfitment_w_rr", roundNum(GetVehicleWheelXOffset(plyVeh, 3), 2))
            ...
            TriggerServerEvent("qpixel-wheelfitment_sv:saveWheelfitment", plate, fitment)
        ...
        RPC.execute("qpixel-wheelfitment_sv:setIsWheelFitmentInUse", false)
    end
end
...
function checkVehicleFitment()
    ...
    for _, veh in ipairs(vehicles) do
        ...
        if isNear(plyPos, GetEntityCoords(veh), 150) then
            if DecorExistOn(veh, "qpixel-wheelfitment_applied") then
                vehiclesToCheckFitment[#vehiclesToCheckFitment + 1] = {
                    vehicle = veh,
                    w_width = DecorGetFloat(veh, "qpixel-wheelfitment_w_width"),
                    w_fl = DecorGetFloat(veh, "qpixel-wheelfitment_w_fl"),
                    w_fr = DecorGetFloat(veh, "qpixel-wheelfitment_w_fr"),
                    w_rl = DecorGetFloat(veh, "qpixel-wheelfitment_w_rl"),
                    w_rr = DecorGetFloat(veh, "qpixel-wheelfitment_w_rr")
                }
            end
        end
    end
...
Citizen.CreateThread(function()
    isWheelFitmentInUse = RPC.execute("qpixel-wheelfitment_sv:getIsWheelFitmentInUse")
    isPlyWhitelisted = RPC.execute("qpixel-wheelfitment_sv:checkIfWhitelisted")
...
AddEventHandler("qpixel-polyzone:enter", function(zone, data)
    if zone ~= "qpixel-wheelfitment:zone1" then return end
    Citizen.CreateThread(function()
        local plyPed = PlayerPedId()
        inZone = true
        while inZone do
                ...
                if not isMenuShowing then
                    ...
                    if isNear(plyPos, cfg_wheelFitmentPos, 2.0) then
                        ...
                        if IsControlJustReleased(1, 38) then -- Key: E
                            ...
                            RPC.execute("qpixel-wheelfitment_sv:setIsWheelFitmentInUse", true)
...
RegisterNetEvent('qpixel-fitment:devaleks',function()
    devwheelMenu ()
end)


AddEventHandler("qpixel-polyzone:exit", function(zone)
    if zone ~= "qpixel-wheelfitment:zone1" then
        return
    end
    inZone = false
end)
...
-- #[Event Handlers]#--
RegisterNetEvent("qpixel-wheelfitment_cl:applySavedWheelFitment")
AddEventHandler("qpixel-wheelfitment_cl:applySavedWheelFitment", function(plyVeh, VehicleWheel)
    ...
    DecorSetFloat(plyVeh, "qpixel-wheelfitment_w_width", wheelFitments.width)
    DecorSetFloat(plyVeh, "qpixel-wheelfitment_w_fl", wheelFitments.fl)
    DecorSetFloat(plyVeh, "qpixel-wheelfitment_w_fr", wheelFitments.fr)
    DecorSetFloat(plyVeh, "qpixel-wheelfitment_w_rl", wheelFitments.rl)
    DecorSetFloat(plyVeh, "qpixel-wheelfitment_w_rr", wheelFitments.rr)

    performVehicleCheck = true
    checkVehicleFitment()
end)

RegisterNetEvent("qpixel-wheelfitment_cl:forceMenuClose")
AddEventHandler("qpixel-wheelfitment_cl:forceMenuClose", function()
    if isMenuShowing then
        local plyPed = PlayerPedId()
        local plyVeh = GetVehiclePedIsIn(plyPed, false)
...
RegisterCommand("leavefitment", function()
    TriggerEvent("qpixel-wheelfitment_cl:forceMenuClose")
end)