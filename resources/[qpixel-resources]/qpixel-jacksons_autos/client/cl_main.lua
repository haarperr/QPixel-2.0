...
function updateCarTable(model,price,name)
	for i=1, #carTable do
		...
		TriggerServerEvent("qpixel-jacksons_autos:CarTable",carTable)
	end
end

local myspawnedvehs = {}

RegisterNetEvent("qpixel-jacksons_autos:testdrive")
AddEventHandler("qpixel-jacksons_autos:testdrive", function()
	if exports["qpixel-business"]:IsEmployedAt('jacksons_autos') or #(vector3(-775.9081, -1032.289, 13.719951) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("qpixel-jacksons_autos:EnableBuy")
AddEventHandler("qpixel-jacksons_autos:EnableBuy", function()
	if #(vector3(-775.4795, -1032.456, 14.129936) - GetEntityCoords(PlayerPedId())) > 50.0 and exports["qpixel-business"]:IsEmployedAt('jacksons_autos') then
		return
	end
	...
	TriggerServerEvent("qpixel-jacksons_autos:BuyEnabledSV",vehplate)
end)

RegisterNetEvent("qpixel-jacksons_autos:BuyEnabledCL")
AddEventHandler("qpixel-jacksons_autos:BuyEnabledCL", function(addplate)
	buyPlate[addplate] = true
	Citizen.Wait(60000)
	buyPlate[addplate] = nil
end)

RegisterNetEvent("qpixel-jacksons:CommissionPrice")
AddEventHandler("qpixel-jacksons:CommissionPrice", function(newAmount)
	if eexports["qpixel-business"]:IsEmployedAt('jacksons_autos') or #(vector3(-775.4795, -1032.456, 14.129936) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("qpixel-jacksons_autos:ReturnCarTable")
AddEventHandler("qpixel-jacksons_autos:ReturnCarTable", function(newTable)
	carTable = newTable
	DespawnSaleVehicles()
	SpawnSaleVehicles()
end)
...
function AttemptBuy(tableid)
	...
	TriggerServerEvent('qpixel-jacksons_autos:ChechMoney',name, model, price)
...
function DrawPrices()
	for i = 1, #pSpawnLocations do
		if #(vector3(pSpawnLocations[i]["x"],pSpawnLocations[i]["y"],pSpawnLocations[i]["z"]) - GetEntityCoords(PlayerPedId())) < 1.5 then
			...
			if exports["qpixel-business"]:IsEmployedAt('jacksons_autos') then
...
function SpawnSaleVehicles()
	if not hasspawned then
		TriggerServerEvent("qpixel-jacksons_autos:RequestCarTable")
		Citizen.Wait(1500)
	end
...
function CloseCreator(name, veh, price, plate)
	Citizen.CreateThread(function()
		...
		if not boughtcar then
			...
			TriggerServerEvent('qpixel-jacksons_autos:BuyVehicle', plate, vehname, vehicle, price, VehicleProps)
			TriggerServerEvent('qpixel-cardealer:log', exports['isPed']:isPed('cid'), vehname, price, plate, 'Jacksons Autos')
			TriggerServerEvent('qpixel-financials:business_money', price / 10, 'jacksons_autos', 'add')
			exports['qpixel-interface']:hideInteraction()
			DespawnSaleVehicles()
			SpawnSaleVehicles()
		end
...
RegisterNetEvent("qpixel-jacksons_autos:FailedPurchase")
AddEventHandler("qpixel-jacksons_autos:FailedPurchase", function()
...
function ButtonSelected(button)
	...
	elseif this == "b-class" or this == "s-class" then
		if ownerMenu then
			...
			TriggerServerEvent('qpixel-jacksons_autos:ChechMoney',button.name, button.model, button.costs)
		end
	end
end
	...
	while true do
		...
		if vehshop.opened then
			...
			for i,button in pairs(menu.buttons) do
				...
				if exports["qpixel-business"]:IsEmployedAt('jacksons_autos') and i >= vehshop.menu.from and i <= vehshop.menu.to then
...
RegisterCommand('commission', function(source, args, raw)
	if exports["qpixel-business"]:IsEmployedAt('jacksons_autos') then
		...
		if tonumber(amount) > 0 and tonumber(amount) <= 60 then
			TriggerEvent('qpixel-jacksons_autos:CommissionPrice', amount)
		else
...
RegisterCommand('testdrive', function(source, args, raw)
	if exports["qpixel-business"]:IsEmployedAt('jacksons_autos') then
		TriggerEvent('qpixel-jacksons_autos:testdrive')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)
...
RegisterCommand('enableBuy', function(source, args, raw)
	if exports["qpixel-business"]:IsEmployedAt('jacksons_autos') then
		TriggerEvent('qpixel-jacksons_autos:EnableBuy')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)