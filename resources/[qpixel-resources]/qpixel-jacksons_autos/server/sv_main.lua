...
RegisterServerEvent('qpixel-jacksons_autos:CarTable')
AddEventHandler('qpixel-jacksons_autos:CarTable', function(table)
    if table ~= nil then
        carTable = table
        TriggerClientEvent('qpixel-jacksons_autos:ReturnCarTable', -1, carTable)
        for i=1, #carTable do
...
RegisterServerEvent('qpixel-jacksons_autos:FinaceEnabledSV')
AddEventHandler('qpixel-jacksons_autos:FinaceEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('qpixel-jacksons_autos:FinaceEnabledCL', -1, plate)
    end
end)

RegisterServerEvent('qpixel-jacksons_autos:BuyEnabledSV')
AddEventHandler('qpixel-jacksons_autos:BuyEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('qpixel-jacksons_autos:BuyEnabledCL', -1, plate)
    end
end)

-- return table
-- TODO (return db table)
RegisterServerEvent('qpixel-jacksons_autos:RequestCarTable')
AddEventHandler('qpixel-jacksons_autos:RequestCarTable', function()
    local user = source
    exports.oxmysql:execute("SELECT * FROM vehicle_display", {}, function(display)
        ...
        TriggerClientEvent('qpixel-jacksons_autos:ReturnCarTable', user, carTable)
    end)
end)

-- Check if player has enough money
RegisterServerEvent('qpixel-jacksons_autos:ChechMoney')
AddEventHandler('qpixel-jacksons_autos:ChechMoney', function(name, model, price)
    local src = source
    ...
    elseif tonumber(cash) <= price then
        ...
        TriggerClientEvent('qpixel-jacksons_autos:FailedPurchase', src)
    end
end)

RegisterServerEvent('qpixel-jacksons_autos:BuyVehicle')
AddEventHandler('qpixel-jacksons_autos:BuyVehicle', function(plate, name, vehicle, price, personalvehicle)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
