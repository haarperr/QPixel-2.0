...
local function benchObjectHandler(pBenchType, pItem)
    ...
    local id = exports['qpixel-objects']:PlaceAndSaveObject(
...
local itemHandlers = {
    ...
    ['pailshovel'] = function(info)
        ...
        local finished = exports["qpixel-taskbar"]:taskBar(15000, "Building Sandcastle", false, true, false, false, nil, 5.0, ped)
        ClearPedTasks(ped)
    ...
    ['letterbox'] = function(info)
        ...
        local properties = exports['qpixel-housing']:getHousingData()
        ...
        local name
        if elements then
            local prompt2 = exports['qpixel-ui']:OpenInputMenu(elements)
            if not prompt2 then
                return false
            end
            ...
            if mailType == 'property' then
                local housingTable = exports['qpixel-housing']:retriveHousingTable()
            ...
            elseif mailType == 'business' then
                local zones = exports['qpixel-polyzone']:GetZones(plyCoords)
                if not zones then
        ...
        local results = exports['qpixel-objects']:PlaceObject(
            model,
            {
                forceGroundSnap = true,
                zOffset = -0.05,
        ...
        local objId = RPC.execute('qpixel-usableprops:placeLetterbox', model, results[2].coords, results[2].rotation, mailType, foundInfo, name)
        if objId then
...
AddEventHandler('qpixel-inventory:itemUsed', function(item, info)
...
local function getMailboxInfo(pEntity)
    local object = exports['qpixel-objects']:GetObjectByEntity(pEntity)
    if not object then
        return false
    end
...
-- Citizen.CreateThread(function()
--     exports["qpixel-interact"]:AddPeekEntryByModel({ `prop_sign_road_01a` }, {{
--         event = "qpixel-usableprops:stealStopSign",
--         id = "qpixel-usableprops:stealStopSign",
--         icon = "hammer",
--         label = "Steal!",
--         parameters = {},
--     }}, {
...
--     exports["qpixel-interact"]:AddPeekEntryByModel({ 2108567945, -1940238623 }, {{
--         event = "qpixel-usableprops:payParkingMeter",
--         id = "qpixel-usableprops:parking_pay",
--         icon = "dollar-sign",
...
--     exports["qpixel-interact"]:AddPeekEntryByModel({ `prop_skid_tent_01`, `prop_skid_tent_03` }, {{
--         event = "qpixel-usableprops:pickupTent",
--         id = "qpixel-usableprops:pickup_tent",
--         icon = "arrow-up",
--         label = "pickup tent",
--         parameters = {},
--     }}, {
--         distance = { radius = 2.0 },
--         isEnabled = function(pEntity)
--           return exports['qpixel-objects']:GetObjectByEntity(pEntity) ~= nil
--         end,
--     })

--     exports["qpixel-interact"]:AddPeekEntryByModel({ `prop_bench_04`, `prop_ld_bench01`, `prop_snow_bench_01` }, {{
--         event = "qpixel-usableprops:pickupBench",
--         id = "qpixel-usableprops:pickup_bench",
--         icon = "arrow-up",
--         label = "pickup bench",
--         parameters = {},
--     }}, {
...
--     exports["qpixel-interact"]:AddPeekEntryByModel({ `prop_letterbox_01`, `prop_letterbox_02`, `prop_letterbox_03`, `prop_letterbox_04`, `prop_postbox_01a` }, {{
--         event = "qpixel-usableprops:openMail",
--         id = "qpixel-usableprops:open_mailbox",
--         icon = "envelope",
--         label = "deposit mail",
--         parameters = {},
--     }}, {
...
--     exports["qpixel-interact"]:AddPeekEntryByModel({ `prop_letterbox_01`, `prop_letterbox_02`, `prop_letterbox_03`, `prop_postbox_01a` }, {{
--         event = "qpixel-usableprops:checkMail",
--         id = "qpixel-usableprops:check_mailbox",
--         icon = "envelope-open",
--         label = "check mail",
--         parameters = {},
--     }}, {
...
--             return (mailType == 'business' and (exports['qpixel-business']:HasPermission(info, 'stash_access') or isPD())) or
--             (mailType == 'property' and exports['qpixel-housing']:hasPropertyAccess(info, true)) or
--             (mailType == 'mrpd' and isPD()) or
--             (mailType == 'pillbox' and (job == 'ems' or job == 'doctor'))
--         end,
--     })

--     exports["qpixel-interact"]:AddPeekEntryByModel({ `prop_letterbox_01`, `prop_letterbox_02`, `prop_letterbox_03` }, {{
--         event = "qpixel-usableprops:removeMailbox",
--         id = "qpixel-usableprops:remove_mailbox",
--         icon = "trash-alt",
...
--         isEnabled = function(pEntity)
--             local info, mailType = getMailboxInfo(pEntity)
--             return mailType == 'business' and (exports['qpixel-business']:HasPermission(info, 'stash_access')) or
--             (mailType == 'property' and exports['qpixel-housing']:hasPropertyAccess(info, false))
--         end,
--     })

-- end)

AddEventHandler("qpixel-usableprops:stealStopSign", function(pArgs, pEntity, pContext)
    ...
    local finished = exports["qpixel-taskbar"]:taskBar(15000, "Stealing", false, true, false, false, nil, 5.0, ped)
    ClearPedTasks(ped)
    if finished == 100 then
...
RegisterNetEvent("qpixel-usableprops:deleteEntity", DeleteEntityAtCoords)
...
AddEventHandler("qpixel-usableprops:payParkingMeter", function(args, entity, context)
    PaidMeters[entity] = true
    local success = RPC.execute('qpixel-usableprops:payParkingMeter', args.price)
    if success then
        local pos = GetEntityCoords(entity)
        TriggerServerEvent("qpixel-scenes:addScene", {
...
AddEventHandler("qpixel-usableprops:pickupTent", function(args, entity, context)
    local tent = exports['qpixel-objects']:GetObjectByEntity(entity)
    if tent then
        local success = exports['qpixel-objects']:DeleteObject(tent.id)
        if success then
...
AddEventHandler("qpixel-usableprops:pickupBench", function(args, entity, context)
    local bench = exports['qpixel-objects']:GetObjectByEntity(entity)
    if bench then
        local success = exports['qpixel-objects']:DeleteObject(bench.id)
        if success then
...
local closeData = {}
AddEventHandler("qpixel-usableprops:openMail", function(args, entity, context)
    local info, mailType, object = getMailboxInfo(entity)
    waitForClose = RPC.execute('qpixel-usableprops:openMailbox', info, mailType, object.id)
...
AddEventHandler("qpixel-usableprops:checkMail", function(args, entity, context)
    if isPD() and mailType == 'business' then
        local prompt = exports['qpixel-ui']:OpenInputMenu({
    ...
    local info, mailType, object = getMailboxInfo(entity)
    RPC.execute('qpixel-usableprops:checkMailbox', info, mailType, object.id)
end)

AddEventHandler('qpixel-inventory:closed', function(pName)
    if waitForClose and pName == 'mailtemp:' .. closeData.mailType .. ':' .. closeData.objId then
        RPC.execute('qpixel-usableprops:closeMailbox', closeData.info, closeData.mailType, closeData.objId)
        waitForClose = false
        closeData = {}
    end
end)

AddEventHandler('qpixel-usableprops:removeMailbox', function(args, entity, context)
    TriggerEvent("animation:PlayAnimation","mechanic3")
    local finished = exports['qpixel-taskbar']:taskBar(15000, "Removing mailbox", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local obj = exports['qpixel-objects']:GetObjectByEntity(entity)
        if not obj then
            return
        end
        exports["qpixel-objects"]:DeleteObject(obj.id)
    end
end)

AddEventHandler('qpixel-objects:objectCreated', function(pObject, handle)
    if pObject.data.metadata.mail then
        FreezeEntityPosition(handle, false)
    end
end)

AddEventHandler('qpixel-spawn:characterSpawned', function(pCharacterId)
    RPC.execute('qpixel-usableprops:checkMailAlerts', pCharacterId)
end)
