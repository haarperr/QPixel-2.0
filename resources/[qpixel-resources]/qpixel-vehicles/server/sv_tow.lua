...
RegisterServerEvent("qpixel-vehicles:checkrepo")
AddEventHandler("qpixel-vehicles:checkrepo", function(plate)
    local src = source
...
-- RegisterServerEvent("qpixel-vehicles:repo")
-- AddEventHandler("qpixel-vehicles:repo", function(plate)
--     local src = source
-- 	local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
--                         ['@repoed'] = "1"
--                     })
--                     TriggerClientEvent("qpixel-vehicles:repo:success", src)
...
-- RegisterServerEvent("qpixel-vehicles:judgerepo")
-- AddEventHandler("qpixel-vehicles:judgerepo", function(plate)
--     local src = source
-- 	local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
-- RegisterServerEvent("qpixel-vehicles:judgerepodelete")
-- AddEventHandler("qpixel-vehicles:judgerepodelete", function(plate)
--     local src = source
-- 	local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
--     exports.oxmysql:execute("SELECT `license_plate`, `repoed` FROM `characters_cars` WHERE license_plate = ?", {char.id}, function(data)
...
-- RegisterServerEvent("qpixel-vehicles:repo2")
-- AddEventHandler("qpixel-vehicles:repo2", function(plate)
--     local src = source
-- 	local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
--                     ['@vehicle_state'] = "In"
--                 })
--                 TriggerClientEvent("qpixel-vehicles:repo:success2", src)
--             else
--                 TriggerClientEvent("qpixel-vehicles:repo:success2", src)
--             end
--         end
...
-- RegisterServerEvent("qpixel-vehicles:release:vehicle")
-- AddEventHandler("qpixel-vehicles:release:vehicle", function(ServerID, plate)
...
--         ['repoed'] = "0"
--     })
--     exports['qpixel-exports']:removeCash(SrcID, 500)
...
RegisterCommand('towbill', function(source, args)
	...
	exports.oxmysql:execute('SELECT * FROM jobs_whitelist WHERE cid = ?', {character2.id}, function(result)
		if result[1].job == 'towunion' then
			...
			TriggerClientEvent("qpixel-vehicles:towBill", src, tonumber(args[2]))
		else
		end
	end)
end)