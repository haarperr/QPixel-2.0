...
RegisterNetEvent("qpixel-nitro:client:placeNitro")
AddEventHandler("qpixel-nitro:client:placeNitro", function() 
...
RegisterNetEvent('qpixel-nitro:client:particlefx')
AddEventHandler('qpixel-nitro:client:particlefx', function (veh)	
	local exhaustNames = {
...
RegisterNetEvent('qpixel-nitro:client:particlefisfis')
AddEventHandler('qpixel-nitro:client:particlefisfis', function (type, veh)
		...
		if nitroactive and pNitrousLevel > 0 and IsPedInVehicle(PlayerPedId(), GetVehiclePedIsIn(PlayerPedId(), false), false) then
			...
			if fisfismode then
				if IsControlPressed(0, 21) and nitroveh == GetVehiclePedIsIn(PlayerPedId(), false) then -- fışfış
					TriggerServerEvent('qpixel-nitro:server:particlefisfis', 'fisfisacc', nitroveh)
					SetVehicleBoostActive(nitroveh, 4) --Boost Sound
					pNitrousInUse = true
					pNitrousLevel = pNitrousLevel - 0.1
				else
					pNitrousInUse = false
					TriggerServerEvent('qpixel-nitro:server:particlefisfis', 'fisfiskapatt', nitroveh)
				end
			elseif nitromode then
				if IsControlPressed(0, 21) and nitroveh == GetVehiclePedIsIn(PlayerPedId(), false) then -- nitro
					TriggerServerEvent('qpixel-nitro:server:particlefx', nitroveh)
					SetVehicleBoostActive(nitroveh, 4) --Boost Sound
					pNitrousInUse = true
					pNitrousLevel = pNitrousLevel - 0.1
...
AddEventHandler('onResourceStart', function (resource)
	pNitrousLevel = 0
	nitroactive = false
	TriggerServerEvent('qpixel-nitro:server:particlefisfis', 'fisfiskapatt')
	nitroveh = nil
	pNitrousInUse = false 
end)
...
Citizen.CreateThread(function()
    exports["qpixel-keybinds"]:registerKeyMapping("", "Nitrous", "Change Mode", "+nosMode", "-nosMode", "")

	exports["qpixel-keybinds"]:registerKeyMapping("", "Nitrous", "Flow Rate Higher", "+changeFlowPlus", "-changeFlowPlus", "")

	exports["qpixel-keybinds"]:registerKeyMapping("", "Nitrous", "Flow Rate Lower", "+changeFlowMinus", "-changeFlowMinus", "")
end)