...
function preventVehicleExit()
    Citizen.CreateThread(function()
        ...
        disableControl = true
        local finished = exports["qpixel-taskbar"]:taskBar(options[math.random(1,3)],"Taking off Harness",true)
        if finished == 100 then
            TriggerEvent('qpixel-hud:harness_values', 0)
    ...
    if not pStalled then
        if GetVehicleEngineHealth(currentVehicle) <= 100.0 then
            ...
            if harness then
                ...
                TriggerEvent('qpixel-hud:harness_values', harnessDurability)
            end
            Citizen.Wait(10000)
            pStalled = false
        end
...
function toggleHarness()
    ...
    local finished = 0
    if harness then
        finished = exports["qpixel-taskbar"]:taskBar(5000,"Taking off Harness",true)
        TriggerEvent('qpixel-hud:harness_values', 0)
        TriggerEvent('seatbelt', false)
    else
        finished = exports["qpixel-taskbar"]:taskBar(5000,"Putting on Harness",true)
        TriggerEvent('qpixel-hud:harness_values', harnessDurability)
