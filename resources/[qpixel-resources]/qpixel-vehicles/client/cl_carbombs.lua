...
local function explodeVehicle(pEntity)
  exports['qpixel-sync']:SyncedExecution('NetworkExplodeVehicle', pEntity, 1, 0, 0)
  TriggerServerEvent('qpixel-miscscripts:carbombs:removeBomb', NetworkGetNetworkIdFromEntity(pEntity))
end
...
local function checkForCarBomb(pEntity) 
  ...
  local metaData = exports['qpixel-vehicles']:GetVehicleMetadata(pEntity)
  local carBombMeta = metaData.carBombData or false
  local remoteBomb = DPX.Procedures.execute('qpixel-usableprops:hasPhoneBomb', NetworkGetNetworkIdFromEntity(pEntity))

  if remoteBomb and remoteBomb.planted then
    local number = remoteBomb.bomb.number
    TriggerServerEvent('qpixel-usableprops:defusePhoneBomb', number)
...
AddEventHandler('qpixel-bomb:test', function (name, info)
  ...
  local prompt = exports['qpixel-interface']:OpenInputMenu(elements)
  ...
  local added = DPX.Procedures.execute('qpixel-miscscripts:carbombs:addCarBomb', netId, minSpeed, ticksBeforeExplode, ticksForRemoval, gridSize, coloredSquares, timeToComplete)
...
AddEventHandler('qpixel-miscscripts:carBombs:removeBomb', function (pData, pEntity)
  ...
  if carBombMeta and carBombMeta.hasCarBomb then
    ...
    exports['qpixel-ui']:openApplication('memorygame', {
...
--[[ RegisterUICallback('qpixel-vehicles:carBombs:completeHacking', function(data, cb)
  ...
  Citizen.Wait(2000)
  exports['qpixel-ui']:closeApplication('memorygame')
end) ]]
