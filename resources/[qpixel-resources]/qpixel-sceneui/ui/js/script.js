...
$(function() {
    sleep(5000,function(){
        $.post('https://qpixel-sceneui/qpixel-sceneui:getKVPValue', JSON.stringify({
            key : "qpixel-preferences",
        }),function(result){
            ...
            $.post('https://qpixel-sceneui/qpixel-sceneui:hudSetPreferences', JSON.stringify(settings),function(result){
...
document.querySelector('#setting_hud_save').addEventListener('click', function(){
    ...
    $.post('https://qpixel-sceneui/qpixel-sceneui:setKVPValue', JSON.stringify({
        key : "qpixel-preferences",
        value:settings
    }),function(res){
        ...
        if(data_captcha.solution === answer){
            ...
            if( (mode === 'vault' && streakcaptcha === roundTotal_captcha) ){
                ...
                sleep(5000, function(){ 
                    ...
                    $.post('https://qpixel-sceneui/'+minigame_captcha_endpoint, JSON.stringify({
                        result : true
                    }));
                });
            }else{
...
function correct_skillbar(){
    ...
    $.post('https://qpixel-sceneui/qpixel-sceneui:taskBarSkillResult', JSON.stringify({
        success: true,
    }));
}
...
function reset(checked){
    ...
    if (checked) {
        ...
        sleep(4000, function(){
            ...
            $.post('https://qpixel-sceneui/'+minigame_memory_endpoint, JSON.stringify({
                result : true
            }));
        })
    }else{
...
function checkanswernumbers(){
    ...
    if(game_started_numbers && response === answer_numbers.join('')){
        ...
        sleep(3000, function(){
            ...
            $.post('https://qpixel-sceneui/'+gameFinishedEndpointNumbers, JSON.stringify({
                success : true
            }));
        });
    }else{
...
document.querySelector('#textbox_submit').addEventListener('click', function(){
    ...
    $.post('https://qpixel-sceneui/qpixel-sceneui:closeApp', JSON.stringify({}),function(res){
        $.post('https://qpixel-sceneui/'+textCallback, JSON.stringify({
            key : textKey,
            values : textData,
        }),function(cb){
            textCallback = null;
            ...
            $.post('https://qpixel-sceneui/qpixel-sceneui:applicationClosed', JSON.stringify({
                name : "textbox",
                fromEscape : false
            }));
        });
            ...
            if (event.target.id.split("-")[0] === 'contextmenu') {
                ...
                if (action) {
                    if (children) {
                        ...
                        $.post('https://qpixel-sceneui/'+action, JSON.stringify({
                            key:key
                        }));
                    }else{
                        contextActive = false;
                ...
                $.post('https://qpixel-sceneui/'+subaction, JSON.stringify({
                    key:subkey
                }),function (res) { 
...
document.querySelector('.vol-up-wrapper').addEventListener('click', function(){
    $.post('https://qpixel-sceneui/qpixel-sceneui:radioVolumeUp', JSON.stringify({}));
});

document.querySelector('.vol-down-wrapper').addEventListener('click', function(){
    $.post('https://qpixel-sceneui/qpixel-sceneui:radioVolumeDown', JSON.stringify({}));
});

document.querySelector('.on-off-wrapper').addEventListener('click', function(){
    radiostate = !radiostate
    if (radiostate == true) {
        $.post('https://qpixel-sceneui/qpixel-sceneui:toggleRadioOn', JSON.stringify({}));
    }else if (radiostate == false) {
        $.post('https://qpixel-sceneui/qpixel-sceneui:toggleRadioOff', JSON.stringify({}));
    }
});
...
$(document).on('mousedown', (event) => {
    ...
    if (element.id.split("-")[0] === 'target') {
        ...
        $.post('https://qpixel-sceneui/qpixel-sceneui:targetSelectOption', JSON.stringify({
...
$(document).keydown(function(event) {
    ...
    if (key == 27) {
        if (showroomuiactive) {
            ...
            $.post('https://qpixel-sceneui/qpixel-sceneui:closeApp', JSON.stringify({}),function (cb) { 
                $.post('https://qpixel-sceneui/qpixel-sceneui:applicationClosed', JSON.stringify({
                    name : "showroom",
                    fromEscape : true
                }));
