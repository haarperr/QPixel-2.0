...
--
local cmdsSet = {}
RegisterNUICallback("qpixel-ui:hudSetPreferences", function(data, cb)
    ...
    if data["hud.presets"] then
      ...
      for _, p in pairs(data["hud.presets"]) do
        ...
        if not cmdsSet[idx] then
          ...
          exports["qpixel-keybinds"]:registerKeyMapping("", "HUD", "Enable " .. s, "+hud_enable_" .. s, "-hud_enable_" .. s)
          RegisterCommand('+hud_enable_' .. s, function()
            exports["qpixel-ui"]:sendAppEvent("preferences", eventOptions)
          end, false)
    ...
    TriggerEvent("qpixel-preferences:setPreferences", data)
end)
...
AddEventHandler("qpixel-ui:watch", function()
...
RegisterNetEvent('qpixel-inventory:itemCheck')
AddEventHandler('qpixel-inventory:itemCheck', function (item, hasItem, quantity)
...
AddEventHandler('qpixel-vehicles:pursuitMode', function (pEnabled, pMode, pModes)
...
AddEventHandler('qpixel-jail:attachedCollar', function (pAttached)
...
RegisterNetEvent('qpixel-boosting:client:setBoostCompletions')
AddEventHandler('qpixel-boosting:client:setBoostCompletions', function (pAmount)
...
RegisterNetEvent('qpixel-weapons:client:setWeaponFireRate')
AddEventHandler('qpixel-weapons:client:setWeaponFireRate', function (pAmount)
    weaponFireRate = pAmount
    setHudValue("weaponFireRate", weaponFireRate)
end)

RegisterNetEvent('qpixel-weapons:client:showWeaponFireRate')
AddEventHandler('qpixel-weapons:client:showWeaponFireRate', function (pToggle)
    showWeaponFireRate = pToggle
    setHudValue("showWeaponFireRate", pToggle)
end)

RegisterNetEvent('qpixel-ui:hud:forceShowMinimap', function(pForce)
...
function roundedRadar()
    ...
    Citizen.CreateThread(function()
        ...
        -- for k,v in pairs(exports["qpixel-base"]:getModule("Blips")) do
...
function getFuel()
    fuel = exports["qpixel-vehicles"]:CurrentFuel() or 0
end
    ...
    Citizen.CreateThread(function()
        while engineOn do
            ...
            if GetPedInVehicleSeat(veh, -1) == GetPed() then
                harnessDurability = exports["qpixel-vehicles"]:GetVehicleMetadata(veh, 'harness')
            end

            local degradation = exports["qpixel-vehicles"]:GetVehicleDegradation(veh)
...
AddEventHandler('qpixel-ui:setGaze', function (pEntity, pDelay)
    GazeEntity = pEntity
    GazeDelay = pDelay
end)
...
Citizen.CreateThread(function()
  while true do
    ...
    if toggleParachuteOff and pState == -1 then
      ...
      exports["qpixel-ui"]:sendAppEvent("hud", { hasParachute = false })
...
AddEventHandler("hud:equipParachute", function()
  hasParachute = true
  exports["qpixel-ui"]:sendAppEvent("hud", { hasParachute = true })
end)

AddEventHandler("qpixel-island:onIsland", function(pState)
    onIsland = pState
end)
