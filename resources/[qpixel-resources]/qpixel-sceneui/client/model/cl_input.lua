...
RegisterNUICallback('qpixel-ui:applicationClosed', function(data, cb)
    if (data.name ~= 'textbox' or data.callbackUrl ~= 'qpixel-ui:inputResponse') then return end
...
RegisterNUICallback('qpixel-sceneui:inputResponse', function(data, cb)
    ...
    exports['qpixel-sceneui']:closeApplication('textbox')
end)
...
function OpenInputMenu(pEntries, pValidation)
    ...
    exports['qpixel-sceneui']:openApplication('textbox', {
        callbackUrl = 'qpixel-ui:inputResponse',
        key = inputId,
        items = pEntries,
        show = true,
    })
