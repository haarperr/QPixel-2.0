RegisterNUICallback("data_status", function(data)
    if soundInfo[data.id] ~= nil then
        if data.type == "finished" then
            ...
            TriggerEvent("qpixel-musicplayer:songStopPlaying", data.id)
        end
...
RegisterNetEvent("qpixel-musicplayer:stateSound")
AddEventHandler("qpixel-musicplayer:stateSound", function(state, data)
