...
local function get_info_from_db ()
  local src = source
  local user = exports['qpixel-base']GetModule('Player')(src)
  local cid = user.id
...
RPC.register('qpixel-meth:getConfig',function(pData, pInfo)
  print(pData, pInfo)
  local src = source
  table = {
    ACTIVE_CORNERS = {
...
RegisterServerEvent('qpixel-meth:corneringFoundVehicle',function (pCorner , pSrc , pTable)
  print('Hmmm',json.encode(pTable))
  
end)

RPC.register('qpixel-meth:startCornering', function(pID)
  local src = source
  TriggerClientEvent('qpixel-meth:findCandidateVehicle', 1 , src)
  return true
end)

RPC.register('qpixel-meth:corneringFoundVehicle',function(pData, pInfo , pTable)
  local src = source
  return false , 'No vehicle found bro'
end)

RPC.register('qpixel-meth:attemptCornerSale', function (pCar , pInfo)
...
RPC.register('qpixel-meth:stopCornering', function()
  local src = source 
  TriggerClientEvent('qpixel-meth:showSellDrugsMenuItem',src,"cancorner" ,false)
end)

RPC.register('qpixel-meth:purchaseLabKey', function()
  local src = source
