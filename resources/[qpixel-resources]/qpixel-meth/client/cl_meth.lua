...
AddEventHandler("qpixel-inventory:itemUsed", function(item, info)
    ...
    if not exports["qpixel-inventory"]:hasEnoughOfItem("qualityscales", 1, false) then
        TriggerEvent("DoLongHudText", "Need something to weigh with...")
        return
    end
    if not exports["qpixel-inventory"]:hasEnoughOfItem("emptybaggies", 1, false) then
        TriggerEvent("DoLongHudText", "Need something to pack with...")
        return
    end
    local finished = exports["qpixel-taskbar"]:taskBar(1000, "Packing")
    if finished ~= 100 then return end
    RPC.execute("qpixel-meth:packCuredMeth", json.decode(info))
...
Citizen.CreateThread(function()
    ...
    local rv = exports["qpixel-npcs"]:RegisterNPC(data, 'qpixel-meth')
end)
