...
local function processAction(action, pEntity, fnCb)
    ...
    if animSettings then
        ...
        result:next(function (data)
            if data == 100 then
                RPC.execute("qpixel-meth:doAction", insideLab, action)
                if fnCb then
                  fnCb()
                end
            else
...
--[[ RegisterUICallback("qpixel-ui:submitRangeValues", function(data, cb)
    ...
    ClearPedTasksImmediately(PlayerPedId())
    Wait(100)
    exports["qpixel-ui"]:closeApplication("range-picker")
    
    local result = RPC.execute("qpixel-meth:doAction", insideLab, "START_COOKING", data)
...
AddEventHandler("qpixel-meth:pickupIngredient", function(pArgs, pEntity)
...
AddEventHandler("qpixel-meth:adjustFridgeTemp", function(pArgs, pEntity)
    processAction("FRIDGE_TEMPERATURE", pEntity)
end)

AddEventHandler("qpixel-meth:adjustSteamLevel", function(pArgs, pEntity)
    processAction("DISTIL_STEAM", pEntity)
end)

AddEventHandler("qpixel-meth:adjustDistilSettings", function(pArgs, pEntity)
    processAction("DISTIL_SETTINGS", pEntity)
end)

AddEventHandler("qpixel-meth:adjustMixerTemp", function(pArgs, pEntity)
    processAction("MIXER_TEMPERATURE", pEntity)
end)

AddEventHandler("qpixel-meth:addIngredient", function(pArgs, pEntity)
...
AddEventHandler("qpixel-meth:adjustMixerSettings", function(pArgs, pEntity)
...
--     RPC.execute("qpixel-meth:useDoorKey", NetworkGetNetworkIdFromEntity(PlayerPedId()), info)
-- end)

AddEventHandler("qpixel-meth:purchaseMethLabKey", function()
    local wallet, message = RPC.execute("phone:adjustCryptoBalance", 1, getConfig())
    if not wallet then
        TriggerEvent("DoLongHudText", message, 2)
        return
    end
    local result = RPC.execute("qpixel-meth:purchaseLabKey")
end)
...
--     if foundObject then
--         foundObject = nil
--         TriggerEvent("qpixel-menu:updateMethObject", foundObject)
--     end
--     return
--   end
...
RegisterNetEvent("qpixel-meth:keyPurchasedEmail")
AddEventHandler("qpixel-meth:keyPurchasedEmail", function()
  TriggerEvent(
    "phone:emailReceived",
    "Dark Market",
    "Lab Information",
