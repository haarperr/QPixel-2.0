...
AddEventHandler("qpixel-meth:cornerSellProduct", function(pArgs, pEntity)
    local gameTimer = GetGameTimer()

    if lastCornerSale and (gameTimer - lastCornerSale) <= 4000 then
        return TriggerEvent('qpixel-meth:cleanAttempt', lastCornerSale - gameTimer)
    end

    lastCornerSale = gameTimer

    if not exports["qpixel-inventory"]:hasEnoughOfItem("methlabproduct", 1, false, true) then
    ...
    local success = RPC.execute("qpixel-meth:attemptCornerSale", NetworkGetNetworkIdFromEntity(pEntity), inCornerId)
    if success then
...
AddEventHandler("qpixel-meth:cornerStartSelling", function()
    local result = RPC.execute("qpixel-meth:startCornering", inCornerId)
    if not result then return end
    TriggerEvent("qpixel-meth:showSellDrugsMenuItem", "cancorner", false)
    playerCornering = true
end)

AddEventHandler("qpixel-polyzone:enter", function(name, data)
    if name ~= "meth_corner" then return end
    if not exports["qpixel-inventory"]:hasEnoughOfItem("methlabproduct", 1, false) then return end
    inCorner = true
    inCornerId = 1
    -- inCornerId = data.id
    TriggerEvent("DoLongHudText", "Looks like a good spot to sell...")
    TriggerEvent("qpixel-meth:showSellDrugsMenuItem", "cancorner", true)
end)
AddEventHandler("qpixel-polyzone:exit", function(name, data)
    if name ~= "meth_corner" then return end
    if not inCorner then return end
    if playerCornering then
        RPC.execute("qpixel-meth:stopCornering")
...
RegisterNetEvent("qpixel-meth:findCandidateVehicle")
AddEventHandler("qpixel-meth:findCandidateVehicle", function(pCornerId, pSrc)
  PopulateNow()
  Wait(0)
  ...
  TriggerServerEvent("qpixel-meth:corneringFoundVehicle", pCornerId, pSrc, { vehicle = cornerVehicle, driver = cornerPed })
end)

RegisterNetEvent('qpixel-meth:findCandidate', function()
  PopulateNow()
  Wait(0)
  local vehicles = GetGamePool('CVehicle')
