...
function Login.NUICallback(data)
	...
	if data.action == "currentHover" then
		...
		local coords = exports["qpixel-base"]:getModule("Util"):ScreenRelToWorld(GetCamCoord(LoginSafe.Cam), GetCamRot(LoginSafe.Cam,2),offset)
	...
	if data.action == "deleteCharacter" then
		...
		events:Trigger("qpixel-base:deleteCharacter", data.actionData, function(deleted)
            Login.getCharacters(true)
        end)
	end
	...
	if data.action == "newCharacter" then
		...
		TriggerServerEvent('qpixel-spawn:createCharacter:log', cData.first_name, cData.last_name, cData.dob, pGender)
		
        if not chardata then return end

		events:Trigger("qpixel-base:createCharacter", chardata, function(created)
            if not created then
...
RegisterNetEvent("qpixel-base:spawnInitialized")
