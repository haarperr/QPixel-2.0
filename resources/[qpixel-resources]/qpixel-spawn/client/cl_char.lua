...
function Login.CreatePlayerCharacterPeds(characterModelData,isReset)
    ...
    TriggerEvent("qpixel-weathersync:client:DisableSyncSpawn")

    local events = exports["qpixel-base"]:getModule("Events")
    events:Trigger("qpixel-base:fetchPlayerCharacters", nil, function(data)
        if data.err then
            return
        end
...
--[[
    ...
    Description: dealing with qpixel-base functionality in order to set/get the correct information for chars
]]
...
function Login.getCharacters(isReset)
    ...
    if not isReset then
        ...
        events:Trigger("qpixel-base:loginPlayer", nil, function(data)
...
function Login.SelectedChar(data)
	...
	local events = exports["qpixel-base"]:getModule("Events")
	events:Trigger("qpixel-base:selectCharacter", data.actionData, function(returnData)
        ...
        if Login.CurrentClothing[data.actionData] == nil then
	       ...
	       -- SetPlayerInvincible(PlayerPedId(), true)
	        TriggerEvent("qpixel-base:firstSpawn")
	    end
    end)
end
...
function Login.setClothingForChar()
    ...
    TriggerServerEvent('qpixel-spawn:SetNewRouting', math.random(1, 999))
    --TriggerEvent('qpixel_clothing:setclothes', {}, nil, true)
...
RegisterNetEvent('qpixel-spawn:justCreated')
AddEventHandler('qpixel-spawn:justCreated', function()
    TriggerServerEvent("qpixel-spawn:SetOldRouting")
    spawnChar()
    TriggerServerEvent("qpixel-spawn:licenses")
    Citizen.Wait(5000)
    TriggerEvent('qpixel-spawn:spawnCharacter')
end)

RegisterNetEvent("qpixel-spawn:finishedClothing")
AddEventHandler("qpixel-spawn:finishedClothing", function(endType)
...
function spawnChar()
    Login.actionsBlocked = false
    deleteTrain()
    --TriggerEvent("qpixel-weathersync:spawned")
    TriggerEvent("qpixel-weathersync:client:EnableSync")
    -- SetPlayerInvincible(PlayerPedId(), true)
    TriggerEvent("qpixel-base:firstSpawn")
...
AddEventHandler('qpixel-polyzone:enter', function(name)
    if name == "scuff_shit" then
        pInZone = true
        exports["qpixel-interface"]:showInteraction("[E] Information")
        TPFUNCTION()
    end
end)
