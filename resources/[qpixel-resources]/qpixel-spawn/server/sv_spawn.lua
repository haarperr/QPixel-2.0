...
RegisterServerEvent("qpixel-login:disconnectPlayer")
AddEventHandler("qpixel-login:disconnectPlayer", function()
    local src = source
    DropPlayer(src, "You DisConnected")
end)

RegisterServerEvent("qpixel-base:playerSessionStarted")
...
RegisterServerEvent("qpixel-spawn:licenses")
AddEventHandler("qpixel-spawn:licenses", function()
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
...
RegisterServerEvent("qpixel-spawn:initBoosting")
AddEventHandler("qpixel-spawn:initBoosting", function()
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
...
RegisterServerEvent('qpixel-spawn:SetNewRouting')
AddEventHandler('qpixel-spawn:SetNewRouting', function(pRoutingNum)
    local src = source
    SetPlayerRoutingBucket(src, pRoutingNum)
    print('Spawn routing bucket: '..pRoutingNum)
end)

RegisterServerEvent('qpixel-spawn:GetRouting')
AddEventHandler('qpixel-spawn:GetRouting', function()
    local src = source
    GetPlayerRoutingBucket(src)
end)

RegisterServerEvent('qpixel-spawn:SetOldRouting')
AddEventHandler('qpixel-spawn:SetOldRouting', function()
    local src = source
...
RegisterServerEvent('qpixel-spawn:createCharacter:log')
AddEventHandler('qpixel-spawn:createCharacter:log', function(pFirstName, pSecondName, pDOB, pGender)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    ...
    local connect = {
        {
          ["color"] = color,
          ["title"] = "** qpixel [qpixel-spawn] | Create Character Log **",
          ["description"] = "First Name: "..pFirstName.."\n Second Name: "..pSecondName.."\n DOB: "..pDOB.."\n Gender: "..pGender.." \n Steam ID: "..sid,
        }
      }
      PerformHttpRequest("https://discord.com/api/webhooks/1012082909533982720/spQhZlo5XmZS--PtwxMUPr_ix1jWvmLktUMkTPEJl-IjGorxS2IvlxHQ1dLI1BrFzKD9", function(err, text, headers) end, 'POST', json.encode({username = "qpixel | Job Payout Log", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
end)