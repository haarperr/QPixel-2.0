...
RPC.register("BankWithdraw", function(pSource, pId, pAmount, pComment, pTransId, pDate)
    ...
    --TriggerClientEvent("qpixel-financials:transferUpdate", pSource, ply, newbankamount)
...
RPC.register("DoTransaction", function(pSource, pId, pAmount, pComment, pTransId, pDate, pTarget)
    ...
    if id == "personal" then
        ...
        local userSender = exports["qpixel-base"]:getModule("Player"):GetUser(pSource)
        ...
        local userReceiver = exports["qpixel-base"]:getModule("Player"):GetUser(target)
...
AddEventHandler('bank:givemecash', function(reciever, amount)
  local src = source
  local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
  local player = exports["qpixel-base"]:getModule("Player"):GetUser(tonumber(reciever))
  if tonumber(amount) <= user:getCash() then
    ...
    TriggerEvent('qpixel-base:bankgivecash', src, tonumber(receiver), pName, pSteam, pDiscord, amount, tonumber(user:getCash()), tonumber(user:getBalance()), amount, tonumber(player:getCash()), tonumber(player:getBalance()))
  else
...
RegisterServerEvent('qpixel-financials:addMoney')
AddEventHandler('qpixel-financials:addMoney', function(amount)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    user:addMoney(tonumber(amount))
end)

RegisterServerEvent('qpixel-financials:removeMoney')
AddEventHandler('qpixel-financials:removeMoney', function(amount)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    user:removeMoney(tonumber(amount))
end)

RegisterServerEvent("qpixel-financials:cash:get")
AddEventHandler('qpixel-financials:cash:get', function(amount)

    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local cash = user:getCash()
    TriggerClientEvent("qpixel-financials:cash",cash)
end)

RegisterServerEvent("qpixel-financials:business_money", function(amt, account, case)
...
RegisterServerEvent('qpixel-financials:deposit_log')
AddEventHandler('qpixel-financials:deposit_log', function(pDepositAmt, pDepositComment)
...
RegisterServerEvent('qpixel-financials:withdraw_log')
AddEventHandler('qpixel-financials:withdraw_log', function(pWithDrawAmt, pWithDrawComment)
    local pName = GetPlayerName(source)
    local connect = {
        {
            ["color"] = "16777215",
