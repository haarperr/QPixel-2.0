...
RegisterNetEvent("qpixel-scenes:getScenes")
AddEventHandler("qpixel-scenes:getScenes", function()
    local src = source
    local data = Await(SQL.execute("SELECT * FROM __scenes", {}))

    if not data then
        TriggerClientEvent("qpixel-scenes:allScenes", src, {})
        return
    end
    ...
    TriggerClientEvent("qpixel-scenes:allScenes", src, scenes)
end)

RegisterNetEvent("qpixel-scenes:addScene")
AddEventHandler("qpixel-scenes:addScene", function(pData)
    local src = source
    ...
    TriggerClientEvent("qpixel-scenes:refreshScenes", -1, data[1].id, data[1])
end)

RegisterNetEvent("qpixel-scenes:deleteScene")
AddEventHandler("qpixel-scenes:deleteScene", function(pCoords)
    local data = Await(SQL.execute("SELECT * FROM __scenes", {}))
    for k,v in pairs(data) do
        ...
        if #(acCoord - pCoords) < 2.0 then
            ...
            if delete then
                TriggerClientEvent("qpixel-scenes:deleteScene", -1, v.id)
                return
            end
        end
    end
