function QPX.Core.LoginPlayer(self, args, src, callback)
    ...
    local function fetchData(_err)
        ...
        QPX.DB:FetchPlayerData(src, function(data, err)
            ...
            if not err then TriggerEvent("qpixel-base:playerLoggedIn", user) TriggerClientEvent("qpixel-base:playerLoggedIn", src) end
        end)
    end
	...
	QPX.DB:PlayerExistsDB(src, function(exists, err)
		...
		if not exists then
			QPX.DB:CreateNewPlayer(src, function(created)
				...
				TriggerClientEvent('qpixel-admin:InsertPrioCL', src)
...
function QPX.Core.FetchPlayerCharacters(self, args, src, callback)
	...
	QPX.DB:FetchCharacterData(user, function(data, err)
		if err then
			...
			TriggerEvent("qpixel-base:charactersLoaded", user, data)
			TriggerClientEvent("qpixel-base:charactersLoaded", src, data)
		end
...
function QPX.Core.SelectCharacter(self, id, src, callback)
	...
	TriggerClientEvent('qpixel-base:setcontrols', src)
	TriggerClientEvent('updatepasses', src)

	TriggerEvent("qpixel-base:characterLoaded", user, selectedCharacter)
	TriggerClientEvent("qpixel-base:characterLoaded", src, selectedCharacter)
