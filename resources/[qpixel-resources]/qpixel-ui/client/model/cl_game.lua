...
function isGrapeBarrel(p2)
  ...
  AddEventHandler("qpixel-inventory:itemUsed", function(pItem, pData)
  ...
  AddEventHandler("qpixel-vineyard:pickupBarrel", function(p1, p2, p3)
    ...
    exports['qpixel-objects']:DeleteObject(p3.meta.id)
    TriggerEvent("player:receiveItem", "vineyardbarrel", 1, false, data, data)
  end)
  
  
  RegisterUICallback("qpixel-vineyard:changeBarrelName", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = "done" } })
    exports["qpixel-ui"]:closeApplication("textbox")
    RPC.execute("qpixel-vineyard:changeBarrelName", data.key, data.values.name)
  end)
  
  RegisterUICallback("qpixel-vineyard:addGrapesToBarrel", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = "done" } })
    exports["qpixel-ui"]:closeApplication("textbox")
    local newData = data.key.newData
    local objectId = data.key.objectId
    local items = exports["qpixel-inventory"]:getItemsOfType("vineyardgrapes", false, true)
    ...
    RPC.execute("qpixel-vineyard:changeBarrelData", objectId, newData)
  end)
  
  local function getTimestamp(data)
    local remaining = math.floor((exports["qpixel-misc-code"]:getCurrentTime() - data.created_at) / 1000)
  ...
  AddEventHandler("qpixel-vineyard:barrelAction", function(p1, p2, p3)
    if not isGrapeBarrel(p3) then return end
    local data = p3.meta.data.metadata
    ...
    if p1.action == "change_name" then
      exports["qpixel-ui"]:openApplication("textbox", {
        callbackUrl = "qpixel-vineyard:changeBarrelName",
        key = objectId,
        items = {
    ...
    if p1.action == "seal" then
      ...
      local newData = { sealed = true, created_at = exports["qpixel-misc-code"]:getCurrentTime() }
      RPC.execute("qpixel-vineyard:changeBarrelData", objectId, newData)
      return
    end
    ...
    if p1.action == "add_grapes" then
      ...
      if #items == 0 then return end
      exports["qpixel-ui"]:openApplication("textbox", {
        callbackUrl = "qpixel-vineyard:addGrapesToBarrel",
    ...
    if p1.action == "pour_bottles" then
      local hasEnough = exports["qpixel-inventory"]:hasEnoughOfItem("vineyardwinebottleempty", 1, false, true)
      if not hasEnough then
        TriggerEvent("DoLongHudText", "You need empty wine bottles!", 2)
        return
      end
      TriggerEvent("doAnim","cokecut")
      local finished = exports["qpixel-taskbar"]:taskBar(10000, "Bottling Wine")
      ClearPedTasks(PlayerPedId())
      if finished ~= 100 then return end
      exports['qpixel-objects']:DeleteObject(objectId)
  ...
  AddEventHandler("qpixel-inventory:attachmentsToggle", function(pEnabled, pId)
  ...
  Citizen.CreateThread(function()
    exports["qpixel-interact"]:AddPeekEntryByModel({ `prop_wooden_barrel` }, {
      {
        id = "peek_prop_vineyard_barrel",
        event = "qpixel-vineyard:pickupBarrel",
        icon = "shopping-basket",
        label = "Pick Up",
      },
      {
        id = "peek_prop_vineyard_barrel_details",
        event = "qpixel-vineyard:barrelAction",
        icon = "info-circle",
        label = "View Label",
    ...
    exports["qpixel-interact"]:AddPeekEntryByModel({ `prop_wooden_barrel` }, {
      ...
      isEnabled = function(p1, p2, p3)
        ...
        local hasEnough = exports["qpixel-inventory"]:hasEnoughOfItem("vineyardbarrelheadopener", 1, false, true)
        return hasEnough
      end,
    })
  end)
