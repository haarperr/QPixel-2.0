...
function correct(){
    ...
    $.post('https://qpixel-ui/qpixel-ui:taskBarSkillResult', JSON.stringify({
        success: true,
    }));
}
...
const PostData = (data) => {
    return $.post(`https://qpixel-ui/dataPost`, JSON.stringify(data))
}
    ...
    switch (action) {
        ...
        case "eye":
            ...
            if (msg.data.action == "refresh") {
                ...
                $.post('https://qpixel-ui/closecontext', JSON.stringify({}));
            }
            break;
        case "sniper-scope":
...
$(document).on('mousedown', (event) => {
    ...
    if (element.id.split("-")[0] === 'target') {
        ...
        $.post('https://qpixel-ui/qpixel-ui:targetSelectOption', JSON.stringify({
            entity : entitydata,
            option :{
                event: eventData,
...
$(document).keydown(function(event) {
    ...
    if (key == 27) {
        ...
        if (eyeintactive){
            eyeintactive = false
            $.post('https://qpixel-ui/closeeyeint', JSON.stringify({}));
        }
    }
});
        ...
        if (event.target.id == "backmenu") {
            ...
            if (event.target.id.split("-")[0] === 'contextmenu') {
                ...
                if (children) {
                    ...
                    $.post('https://qpixel-ui/'+action, JSON.stringify({
                        key:key
                    }));
                ...
                $(".buttoncontext").html("");
                $(".containercontext").hide();
                $.post('https://qpixel-ui/'+subaction, JSON.stringify({
                    key:subkey
                }));
            }
        }
