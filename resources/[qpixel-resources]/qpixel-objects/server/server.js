!(function () {
    ...
    onNet('qpixel-objects:requestObjects', async () => {
      ...
      emitNet('qpixel-objects:loadObjects', src, objTable)
    })
    onNet(
    ...
    RPC.register('qpixel-objects:DeleteObject', async (pSrc, pId) => {
      let query = await SQL.execute(
      ...
      return (
        !!(await SQL.execute('DELETE FROM __objects WHERE id = @id', {
          id: pId,
        })) && (emitNet('qpixel-objects:clearObjects', -1, stackedData), true)
      )
    })
    RPC.register(
      ...
      async (pSrc, pId, pModel) => {
        ...
        return emitNet('qpixel-objects:updateObjects', -1, stackedData), true
      }
    )
  })()
