;(function () {
    ...
    !(function () {
      ...
      setImmediate(async () => {
        ...
        Number(pObjData.length) === 0 && emitNet('qpixel-objects:requestObjects')
      })
      onNet('qpixel-objects:loadObjects', async (pObjects) => {
      ...
      onNet('qpixel:objects:prepareNewObject', async (pObject) => {
        addObject(pObject)
      })
      on('qpixel-polyzone:enter', async (pName, pID) => {
      ...
      on('qpixel-polyzone:exit', (pZone, pData) => {
      ...
      onNet('qpixel-objects:clearObjects', (pObjects) => {
      ...
      onNet('qpixel-objects:updateObjects', (pObjects) => {
      ...
      async function addObject(pObject) {
        console.log('[qpixel-objects] Adding object: ', pObject)
        let pCoords = {
      ...
      on('onResourceStop', (pResourceName) => {
        if (pResourceName !== 'qpixel-objects') {
          return
        }
      ...
      function DelObject(pObj) {
        let pResult = RPC.execute('qpixel-objects:DeleteObject', pObj)
        return pResult
      }
      global.g.exports('DelObject', DelObject)
      function updataObject(pObj, pData) {
        RPC.execute('qpixel-objects:UpdateObject', pObj, pData)
      }
      ...
      RegisterCommand(
        ...
        (src, args, users) => {
          ...
          console.log('[qpixel-objects] getObject return: ', obj)
        },
        false
      )
      RegisterCommand(
    ...
    !(function () {
      ...
      on('onResourceStop', (resourceName) => {
        if (resourceName !== 'qpixel-objects') {
          return
        }
