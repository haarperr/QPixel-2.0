...
function updateCarTable(model,price,name)
	for i=1, #carTable do
		...
		TriggerServerEvent("qpixel-pdm:CarTablePDM",carTable)
	end
end

local myspawnedvehs = {}

RegisterNetEvent("qpixel-pdm:testdrive")
AddEventHandler("qpixel-pdm:testdrive", function()
	if exports["qpixel-business"]:IsEmployedAt("pdm") or #(vector3(-30.31887, -1089.345, 26.04948) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("qpixel-pdm:EnableBuy")
AddEventHandler("qpixel-pdm:EnableBuy", function()
	if #(vector3(-30.31887, -1089.345, 26.04948) - GetEntityCoords(PlayerPedId())) > 50.0 and exports["qpixel-business"]:IsEmployedAt("pdm") then
		return
	end
	...
	TriggerServerEvent("qpixel-pdm:BuyEnabledSV",vehplate)
end)

RegisterNetEvent("qpixel-pdm:BuyEnabledCL")
AddEventHandler("qpixel-pdm:BuyEnabledCL", function(addplate)
	buyPlate[addplate] = true
	Citizen.Wait(60000)
	buyPlate[addplate] = nil
end)

RegisterNetEvent("qpixel-pdm:PDMCommission")
AddEventHandler("qpixel-pdm:PDMCommission", function(newAmount)
	if exports["qpixel-business"]:IsEmployedAt("pdm") or #(vector3(-30.31887, -1089.345, 26.04948) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("qpixel-pdm:ReturnPDMTTable")
AddEventHandler("qpixel-pdm:ReturnPDMTTable", function(newTable)
	carTable = newTable
	DespawnSaleVehicles()
	SpawnSaleVehicles()
end)
...
function AttemptBuy(tableid)
	...
	TriggerServerEvent('qpixel-pdm:ChechMoney',name, model, price)
...
function DrawPrices()
	for i = 1, #carspawns do
		if #(vector3(carspawns[i]["x"],carspawns[i]["y"],carspawns[i]["z"]) - GetEntityCoords(PlayerPedId())) < 1.5 then
			...
			if exports["qpixel-business"]:IsEmployedAt("pdm") then
				if buyPlate[addplate] ~= nil then
					exports['qpixel-interface']:showInteraction("$" .. math.ceil(price) .. " | Com: %" ..commission.. " | [E] to change | [G] to buy.")
				else
					exports['qpixel-interface']:showInteraction("$" .. math.ceil(price) .. " | Com: %" ..commission.. " | [E] to change")
				end
			else
				if buyPlate[addplate] ~= nil then
					exports['qpixel-interface']:showInteraction("$" .. math.ceil(price) .. " [G] to buy.")
				else
					exports['qpixel-interface']:showInteraction("Buy Price: $" .. math.ceil(price) .. " ")
				end
			end
...
function SpawnSaleVehicles()
	if not hasspawned then
		TriggerServerEvent("qpixel-pdm:RequestPDMTTable")
		Citizen.Wait(1500)
	end
...
function CloseCreator(name, veh, price, plate)
	Citizen.CreateThread(function()
		...
		if not boughtcar then
			...
			local VehicleProps = exports['qpixel-bennys']:FetchVehProps(personalvehicle)
			...
			TriggerServerEvent('qpixel-pdm:BuyVehicle', plate, vehname, vehicle, price, VehicleProps)
			TriggerServerEvent('qpixel-cardealer:log', exports['isPed']:isPed('cid'), vehname, price, plate, 'PDM')
			TriggerServerEvent('qpixel-financials:business_money', price / 10, 'pdm', 'add')
			exports['qpixel-interface']:hideInteraction()
			DespawnSaleVehicles()
			SpawnSaleVehicles()
		end
...
RegisterNetEvent("qpixel-pdm:FailedPurchase")
AddEventHandler("qpixel-pdm:FailedPurchase", function()
...
function ButtonSelected(button)
	...
	elseif this == "jobvehicles" or this == "a-class" or this == "b-class" or this == "s-class" or this == "pdm-tuners" or this == "c-class" or this == "casino" or this == "d-class" or this == "offroad" or this == "suvs" or this == "vans" or this == "industrial" or this == "cycles" or this == "motorcycles" then
		if ownerMenu then
			...
			TriggerServerEvent('qpixel-pdm:ChechMoney',button.name, button.model, button.costs)
		end
	end
end
	...
	while true do
		...
		if vehshop.opened then
			...
			for i,button in pairs(menu.buttons) do
				...
				if exports["qpixel-business"]:IsEmployedAt("pdm") and i >= vehshop.menu.from and i <= vehshop.menu.to then
...
RegisterCommand('commissionpdm', function(source, args, raw)
	if exports["qpixel-business"]:IsEmployedAt("pdm") then
		...
		if tonumber(amount) > 0 and tonumber(amount) <= 60 then
			TriggerEvent('qpixel-pdm:PDMCommission', amount)
		else
...
RegisterCommand('testdrivepdm', function(source, args, raw)
	if exports["qpixel-business"]:IsEmployedAt("pdm") then
		TriggerEvent('qpixel-pdm:testdrive')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)
...
RegisterCommand('enableBuypdm', function(source, args, raw)
	if exports["qpixel-business"]:IsEmployedAt("pdm") then
		TriggerEvent('qpixel-pdm:EnableBuy')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)

RegisterNetEvent('qpixel-fuel:pdmmenushit', function()
...
RegisterInterfaceCallback('qpixel-pdm:pdmcom', function(data, cb, pParams)
	cb({ data = {}, meta = { ok = true, message = '' } })
	exports['qpixel-interface']:openApplication('textbox', {
		callbackUrl = 'qpixel-pdm:send_pdm',
		key = 1,
		items = {
		  {
			icon = "address-card", 
...
RegisterInterfaceCallback('qpixel-pdm:send_pdm', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
	TriggerServerEvent("qpixel-pdm:wtf", data.values.pPDMPayCom)
end) 



RegisterNetEvent("qpixel-pdm:commissionpdm") 
AddEventHandler("qpixel-pdm:commissionpdm", function(args)
	print("YO")
	if exports["qpixel-business"]:IsEmployedAt("pdm") then
		print("IM PDM WORKER")
		local amount = args
...
RegisterNetEvent("qpixel-pdm:testdrivepdm")
AddEventHandler("qpixel-pdm:testdrivepdm", function()
	if exports["qpixel-business"]:IsEmployedAt("pdm") then
		TriggerEvent('qpixel-pdm:testdrive')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
...
RegisterNetEvent("qpixel-pdm:enableBuypdm")
AddEventHandler("qpixel-pdm:enableBuypdm", function()
	if exports["qpixel-business"]:IsEmployedAt("pdm") then
		TriggerEvent('qpixel-pdm:EnableBuy')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
