...
RegisterServerEvent('qpixel-pdm:CarTablePDM')
AddEventHandler('qpixel-pdm:CarTablePDM', function(table)
    if table ~= nil then
        carTable = table
        TriggerClientEvent('qpixel-pdm:ReturnPDMTTable', -1, carTable)
        for i=1, #carTable do
...
RegisterServerEvent('qpixel-pdm:FinaceEnabledSV')
AddEventHandler('qpixel-pdm:FinaceEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('qpixel-pdm:FinaceEnabledCL', -1, plate)
    end
end)

RegisterServerEvent('qpixel-pdm:BuyEnabledSV')
AddEventHandler('qpixel-pdm:BuyEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('qpixel-pdm:BuyEnabledCL', -1, plate)
    end
end)

-- return table
-- TODO (return db table)
RegisterServerEvent('qpixel-pdm:RequestPDMTTable')
AddEventHandler('qpixel-pdm:RequestPDMTTable', function()
    local user = source
    exports.oxmysql:execute("SELECT * FROM vehicle_display", {}, function(display)
        ...
        TriggerClientEvent('qpixel-pdm:ReturnPDMTTable', user, carTable)
    end)
end)

-- Check if player has enough money
RegisterServerEvent('qpixel-pdm:ChechMoney')
AddEventHandler('qpixel-pdm:ChechMoney', function(name, model, price)
    local src = source
    ...
    elseif tonumber(cash) <= price then
        ...
        TriggerClientEvent('qpixel-pdm:FailedPurchase', src)
    end
end)

RegisterServerEvent('qpixel-pdm:BuyVehicle')
AddEventHandler('qpixel-pdm:BuyVehicle', function(plate, name, vehicle, price, personalvehicle)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
...
RegisterServerEvent('qpixel-pdm:wtf')
AddEventHandler('qpixel-pdm:wtf', function(comis)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    TriggerClientEvent("qpixel-pdm:commissionpdm", src, comis)
end)