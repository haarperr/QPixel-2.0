...
local function drawStatusHUD(state, pValues)
  ...
  local values = {}
  isSlotMachinesEnabled, currentJackPot = RPC.execute("qpixel-slotmachines-sv:checkIfSlotMachinesState")
  ...
  -- exports["qpixel-textui"]:sendAppEvent("status-hud", {
  --   show = state,
  --   title = title,
  --   values = values
  -- })
    ...
    if isSeatedAtSlotMachine then
      if not isBetContextShowing then
        if currentSlotMachineDetails ~= nil then
          ...
          table.insert(data, {
            action = "qpixel-textui-slotmachines:placeBet",
            title = "Place bet",
            description = "$100.00",
            key = {
              coinValue = 100,
          ...
          table.insert(data, {
            action = "qpixel-textui-slotmachines:leaveMachine",
            title = "Leave Machine",
            description = "",
            key = {},
          })

          exports["qpixel-ui"]:showContextMenu(data) 
        end
      end
    end
        ...
        while true do
          ...
          if not isNear(plyPos, v.chairPos, 0.75) then
            if isInteractionShowing then
              ...
              exports["qpixel-ui"]:hideInteraction(interactionBackground)
            end
          ...
          if not isInteractionShowing then
            ...
            exports["qpixel-ui"]:showInteraction("[E] Play " .. v.label, interactionBackground)
          end
...
AddEventHandler("qpixel-casino:casinoEnteredEvent", function()
  isInsideCasino = true
  isSlotMachinesEnabled, currentAccJackpot = RPC.execute("qpixel-slotmachines-sv:checkIfSlotMachinesState")
...
AddEventHandler("qpixel-casino:casinoExitedEvent", function()
  ...
  isSlotMachinesEnabled = RPC.execute("qpixel-slotmachines-sv:checkIfSlotMachinesState")
  if not isSlotMachinesEnabled then
    return
  end
...
--#[NUI Callbacks]#--
RegisterInterfaceCallback("qpixel-textui-slotmachines:placeBet", function(data, cb)
  ...
  if currentSlotMachine and not isSlotMachineSpinning then
    ...
    local useChipsResult = RPC.execute("qpixel-casino:useChips", currentBet)
    ...
    local user = exports["qpixel-fw"]:getModule("LocalPlayer")
...
RegisterInterfaceCallback("qpixel-textui-slotmachines:leaveMachine", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  getUpFromSlotMachine()
  SendNUIMessage({
    app = "status-hud",
