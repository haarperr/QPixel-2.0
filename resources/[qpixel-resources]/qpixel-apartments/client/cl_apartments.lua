...
RegisterNetEvent('qpixel-binds:keyEvent')
AddEventHandler('qpixel-binds:keyEvent', function(name,onDown)
...
function Apart.processBuildType(numMultiplier,roomType,isSpawn)
    ...
    local isBuiltCoords = exports["qpixel-build"]:getModule("func").buildRoom(name,numMultiplier,false)
    ...
    if isBuiltCoords then
        ...
        if isSpawn then
            ...
            TriggerEvent("qpixel-spawn:characterSpawned", cid)
        else
...
function Apart.leaveApartment()
    ...
    if Apart.exitPoint[Apart.currentRoomType] == nil then
        exports["qpixel-build"]:getModule("func").exitCurrentRoom(Apart.Locations[Apart.currentRoomType][Apart.currentRoomNumber])
    else
        if type(Apart.exitPoint[Apart.currentRoomType]) == typeVector then
            exports["qpixel-build"]:getModule("func").exitCurrentRoom(Apart.exitPoint[Apart.currentRoomType])
        else
            local rnd = math.random(1,#Apart.exitPoint[Apart.currentRoomType])
            exports["qpixel-build"]:getModule("func").exitCurrentRoom(Apart.exitPoint[Apart.currentRoomType][rnd])
        end
    end
...
function Apart.OpenStash()
    if not exports['qpixel-taskbar']:pInInv() then
        local cid = exports["isPed"]:isPed("cid")
...
function garageToWorld()
    exports["qpixel-build"]:getModule("func").exitCurrentRoom(vector3(4.67, -724.85, 32.18))
    Apart.currentRoomNumber = Apart.defaultInfo[2]
    Apart.currentRoomType = Apart.defaultInfo[1]
    Apart.currentHotelInformation = nil
    Apart.currentGarageNumber = 0
...
function Apart.WakeFromBed()
    local bedOffset = exports["qpixel-build"]:getModule("func").getCurrentBed()
...
function nearClothing()
    ...
    local isNear = exports["qpixel-build"]:getModule("func").isNearCurrentInteract(1,5.5)
    if isNear then
        return true
    end
...
AddEventHandler('apartment:removeFromBuilding', function(roomNumber,buildType)
    if Apart.currentRoomType == buildType and Apart.currentRoomNumber == roomNumber then
        if exports["qpixel-build"]:getModule("func").isInBuilding() then
...
RegisterInterfaceCallback("qpixel-apartments:handler", function(data, cb)
...
AddEventHandler('apartments:menuAction', function(action)
    ...
    elseif action == "forfeit" then
        exports["qpixel-interface"]:showContextMenu(MenuData["apartment_check"])
    end
end)
