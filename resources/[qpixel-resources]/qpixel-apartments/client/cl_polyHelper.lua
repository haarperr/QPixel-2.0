...
function polyHelperEnter(apartmentLocNumber,apartmentNumber)
    ...
    listening = true
    exports["qpixel-interface"]:showInteraction(getInteractionMessage(apartmentLocNumber,apartmentNumber))
...
local function showApartmentMenu(apartmentLocNumber,apartmentNumber)
    ...
    for k,v in pairs(Apart.currentRoomLocks[apartmentLocNumber]) do
        if v == false then
            unlockedApartments[#unlockedApartments + 1] = { title = "Apartment # "..k, action = "qpixel-interface:apartmentsContext", key = {"EnterUnlocked",apartmentLocNumber,k} }
        end 
    end
    ...
    if Apart.defaultInfo[1] == apartmentLocNumber and Apart.defaultInfo[2] == apartmentNumber then
        data[#data+1] = {
            ...
            action = "qpixel-interface:apartmentsContext"
        }
    end
...
RegisterInterfaceCallback("qpixel-interface:apartmentsContext", function(data, cb)
    ...
    if action == "locks" then
        ...
        exports["qpixel-interface"]:showInteraction(getInteractionMessage(globalLocNumber,globalNumber))
    end
    ...
    if action == "lockdownCid" then
        ...
        local prompt = exports['qpixel-interface']:OpenInputMenu(elements, function(values)
