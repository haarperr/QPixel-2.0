-- Key binds:
-- Usage:
-- exports["qpixel-keybinds"]:registerKeyMapping(category, description, onKeyDownCommand, onKeyUpCommand, defaultKey)
...
AddEventHandler("qpixel-binds:should-execute", function(shouldExecute)
...
exports('registerKeyMapping', function(name, category, description, onKeyDownCommand, onKeyUpCommand, default, event, type)
    ...
    RegisterCommand(cmdStringUp, function()
      if not shouldExecuteBind then return end
      if event then TriggerEvent("qpixel-binds:keyEvent", name, false) end
