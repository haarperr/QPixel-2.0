		...
		Citizen.Wait(1)
			if(Vdist(methlocation["x"],methlocation["y"],methlocation["z"], pos.x, pos.y, pos.z) < 10.0)then
				...
				if(Vdist(methlocation["x"],methlocation["y"],methlocation["z"], pos.x, pos.y, pos.z) < 5.0)then
					...
					local finished = exports["qpixel-taskbar"]:taskBar(60000,"Searching...")
...
RegisterNetEvent('qpixel-shops:exchangeGold')
AddEventHandler('qpixel-shops:exchangeGold', function()
	if exports["qpixel-inventory"]:hasEnoughOfItem("goldbar",70,true) then
		local finished = exports["qpixel-taskbar"]:taskBar(120000,"Trading Bars")
		if finished == 100 then
			if exports["qpixel-inventory"]:hasEnoughOfItem("goldbar",70,true) then
				TriggerServerEvent('qpixel-banking:addMoney', 35000)
...
AddEventHandler('slushy:general', function()
	...
	local finished = exports ["qpixel-ui"]:taskBarSkill(18000,math.random(10,20))
	if (finished == 100) then
		ExecuteCommand("e handshake")
		local finished = exports ["qpixel-ui"]:taskBarSkill(18000,math.random(10,20))
		if (finished == 100) then
...
AddEventHandler('lockpickshit:general', function()
	local finished = exports["qpixel-taskbar"]:taskBar(60000,"What dis????")
	if (finished == 100) then
...
AddEventHandler("illegal:crafting", function()
	...
	local finished = exports['qpixel-taskbar']:taskBar(12000, 'Inspecting Table')
	if (finished == 100) then
...
-- 	local finished = exports['qpixel-taskbar']:taskBar(20000, 'Smelting')
-- 	if (finished == 100) then
...
RegisterNetEvent("qpixel-shops:redCounter")
AddEventHandler("qpixel-shops:redCounter", function()
    TriggerEvent("server-inventory-open", "1", "redcircle_counter");
end)

RegisterNetEvent("qpixel-shops:redCounter2")
AddEventHandler("qpixel-shops:redCounter2", function()
...
AddEventHandler('weapon:general', function()
	TriggerServerEvent('qpixel-shops:WeaponShopStatus')
end)
...
AddEventHandler("hunting:level1", function()
	if exports["qpixel-inventory"]:getQuantity("coyotepelt") >= 1 then
		playerAnim()
		local finished = exports["qpixel-taskbar"]:taskBar(4000,"Selling Coyotepelt",true,false,playerVeh)
		if finished == 100 then
			if exports["qpixel-inventory"]:getQuantity("coyotepelt") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'coyotepelt', 1)
				TriggerServerEvent('qpixel-banking:addMoney', math.random(80, 150))
			else
...
AddEventHandler("hunting:level2", function()
	if exports["qpixel-inventory"]:getQuantity("buck") >= 1 then
		playerAnim()
		local finished = exports["qpixel-taskbar"]:taskBar(4000,"Selling Buck",true,false,playerVeh)
		if finished == 100 then
			if exports["qpixel-inventory"]:getQuantity("buck") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'buck', 1)
				TriggerServerEvent('qpixel-banking:addMoney', math.random(160, 200))
			else
...
AddEventHandler("hunting:level3", function()
	if exports["qpixel-inventory"]:getQuantity("boar") >= 1 then
		playerAnim()
		local finished = exports["qpixel-taskbar"]:taskBar(4000,"Selling Boar",true,false,playerVeh)
		if finished == 100 then
			if exports["qpixel-inventory"]:getQuantity("boar") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'boar', 1)
				TriggerServerEvent('qpixel-banking:addMoney', math.random(350, 500))
			else
...
--     exports['qpixel-menuinteract']:AddButton("Sell Coyotepelt", "Wow you suck", "hunting:level1", 'example button 1', "jobmenu")
--     exports['qpixel-menuinteract']:AddButton("Sell Buck", "Wow you kinda okay", "hunting:level2", 'example button 2', "jobmenu")
--     exports['qpixel-menuinteract']:AddButton("Sell Boar", "Wow you good", "hunting:level3", 'example button 3', "jobmenu")
--     exports['qpixel-menuinteract']:SubMenu("Hunting Menu" , "Click to select sell!" , "jobmenu")
-- end
