...
--   exports["qpixel-ui"]:sendAppEvent("status-hud", {
--     show = true,
--     title = title,
--     values = values,
--   })
            ...
            if closestChair ~= nil and closestChairDist < 1.6 then
                if not timeoutHowToBlackjack then
                    if not uiIntShown then
                        ...
                        -- exports["qpixel-ui"]:showInteraction(
                        --   "[E] Play! " .. dwGetLimitTitleString(),
                        --   background
                        -- )
                        exports["qpixel-ui"]:showInteraction("[E] Play! " .. dwGetLimitTitleString() , background)
                        uiIntShown = true
                    end
                end
...
RegisterUICallback("qpixel-ui:blackjackHitStand", function(data, cb)
...
AddEventHandler("qpixel-ui:application-closed", function(name)
...
Citizen.CreateThread(function()
    while true do
        ...
        --         data[#data + 1] = {
        --           action = "qpixel-ui:blackjackHitStand",
...
RegisterUICallback("qpixel-ui:blackJackBet", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
...
AddEventHandler("qpixel-ui:application-closed", function(name, data)
