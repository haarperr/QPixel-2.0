...
function dwStatusHud(pValues)
  if dwRefreshChipCount then
    ...
    dwCurrentChipCount = RPC.execute("qpixel-casino:getCurrentChipCount")
  end
...
Citizen.CreateThread(function()
  --Wait(10000)
  dealerPedSeeds = RPC.execute("qpixel-blackjack:getDealerPedSeeds")
    ...
    while true do 
        if not sittingAtBlackjackTable then
            if closestChair ~= nil and closestChairDist < 1.6 then
                if not timeoutHowToBlackjack then
                    if not uiIntShown then
                        ...
                        if exports['qpixel-inventory']:hasEnoughOfItem("casinomember", 1, false, true) and not isHighRoller then
                            background = "success"
                        ...
                        exports["qpixel-interface"]:showInteraction(
            ...
            elseif uiIntShown then
                uiIntShown = false
                local background = "info"
                if exports['qpixel-inventory']:hasEnoughOfItem("casinomember", 1, false, true) or exports['qpixel-inventory']:hasEnoughOfItem("casinovip", 1, false, true) then
                    background = "success"
                else
                    background = "error"
                end
                exports["qpixel-interface"]:hideInteraction(background)
            end
        elseif uiIntShown then
            uiIntShown = false
            local background = "info"
...
RegisterInterfaceCallback("qpixel-interface:blackjackHitStand", function(data, cb)
  ...
  elseif action == "double" then
    local dwUseChipsResult = RPC.execute("qpixel-casino:useChips", currentBetAmount)
  ...
  elseif action == "surrender" then
    ...
    -- RPC.execute("qpixel-casino:winChips", currentBetAmount / 2)
...
Citizen.CreateThread(function()
    while true do
        ...
        --         data[#data + 1] = {
        --           action = "qpixel-interface:blackjackHitStand",
...
Citizen.CreateThread(function()
    while true do
        if drawTimerBar and not timerBarHasBeenDrawn then
            ...
            exports["qpixel-interface"]:sendAppEvent("hud", {
...
RegisterInterfaceCallback("qpixel-interface:blackJackBet", function(data, cb)
...
AddEventHandler("qpixel-interface:application-closed", function(name, data)
...
function dwWaitForBet(act)
  ...
  if waitingForBetState then
    ...
    if sittingAtBlackjackTable then
      ...
      local data = {}
      data[#data + 1] = {
        action = "qpixel-interface:blackJackBet",
        title = "Place bet",
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    -- RPC.execute("qpixel-casino:winChips", currentBetAmount)
    dwRefreshChipCount = true
    if closeToCasino then
        blackjackGameInProgress = false
        dealerPed = getDealerFromTableId(tableId)
...
AddEventHandler("Blackjack:blackjackWin",function(tableId, dwHitBlackjack)
    ...
    -- RPC.execute("qpixel-casino:winChips", (currentBetAmount * 2) + extra)
    dwRefreshChipCount = true
    if closeToCasino then
        blackjackGameInProgress = false
        dealerPed = getDealerFromTableId(tableId)
