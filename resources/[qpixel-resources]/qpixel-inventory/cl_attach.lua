...
AddEventHandler("AttachWeapons", function()
	...
	DeleteAttached()
	for i = 1, #w do
		if exports["qpixel-inventory"]:getQuantity(w[i]["id"]) > 0 then
			...
			elseif w[i]["type"] == 4 and not sheathed then
				TriggerEvent("qpixel-inventory:attachmentsToggle", true, w[i]["id"])
				sheathed = true
...
function DeleteAttached()
	TriggerEvent("qpixel-inventory:attachmentsToggle", false)
	for i = 1, #ag do
		DeleteEntity(ag[i])
	end
	for i = 1, #ad do
