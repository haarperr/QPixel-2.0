...
function getQuantity(pServerId, pItemId)
    local user, amount = exports["qpixel-base"]:getModule("Player"):GetUser(pServerId), 0
    ...
    if user then
        ...
        if checkQuality then
            decayDate = exports['qpixel-inventory']:GetDecayDate(pItemId)
...
function removeItem(pServerId, pItemId, pAmount, pShouldNotify)
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(pServerId)
