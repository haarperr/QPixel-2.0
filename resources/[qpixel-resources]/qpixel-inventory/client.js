...
on('qpixel-base:playerSpawned', async (broughtData) => {
...
//  $.post("http://qpixel-inventory/SlotJustUsed", JSON.stringify({target: targetSlot, origin: originSlot, itemid: itemidsent }));
let recentused = [];
...
on('inventory:open_hidden', (penis, vehicleFound) => {
    let vehId = exports['qpixel-vehicles'].GetVehicleIdentifier(vehicleFound)
...
on('inventory-open-request', async () => {
  ...
  //  const apartmentFloor = exports["qpixel-apartments"].getModule("func").getApartment()
    let targetid = 0;
    cid = exports.isPed.isPed("cid");
    ...
    const currentTarget = exports['qpixel-target'].GetCurrentEntity()
    ...
    if (isInVehicle) {
       ...
       // let serverCode = exports["qpixel-config"].GetServerCode();
...
on('qpixel-items:SetAmmo', (sentammoTable) => {
    if (sentammoTable) {
...
on('__cfx_nui:ServerCloseInventory', (data, cb) => {
    ...
    if (data.name != 'none') {
        ...
        emit('qpixel-inventory:closed', data.name);
    }
    cb({});
});
...
on('__cfx_nui:craftProgression', (data, cb) => {
    cb("ok");
    emit("qpixel-inventory:craftProgression", data);
    emitNet("qpixel-inventory:craftProgression", data);
});
...
on('__cfx_nui:move', (data, cb) => {
    if (global.exports['qpixel-taskbar'].pInInv()) {
        emit('DoLongHudText', 'Nice try exploiting mate.', 2)
        emitNet('qpixel-inventory:exploiterKick')
    } else {
...
function SetCustomNuiFocus(hasKeyboard, hasMouse) {
    ...
    //   if (HasNuiFocus === true) {
    //   	emit("qpixel-voice:focus:set", HasNuiFocus, hasKeyboard, hasMouse);
    //   } else {
    // 	  setTimeout(() => {if (HasNuiFocus !== true) emit("qpixel-voice:focus:set", false, false, false);}, 1000)
    //   }
}
...
exports('setPlayerWeight', (cid, weight) => {
    ...
    emitNet('qpixel-inventory:server:weightChange', cid, weight);
