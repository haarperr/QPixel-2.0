...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("qpixel-bennys:attemptPurchase",cheap, type, upgradeLevel)
...
function RepairVehicle()
    ...
    ExitBennys()
    local finished = exports["qpixel-taskbar"]:taskBar(10000 + (missingBodyHealth / 50), "Repairing vehicle...", false, true)
...
function ExitBennys()
    ...
    TriggerServerEvent("qpixel-bennys:removeFromInUse", currentBennys)
    if currentBennys:find('^devbennys') then
        --
    else
        exports["qpixel-interface"]:showInteraction("Bennys")
    end
    currentBennys = nil
end
...
--#[Event Handlers]#--
RegisterNetEvent("qpixel-bennys:purchaseSuccessful")
AddEventHandler("qpixel-bennys:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
end)

RegisterNetEvent("qpixel-bennys:purchaseFailed")
AddEventHandler("qpixel-bennys:purchaseFailed", function()
...
AddEventHandler("bennys:enter", function()
    ...
    --[[local isBennysInUse = RPC.execute("qpixel-bennys:checkIfUsed", currentBennys)
    ...
    ::hasAccess::
    TriggerServerEvent("qpixel-bennys:addToInUse", currentBennys)
    print(currentBennys)
    ...
    elseif currentBennys == "bennyshayes" then
        if exports['qpixel-business']:IsEmployedAt('hayes_autos') then
            enterLocation(bennys, repair)
        end
    ...
    elseif currentBennys == "bennysharmony" then
        if exports['qpixel-business']:IsEmployedAt('harmony_repairs') then
            enterLocation(bennys, repair)
        end
    elseif currentBennys == "ottosbay1" then
        if exports['qpixel-business']:IsEmployedAt('ottos_autos') then
            enterLocation(bennys, repair)
        end
...
RegisterNetEvent("qpixel-bennys:repairVeh")
AddEventHandler('qpixel-bennys:repairVeh', function()
    ...
    if car ~= 0 then
        ...
        local finished = exports["qpixel-taskbar"]:taskBar(4000,"Repairing Engine",false,false,playerVeh)
        Citizen.Wait(1000)
        local finished = exports["qpixel-taskbar"]:taskBar(3000,"Repairing Body",false,false,playerVeh)
        if (finished == 100) then
...
RegisterNetEvent('qpixel-public-bennys')
AddEventHandler('qpixel-public-bennys', function()
    TriggerServerEvent('qpixel-bennys:payPublicBennys')
end)

Citizen.CreateThread(function()
    exports["qpixel-polyzone"]:AddBoxZone("pubbennys", vector3(-33.2, -1053.37, 28.4), 8.0, 9.4, {
        heading=340,
        minZ=25.72,
        maxZ=29.52
    })
