...
--   local balance = RPC.execute("qpixel-casino:getCurrentChipCount")
--  --local balance = 400
--   local values = {}
    if state then 
...
--     -- exports["qpixel-interface"]:sendAppEvent("status-hud", {
--     --   show = state,
--     --   title = title,
--     --   values = values
--     -- })
--     exports["qpixel-textdisplay"]:DisplayText({
--       header = title,
--         rows = {
--           {
--               id = 0, 
...
--   })
    ...
    exports["qpixel-textdisplay"]:HideText()
  end
end
...
local function getUpFromSlotMachine()
  ...
  TriggerServerEvent("qpixel-slotmachines-sv:getUpFromSlotMachine")
  drawStatusHUD(false, {})
    ...
    if isSeatedAtSlotMachine then
      if not isBetContextShowing then
        if currentSlotMachineDetails ~= nil then
          ...
          table.insert(data, {
            action = "qpixel-interface-slotmachines:placeBet",
            title = "Place bet",
            description = "$100.00",
            key = {
              coinValue = 100,
          ...
          table.insert(data, {
            action = "qpixel-interface-slotmachines:leaveMachine",
            title = "Leave Machine",
            key = {},
          })
...
local function initializeSlotMachines()
  ...
  TriggerServerEvent("qpixel-slotmachines-sv:getMachineData")
        ...
        while true do
          ...
          if not isNear(plyPos, v.chairPos, 0.75) then
            if isInteractionShowing then
              ...
              exports["qpixel-interface"]:hideInteraction(interactionBackground)
            end
          ...
          if not isInteractionShowing then
            ...
            if exports["qpixel-inventory"]:hasEnoughOfItem("casinomember", 1, false, true) then
              interactionBackground = "success"
              canPlayAtSlotMachine = true
            elseif exports["qpixel-inventory"]:hasEnoughOfItem("casinovip", 1, false, true) then
              interactionBackground = "success"
              canPlayAtSlotMachine = true
            else
            ...
            exports["qpixel-interface"]:showInteraction("[E] Play " .. v.label, interactionBackground)
          end
        
          if IsControlJustReleased(1, 38) and canPlayAtSlotMachine then --Key: E
            TriggerServerEvent("qpixel-slotmachines-sv:sitDownAtSlotMachine", k)
            --print("E")
            break
          end
...
--#[Event Handlers]#--
RegisterNetEvent("qpixel-slotmachines-cl:getMachineData")
AddEventHandler("qpixel-slotmachines-cl:getMachineData", function(data)
...
RegisterNetEvent("qpixel-slotmachines-cl:updateMachineData")
AddEventHandler("qpixel-slotmachines-cl:updateMachineData", function(machine, ply)
...
RegisterNetEvent("qpixel-slotmachines-cl:sitDownAtSlotMachine")
AddEventHandler("qpixel-slotmachines-cl:sitDownAtSlotMachine", function(machine, coinValue)
  local plyPed = PlayerPedId()
  local plyPos = GetEntityCoords(plyPed)
...
RegisterNetEvent("qpixel-slotmachines-cl:playSlotMachineSound")
AddEventHandler("qpixel-slotmachines-cl:playSlotMachineSound", function(machinePos, machineModel, sound)
  if isSMSetup then
    local soundRef = ""
...
RegisterNetEvent("qpixel-slotmachines-cl:stopReels")
AddEventHandler("qpixel-slotmachines-cl:stopReels", function(reel, rot)
...
RegisterNetEvent("qpixel-slotmachines-cl:spinFinished")
AddEventHandler("qpixel-slotmachines-cl:spinFinished", function(amountWon)
  if amountWon > 0 then
    -- RPC.execute("qpixel-casino:winChips", amountWon)
...
RegisterNetEvent("qpixel-slotmachines-cl:toggleSlotMachines")
AddEventHandler("qpixel-slotmachines-cl:toggleSlotMachines", function(state)
...
AddEventHandler("qpixel-casino:casinoEnteredEvent", function()
  isInsideCasino = true
...
AddEventHandler("qpixel-casino:casinoExitedEvent", function()
...
--#[NUI Callbacks]#--
RegisterInterfaceCallback("qpixel-interface-slotmachines:placeBet", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  exports["qpixel-interface"]:hideContextMenu(data)
  ...
  if currentSlotMachine and not isSlotMachineSpinning then
    ...
    if not exports["qpixel-inventory"]:hasEnoughOfItem("redchip", currentBet, false, true) then 
    ...
    TriggerServerEvent(
      "qpixel-slotmachines-sv:spinMachine",
    ...
    local title = "Slot Machine - " .. currentSlotMachineLabel
    --local balance = RPC.execute("qpixel-casino:getCurrentChipCount")
    local balance = exports["qpixel-inventory"]:getQuantity("redchip")
    --local values = {}
    --table.insert(values, "Balance: $" .. balance)
      exports["qpixel-textdisplay"]:DisplayText({
      header = title,
...
RegisterInterfaceCallback("qpixel-interface-slotmachines:leaveMachine", function(data, cb)
...
function spinSlot(tickRate)
  ...
  for i=1,300,1 do
    ...
    elseif i == 180 then
      ...
      TriggerEvent("qpixel-slotmachines-cl:stopReels", 1, ang1)
    end
    if i < 240 then
    elseif i == 240 then
      ang2 = tickRate.b*22.5-180+0.0
      print(ang2)
      TriggerEvent("qpixel-slotmachines-cl:stopReels", 2, ang2)
    end
    if i < 300 then
    elseif i == 300 then
      ang3 = tickRate.c*22.5-180+0.0
      TriggerEvent("qpixel-slotmachines-cl:stopReels", 3, ang3)
    end				
    Citizen.Wait(10)
  end
  TriggerServerEvent('qpixel-slotmachines-sv:slotsCheckWin', tickRate, data)
end

RegisterNetEvent('qpixel-slotmachines-cl:startSpin')
AddEventHandler('qpixel-slotmachines-cl:startSpin',function(tickRate)
  spinSlot(tickRate)
end)
