...
Entries[#Entries + 1] = {
    ...
    options = {
        distance = { radius = 1.8 },
        isEnabled = function(pEntity, pContext)
            return exports['qpixel-activities']:canHandOffPackages() and exports['qpixel-inventory']:hasEnoughOfItem('darkmarketpackage', 1)
        end
    }
}
    ...
    data = {
        {
            ...
            event = "qpixel-fuel:RefillVehicle",
            parameters = {}
        }
    },
    options = {
    ...
    options = {
        ...
        isEnabled = function(pEntity, pContext)
            return (not (isCloseToHood(pEntity, PlayerPedId()) or isCloseToBoot(pEntity, PlayerPedId(), 2.0, pContext.model))
              or pContext.model == GetHashKey("emsnspeedo"))
              and not IsPedInAnyVehicle(PlayerPedId(), false)
              and NetworkGetEntityIsNetworked(pEntity) and exports['qpixel-boosting']:pCanFinishScratch()
        end
    }
}
