...
function hasJobPermission(pJob, pPermission)
  ...
  local hasJobPermission = function()
    if pPermission == "employee" then
      ...
      return RPC.execute('qpixel-business:hasPermission', pJob, pPermission)
    end
  end
...
RegisterNetEvent("qpixel-interact:bustVehicleOwnerShipCache", function(pEntity)
...
function getVehicleOwner(pEntity)
  ...
  local vin = exports['qpixel-vehicles']:GetVehicleIdentifier(pEntity)
  local vehicleOwner = RPC.execute("qpixel-ottosauto:getVehicleOwner", vin)
...
AddEventHandler("qpixel-fishing:currentlyRentedBoat", function(handle)
