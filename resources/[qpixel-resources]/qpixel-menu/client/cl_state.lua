...
AddEventHandler('qpixel-polyzone:entered:garages', polyZoneEnter)
AddEventHandler('qpixel-polyzone:exited:garages', polyZoneExit)
...
RegisterNetEvent("menu:send:qpixel-housing:garages", function(pTable, pHid, pHeading)
        exports["qpixel-polyzone"]:AddBoxZone("housingGarage-"..pHid, pTable, 10, 10, {
            heading = pHeading,
            minZ = -25,
            maxZ = 100,
          })
...
AddEventHandler('qpixel-polyzone:enter', function(name)
    ...
    for k, v in pairs (sharedGarages) do
        if name == v then
            local garageData = exports["qpixel-garages"]:getGarageData(name)
