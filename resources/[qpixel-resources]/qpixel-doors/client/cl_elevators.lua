...
Citizen.CreateThread(function()
    Elevators = RPC.execute('qpixel-doors:elevators:fetch')
    ...
    for _, elevator in ipairs(Elevators) do
        ...
        for floorId, floor in ipairs(floors) do
            ...
            local data, lib = zone.options.data or {}, zone.target and 'qpixel-polytarget' or 'qpixel-polyzone'

            data.floorId = floorId
            data.elevatorId = elevator.id

            exports[lib]:AddBoxZone("qpixel-doors:elevator", zone.center, zone.width, zone.length, zone.options)
        end
    end

    exports['qpixel-interact']:AddPeekEntryByPolyTarget('qpixel-doors:elevator', {
        {
            id = "elevatorPrompt",
            event = "qpixel-doors:elevator:prompt",
            icon = "chevron-circle-up",
            label = "Elevator"
        }}, { distance = { radius = 1.5 } })
end)

AddEventHandler('qpixel-doors:elevator:prompt', function(pParameters, pEntity, pContext)
    local data = pContext.zones and pContext.zones['qpixel-doors:elevator']

    if not data or not Elevators[data.elevatorId] then return end

    local hasHackingDevice = exports["qpixel-inventory"]:hasEnoughOfItem('elevatorhackingdevice', 1, false, true)
...
function OpenElevatorMenu(pElevator, pCurrentFloor, pBreachingDevice)
    ...
    for floorId, floor in ipairs(elevator.floors) do
        ...
        elements[#elements+1] = {
            ...
            action = (not isCurrentFloor and not isRestricted) and 'qpixel-doors:elevator:teleport' or '',
            key = floor.teleport,
        ...
        if hasAccess then
            access[#access+1] = {
                ...
                action = 'qpixel-doors:elevator:access',
    ...
    if not hasAccess and pBreachingDevice then
        elements[#elements+1] = {
            ...
            action = 'qpixel-doors:elevator:hackControlPanel',
            key = { type = "elevator", id = pElevator }
        }
    end

    exports['qpixel-interface']:showContextMenu(reverse(elements))
end
...
RegisterNetEvent('qpixel-doors:elevators:updateState')
AddEventHandler('qpixel-doors:elevators:updateState', function (pElevatorId, pFloorId, pRestricted, pForceUnlock)
...
RegisterInterfaceCallback('qpixel-doors:elevator:teleport', function (data, cb)
    ...
    local finished = exports["qpixel-taskbar"]:taskBar(time, "Waiting for the Elevator", false)
...
RegisterInterfaceCallback('qpixel-doors:elevator:access', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    ...
    TriggerServerEvent('qpixel-doors:change-elevator-state', elevatorId, floorId, not locked)
end)

RegisterInterfaceCallback('qpixel-doors:elevator:hackControlPanel', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })

    local id = data.key.id
    local accessType = data.key.type

    local hackId = RPC.execute('qpixel-doors:startAccessHack', accessType, id)
    ...
    exports['qpixel-interface']:openApplication('memorygame', {
        gameFinishedEndpoint = 'qpixel-doors:elevator:completeHacking',
...
RegisterInterfaceCallback('qpixel-doors:elevator:completeHacking', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })

    local success = data.success
    local hackId = data.parameters.hackId

    RPC.execute('qpixel-doors:completeAccessHack', success, hackId)
