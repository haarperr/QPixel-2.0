...
RegisterNetEvent('qpixel-doors:initial-lock-state')
AddEventHandler('qpixel-doors:initial-lock-state', function(pDoors)
    doors = pDoors
...
RegisterNetEvent('qpixel-doors:change-lock-state')
AddEventHandler('qpixel-doors:change-lock-state', function(pDoorId, pDoorLockState, pDoorForceUnlock)
...
local function listenForKeypress()
    ...
    Citizen.CreateThread(function()
        ...
        if not hasAccess and currentDoorLockState then
            exports["qpixel-interface"]:showInteraction('Locked', 'error')
        end
        ...
        while listening do
            ...
            if currentDoorLockState ~= newLockState then
                if #(GetOffsetFromEntityGivenWorldCoords(PlayerPedId(), currentDoorCoords)) <= 1.2 then
                    ...
                    if hasAccess then
                        exports["qpixel-interface"]:showInteraction(("[E] %s"):format(newLockState and 'Locked' or 'Unlocked'), newLockState and 'error' or 'success')
                    else
                        exports["qpixel-interface"]:showInteraction((newLockState and 'Locked' or 'Unlocked'), newLockState and 'error' or 'success')
                    end
                else
                    idle = 100
                end
            ...
            if currentDoorId ~= nil and hasAccess and IsControlJustReleased(0, 38) and #(GetOffsetFromEntityGivenWorldCoords(PlayerPedId(), currentDoorCoords)) <= 1.2 then
                ...
                TriggerServerEvent("qpixel-doors:change-lock-state", currentDoorId, not currentDoorLockState)
            end

            Wait(idle)
        end

        exports["qpixel-interface"]:hideInteraction((not hasAccess or newLockState) and 'error' or 'success')
    end)
end
...
AddEventHandler("qpixel-doors:doorKeyFob", function()
    ...
    TriggerServerEvent("qpixel-doors:change-lock-state", doorId, isBollard and (not isLocked and 6 or 0) or (not isLocked))
end)

Citizen.CreateThread(function()
    exports["qpixel-polyzone"]:AddBoxZone("mrpd_bollards_01", vector3(411.66, -1027.95, 29.24), 7.8, 23.4, {
        heading=0,
        minZ=28.14,
        maxZ=32.14
    })
    exports["qpixel-polyzone"]:AddBoxZone("mrpd_bollards_02", vector3(411.66, -1020.09, 29.34), 7.8, 23.4, {
        heading=0,
        minZ=28.14,
        maxZ=32.14
    })
    TriggerServerEvent("qpixel-doors:request-lock-state")
end)

RegisterCommand("addnewdoor", function()
    local pEntity = exports['qpixel-target']:GetEntityPlayerIsLookingAt(10.0, 2.0, 16)
    TriggerServerEvent("qpixel-door:add", GetEntityCoords(pEntity), GetEntityModel(pEntity))
end, false)

RegisterNetEvent("qpixel-doors:add")
AddEventHandler("qpixel-doors:add", function(pArgs, pEntity, pContext)
...
AddEventHandler("qpixel-doors:detCoord", function ()
  ...
  Citizen.CreateThread(function()
    ...
    ClearPedTasks(ped) 
    DeleteObject(bomba)
    TriggerServerEvent("qpixel-doors:change-lock-state", targetDoor, 0)
