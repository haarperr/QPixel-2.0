...
RegisterNetEvent('qpixel-doors:request-lock-state')
AddEventHandler('qpixel-doors:request-lock-state', function()
    local src = source
    TriggerClientEvent('qpixel-doors:initial-lock-state', src, doors)
end)

RegisterNetEvent('qpixel-doors:change-lock-state')
AddEventHandler('qpixel-doors:change-lock-state', function(pDoorId, pDoorLockState)
    if doors[pDoorId] then
        doors[pDoorId].lock = pDoorLockState
        TriggerClientEvent('qpixel-doors:change-lock-state', -1, pDoorId, pDoorLockState)
    end
end)
...
-- RegisterNetEvent("qpixel-doors:save-config")
-- AddEventHandler("qpixel-doors:save-config", function(pDoorData)
