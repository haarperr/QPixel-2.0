...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if isNear then
            TriggerServerEvent('qpixel-paleto:getGetDoorStateSV')
...
RegisterNetEvent("qpixel-paleto:getDoorCheckCL")
AddEventHandler("qpixel-paleto:getDoorCheckCL", function(result)
...
RegisterNetEvent("qpixel-paleto:resetDoorState")
AddEventHandler("qpixel-paleto:resetDoorState", function(name)
    Freeze[name] = 0
end)

RegisterNetEvent("qpixel-paleto:lootup_c")
AddEventHandler("qpixel-paleto:lootup_c", function(var, var2)
    LootCheck[var][var2] = true
end)

RegisterNetEvent("qpixel-paleto:outcome")
AddEventHandler("qpixel-paleto:outcome", function(oc, arg)
    for i = 1, #Check, 1 do
        Check[i] = false
    end
    ...
    if oc then
        Check[arg] = true
        TriggerEvent("qpixel-paleto:startheist", Paleto.Banks[arg], arg)
    elseif not oc then
        TriggerEvent("DoLongHudText", arg, 2)
    end
end)

RegisterNetEvent("qpixel-paleto:startLoot_c")
AddEventHandler("qpixel-paleto:startLoot_c", function(data, name)
    currentname = name
            ...
            while true do
                ...
                if dst < 40 then
                    ...
                    if LootCheck[name].Stop or (LootCheck[name].Loot) then
                        ...
                        if initiator then
                            TriggerEvent("qpixel-paleto:reset", name, data)
                            return
                        end
                        return
                    end
...
RegisterNetEvent("qpixel-paleto:stopHeist_c")
AddEventHandler("qpixel-paleto:stopHeist_c", function(name)
    LootCheck[name].Stop = true
end)

-- MAIN DOOR UPDATE --

AddEventHandler("qpixel-paleto:freezeDoors", function()
    Citizen.CreateThread(function()
...
RegisterNetEvent("qpixel-paleto:ClosePaletoDoor")
AddEventHandler("qpixel-paleto:ClosePaletoDoor", function()
    local count = 0
    local VaultDoor = GetClosestObjectOfType(-104.8747253418,6472.1274414062,31.621948242188, 25.0, `v_ilev_cbankvauldoor01`, 0, 0, 0)
    repeat
        ...
        Citizen.Wait(10000)
    until count == 900
    TriggerServerEvent('qpixel-paleto:getGetDoorStateSVSV', false)
end)


-- RegisterNetEvent("qpixel-paleto:toggleVault")
...
--             TriggerServerEvent("qpixel-paleto:updateVaultState", key, Doors[key][1].state)
--         elseif state then
--             local obj = GetClosestObjectOfType(Paleto.Banks[key].doors.startloc.x, Paleto.Banks[key].doors.startloc.y, Paleto.Banks[key].doors.startloc.z, 2.0, GetHashKey(Paleto.vaultdoor), false, false, false)
--             local count = 0
...
RegisterNetEvent("qpixel-paleto:OpenPaletoDoor")
AddEventHandler("qpixel-paleto:OpenPaletoDoor", function()
    TriggerServerEvent('qpixel-paleto:getGetDoorStateSVSV', true)
    local count = 0
    local VaultDoor = GetClosestObjectOfType(-104.8747253418, 6472.1274414062, 31.621948242188, 25.0, `v_ilev_cbankvauldoor01`, 0, 0, 0)
    repeat
...
AddEventHandler("qpixel-paleto:reset", function(name, data)
    ...
    TriggerServerEvent("qpixel-paleto:closeDoor")
    -- TriggerServerEvent("qpixel-paleto:toggleVault", name, true)
    TriggerEvent("qpixel-paleto:cleanUp", data, name)
end)


AddEventHandler("qpixel-paleto:startheist", function(data, name)
    disableinput = true
    currentname = name
  --  currentcoords = vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z)
    initiator = true
    ...
    TriggerServerEvent('qpixel-paleto:openDoor')
    TriggerServerEvent("qpixel-paleto:setCooldown", name)
    --TriggerServerEvent("qpixel-paleto:toggleVault", name, false)
    startdstcheck = true
    currentname = name
    SpawnTrolleys(data, name)
end)

-- AddEventHandler("qpixel-paleto:cleanUp", function(data, name)
--     Citizen.Wait(10000)
...
function SpawnTrolleys(data, name)
    ...
    TriggerServerEvent("qpixel-paleto:startLoot", data, name, missionplayers)
    done = false
end
        ...
        if startdstcheck then
            if initiator then
                ...
                if (GetDistanceBetweenCoords(playercoord, currentcoords, true)) > 20 then
                    ...
                    startdstcheck = false
                    TriggerServerEvent("qpixel-paleto:stopHeist", currentname)
                end
            end
            Citizen.Wait(1)
        else
...
RegisterNetEvent('qpixel-paleto:darkmail')
AddEventHandler('qpixel-paleto:darkmail', function()
    Citizen.Wait(1000)
...
RegisterNetEvent('qpixel-paleto:UseBlueLapTop')
AddEventHandler('qpixel-paleto:UseBlueLapTop', function(item)
    local ply = PlayerPedId()
    local plyCoords = GetEntityCoords(ply)
    local p = promise:new()

    Paleto.Banks, doors = RPC.execute("qpixel-paleto:getBanks")
	TriggerEvent("qpixel-paleto:freezeDoors")
    ...
    for k, v in pairs(Paleto.Banks) do
        if not v.onaction then
            ...
            if dist < 1.0 then
                ...
                if exports["qpixel-inventory"]:hasEnoughOfItem("heistlaptop2", 1) then
                    -- if HasItem then
                    ...
                    exports['hacking']:OpenHackingGame(function(Success)
                        if Success then
                            ...
                            Citizen.SetTimeout(timeout, function()
                                TriggerServerEvent("qpixel-paleto:startcheck", k)
                            end)
...
AddEventHandler('paletogate:doorburn', function()
	TriggerServerEvent('qpixel-doors:change-lock-state', 498, false)
end)

RegisterNetEvent('paletogate2:doorburn') -- Doorlock
AddEventHandler('paletogate2:doorburn', function()
	TriggerServerEvent('qpixel-doors:change-lock-state', 517, false)
end)
