QPX.Jobs.CurPlayerJobs = {}

for k,v in pairs(QPX.Jobs.ValidJobs) do
    QPX.Jobs.CurPlayerJobs[k] = {}
end

function QPX.Jobs.IsWhiteListed(self, hexId, characterId, job, callback)
...
function QPX.Jobs.JobExists(self, job)
    return QPX.Jobs.ValidJobs[job] ~= nil
end

function QPX.Jobs.CountJob(self, job)
    if not QPX.Jobs:JobExists(job) then return 0 end

    local count = 0
    for k,v in pairs(QPX.Jobs.CurPlayerJobs[job]) do
        if job == "ems" then
...
function QPX.Jobs.CanBecomeJob(self, user, job, callback)
    ...
    -- if QPX.Jobs.ValidJobs[job].requireDriversLicense and not exports["police"]:CheckLicense(characterId, "Drivers License") then
    ...
    if not hexId or not characterId or not src then callback(false, "Id's don't exist") return end
        if not QPX.Jobs.ValidJobs[job] then callback(false, "Job isn't a valid job") return end
        
        TriggerEvent("qpixel-jobmanager:attemptBecomeJob", src, characterId, function(allowed, reason)
            if not allowed then callback(false, reason) return end
        end)

        if WasEventCanceled() then callback(false) return end

        -- if QPX.Jobs:CountJob(job) < 1 and QPX.Jobs.ValidJobs[job].name == "EMS" then
        --     callback(true)
        --     return
        -- else
        --     callback(false)
        ...
        if QPX.Jobs.ValidJobs[job].whitelisted then
            QPX.Jobs:IsWhiteListed(hexId, characterId, job, function(whiteListed, rank)
        ...
        if QPX.Jobs:JobExists(job) then
            local jobTable = QPX.Jobs.ValidJobs[job]
            if jobTable and jobTable.max then
                if QPX.Jobs:CountJob(job) >= jobTable.max then callback(false, "There are too many employees for this job right now, try again later") return end
            end
        end
        callback(true)
end

function QPX.Jobs.AddWhiteList(self, user, job, rank)
...
function QPX.Jobs.SetJob(self, user, job, notify, callback)
    ...
    QPX.Jobs:CanBecomeJob(user, job, function(allowed, reason, rank) 
        ...
        if oldJob then
            QPX.Jobs.CurPlayerJobs[oldJob][src] = nil
        end

        user:setVar("job", job)
        QPX.Jobs.CurPlayerJobs[job][src] = {rank = rank and rank or 0, lastPayCheck = GetGameTimer(),isWhiteListed = false} 

        local name = QPX.Jobs.ValidJobs[job].name

        TriggerClientEvent("qpixel-jobmanager:playerBecameJob", src, job, name, false)
        TriggerClientEvent("qpixel-jobmanager:playerBecomeEvent", src, job, name, notify)

        if QPX.Jobs:CountJob("trucker") >= 1 then
            TriggerEvent("lscustoms:IsTruckerOnline",true)
        elseif QPX.Jobs:CountJob("trucker") <= 0 then
...
AddEventHandler("playerDropped", function(reason)
    ...
    for j,u in pairs(QPX.Jobs.CurPlayerJobs) do
        for k,s in pairs(u) do
            if k == src then QPX.Jobs.CurPlayerJobs[j][k] = nil end
        end
    end
end)

AddEventHandler("qpixel-base:characterLoaded", function(user, char)
    QPX.Jobs:SetJob(user, "unemployed", false)
end)
...
Citizen.CreateThread(function()
    while true do
        ...
        for job,tbl in pairs(QPX.Jobs.CurPlayerJobs) do
            if QPX.Jobs.ValidJobs[job].paycheck then
                local payCheck = QPX.Jobs.ValidJobs[job].paycheck
                if QPX.Jobs.ValidJobs[job].name == "Police Officer" then
                    payCheck = payCheck + policebonus

                elseif QPX.Jobs.ValidJobs[job].name == "EMS" then
                ...
                for src,data in pairs(tbl) do
                    ...
                    if user then
                        ...
                        if tonumber(curTime) == tonumber(data.lastPayCheck) or tonumber(data.lastPayCheck) >= 480 then
                            ...
                            QPX.Jobs.CurPlayerJobs[job][src].lastPayCheck = curTime
                            TriggerEvent("server:givepayJob", job, math.floor(payCheck), src)
                            --exports["qpixel-log"]:AddLog("Job Pay", user, "User recieved paycheck, amount: " .. tostring(payCheck))
                        else
...
RPC.register("qpixel-jobmanager:jobCount", function(pSource, pJob)
    return QPX.Jobs:CountJob(pJob)
end)
