local LAUNDRY_COORDS = {
  ...
  Citizen.CreateThread(function()
    for _, v in pairs(LAUNDRY_COORDS) do
      exports["qpixel-polytarget"]:AddBoxZone("casino_hotel_laundry_machine", v[1], v[2], v[3], v[4])
    end
    exports["qpixel-interact"]:AddPeekEntryByPolyTarget("casino_hotel_laundry_machine", {{
      id = "casino_laundry_interact",
      label = "Do Laundry",
      icon = "money-bill-alt",
      event = "qpixel-casino:hotel:doLaundry",
    }}, { distance = { radius = 1.5 } })
  end)
  
  AddEventHandler("qpixel-casino:hotel:doLaundry", function(p1, p2, pContext)
    local laundryId = pContext.zones["casino_hotel_laundry_machine"].id
    local coins = exports["qpixel-inventory"]:getItemsOfType("casinogoldcoin", 1, true)
    local coin = coins[1]
    if not coin then
    ...
    local enough = exports["qpixel-inventory"]:hasEnoughOfItem("markedbills",5,false)

    if enough then

    local canLaunder = RPC.execute("qpixel-casino:hotel:canUseLaundry", laundryId, true)
    ...
    RPC.execute("qpixel-casino:hotel:finishup", laundryId , {} )
    end)
else
