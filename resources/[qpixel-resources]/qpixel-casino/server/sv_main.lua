...
RegisterNetEvent("qpixel-casino:casinoPurchaseChips",function(qty)
    local src = source
...
RegisterNetEvent("qpixel-casino:withdraw",function(moneytype,qty)
    local src = source
	local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local quantity = tonumber(qty)
...
RegisterNetEvent("qpixel-casino:purchageMembership",function(cardtype, cid)
    local src = source
	local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
...
RegisterServerEvent('qpixel-luckywheel:getLucky')
AddEventHandler('qpixel-luckywheel:getLucky', function()
    local src = source
    if not isRoll then
        isRoll = true
        ...
        SetTimeout(12000, function()
            ...
            TriggerClientEvent("qpixel-luckywheel:rollFinished", -1)
        end)
        TriggerClientEvent("qpixel-luckywheel:doRoll", -1, _priceIndex)
        --print(_priceIndex)
    end
end)
