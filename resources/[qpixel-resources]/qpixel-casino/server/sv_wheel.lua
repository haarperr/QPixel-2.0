...
RPC.register('qpixel-casino:purchaseMembershipCard',function(pCid)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    if user:getCash() >= 250 then
          user:removeMoney(250)
          TriggerClientEvent('qpixel-casino:purchaseMembership',src)
    end
end)

RegisterServerEvent('qpixel-luckywheel:getLucky')
AddEventHandler('qpixel-luckywheel:getLucky', function(money)
    local src = source
...
RegisterServerEvent("attempt:spin", function()
    local pSrc = source
    if CasinoSpin then
        TriggerClientEvent("qpixel-luckywheel:spinit", pSrc, true)
    else
