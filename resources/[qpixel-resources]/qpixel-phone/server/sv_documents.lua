...
local documents = {}
RPC.register('qpixel-phone:getDocuments', function(pSource, pType)
...
RPC.register("qpixel-phone:shareDocumentPermanent", function(pSource, dID, tCID)
...
RPC.register("qpixel-phone:acceptDocumentReceiveRequest", function(pSource, pDocumentID)
    local doc = MySQL.query.await([[
        SELECT * FROM __documents WHERE id = ?
    ]], {pDocumentID})
...
RPC.register('qpixel-phone:deleteDocument', function(pSource, pId)
    exports.oxmysql:execute("DELETE FROM __documents WHERE id = ?",{pId})
end)

RPC.register('qpixel-phone:updateDocument', function(pSource, pId, pTitle, pText)
...
RPC.register("qpixel-phone:createDocument", function(pSource, pOwner, pTitle, pText, pType)
    MySQL.query.await([[
