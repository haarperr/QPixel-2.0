...
RPC.register("qpixel-phone:getIdleGroup", function(source)
    local src = source
    local idle = {}
...
RPC.register("qpixel-phone:getMyGroup", function(source)
    local src = source
    local idle = {}
    local g = MySQL.query.await([[
        SELECT * FROM job_group
...
RPC.register("qpixel-phone:c_group", function(source)
    local src = source
...
RPC.register("qpixel-phone:j_group", function(source, id)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
...
RPC.register("qpixel-phone:get_groupsMem", function(source, id)
...
RPC.register('qpixel-phone:g_ready', function(source,stat,id)
    local src = source
    local status = stat
    local gid = id
...
RPC.register('qpixel-phone:leave_group', function(source,id)
    local src = source
    local gId = id
...
--         end
--       end
--     --   TriggerLatentClientEvent('mhqpixel-housing:collectHouses', -1, 30000, Houses)
--     end)
-- end)