...
RPC.register("qpixel-phone:addToCurrentBanks", function(pSource, pBank)
...
RPC.register("qpixel-phone:getCurrentBanks", function(pSource)
    return currentBanks
end)

RPC.register("qpixel-phone:requestPlayerToGroup", function(pSource, pCid, pGroupID)
...
RPC.register("qpixel-phone:addPlayerToGroup", function(pSource, pCid, pGroupID, pSenderID)
...
RPC.register('qpixel-phone:removePlayerFromGroup', function(pSource, pCid, pGroupID)
...
RPC.register("qpixel-phone:createGroup", function(pSource)
...
RPC.register('qpixel-phone:deleteGroup', function(pSource, pGroupID)
    local jsonDecodedGroups = json.decode(groupTable)
    table.remove(jsonDecodedGroups, pGroupID)
    groupTable = json.encode(jsonDecodedGroups)
end)

RPC.register('qpixel-phone:getMyGroup', function(pSource, pCid)
    local jsonDecodedGroups = json.decode(groupTable)
    for k,v in pairs(jsonDecodedGroups) do
        local jsonDecodedGroupMembers = json.decode(v.members)
        for j,s in pairs(jsonDecodedGroupMembers) do
