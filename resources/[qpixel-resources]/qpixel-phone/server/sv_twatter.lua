...
AddEventHandler('Tweet', function(handle, data, time)
  ...
  TriggerClientEvent('qpixel-phone:insertTwat', -1, handle,data.text,time,data.attachment)
  -- TriggerClientEvent('Client:UpdateTweet1', src, newtwat,allTweet)
 
end)

RegisterNetEvent('qpixel-phone:twatSendNotification')
AddEventHandler('qpixel-phone:twatSendNotification', function(handle, data, time)

  local newtwat = { ['handle'] = handle, ['message'] = data, ['time'] = time}
  TriggerClientEvent('qpixel-phone:TwatNotify', -1, newtwat)
end)

RegisterNetEvent('qpixel-phone:openGetTweets')
AddEventHandler('qpixel-phone:openGetTweets', function()
  local src = source
  exports.oxmysql:execute('SELECT * FROM (SELECT * FROM tweets ORDER BY `time` DESC LIMIT 50) sub ORDER BY time ASC', {}, function(tweets) -- Get most recent 100 tweets
...
RPC.register('qpixel-phone:getTwatters', function(source)
...
RPC.register("qpixel-phone:twitterhandle", function(pSource)
  local src = source
  local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
