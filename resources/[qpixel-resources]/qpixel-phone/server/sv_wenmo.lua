RegisterNetEvent('qpixel-phone:transferWenmo', function(num,amount,comm)
    local src = source
    local zcid = getIdentifierByPhoneNumber(num)
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(tonumber(src))
...
RPC.register('qpixel-phone:getWenmo', function(source)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(tonumber(src))
