...
RegisterNetEvent('qpixel-phone:fireEmp')
AddEventHandler('qpixel-phone:fireEmp', function(id, job, name)
  local src = source
...
RPC.register("qpixel-phone:chargeCustomerBus", function(pSource, amount, GroupID, stateID, sID)
    ...
    TriggerClientEvent("qpixel-phone:businessRegisterNoti", tSrc, "SERVICE CHARGE", "Payment Successful.")
    TriggerClientEvent("qpixel-phone:businessRegisterNoti", tSrc, "SERVICE CHARGE", "You have been deducted " .. amtTax .. " from you're bank account")
    TriggerClientEvent("qpixel-phone:businessRegisterNoti", sID, "SERVICE CHARGE", "They accepted the payment")
    return
end)
...
RegisterServerEvent('qpixel-phone:business_hired', function(StateID, Rank, pBusiness)
...
RegisterServerEvent('qpixel-phone:business_edit', function(pRank, pStateID, pPassType)
...
RPC.register('qpixel-phone:business_paycheck', function(source,bus,p,a)
    local src = source
    local bId = bus
    local cid = p
    local amount = a
...
function getPlayerFromCid(zcid, amountAdded)
    for _, player in ipairs(GetPlayers()) do
        local user = exports["qpixel-base"]:getModule("Player"):GetUser(tonumber(player))
...
RPC.register("qpixel-business:payExternal", function(pSource, pCid, pAmount, pBusinessID)
...
RPC.register('qpixel-phone:bus_addBank', function(source,id,a)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local ply = user:getCurrentCharacter()
    local bId = id
...
RegisterServerEvent('qpixel-phone:business_fire', function(pBusinessName, pStateID)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
