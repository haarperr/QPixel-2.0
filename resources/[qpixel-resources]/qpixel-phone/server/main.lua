...
RPC.register("qpixel-phone:getPhoneNumber", function(pSource)
...
RPC.register("qpixel-phone:sendThePM", function(src, msg)
...
RPC.register("acceptBusinessCharge", function(pSource, pBID, pAmt, pCID)
    ...
    local user = exports['qpixel-base']:getModule("Player"):GetUser(SID)
    local char = user:getCurrentCharacter()
...
RegisterNetEvent("qpixel-phone:h-Lock:sv", function(hid, status)
    print("HELLO? " .. " " .. hid .. " " ..status)
    exports.oxmysql:execute("UPDATE housing SET status = ? WHERE hid = ?",{status,hid})
end)

RPC.register("qpixel-financials:getCurrentBank", function(pSource)
    local user = exports['qpixel-base']:getModule("Player"):GetUser(pSource)
    local char = user:getCurrentCharacter()
    local cid = char.id
...
RPC.register("qpixel-apartments:getCurrentApartment", function(src)
...
function getOrGeneratePhoneNumber (sourcePlayer, identifier, cb)
  ...
  local user = exports["qpixel-base"]:getModule("Player"):GetUser(sourcePlayer)
...
AddEventHandler('racing-join-race', function(identifier)
  local src = source
  local player = exports["qpixel-base"]:getModule("Player"):GetUser(src)
...
--     local src = source
--     local user = exports["qpixel-base"]:getModule("Player"):GetUser(tonumber(id))
--     local char = user:getCurrentCharacter()
  ...
  exports.oxmysql:execute("SELECT * FROM character_passes WHERE cid = @cid", {['cid'] = ply.id}, function(result)
      if result[1] ~= nil then
          ...
          if result[1].pass_type == "police" or result[1].pass_type == "ems" then
              ...
              TriggerClientEvent("qpixel-jobmanager:playerBecameJob", src, result[1].pass_type, result[1].pass_type, false)
          end
      end
  end)
end)
...
RegisterServerEvent("qpixel-phone:grabWallpaper")
AddEventHandler("qpixel-phone:grabWallpaper", function()
  ...
  local cid = char.id
    exports.oxmysql:execute("SELECT `wallpaper` FROM characters WHERE id = @id",{['id'] = cid}, function(data)
      if data[1].wallpaper ~= nil then
          TriggerClientEvent('qpixel-phone:grabBackground', src, data[1].wallpaper)
        --   print('Grabbbing',data[1].wallpaper)
      else
          TriggerClientEvent('qpixel-phone:grabBackground', src, "https://i.imgur.com/JVPavDg.jpg")
        --   print('Not Grabbbing')
      end
    end)
end)

RegisterServerEvent("qpixel-phone:GiveCrypto")
AddEventHandler("qpixel-phone:GiveCrypto", function(type,amount)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("qpixel-phone:getChips", function(pSource)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("qpixel-phone:getGroupSanitation", function(pSource)
    local src = source
    -- local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    -- local char = user:getVar("character")
    -- local id = char.id
...
RPC.register("qpixel-phone:incomingCrypto", function(pSource)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
...
RPC.register("qpixel-phone:blueprintid", function(pSource)
    return BPid
end)

RPC.register("qpixel-phone:exchangeCrypto", function(pSource,type,amount)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("qpixel-phone:transferCrypto", function(pSource,type,amount,target)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("qpixel-phone:GetShungite", function (pSource)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("qpixel-phone:GetTgb", function (pSource)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("qpixel-phone:GetDvd", function (pSource)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("qpixel-phone:buyCrypto", function (pSource,ctype,amount)
    local src = source
    local amount = amount
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
...
RPC.register("qpixel-phone:purchaseDarkMarket", function (pSource,type,price)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local id = char.id
...
RPC.register('qpixel-phone:getMyGroup', function(cid)
...
RPC.register('qpixel-phone:getHouses', function(src)
...
RPC.register('qpixel-phone:apt', function(source)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
...
RPC.register('qpixel-phone:getCurrentOwned', function(source)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
    local house
...
RegisterNetEvent('qpixel-phone:house:status')
AddEventHandler('qpixel-phone:house:status', function(hid,status)
     local src = source
...
RPC.register("qpixel-housing:getKeyHolderz", function(src,hid)
    print(src,hid)
    local myKeyz = {}
...
RPC.register("qpixel-housing:removeKey:phone", function(src,cid,hid)
    local keys = MySQL.query.await([[
...
RPC.register('qpixel-phone:getHousePrice', function(source,hid)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local hIds = hid
...
RPC.register('qpixel-phone:GiveAccess', function(source,hid,cid)
    local src = source
    local hIds = hid
    local cid = cid
    local access
...
-- RPC.register('qpixel-phone:getAccessHouse', function(source)
--     local src = source
--     local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
--     local cid = user:getCurrentCharacter().id
--     local house
...
RPC.register('qpixel-phone:getAccessHouse', function(source,house)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
    local hid = house
...
RPC.register('qpixel-phone:getAccessHouse_2', function(house)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
    local hid = house
...
RPC.register("qpixel-phone:addDOJ", function(pSource, pJob)
    local user = exports['qpixel-base']:getModule("Player"):GetUser(pSource)
    local char = user:getCurrentCharacter()
    local cid = char.id
...
RPC.register("qpixel-phone:removeDOJ", function(pSource, pJob)
    local user = exports['qpixel-base']:getModule("Player"):GetUser(pSource)
    local char = user:getCurrentCharacter()
    local cid = char.id
...
RPC.register("qpixel-phone:getDOJ", function(src)
    return availableDOJ
end)

local availableDrivers = {}

RPC.register("qpixel-phone:addAbdulTaxi", function(src)
    local user = exports['qpixel-base']:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
    local phonenumber = char.phone_number
...
RPC.register("qpixel-phone:getAbdulTaxies", function(src)
    return availableDrivers
end)

RegisterNetEvent("qpixel-phone:getAbdulTaxies", function()
    TriggerClientEvent("qpixel-phone:manualUpdateAbdulTaxies", source)
end)

RPC.register("qpixel-phone:removeAbdulTaxi", function(src)
    local user = exports['qpixel-base']:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
...
RPC.register("qpixel-phone:removeKeys", function(src, pHouseId, pPlayerId)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
...
-- RPC.register("qpixel-phone:Groups", function(source,gIp)
--     local src = source
...
RegisterServerEvent('qpixel-phone:sendVehBill')
AddEventHandler('qpixel-phone:sendVehBill', function(paypal,plate,amount)
    src = source
    TriggerClientEvent('qpixel-phone:sellCar', paypal, plate, amount,src)
end)


RPC.register('qpixel-phone:sellVehicle', function(pSource,pl,py,am)
    local src = pSource
    local plate = pl
    local paypal = py
    local amount = am
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local player = exports["qpixel-base"]:getModule("Player"):GetUser(paypal)
    local ply = user:getCurrentCharacter()
...
RegisterNetEvent('qpixel-phone:send_gas_bill')
AddEventHandler('qpixel-phone:send_gas_bill', function(targetId, pFuelCost)
    local pSrc = source
    
	if targetId ~= nil then
        TriggerClientEvent('qpixel-hud:send_fuel_bill', targetId, pFuelCost, pSrc)
    end
end)
