...
RPC.register('qpixel-phone:debt', function(source)
    local src = source
...
function buildInfo(house)
    ...
    for k,v in pairs(house) do
        ...
        local street = exports['qpixel-housing']:HouseStreet(v.hid)
...
RPC.register('qpixel-phone:debtPrice', function(source, id)
    local src = source
    local hId = id
    return getPrice(hId)
end)
...
RPC.register('qpixel-phone:car_payment', function(source, pl)
    local src = source
    local plate = pl
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local status
...
RPC.register('qpixel-phone:h_payment', function(source,id,pay)
    local src = source
    local hId = id
    local payment = pay
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
...
RPC.register('qpixel-phone:p_payment', function(source,id,p,did)
    local src = source
    local pId = id
    local dId = did
    local payment = p
...
RPC.register('qpixel-phone:addBill', function(source,id,a,t,c)
    local src = source
    local cid = id
    local amount = a
    local type = t
