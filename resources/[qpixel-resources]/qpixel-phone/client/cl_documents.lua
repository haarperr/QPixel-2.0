RegisterNUICallback("getDocuments", function(data, cb)
    SendNUIMessage({
        ...
        documentData = RPC.execute("qpixel-phone:getDocuments", data.pType)
    })
end)

RegisterNUICallback("deleteDocument", function(data, cb)
    RPC.execute("qpixel-phone:deleteDocument", data.documentID)
end)

RegisterNUICallback("updateDocument", function(data, cb)
    RPC.execute("qpixel-phone:updateDocument", data.documentID, data.documentTitle, data.documentText)
end)

RegisterNUICallback("createDocument", function(data, cb)
    RPC.execute("qpixel-phone:createDocument", exports['isPed']:isPed("cid"), "New Document", "Text Goes Here", data.documentType)
end)

RegisterNUICallback("shareDocumentPermanent", function(data)
    RPC.execute("qpixel-phone:shareDocumentPermanent", data.documentID, data.targetCID)
end)
...
RegisterNUICallback("acceptDocumentReceiveRequest", function(data)
    local pDocShared = RPC.execute("qpixel-phone:acceptDocumentReceiveRequest", data.documentID)
    SendNUIMessage({
