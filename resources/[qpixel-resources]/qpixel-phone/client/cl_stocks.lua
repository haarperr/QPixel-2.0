...
RegisterNUICallback('buying-darkMarket', function()
  SendNUIMessage({
    openSection = "buydarkMarket", materials = exports['qpixel-phone']:getBuying(), blueprintid = RPC.execute("qpixel-phone:blueprintid")
  })
end)
...
RegisterNUICallback('stocksTradeToPlayer', function(data, cb)
  ...
  if data.identifier == "Shungite" then
      status = RPC.execute("qpixel-phone:buyCrypto",1,data.amount)
    if status then
  ...
  elseif data.identifier == "Guinea" then
    status = RPC.execute("qpixel-phone:buyCrypto",2,data.amount)
    if status then
  ...
  elseif data.identifier == "DVD Prime" then
    status = RPC.execute("qpixel-phone:buyCrypto",3,data.amount)
    if status then
...
RegisterNUICallback('exchangeCrypto', function(e)
  ...
  if playerid == characterId then
    RPC.execute("qpixel-phone:exchangeCrypto",popo,amount)
  else
    RPC.execute("qpixel-phone:transferCrypto",popo,amount,playerid)
  end
...
RegisterNUICallback('buyCrypto', function(e)
  ...
  elseif e.stockID == "DVD Prime" then
    ...
    RPC.execute("qpixel-phone:buyCrypto",popo,amount)
    sendStocksToPhone()
end)
...
function sendStocksToPhone(isRefresh)
  ...
  if isRefresh then
    ...
    serverstockvalues = RPC.execute("qpixel-phone:incomingCrypto")
  else
    serverstockvalues = RPC.execute("qpixel-phone:incomingCrypto")
  end

  Shungitelol = RPC.execute("qpixel-phone:GetShungite")
  Tgblol = RPC.execute("qpixel-phone:GetTgb")
  Dvdlol = RPC.execute("qpixel-phone:GetDvd")
...
RegisterNUICallback('purchase-darkMarket', function(data)
    ...
    local colorLaptop = exports['qpixel-phone']:getBuying()
    local time = math.random(60000,120000)

    if not orderItem then
      if data.item == "vpn" then
        result = RPC.execute('qpixel-phone:purchaseDarkMarket',1,20)
        print(result)
          if result then
            itemDM[#itemDM+1] = {
              itemDM = "vpnxj"
      ...
      elseif data.item == "deliverylist" then
        result = RPC.execute('qpixel-phone:purchaseDarkMarket',1,10)
          if result then
            itemDM[#itemDM+1] = {
        ...
        elseif data.item == "choplist" then
          result = RPC.execute('qpixel-phone:purchaseDarkMarket',2,20)
            if result then
              itemDM[#itemDM+1] = {
                itemDM = "choplist"
              }
      ...
      elseif data.item == "racingusb2" then
        result = RPC.execute('qpixel-phone:purchaseDarkMarket',2,50)
          if result then
            itemDM[#itemDM+1] = {
              itemDM = "racingusb2"
            }
      ...
      elseif data.item == "boostlaptop" then
        -- result = RPC.execute('qpixel-phone:purchaseDarkMarket',1,100)
        -- if result then
      ...
      elseif data.item == "laptopred" then
        result = RPC.execute('qpixel-phone:purchaseDarkMarket',1,65)
        if result then
          itemDM[#itemDM+1] = {
            itemDM = "heistlaptop4"
          }
      ...
      elseif data.item == "laptopgreen" then
        result = RPC.execute('qpixel-phone:purchaseDarkMarket',1,50)
          if result then
            itemDM[#itemDM+1] = {
              itemDM = "heistlaptop3"
            }
...
RegisterNetEvent('qpixel-phone:takeDMarket')
AddEventHandler('qpixel-phone:takeDMarket', function()
  if orderItem then
