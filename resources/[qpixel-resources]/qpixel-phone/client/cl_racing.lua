...
RegisterNUICallback('racing:event:start', function(data)
  local canJoinOrErr = exports["qpixel-racing"]:canJoinOrStartRace()
  if canJoinOrErr ~= true then
    exports["qpixel-phone"]:phoneNotification("racen",canJoinOrErr,"RACING", 3000)
    return
  end
  print(data.id, json.encode(data))
    exports["qpixel-racing"]:createPendingRace(data.id, data)
    Wait(100)
    SendNUIMessage({
...
RegisterNUICallback("racing:event:join", function(data)
  ...
  exports["qpixel-racing"]:joinRace(id, alias, cid)
  Wait(100)
  SendNUIMessage({
    openSection = "racing:event:update",
    canJoin = false,
...
RegisterNetEvent("qpixel-racing:removedActiveRaced")
AddEventHandler("qpixel-racing:removedActiveRaced", function(id)
  -- print("PUTANG INA")
    openRacingApp()
...
function openRacingApp()
  ...
  local alias = RPC.execute("GetAlias")
  print(alias)
  local _races = exports["qpixel-racing"]:getAllRaces()
...
AddEventHandler("qpixel-racing:api:updatedState", function()
...
RegisterNUICallback("racing:map:save", function(data)
  if creating then
      ...
      if veh ~= 0 and GetPedInVehicleSeat(veh, -1) == ped then
          ...
          TriggerEvent("qpixel-racing:cmd:racecreate", options)
          closeGui()
...
RegisterNUICallback('racing:map:cancel', function()
  creating = false
  TriggerEvent('qpixel-racing:cmd:racecreatecancel')
  openRacingApp()
end)
...
RegisterNUICallback('qpixel-phone:racing:setGPS', function(data)
  local mapID = data.mID
  exports["qpixel-racing"]:locateRace(mapID)
  phoneNotification("racen","GPS Set to your map.","RACING")
end)

RegisterNUICallback('qpixel-phone:racing:seePreview', function(data)
  local mapID = data.mID
  if not mapID then return end
  id = tostring(mapID)
  exports["qpixel-racing"]:previewRace(mapID)
...
RegisterNUICallback('qpixel-phone:r-users', function(data, cb)
  ...
  if race ~= nil then
    ...
    local racer = RPC.execute('qpixel-phone:racing:getRacer', raceID)
    cb(racer)
  end
  print(json.encode(newPendingRace[raceID]))
end)

AddEventHandler("qpixel-racing:api:startingRace", function(startTime)
  TriggerEvent("DoLongHudText", "Starting race in " .. tostring(startTime / 1000) .. " seconds")
end)

AddEventHandler("qpixel-racing:api:playerJoinedYourRace", function(characterId, name)
  TriggerEvent("chatMessage", "", {255, 0, 0}, "^1" .. name .. " joined your race")
end)

AddEventHandler("qpixel-racing:api:playerLeftYourRace", function(characterId, name)
  TriggerEvent("chatMessage", "", {255, 0, 0}, "^1" .. name .. " left your race")
end)

RegisterNetEvent('qpixel-racing:pendingRacenotif')
AddEventHandler("qpixel-racing:pendingRacenotif", function(race)
  ...
  exports["qpixel-phone"]:phoneNotification("racen","Pending race available...","From the PM", 12000)
end)
-- Citizen.CreateThread(function()
--   while true do
--     Wait(5000)
