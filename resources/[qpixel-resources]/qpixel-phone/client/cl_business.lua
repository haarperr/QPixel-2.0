RegisterNUICallback('addEmployee', function(data)
  RPC.execute("qpixel-business:business:hireEmployee",data.state_id,data.rank,data.business)
end)

RegisterNUICallback('chargeCustomer', function(data)
  local success = RPC.execute('qpixel-phone:sendChargeCustomerNotif',data.GroupName, data.GroupID, data.stateID, data.amount, data.reason)
end)

RegisterNUICallback('editEmployee', function(data)
  RPC.execute("qpixel-business:phone:updateEmployee", data.state_id, data.business, data.rank)
end)

RegisterNetEvent("qpixel-phone:chargeCustomerNotif", function(data)
...
RegisterNUICallback('acceptBusinessCharge', function(data)
    RPC.execute("qpixel-phone:chargeCustomerBus", data.pAmount, data.pBID, data.pTargetID, data.pSenderID)
end)

RegisterNUICallback('fireEmployee', function(data)
    RPC.execute("qpixel-business:business:fireEmployee", data.state_id, data.business)
end)
...
RegisterNUICallback('payCheckEmployee', function(data)
    ...
    local complete, paid = RPC.execute('qpixel-phone:business_paycheck', business,cid,amount)
    if complete then
...
RegisterNUICallback('btnTaskGroups', function()
  ...
  local mypasses = RPC.execute("qpixel-business:phone:GetEmploymentInformation")
  local groupObject = {}

  for k,v in pairs(mypasses) do 
    local myRoles = RPC.execute("qpixel-business:phone:GetRolePermission", v.businessid, v.businessrole)
  ...
  AddEventHandler("group:fullList", function(bank,groupid)
    ...
    local fullGroupList = {}
    local peniashiut = RPC.execute("qpixel-business:hasPermission", groupid)
    local groupseemployyees = RPC.execute("qpixel-businesses:getRoleAndEmployeesList", groupid)
  ...
  RegisterNUICallback('payExternal', function(data)
    local typee
    local success, msg = RPC.execute('qpixel-business:payExternal', data.pStateId, data.pAmount, data.pBusinessID)
    if success then
      typee = 1
    else
      typee = 2
  ...
  RegisterNUICallback('deleteRole', function(data)
    RPC.execute("qpixel-business:business:deleteRole", data.pBusinessID, data.pRole)
  end)

  RegisterNUICallback('createBusinessRole', function(data)
    RPC.execute("qpixel-business:business:createRole", data.pBusinessId, data.pRoleName, data.pPermHire, data.pPermFire, data.pPermChangeRoles, data.pPermKeys, data.pPermStash, data.pPermCraft, data.pPermCharge, data.pPermBank, data.pPermLogs)  
  end)

  RegisterNUICallback("editBusinessRole", function(data)
    RPC.execute("qpixel-business:business:editRole", data.pBusinessId, data.pCurRole, data.pRoleName, data.pPermFire, data.pPermHire, data.pPermChangeRoles, data.pPermKeys, data.pPermStash, data.pPermCraft, data.pPermCharge, data.pPermBank, data.pPermLogs)
  end)
  ...
  RegisterNUICallback('retreiveGroupRanks', function(data)
    ...
    local roles = RPC.execute("qpixel-business:business:GetBusinessRoles", data.bID)
  ...
  RegisterNUICallback('qpixel-phone:fireEmp', function(data,cb)
    TriggerServerEvent('qpixel-phone:fireEmp', data.id,data.group,data.name)
  end)

  RegisterNUICallback('addCashBank', function(data)
    local bid = data.bid
    local amount = data.amount
    local bank = RPC.execute("qpixel-phone:bus_addBank",bid,amount)
    if bank then
