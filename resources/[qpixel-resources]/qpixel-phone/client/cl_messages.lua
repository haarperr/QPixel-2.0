...
function loadSMS(cid)
  print("LOAD SMS")
  local messages, phone = RPC.execute("qpixel-phone:getSMS",cid)
  local _messages = {}
...
function loadSMSBurner(cid)
  print("LOAD SMS")
  local messages, phone = RPC.execute("qpixel-phone:getTextsFromNumber",cid)
  local _messages = {}
  for i, v in ipairs(messages) do
      local name = ""
...
local data = message
local app = "sms"
if exports["qpixel-phone"]:phasPhone() then
  if exports['qpixel-phone']:pNotify() == true then
