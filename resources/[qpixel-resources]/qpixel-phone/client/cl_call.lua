...
RegisterNUICallback('getCallHistory', function()
  local recentCallHistory = RPC.execute("qpixel-phone:getRecentCallsByCid", exports['isPed']:isPed("cid"))
  SendNUIMessage({
...
Citizen.CreateThread(function()
  while true do
    Wait(10000)
    TriggerServerEvent("qpixel-phone:sendRecentCalls", recentcalls, exports['isPed']:isPed("cid"))
  end
end)
...
AddEventHandler('phone:otherClientEndCall', function()
  ...
  Citizen.CreateThread(function()
    exports["qpixel-keybinds"]:registerKeyMapping("", "Player", "Hangup Call", "+hangup", "-hangup", "5")
    exports["qpixel-keybinds"]:registerKeyMapping("", "Player", "Answer Call", "+answer", "-answer", "6")
  end)
  ...
  AddEventHandler('phone:addToCall', function(voipchannel)
    exports['qpixel-voice']:addPlayerToCall(tonumber(voipchannel))
  end)
...
RegisterNetEvent('qpixel-phone:RemoveCall')
AddEventHandler('qpixel-phone:RemoveCall', function()
    SendNUIMessage({
      openSection = 'callnotifyEnd'
    })
    SendNUIMessage({
...
AddEventHandler('phone:removefromToko', function(playerRadioChannel)
  exports['qpixel-voice']:removePlayerFromCall()
end)
...
function endCall()
  ...
  mySourceID = 0
  TriggerEvent('phone:setCallState', isNotInCall)
  TriggerEvent('qpixel-phone:RemoveCall')
  onhold = false
  mySourceHoldStatus = false
  callid = 0
end
...
function checkForPayPhone()
  ...
  local ph = exports["qpixel-applications"]:KeyboardInput({
    header = "Payphone",
    rows = {
      {
        id = 1,
