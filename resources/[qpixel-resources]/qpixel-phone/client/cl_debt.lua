RegisterNUICallback('btnDebt', function()
    local cars, debt = RPC.execute('qpixel-phone:debt')
    SendNUIMessage({
        openSection = "debt",
        cars = cars,
        debt = debt
...
RegisterNUICallback('car_paid', function(data)
    ...
    local complete = RPC.execute('qpixel-phone:car_payment', plate,payment)
    if complete then
...
RegisterNUICallback('h_paid', function(data)
    local id = data.hid
    local payment = data.payment
    local complete = RPC.execute('qpixel-phone:h_payment', id,payment)
    if complete then
...
RegisterNUICallback('p_paid', function(data)
    ...
    local payment = data.payment
    local did = data.did
    local complete = RPC.execute('qpixel-phone:p_payment',id,payment,did)
    if complete then
...
RegisterNetEvent('qpixel-phone:MakeBill', function()
    ...
    if input["cid"] ~= nil and input["amount"] ~= nil then
        ...
        local complete = RPC.execute("qpixel-phone:addBill",cid,amount,type,comment)
        if complete then
