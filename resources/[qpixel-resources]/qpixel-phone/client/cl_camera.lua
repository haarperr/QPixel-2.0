RegisterNUICallback("getCurrentCameras", function(data,cb)
    local cameraTable = {}
    local result = RPC.execute("qpixel-gopros:getSecurityCameraByStateID", exports['isPed']:isPed("cid"))
...
RegisterNUICallback("addUserToCamera", function(data,cb)
    RPC.execute("qpixel-gopros:addUserToCamera", data.cameraId, data.targetPhoneNumber)
end)

RegisterNUICallback("removeUserFromCamera", function(data,cb)
    RPC.execute("qpixel-gopros:removeUserFromCamera", data.cameraId, data.targetPhoneNumber)
end)

RegisterNUICallback("getCameraAccessPlayers", function(data, cb)
    local access = RPC.execute("qpixel-gopros:getSecurityCameraAccessByCameraID", data.cameraId)
    SendNUIMessage({
...
RegisterNUICallback("OpenCamera", function(data,cb)
    ...
    exports['qpixel-gopro']:activateSecurityCameraById(data.cameraId)
end)