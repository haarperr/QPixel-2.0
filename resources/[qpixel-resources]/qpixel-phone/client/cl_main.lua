...
RegisterNetEvent('qpixel-base:playerSpawned')
AddEventHandler('qpixel-base:playerSpawned', function()
  Wait(1000)
...
RegisterCommand('renten', function()
  TriggerEvent('qpixel-housing:rent_activate_cl')
end)

RegisterNetEvent('qpixel-housing:rent_activate_cl')
AddEventHandler('qpixel-housing:rent_activate_cl', function()
...
AddEventHandler('qpixel-phone:sanitationIn', function()
  -- print("SANITATION SIGN IN")
  sanitation = true
end)

RegisterNetEvent("qpixel-phone:grabBackground")
AddEventHandler("qpixel-phone:grabBackground", function(link)
    wallPaper = link
end)
...
RegisterNUICallback('wallpaper', function(data, cb)
  ...
  TriggerEvent('qpixel-phone:grabBackground', wallPaperSelecionado)
...
AddEventHandler('phone:reset', function(cidsent)
    ...
    wallPaper = ""
    Wait(1)
    TriggerServerEvent('qpixel-phone:grabWallpaper')
end)

RegisterNUICallback('sendThePM', function(data, cb)
  local thePM = data.thePM
  RPC.execute("qpixel-phone:sendThePM", thePM)
end)

RegisterCommand("fuckmeintheasdasvf", function()
  RPC.execute("qpixel-phone:sendThePM", 'f')
end)
...
RegisterNUICallback('submitSettings', function(data, cb)
  ...
  Wait(5)
  if bg ~= "" then
    TriggerEvent('qpixel-phone:grabBackground', bg)
...
RegisterNetEvent("qpixel-phone:setSomeShit", function(bg, model, twatNotify, callNotify)
  SetResourceKvp(exports['isPed']:isPed('cid').."-bg", bg)
  SetResourceKvpInt(exports['isPed']:isPed('cid').."-pmodel", model)
  SetResourceKvpInt(exports['isPed']:isPed('cid').."-ntwat", twatNotify)
  SetResourceKvpInt(exports['isPed']:isPed('cid').."-ncall", callNotify)
...
RegisterNUICallback('btnHousing', function()
  ...
  allNewHouses = {}
  myAccessHouse = {}
  --local apt = RPC.execute("qpixel-phone:apt")
  local housing = exports['qpixel-housing']:housingForPhone()
  local apartment = exports['qpixel-apartments']:getMyCurrentApartment()
  myHousess = RPC.execute("qpixel-phone:getCurrentOwned")
  accessHouse = RPC.execute('qpixel-phone:getAccessHouse_2')
  ...
  for i,v in pairs(myHousess) do
    ...
    table.insert(newHouses, {
      ...
      cid = v.cid,
      status = v.status,
      cat = exports['qpixel-housing']:getHousingCatFromPropertID(v.hid),
    })
  end
  for l,s in pairs(accessHouse) do
      table.insert(myAccessHouse, {
        ...
        cid = s.cid,
        status = s.status,
        cat = exports['qpixel-housing']:getHousingCatFromPropertID(s.house),
      })
  end
  SendNUIMessage({
    openSection = "housing",
...
RegisterNUICallback('btnKeyHolders', function(data, cb)
  local myKittens = {}
  print(data.hid)
  local keys = RPC.execute('qpixel-housing:getKeyHolderz', data.hid)
  SendNUIMessage({
...
RegisterNUICallback('whatthefuckdoesthisshitwork', function(data,cb)
  local cid = data.cid
  local hid = data.hid
  local success,msg = RPC.execute('qpixel-housing:removeKey:phone',cid, hid)
  if success then
...
RegisterNUICallback('OpenCamera', function(data,cb)
  RPC.execute('qpixel-phone:openCamera', data.cameraId)
end)
...
--   while true do
--     Wait(3000)
--     local myHouse = RPC.execute("qpixel-phone:getCurrentOwned")
...
RegisterInterfaceCallback("qpixel-phone:signInHandler", function(data, cb)
  if data.key.sign_in then
    RPC.execute("qpixel-phone:addDOJ", data.key.job)
  else
    RPC.execute("qpixel-phone:removeDOJ", data.key.job)
  end
...
RegisterNUICallback('getActiveDOJ', function(data,cb)
      SendNUIMessage({
        ...
        penisinmybuttholefuckshit = RPC.execute("qpixel-phone:getDOJ")
      })
end)
...
RegisterNUICallback('abdulSignIn', function(data,cb)
    ...
    if taxiStatus then
      RPC.execute("qpixel-phone:addAbdulTaxi")
      addAbdulTaxi()
    ...
    elseif not taxiStatus then
      RPC.execute("qpixel-phone:removeAbdulTaxi")
      removeAbdulTaxi()
      local availableDrivers = getAbdulTaxi()
      SendNUIMessage({
        openSection = "addTaxi",
...
RegisterNetEvent("qpixel-phone:manualUpdateAbdulTaxies", function(availableDrivers)
  addAbdulTaxi()
  local availableDrivers = getAbdulTaxi()
  SendNUIMessage({
    openSection = "addTaxi",
...
function addAbdulTaxi () 
  currenttable = {}
  local table = RPC.execute("qpixel-phone:getAbdulTaxies")
--  local currenttable = {}
...
RegisterNUICallback('sellVeh', function(data)
  ...
  TriggerServerEvent('qpixel-phone:sendVehBill', data.paypal, data.plate, data.amount)
end)

RegisterNetEvent('qpixel-phone:sellCar')
...
RegisterNUICallback('purchase_gas', function(data)
  TriggerEvent('qpixel-fuel:OfferAccepted')
end)

RegisterNetEvent('qpixel-hud:send_fuel_bill', function(pCost)
...
function SpawnVehicle(vehicle, x,y,z, Fuel, customized, plate, IsViewing, enginehealth, bodyhealth)
  if DoesEntityExist(spawningVeh) then
	...
	Citizen.CreateThread(function()			
        ...
        exports['qpixel-vehicles']:SetVehicleIdentifier(spawningVeh, vid)
        ...
        if not IsViewing then    
            ...
            RPC.execute("qpixel-garages:states", "Out", plate, exports['qpixel-menu']:currentGarage(), pUpdatedFuel)
        end
    end)
end
...
RegisterNUICallback('sethGps', function(data)
 local house = exports["qpixel-housing"]:retrieveHousingTableMapped()
...
RegisterNUICallback('unlockHouses', function(data)
  ...
  if statuus == "Unlock" then
    --TriggerEvent('qpixel-phone:h-Lock', hId,"Lock")
  else
    --TriggerEvent('qpixel-phone:h-Lock', hId,"Unlock")
  end
  TriggerServerEvent('qpixel-phone:house:status',hId,statuus)
  Wait(200)
  TriggerEvent('updateUIHousing')
end)

RegisterNetEvent("qpixel-phone:h-Lock", function(hId, status)
  TriggerServerEvent("qpixel-phone:h-Lock:sv", hId, status)
...
RegisterNUICallback('editHousingOwn', function()
  -- if exports['qpixel-housing']:isNearProperty() then
    if exports["qpixel-housing"]:canEdit() then
      print("HELLO!")
      local isComplete, _propertyID, dist, zone = exports['qpixel-housing']:findClosetedPerson()
      TriggerEvent("qpixel-housing:editOptions",{action = "enter", propertyId = _propertyID})
    else
      SendNUIMessage({
...
RegisterNUICallback('sellOwnHouse', function(data)
  local price = RPC.execute('qpixel-phone:getHousePrice', data.hid)
end)

RegisterNUICallback('giveAccess', function(data)
  local complete = RPC.execute('qpixel-phone:GiveAccess', data.hid,data.cid)
...
RegisterNUICallback('houseAccess', function(data)
  ...
  local access = RPC.execute('qpixel-phone:getAccessHouse', data.hid)
  local data = {
      {
        title = "Close",
        action = "qpixel-housing:close_access",
        key = {},
    },
  }
  local player = {}
  for i,k in pairs(access) do
    data[#data + 1] = {
          ...
          key = {"lockdown"},
          -- action = "qpixel-ui:apartmentsContext"
          children = {
            {
                title = "Remove Key",
  ...
  exports["qpixel-contexts"]:showContext(data)
end)

AddEventHandler("qpixel-housing:close_access", function()
  exports["qpixel-contexts"]:hideContext()
end)

AddEventHandler('qpixel-ui:apartmentsContext', function(data)
  RPC.execute("qpixel-phone:removeKeys",data[2],data[1])
  exports["qpixel-ui"]:hideContext(data)
end)
...
function hasDecrypt2()
    if exports["qpixel-inventory"]:hasEnoughOfItem("vpnxj",1,false,true) then
      return true
    else
      return false
    end
...
function hasTrucker()
    if exports["qpixel-base"]:getModule("LocalPlayer"):getVar("job") == "trucker"  then
      return true
    else
      return false
    end
...
function hasDecrypt()
    if exports["qpixel-inventory"]:hasEnoughOfItem("decrypterenzo",1,false) or exports["qpixel-inventory"]:hasEnoughOfItem("decryptersess",1,false) or exports["qpixel-inventory"]:hasEnoughOfItem("decrypterfv2",1,false) and not exports["isPed"]:isPed("disabled") or exports["qpixel-inventory"]:hasEnoughOfItem(80,1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasDevice()
    if exports["qpixel-inventory"]:hasEnoughOfItem("mk2usbdevice",1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasPhone()
    if
      (
      (exports["qpixel-inventory"]:hasEnoughOfItem("mobilephone",1,false) or 
      ...
      exports["qpixel-inventory"]:hasEnoughOfItem("assphone",1,false) or
...
function hasRadio()
    if (exports["qpixel-inventory"]:hasEnoughOfItem("radio",1,false) or exports["qpixel-inventory"]:hasEnoughOfItem("pdradio",1,false)) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function openGui(isBurner, cid)
   ...
   TriggerEvent("qpixel-spawn:phonerefresh") --Refreshes jobs
  ...
  if hasPhone() then
    ...
    local near = exports['qpixel-housing']:isNearProperty()
    local house = RPC.execute("qpixel-phone:getHouses")
    ...
    SendNUIMessage({
      ...
      hasDevice = device, 
      hasDecrypt = decrypt, 
      pNum = RPC.execute("qpixel-phone:getPhoneNumber"),
...
function canCreateRace()
  local racingCreateUsbItem = exports["qpixel-inventory"]:GetInfoForFirstItemOfName("racingusb0")
...
function canOpenRaceApp()
  local racingUsbItem = exports["qpixel-inventory"]:GetInfoForFirstItemOfName("racingusb2")
...
function canJoinRaces()
  local hasRaceUsbAndAlias = exports["qpixel-racing"]:getHasRaceUsbAndAlias()
...
function closeGui()
  ...
  -- TriggerEvent('qpixel-phone:UpdateStatePhone')
  pPhoneOpen = false
  insideDelivers = false
end

-- RegisterNetEvent('qpixel-phone:UpdateStatePhone')
-- AddEventHandler('qpixel-phone:UpdateStatePhone', function()
--     Wait(5)
...
RegisterNUICallback('updateMyWallpaper', function(data, cb)
  wallPaper = ""
  Wait(5)
  TriggerEvent('qpixel-phone:grabBackground', data.name .."?auto=compress&cs=tinysrgb&h=350")
...
AddEventHandler('phone:casino_paid', function(message)
  if exports['qpixel-phone']:pOpen() == false then 
    SendNUIMessage({openSection = "emailnotify", pEHandle = 'Payment Received.', pEMessages = message})
    SendNUIMessage({openSection = "phonemedio", timeout = "5200", pOpen = exports['qpixel-phone']:pOpen()}) 
  end
...
RegisterNetEvent("phone:sendThePM", function(name, msg)
  if exports["qpixel-inventory"]:hasEnoughOfItem("thepmdongle",1,false,true) then
...
RegisterNetEvent('qpixel-spawn:confirm_spawned')
AddEventHandler('qpixel-spawn:confirm_spawned', function()
  serverSpawn = true
end)
...
RegisterNUICallback('jobcenter-setgps', function(data)	
  ...
  if data.item == "oxy" then	
    TriggerServerEvent("qpixel-deliveries:JobCenter")	
...
RegisterCommand('house', function()
  local isComplete, propertyID, dist, zone = exports['qpixel-housing']:findClosetedPerson()
  print("HOUSING",isComplete, propertyID, dist, zone)
end)
RegisterNUICallback('checkHouse', function()
  
 -- if exports["qpixel-housing"]:canEdit() then
    TriggerEvent('qpixel-housing:phone:check')
 -- else
 --   SendNUIMessage({
 --     openSection = "noProperty"
 --   })
...
RegisterNUICallback('leaveEditMode', function()
  print("LEAVING EDIT")
  TriggerEvent('qpixel-housing:editOptions', { action = "exit" })
end)
...
RegisterNUICallback('openFurnitureUI', function()
  ...
  TriggerEvent('qpixel-housing:editOptions', { action = "furniture" })
end)

RegisterNUICallback('setBackdoorUI', function()
  print("SET BACKDOOR")
  TriggerEvent('qpixel-housing:editOptions', { action = "backdoor" })
end)

RegisterNUICallback('setWardrobeUI', function()
  print("SET WARDROBE")
  TriggerEvent('qpixel-housing:editOptions', { action = "wardrobe" })
end)

RegisterNUICallback('setCraftingUI', function()
  print("SET CRAFTING")
  TriggerEvent('qpixel-housing:editOptions', { action = "crafting" })
end)

RegisterNUICallback('setGarageUI', function()
  print("SET GARAGE")
  TriggerEvent('qpixel-housing:editOptions', { action = "garage" })
end)

RegisterNUICallback('setStashUI', function()
  print("SET STASH")
  TriggerEvent('qpixel-housing:editOptions', { action = "stash" })
end)

RegisterNUICallback('sellsHouse', function()
  print("EDITING")
  TriggerEvent('qpixel-housing:phone:check',"edit")
end)

AddEventHandler('qpixel-phone:sentHouse', function(hasPrice,id,street,price,category)
...
AddEventHandler('qpixel-phone:editingHouse', function(id,street,price)
  print("EDITING HERE")
  SendNUIMessage({
...
RegisterNUICallback('purchaseHouse', function(data)
  ...
  if rentEnabled then
      print("can purchase")
   --   TriggerEvent('qpixel-phone:sentHouse')
      local success, msg = RPC.execute("qpixel-housing:buy", data.hid, data.price, 0, exports['isPed']:isPed("fullname"))
      TriggerEvent("DoLongHudText",msg,1)
     return
  end
...
-- AddEventHandler('qpixel-phone:noProperty', function()
...
RegisterNUICallback('editHousePrice', function(data)
  TriggerServerEvent('qpixel-housing:editPrice', data.hid,data.price)
end)

AddEventHandler('qpixel-phone:EnableSellRent', function()
    exports['qpixel-ui']:openApplication('textbox', {
    callbackUrl = 'qpixel-phone:SellRentEnable',
...
RegisterInterfaceCallback("qpixel-phone:SellRentEnable", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  print('THIS IS ENABLED RENT/SALE FOR PLAYER', data[1].value,json.encode(data))
  TriggerServerEvent('qpixel-housing:rent_activate_sv', data[1].value)
  exports["qpixel-ui"]:hideContextMenu()
  cb({ data = {}, meta = { ok = true, message = '' } })
end)
...
RegisterNetEvent('qpixel-phone:signInInfoTowing')
AddEventHandler('qpixel-phone:signInInfoTowing', function()
  phoneNotification("email", "Grab the towtruck parked in the bay.", "Job Center")
end)

RegisterNetEvent('qpixel-phone:towing_offer')
AddEventHandler('qpixel-phone:towing_offer', function()
...
RegisterNUICallback('accept_tow_job', function(data)
  TriggerEvent('qpixel-jobs:towing_accept')
end)

-- 24/7 Delivery --

RegisterNetEvent('qpixel-phone:signInInfoDelivery')
AddEventHandler('qpixel-phone:signInInfoDelivery', function()
  phoneNotification("email", "Signed in as 24/7 Delivery Driver.", "Job Center")
end)

RegisterNetEvent('qpixel-phone:delivery_offer')
AddEventHandler('qpixel-phone:delivery_offer', function()
...
RegisterNUICallback('accept_delivery_job', function(data)
    TriggerEvent('qpixel-jobs:deliveries:getCar')
end)

-- Dodo Logistics --

RegisterNetEvent('qpixel-phone:signInDodo')
AddEventHandler('qpixel-phone:signInDodo', function()
  phoneNotification("email", "Signed in as a Dodo Logistics worker.", "Job Center")
end)

RegisterNetEvent('qpixel-phone:dodoJobOffer')
AddEventHandler('qpixel-phone:dodoJobOffer', function()
...
RegisterNUICallback('accept_dodo_job', function(data)
    TriggerEvent('qpixel-jobs:spawn-truck')
end)

RegisterNetEvent('qpixel-phone:goToLocation')
AddEventHandler('qpixel-phone:goToLocation', function()
  phoneNotification("email", "Go to the delivery location on the GPS.", "Job Center")
end)

RegisterNetEvent('qpixel-phone:goBackDepo')
AddEventHandler('qpixel-phone:goBackDepo', function()
  phoneNotification("email", "Return back to the depo.", "Job Center")
end)

-- House Robberies --

RegisterNetEvent('qpixel-phone:signInInfoHouseRobbery')
AddEventHandler('qpixel-phone:signInInfoHouseRobbery', function()
  phoneNotification("email", "Signed in as Criminal.", "Job Center")
end)

RegisterNetEvent('qpixel-phone:house_rob_offer')
AddEventHandler('qpixel-phone:house_rob_offer', function()
...
RegisterNUICallback('accept_house_robbery', function(data)
    TriggerEvent('qpixel-activities:acceptJob')
end)

-- Chopshop --

RegisterNetEvent('qpixel-phone:signInInfoChopshop')
AddEventHandler('qpixel-phone:signInInfoChopshop', function()
  phoneNotification("email", "Signed in as Chopshop.", "Job Center")
end)

RegisterNetEvent('qpixel-phone:chop_shop_offer')
AddEventHandler('qpixel-phone:chop_shop_offer', function()
...
-- Oxy Runs --
RegisterNetEvent('qpixel-phone:oxyRunNoti')
AddEventHandler('qpixel-phone:oxyRunNoti', function(pText)
  phoneNotification("email", pText, "Job Center")
end)

RegisterNetEvent('qpixel-phone:oxyrun:offer')
AddEventHandler('qpixel-phone:oxyrun:offer', function()
...
RegisterNUICallback('accept_oxy_run', function(data)
    TriggerEvent('qpixel-oxy:Accept:Offer')
end)

-- Paycheck --

RegisterNetEvent('qpixel-phone:paymentNoti')
AddEventHandler('qpixel-phone:paymentNoti', function(pAmt, pType)
  phoneNotification("email", "$"..pAmt..".00 has been added to your "..pType.. " Balance", "Job Center")
end)

RegisterNetEvent('qpixel-phone:businessRegisterNoti')
AddEventHandler('qpixel-phone:businessRegisterNoti', function(pBusinessName, pText)
  phoneNotification("charge", pText, pBusinessName)
end)

-- Fishing --

RegisterNetEvent('qpixel-phone:signInFishing')
AddEventHandler('qpixel-phone:signInFishing', function()
