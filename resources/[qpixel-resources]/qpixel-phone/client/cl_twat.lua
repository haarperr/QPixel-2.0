...
RegisterNUICallback('btnTwatter', function()
    ...
    local twattermessages = RPC.execute("qpixel-phone:getTwatters")
    -- if not getTwat then
...
RegisterNUICallback("qpixel-phone:getTwatter", function(data, cb)
  cb({ data = currentTwats, meta = { ok = success, message = (not success and currentTwats or 'done') } })
end)



RegisterNetEvent('qpixel-phone:insertTwat')
AddEventHandler('qpixel-phone:insertTwat', function(name,msg,time,attach)
  table.insert(currentTwats, {
    handle = name,
    message = msg,
    attachment = attach,
