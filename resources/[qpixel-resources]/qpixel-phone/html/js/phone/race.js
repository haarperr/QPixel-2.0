...
$('.submit-crace').click(function(){
   ...
   let bubble = $("#bubble-remove").is(":checked");
    ...
    $.post("https://qpixel-phone/racing:event:start", JSON.stringify({
      id: id,
...
//    console.log("SUBMIT HERE")
   ...
   setTimeout(() => {
      $.post("https://qpixel-phone/racing:map:save", JSON.stringify({
...
//    console.log("PENDING ENTRIES",pendingRaces.parent().has('.component-paper').length)
   ...
   $('.pGPS').click(function(){
      ...
      $.post('http://qpixel-phone/qpixel-phone:racing:setGPS', JSON.stringify({
         mID: mID,
     }));
   })
   $('.rPreview').click(function(){
      ...
      $.post('http://qpixel-phone/qpixel-phone:racing:seePreview', JSON.stringify({
         mID: mID,
     }));
   })
...
$('.pending-entries').on('click', '.pJoin',function(){
   ...
   $.post('http://qpixel-phone/racing:event:join', JSON.stringify({
      id: eID
   }));
});

$('.pending-entries, .active-entries').on('click', '.pLeave', function(){
   $.post('http://qpixel-phone/racing:event:leave', JSON.stringify({}));
});
...
$('.pending-entries, .active-entries').on('click', '.pEndRace',function(){
   $.post('http://qpixel-phone/racing:event:end', JSON.stringify({}));
});
