...
//<span class="new-badge" style="margin-left: 10px;margin-top: 40px;" data-badge-caption="">${moment(date).local().fromNow()}</span>
    ...
    element.click(function () {
        if(item.unread === 0){
            ...
            $.post('http://qpixel-phone/readMessage', JSON.stringify({ id: item.id }));
        }
        setTimeout(() => {
        ...
        $.post('http://qpixel-phone/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: item.msgDisplayName }));
    });
...
function addMessageOther(item) {
    ...
    element.id = item.id;
    element.click(function () {
        $.post('http://qpixel-phone/messageRead', JSON.stringify({ sender: item.sender, receiver: item.receiver, displayName: receiver, clientPhone: item.clientNumber }));
    });
    $(".messages-entries").prepend(element);
}
