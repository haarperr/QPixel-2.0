...
AddEventHandler('jobssystem:current', function(cb)
  LocalPlayer = exports["qpixel-base"]:getModule("LocalPlayer")
  cb(LocalPlayer:getVar("job"))
end)

local PaycheckSpot, called = false, false

exports["qpixel-polyzone"]:AddBoxZone("paycheck_collect", vector3(241.63, 226.48, 106.29), 4, 4, {
  heading=0
})
...
AddEventHandler('qpixel-polyzone:enter', function(name)
  if name == "paycheck_collect" then
    PaycheckSpot = true
    exports["qpixel-interface"]:showInteraction("[E]Collect Paycheck")
    PaycheckFunction()
  end
end)
  ...
  Citizen.CreateThread(function()
    while PaycheckSpot do
      ...
      if IsControlJustReleased(0, 38) then
        local finished = exports["qpixel-taskbar"]:taskBar(2000,"Collecting Your Paycheck")
