...
AddEventHandler('speaktofredrick1', function()
    if exports["qpixel-inventory"]:hasEnoughOfItem("vpnxj",1,false) then
        FreezeEntityPosition(GetPlayerPed(-1),true)
        ExecuteCommand("e argue")
        local finished = exports["qpixel-taskbar"]:taskBar(13000,"Speaking to fredrick")
...
AddEventHandler('basiclaptop1', function()
	...
	local finished = exports["qpixel-taskbar"]:taskBar(13000,"Fredrick - Give me a sec")
	if finished == 100 then
		FreezeEntityPosition(GetPlayerPed(-1),false)
...
AddEventHandler('jim:laptop1', function()
	if exports["qpixel-inventory"]:hasEnoughOfItem("jimpass",1,false) then
		FreezeEntityPosition(GetPlayerPed(-1),true)
...
AddEventHandler('buy:laptop1', function()
	FreezeEntityPosition(GetPlayerPed(-1),true)
	local finished = exports["qpixel-taskbar"]:taskBar(6000,"ight dawg slide me 5k")
	if finished == 100 then
		FreezeEntityPosition(GetPlayerPed(-1),false)
...
AddEventHandler('kaarensahhoe', function()
    if exports["qpixel-inventory"]:hasEnoughOfItem("rollcash",250,false) then
        FreezeEntityPosition(GetPlayerPed(-1),true)
        ExecuteCommand("e think")
        local finished = exports["qpixel-taskbar"]:taskBar(13000,"Speaking to Karen")
        if finished == 100 then
