RPC.register("qpixel-storageunits:get", function()
    return storage_units
end)


RegisterServerEvent("qpixel-storageunits:purchase", function(args)
    local pSrc = source
...
RPC.register("qpixel-storageunits:occupied", function(storage_unit_id)
    local pSrc = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(pSrc)
        ...
        if result[1] ~= nil then
            ...
            local menu_data = {
                ...
                },
                {
                    ...
                    params = {
                        event = "qpixel-storageunits:attempt:inventory",
                        args = {
                            id = storage_unit_id,
                        }
                    }
            ...
            
                {
                    ...
                    params = {
                        event = "qpixel-storageunits:attempt:access",
                        args = {
                            id = storage_unit_id,
            ...
            if tonumber(storage_owner) == char_id then
                TriggerClientEvent('qpixel-context:sendMenu', pSrc, menu_data)  
            else
                for i=1, #keys do
            ...
            local menu_data = {
                ...
                },
                {
                    ...
                    params = {
                        event = "qpixel-storageunits:attempt:purchase",
                        args = {
                            id = storage_unit_id,
                            price = 70000
                        }
...
RPC.register("qpixel-storageunits:give:access", function(storage_unit_id, given_cid)
    local pSrc = source
...
RPC.register("qpixel-storageunits:remove:access", function(storage_unit_id, given_cid, given_name)
    local pSrc = source
...
RegisterServerEvent("qpixel-storageunit:view", function(storage_unit_id)
    local pSrc = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(pSrc)
    exports["ghmattimysql"]:execute("SELECT `given_access` FROM storage_units WHERE id = @id", {['id'] = storage_unit_id}, function(result)
        if result[1].given_access ~= nil then
            ...
            for i=1, #current_keys do  
                exports["ghmattimysql"]:execute("SELECT `first_name`, `last_name`, `id` FROM characters WHERE id = @id", {['id'] = current_keys[i]}, function(result)
                    if result[1] ~= nil then
                        for i=1, #result do
                            TriggerClientEvent('qpixel-context:sendMenu', pSrc, {
                                {
                                    id = result[i].id,
                                    header = "Name: " ..result[i].first_name .. " " .. result[i].last_name,
                                    txt = "CID: " ..result[i].id,
                                    params = {
                                        event = "qpixel-storageunit:attempt:remove",
                                        args = {
                                            id = storage_unit_id,
...
RPC.register("qpixel-storageunits:attenpt:transfer", function(storage_unit_id, transfer_cid)
    local pSrc = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(pSrc)
    local char_id = user:getCurrentCharacter().id
