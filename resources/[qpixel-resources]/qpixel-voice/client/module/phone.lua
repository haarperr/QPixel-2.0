	...
	Citizen.CreateThread(function()
		...
		while callChannel ~= 0 do
			...
			if NetworkIsPlayerTalking(PlayerId()) and not changed then
				...
				TriggerServerEvent('qpixel-voice:setTalkingOnCall', true)
			elseif changed and NetworkIsPlayerTalking(PlayerId()) ~= 1 then
				changed = false
				MumbleClearVoiceTargetPlayers(1)
				TriggerServerEvent('qpixel-voice:setTalkingOnCall', false)
			end
			Wait(0)
		end
	end)
...
RegisterNetEvent('qpixel-voice:syncCallData', function(callTable, channel)
	callData = callTable
...
RegisterNetEvent('qpixel-voice:setTalkingOnCall', function(tgt, enabled)
	if tgt ~= playerServerId then
...
RegisterNetEvent('qpixel-voice:addPlayerToCall', function(plySource)
	callData[plySource] = false
end)

RegisterNetEvent('qpixel-voice:removePlayerFromCall', function(plySource)
...
function setCallChannel(channel)
	...
	TriggerServerEvent('qpixel-voice:setPlayerCall', channel)
	callChannel = channel
...
RegisterNetEvent('qpixel-voice:clSetPlayerCall', function(_callChannel)
	if GetConvarInt('voice_enablePhones', 1) ~= 1 then return end
