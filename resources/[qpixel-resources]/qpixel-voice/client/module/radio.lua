...
RegisterNetEvent('qpixel-voice:syncRadioData', syncRadioData)
...
RegisterNetEvent('qpixel-voice:setTalkingOnRadio', setTalkingOnRadio)
...
function addPlayerToRadio(plySource)
	TriggerEvent('qpixel-voice:getRadioChannel')
...
RegisterNetEvent('qpixel-voice:addPlayerToRadio', addPlayerToRadio)
...
RegisterNetEvent('qpixel-voice:removePlayerFromRadio', removePlayerFromRadio)
...
function setRadioChannel(channel)
	...
	TriggerServerEvent('qpixel-voice:setPlayerRadio', channel)
	...
	if not exports['ragdoll']:GetDeathStatus() then 
		if not radioPressed and radioEnabled then
			if radioChannel > 0 then
				...
				playerTargets(radioData, NetworkIsPlayerTalking(PlayerId()) and callData or {})
				TriggerServerEvent('qpixel-voice:setTalkingOnRadio', true)
				radioPressed = true
				playMicClicks(true)
				...
				Citizen.CreateThread(function()
					TriggerEvent('qpixel-hud:transmitting', radioPressed)
					TriggerEvent("qpixel-voice:radioActive", true)
...
RegisterCommand('-radiotalk', function()
	...
	if radioChannel > 0 or radioEnabled and radioPressed then
		...
		playerTargets(NetworkIsPlayerTalking(PlayerId()) and callData or {})
		TriggerEvent("qpixel-voice:radioActive", false)
		TriggerEvent('qpixel-hud:transmitting', radioPressed)
		playMicClicks(false)
		if GetConvarInt('voice_enableRadioAnim', 1) == 1 then
			StopAnimTask(PlayerPedId(), "random@arrests", "generic_radio_enter", -4.0)
		end
		TriggerServerEvent('qpixel-voice:setTalkingOnRadio', false)
	end
end, false)
...
RegisterNetEvent('qpixel-voice:clSetPlayerRadio', syncRadio)

RegisterNetEvent("qpixel-voice:getRadioChannel")
AddEventHandler("qpixel-voice:getRadioChannel", function()
	TriggerServerEvent('qpixel-dispatch:GrabRadioChannel', radioChannel)
end)