    ...
    data = {
        {
            ...
            event = 'qpixel-weed:prepareBaggies',
            parameters = {},
        }
    },
    options = {
        distance = { radius = 5.0 },
        isEnabled = function(pEntity, pContext)
            local hasWeed = exports["qpixel-inventory"]:hasEnoughOfItem("weedpackage", 1, false, true)
            return hasWeed
        end
    }
}
...
Citizen.CreateThread(function()
    exports["qpixel-interact"]:AddPeekEntryByEntityType(InteractCorner.group, InteractCorner.data, InteractCorner.options)
    exports["qpixel-interact"]:AddPeekEntryByEntityType(InteractCornerVehicle.group, InteractCornerVehicle.data, InteractCornerVehicle.options)
    ...
    Citizen.CreateThread(function()
        ...
        while CurrentlyCornering do
            ...
            if not foundPed then
                ...
                RPC.execute("qpixel-weed:cornerPed", coords, NetworkGetNetworkIdFromEntity(foundPed), NetworkGetNetworkIdFromEntity(CurrentCornerVehicle))
            end
...
AddEventHandler("qpixel-weed:prepareBaggies", function(pContext, pEntity)
    if CornerConfig.RequireScales then
        local hasScale = exports["qpixel-inventory"]:hasEnoughOfItem("qualityscales", 1, false) or exports["qpixel-inventory"]:hasEnoughOfItem("smallscales", 1, false)
    ...
    local hasBaggies = exports["qpixel-inventory"]:hasEnoughOfItem("emptybaggies", CornerConfig.BaggiesPerBrick / 4, false)
    if not hasBaggies then
    ...
    local finished = exports["qpixel-taskbar"]:taskBar(30000, _L('weed-corner-prepare', 'Preparing'), false, true, false, false, nil, 5.0, pEntity)
    if finished == 100 then
        local weedpackages = exports["qpixel-inventory"]:getItemsOfType("weedpackage", 1, true)
        if #weedpackages == 0 then return end
        local package = weedpackages[1]
        local info = json.decode(package.information)
        RPC.execute("qpixel-weed:prepareBaggies", info)
    end
end)
...
function stopCornering()
    ...
    RPC.execute("qpixel-weed:stopCorner")
end
