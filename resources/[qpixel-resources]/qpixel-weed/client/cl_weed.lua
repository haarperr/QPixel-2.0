...
function showPlantMenu(pPlantId)
    ...
    if growth >= 95 or exports["qpixel-jobs"]:getJob(myjob, "is_police") or myjob == "judge" then
        context[#context+1] = {
            title = "Destroy Plant",
    ...
    exports["qpixel-context"]:showContext(context)
end
...
AddEventHandler("qpixel-weed:addWater", function (pParams)
    ...
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local success = RPC.execute("qpixel-weed:addWater", pParams.id)
        if not success then
...
AddEventHandler("qpixel-weed:addFertilizer", function (pParams)
    ...
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local success = RPC.execute("qpixel-weed:addFertilizer", pParams.id, pParams.type)
        if not success then
...
AddEventHandler("qpixel-weed:addMaleSeed", function (pParams)
    ...
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        RPC.execute("qpixel-weed:addMaleSeed", pParams.id)
        TriggerEvent("inventory:removeItem", "maleseed", 1)
    end
    showPlantMenu(pParams.id)
end)

AddEventHandler("qpixel-weed:removePlant", function (pParams)
    TriggerEvent("animation:PlayAnimation","layspike")
    local finished = exports["qpixel-taskbar"]:taskBar(30000, "Removing Plant", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local getFertilizer = getPlantGrowthPercent(weedPlants[pParams.id]) > 20.0
        local success = RPC.execute("qpixel-weed:removePlant", pParams.id, getFertilizer)
        if not success then
...
AddEventHandler("qpixel-weed:pickPlant", function(pContext, pEntity)
    ...
    TriggerEvent("animation:PlayAnimation", "layspike")
    local finished = exports["qpixel-taskbar"]:taskBar(10000, "Harvesting Plant", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
