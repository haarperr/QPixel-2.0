AddEventHandler("qpixel-inventory:itemUsed", function(item, passedItemInfo, inventoryName, slot)
    if item == "driedbud" then
        local parsedInfo = json.decode(passedItemInfo)
        local finished = exports["qpixel-taskbar"]:taskBar(5000, _L('weed-pack-bud', 'Packing'), false, true, false, false, nil, 5.0, PlayerPedId())
    ...
    if item == "weedpackage" then
        TriggerEvent("qpixel-weed:prepareBaggies", {}, PlayerPedId())
        -- local parsedInfo = json.decode(passedItemInfo)
        -- local budData = {
