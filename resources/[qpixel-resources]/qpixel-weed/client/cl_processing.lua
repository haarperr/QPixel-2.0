...
RegisterNetEvent('qpixel-weed:package_smallbud')
AddEventHandler('qpixel-weed:package_smallbud', function()
    if exports['qpixel-inventory']:hasEnoughOfItem('smallbud', 10) then
        if not pStartedPackaging then
            pStartedPackaging = true
            local finished = exports['qpixel-taskbar']:taskBar(10000, 'Preparing')
...
RegisterNetEvent('qpixel-weed:grab_package')
AddEventHandler('qpixel-weed:grab_package', function()
...
RegisterNetEvent('qpixel-weed:get_delivery')
AddEventHandler('qpixel-weed:get_delivery', function()
    if pDeliverAmt == 3 then
        pDeliverAmt = 0
        inWeedRun = false
        pSentNoti = false
        exports["qpixel-phone"]:DoPhoneNotification("home-screen", "CURRENT", 'Run complete.', 1)
        return
    end
    if inWeedRun then
    ...
    inWeedRun = true
    exports["qpixel-phone"]:DoPhoneNotification("home-screen", "CURRENT", 'Drive to the delivery location with the goods.', 1)
    ...
    TriggerEvent('qpixel-weed:create_ped')
            ...
            if inWeedRun then
                ...
                if distance < 40 then
                    if not pSentNoti then
                        exports["qpixel-phone"]:DoPhoneNotification("home-screen", "CURRENT", 'Deliver the package.', 1)
                        pSentNoti = true
                        pCanDeliver = true
                    end
                end
...
RegisterNetEvent("qpixel-weed:create_ped")
AddEventHandler("qpixel-weed:create_ped", function()
    Citizen.Wait(5000)
...
AddEventHandler("qpixel-weed:deliver_package", function()
    ...
    if pRndCop == 1 then
        TriggerEvent('qpixel-mdt:drugsale')
    end
    ...
    local finished = exports["qpixel-taskbar"]:taskBar(20000, "Weighing Package", false, true, false, false, nil, 5.0, PlayerPedId())
    if finished == 100 then
    PlayAmbientSpeech1(CurrentDeliveryNPC.entity, "Chat_State", "Speech_Params_Force")
    Citizen.Wait(200)
    local finished = exports['qpixel-taskbar']:taskBar(15000, "Counting Bills")
        if finished == 100 then
        RPC.execute("qpixel-weed:deliverpackage")
        pCanDeliver = false
        Wait(1000)
        TriggerEvent("qpixel-weed:drop_weed_package", 0.5)
        ...
        TriggerEvent('qpixel-weed:get_delivery')
        end
    end
end)
...
AddEventHandler("qpixel-weed:drop_weed_package", function(pRandomChance)
    local payment = 0
