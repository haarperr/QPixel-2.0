...
RegisterServerEvent('qpixel-6str_shop:CarTableTunerShop')
AddEventHandler('qpixel-6str_shop:CarTableTunerShop', function(table)
    if table ~= nil then
        carTable = table
        TriggerClientEvent('qpixel-6str_shop:ReturnVehicleTunerTable', -1, carTable)
        for i=1, #carTable do
...
RegisterServerEvent('qpixel-6str_shop:FinaceEnabledSV')
AddEventHandler('qpixel-6str_shop:FinaceEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('qpixel-6str_shop:FinaceEnabledCL', -1, plate)
    end
end)

RegisterServerEvent('qpixel-6str_shop:BuyEnabledSV')
AddEventHandler('qpixel-6str_shop:BuyEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('qpixel-6str_shop:BuyEnabledCL', -1, plate)
    end
end)

-- return table
-- TODO (return db table)
RegisterServerEvent('qpixel-6str_shop:RequestTableTunerShopCars')
AddEventHandler('qpixel-6str_shop:RequestTableTunerShopCars', function()
    local user = source
    exports.oxmysql:execute("SELECT * FROM vehicle_display", {}, function(display)
        ...
        TriggerClientEvent('qpixel-6str_shop:ReturnVehicleTunerTable', user, carTable)
    end)
end)

-- Check if player has enough money
RegisterServerEvent('qpixel-6str_shop:ChechMoney')
AddEventHandler('qpixel-6str_shop:ChechMoney', function(name, model, price)
    local src = source
    ...
    elseif tonumber(cash) <= price then
        ...
        TriggerClientEvent('qpixel-6str_shop:FailedPurchase', src)
    end
end)

RegisterServerEvent('qpixel-6str_shop:BuyVehicle')
AddEventHandler('qpixel-6str_shop:BuyVehicle', function(plate, name, vehicle, price, personalvehicle)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
...
RegisterServerEvent('qpixel-cardealer:log')
AddEventHandler('qpixel-cardealer:log', function(cid, model, price, plate, carDealer)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
