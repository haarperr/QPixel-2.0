...
function updateCarTable(model,price,name)
	for i=1, #carTable do
		...
		TriggerServerEvent("qpixel-6str_shop:CarTableTunerShop",carTable)
	end
end

local myspawnedvehs = {}

RegisterNetEvent("qpixel-6str_shop:testdrive")
AddEventHandler("qpixel-6str_shop:testdrive", function()
	if exports["qpixel-business"]:IsEmployedAt("tuner_shop") or #(vector3(144.35664, -3005.489, 7.0309138) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("qpixel-6str_shop:EnableBuy")
AddEventHandler("qpixel-6str_shop:EnableBuy", function()
	if #(vector3(vector3(144.35664, -3005.489, 7.0309138)) - GetEntityCoords(PlayerPedId())) > 50.0 and exports["qpixel-business"]:IsEmployedAt("tuner_shop") then
		return
	end
	...
	TriggerServerEvent("qpixel-6str_shop:BuyEnabledSV",vehplate)
end)

RegisterNetEvent("qpixel-6str_shop:BuyEnabledCL")
AddEventHandler("qpixel-6str_shop:BuyEnabledCL", function(addplate)
	buyPlate[addplate] = true
	Citizen.Wait(60000)
	buyPlate[addplate] = nil
end)

RegisterNetEvent("qpixel-6str_shop:VehicleCommission")
AddEventHandler("qpixel-6str_shop:VehicleCommission", function(newAmount)
	if eexports["qpixel-business"]:IsEmployedAt("tuner_shop") or #(vector3(-30.31887, -1089.345, 26.04948) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("qpixel-6str_shop:ReturnVehicleTunerTable")
AddEventHandler("qpixel-6str_shop:ReturnVehicleTunerTable", function(newTable)
	carTable = newTable
	DespawnSaleVehicles()
	SpawnSaleVehicles()
end)
...
function AttemptBuy(tableid)
	...
	TriggerServerEvent('qpixel-6str_shop:ChechMoney',name, model, price)
...
function DrawPrices()
	for i = 1, #carspawns do
		if #(vector3(carspawns[i]["x"],carspawns[i]["y"],carspawns[i]["z"]) - GetEntityCoords(PlayerPedId())) < 1.5 then
			...
			if exports["qpixel-business"]:IsEmployedAt("tuner_shop") then
...
function SpawnSaleVehicles()
	if not hasspawned then
		TriggerServerEvent("qpixel-6str_shop:RequestTableTunerShopCars")
		Citizen.Wait(1500)
	end
...
function pCloseCreator(name, veh, price, plate)
	Citizen.CreateThread(function()
		...
		if not boughtcar then
			...
			TriggerServerEvent('qpixel-6str_shop:BuyVehicle', plate, vehname, vehicle, price, VehicleProps)
			TriggerServerEvent('qpixel-cardealer:log', exports['isPed']:isPed('cid'), vehname, price, plate, 'Tuner Shop')
			TriggerServerEvent('qpixel-financials:business_money', price / 10, 'tuner_shop', 'add')
			exports['qpixel-interface']:hideInteraction()
			DespawnSaleVehicles()
			SpawnSaleVehicles()
		end
...
RegisterNetEvent("qpixel-6str_shop:FailedPurchase")
AddEventHandler("qpixel-6str_shop:FailedPurchase", function()
...
function ButtonSelected(button)
	...
	elseif this == "6str-vehicles" or this == "tuner_cars" then
		if ownerMenu then
			...
			TriggerServerEvent('qpixel-6str_shop:ChechMoney',button.name, button.model, button.costs)
		end
	end
end
	...
	while true do
		...
		if vehshop.opened then
			...
			for i,button in pairs(menu.buttons) do
				...
				if exports["qpixel-business"]:IsEmployedAt("tuner_shop") and i >= vehshop.menu.from and i <= vehshop.menu.to then
...
RegisterCommand('commissiontuner', function(source, args, raw)
	if exports["qpixel-business"]:IsEmployedAt("tuner_shop") then
		...
		if tonumber(amount) > 0 and tonumber(amount) <= 60 then
			TriggerEvent('qpixel-6str_shop:VehicleCommission', amount)
		else
...
RegisterCommand('testdrivetuner', function(source, args, raw)
	if exports["qpixel-business"]:IsEmployedAt("tuner_shop") then
		TriggerEvent('qpixel-6str_shop:testdrive')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)
...
RegisterCommand('enableBuytuner', function(source, args, raw)
	if exports["qpixel-business"]:IsEmployedAt("tuner_shop") then
		TriggerEvent('qpixel-6str_shop:EnableBuy')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)