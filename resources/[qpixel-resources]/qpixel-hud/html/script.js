...
document.onkeyup = function (data) {
    if ( data.which == 27 ) { //esc
      $.post('https://qpixel-hud/close', JSON.stringify({}));
      $(".menu").fadeOut(500);
    }
};
...
$(function(){
    $('.jss256').change(function(){
        ...
        if ($('#time').prop('checked')) {
            time = true;
            $.post('https://qpixel-hud/time', JSON.stringify({
                time: true
            }));
        } else {
            time = false;
    ...
    $("#defaultmap").change(function() {
        if ($('#defaultmap').prop('checked')) {
            SetDefaultMap = true
            $.post('https://qpixel-hud/minimap', JSON.stringify({
                action: "default"
            }));
        } else {
            SetDefaultMap = false
...
document.body.addEventListener('click', function(e) {
    ...
    $('.speedmeter-15').click(function(){
        ...
        SetSpeedFPS = 500
        $.post('https://qpixel-hud/carhud', JSON.stringify({
            fps: 500
        }));
    });
   ...
   //     $('.speedmeterOptionBtn').html('60')
    ...
    $('.compass-15').click(function(){
        ...
        SetCompassFPS = 50
        $.post('https://qpixel-hud/compass', JSON.stringify({
            fps: 50
        }));
    });
...
$(document).on('click', '#save-button', function (e) {
    ...
    $.post('https://qpixel-hud/SaveSettings', JSON.stringify({
