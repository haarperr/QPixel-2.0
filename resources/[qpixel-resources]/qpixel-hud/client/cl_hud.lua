...
RegisterNetEvent("qpixel-hud:EnableHud", function(status)
    isLoggedIn = status
    if status == true then
        exports['qpixel-interface']:showInteraction('[F] To pay respects')
        HudActive = false
    end
end)

RegisterNetEvent("qpixel-hud:changeRange", function(pRange)
    voice = pRange or 2
end)
...
RegisterNetEvent('qpixel-weapons:client:setWeaponFireRate', function (pAmount)
--    weaponFireRate = pAmount
end)

RegisterNetEvent("qpixel-weapons:client:showWeaponFireRate", function (pBool)
    weaponFireRate = pBool
end) 

RegisterNetEvent("qpixel-hud:harness_values", function(cinturon)
	harness = cinturon
end)
...
Citizen.CreateThread(function ()
	...
	while true do
        ...
        local pRadioActive = exports['qpixel-voice']:pRadioActive()
...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if isLoggedIn then
            ...
            local pRadio = exports['qpixel-voice']:pRadioActive()
            ...
            SendNUIMessage({
                ...
                gps = gps,
                gpu = gpu,
                nos = exports['qpixel-vehicles']:nitroLevel(),
                dev = developer,
                debug = debug,
...
RegisterNetEvent('qpixel-hud:ChangeThirst', function(amount)
...
RegisterNetEvent('qpixel-hud:ChangeHunger', function(amount)
	if isLoggedIn == true then
...
RegisterNetEvent("client:newStress", function(status, amount)
    ...
    if status == true then
        ...
        Wait(1000)
        TriggerServerEvent('qpixel-hud:UpdateStress_SV', pStress)
...
RegisterNetEvent('qpixel:admin:maxstats', function()
    ...
    TriggerServerEvent("qpixel-death:reviveSV", source)
...
RegisterNUICallback("time", function(data)
    exports['qpixel-carhud']:SetTimeEnabled(data.time)
end)

RegisterNUICallback("SaveSettings", function(data)
    local pInfo = exports["qpixel-base"]:getModule("LocalPlayer")
    ...
    exports['qpixel-carhud']:SetCompassFPS(tonumber(data.compassFPSValue))
    exports['qpixel-carhud']:SetSpeedFPS(tonumber(data.speedFPSValue))
    TriggerEvent('setModel', model)
    TriggerEvent('qpixel-phone:grabBackground', bg)
    TriggerEvent('changeBG', bg)
    TriggerEvent("qpixel-phone:setSomeShit",bg,model,twatNotify,callNotify)
    local circleTaskbar = GetResourceKvpInt(cid..'-circleTaskbar')
    if circleTaskbar == 1 then
        TriggerEvent("qpixel-preferences:setPreferences", true)
    elseif circleTaskbar == 0 then
        TriggerEvent("qpixel-preferences:setPreferences", false)
    end
...
AddEventHandler('qpixel-preferences:setPreferences', function(bool)
    local data = {
        ["hud.misc.circle.taskbar.enabled"] = bool
    }
    exports["qpixel-interface"]:sendAppEvent('preferences', data)
end)

RegisterNetEvent("qpixel-hud:SetValues", function()
    local pInfo = exports["qpixel-base"]:getModule("LocalPlayer")
    local cid = exports['isPed']:isPed('cid')
    ...
    local model = exports['qpixel-phone']:getModelForHud()
    local background = exports['qpixel-phone']:getBackgroundForHud()
    local twat = exports['qpixel-phone']:getTwatForHud()
    local call = exports['qpixel-phone']:getCallForHud()
    ...
    exports['qpixel-carhud']:SetCompassFPS(SetCompassFPSValue)
    exports['qpixel-carhud']:SetSpeedFPS(SetSpeedFPSValue)
    ...
    if SetTimeToggled == 1 then
        exports['qpixel-carhud']:SetTimeEnabled(true)
    elseif SetTimeToggled == 0 then
        exports['qpixel-carhud']:SetTimeEnabled(false)
    end
...
RegisterCommand('ui-r', function()
    ...
    TriggerEvent('qpixel-banking:ResetUI')
    TriggerEvent('qpixel-radio:ResetUI')
    TriggerEvent('qpixel-menu:ResetUI')
    TriggerEvent('qpixel-interact:ResetUI')
    TriggerEvent('qpixel-mdt:ResetUI')
    TriggerEvent('phone:reset')
    TriggerEvent('clothing:close')
    exports['qpixel-interface']:showInteraction("Restarting UI .")
    Wait(1000)
    exports['qpixel-interface']:showInteraction("Restarting UI ..")
    Wait(1000)
    exports['qpixel-interface']:showInteraction("Restarting UI ...")
    Wait(1000)
    exports['qpixel-interface']:showInteraction("UI Successfully Restarted")
    Wait(1000)
    exports['qpixel-interface']:hideInteraction()
...
RegisterNetEvent('qpixel-hud:boosting_value')
AddEventHandler('qpixel-hud:boosting_value', function(pValue)
