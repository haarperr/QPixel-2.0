...
AddEventHandler('carfill:checkmoney', function(costs)
	...
	if exports['qpixel-base']:getModule('GetCash')(src) >= costs then
		TriggerClientEvent("RefuelCarServerReturn", src)
		exports['qpixel-base']:getModule('RemoveCash')(src, costs)
	else
		moneyleft = costs - exports['qpixel-base']:getModule('GetCash')(src)
		TriggerClientEvent('DoLongHudText', src, "Requires $" .. costs)
	end
end)

RegisterServerEvent('qpixel-hud:UpdateStress_SV')
AddEventHandler('qpixel-hud:UpdateStress_SV',function(amount)
	local src = source
