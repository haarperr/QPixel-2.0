...
RegisterNetEvent('qpixel-jobs:passed')
AddEventHandler('qpixel-jobs:passed', function()
    pisInDodoJob = true
    ...
    TriggerEvent('qpixel-phone:goToLocation')
end)

RegisterNetEvent('qpixel-jobs:startDodoLogistics')
AddEventHandler('qpixel-jobs:startDodoLogistics', function()
    TriggerEvent('qpixel-phone:signInDodo')
    ...
    TriggerEvent('qpixel-phone:dodoJobOffer')
end)

RegisterNetEvent('qpixel-jobs:dodoLogisticsPayout')
AddEventHandler('qpixel-jobs:dodoLogisticsPayout', function()
    if pOnWayDodo == true then
        if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("pounder"))  then
            if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("pounder")) then
                TriggerServerEvent('qpixel-jobs:dodoLogisticsPayout')
    ...
    while true do
        ...
        if isToDropLocation == true then
            ...
            if GetDistanceBetweenCoords(px,py,pz, GetEntityCoords(GetPlayerPed(-1),true)) < 2.5 then
                if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("pounder")) then
                    exports['qpixel-interface']:showInteraction('[E] Drop Off Package')
                    if IsControlJustPressed(1,38) then
                        TriggerEvent('qpixel-phone:goBackDepo')
                        exports['qpixel-interface']:hideInteraction()
                            ...
                            local aDist = GetDistanceBetweenCoords(setCoords, coordA)
                                if aDist < 15.0 then
                                    ...
                                    if exports['qpixel-hud']:MoneyBuff() then
                                        TriggerServerEvent('qpixel-jobs:givePayout', math.random(878, 927), "bank", '[Dodo Logistics | With Buff]')
                                    else
                                        TriggerServerEvent('qpixel-jobs:givePayout', math.random(650, 778), "bank", '[Dodo Logistics | Without Buff]')
                                    end
                                end
                            end
                        end)
...
RegisterNetEvent("qpixel-jobs:spawn-truck")
AddEventHandler("qpixel-jobs:spawn-truck", function()
	Citizen.CreateThread(function()
        ...
        TriggerEvent('qpixel-jobs:passed')
...
exports["qpixel-polytarget"]:AddCircleZone("dodo_sign_in", vector3(-437.91, -2795.5, 7.39), 0.6, {
    useZ=true,
})

exports["qpixel-interact"]:AddPeekEntryByPolyTarget("dodo_sign_in", {{
    event = "qpixel-jobs:startDodoLogistics",
    id = "dodo_sign_in",
    icon = "circle",
    label = "Sign In",
    parameters = {},
