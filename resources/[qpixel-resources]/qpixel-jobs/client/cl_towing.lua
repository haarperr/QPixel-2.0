...
RegisterNetEvent('qpixel-jobs:tow_trucker_duty')
AddEventHandler('qpixel-jobs:tow_trucker_duty', function()
	if not clockedon then
		...
		clockedon = true
		TriggerEvent('qpixel-phone:signInInfoTowing')
		TriggerServerEvent('qpixel-duty:tow_trucker')
		TriggerEvent('qpixel-jobs:give_tow_job')
	else
		TriggerEvent('DoLongHudText', 'You\'re already clocked in.', 2)
	end
end)

RegisterNetEvent('qpixel-jobs_tow_trucker_off_duty')
AddEventHandler('qpixel-jobs_tow_trucker_off_duty', function()
	if clockedon then
...
RegisterNetEvent('qpixel-jobs:towing_accept')
AddEventHandler('qpixel-jobs:towing_accept', function()
	GiveTowJobWanker()
end)
...
function GiveTowJobWanker()
	...
	local vehicle = exports['qpixel-vehicles']:GetVehicleTable()
			...
			Citizen.CreateThread(function()
				while Aspect.Looking do
					...
					if #(vector3(Aspect.Location.x, Aspect.Location.y, Aspect.Location.z) - GetEntityCoords(PlayerPedId())) <= 320 then
						...
						setimpoundveh = plate
                        TriggerEvent('qpixel-fuel:setFuel', GetVehiclePedIsIn(PlayerPedId()), 100)
...
RegisterNetEvent('qpixel-jobs:give_tow_job')
AddEventHandler('qpixel-jobs:give_tow_job', function()
	...
	if clockedon then
		TriggerEvent('qpixel-phone:towing_offer')
	end
end)
...
RegisterNetEvent('qpixel-jobs:impound_confirm_shit_idk_anymore')
AddEventHandler('qpixel-jobs:impound_confirm_shit_idk_anymore', function()
	if DesirePolyImp and CamImp then
		TriggerEvent('qpixel-jobs:towing_imp')
	end
end)

RegisterNetEvent('qpixel-jobs:impound_accept')
AddEventHandler('qpixel-jobs:impound_accept', function()
	...
	if DoesEntityExist(vehicle) and NetworkHasControlOfEntity(vehicle) then
		...
		if licensePlate == setimpoundveh then
			...
			if exports['qpixel-hud']:MoneyBuff() then
				TriggerServerEvent('qpixel-jobs:givePayout', math.random(1076, 1554), "cash", '[24/7 Delivery Job | With Buff]')
			else
				TriggerServerEvent('qpixel-jobs:givePayout', math.random(652, 886), "cash", '[24/7 Delivery Job | Without Buff]')
			end
			TriggerEvent('qpixel-jobs:give_tow_job')
...
RegisterNetEvent('qpixel-jobs:imp_decline')
AddEventHandler('qpixel-jobs:imp_decline', function()
...
RegisterNetEvent('qpixel-jobs:towing_imp')
AddEventHandler('qpixel-jobs:towing_imp', function()
	...
	if DoesEntityExist(veh) and NetworkHasControlOfEntity(veh) then
		...
		if licensePlate == setimpoundveh then
			if CanImp then
				TriggerEvent('qpixel-jobs_tow_menu')
			end
		else
...
RegisterNetEvent('qpixel-jobs_tow_menu')
AddEventHandler('qpixel-jobs_tow_menu', function()
	...
	local TowMenu = {
		...
		{
			title = "Accept",
			action = "qpixel-jobs:accept_impound_req",
		},
		{
			title = "Decline",
			action = "qpixel-jobs:decline_impound_req",
		},
	}
	exports["qpixel-interface"]:showContextMenu(TowMenu)
end)

RegisterInterfaceCallback('qpixel-jobs:accept_impound_req', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
	TriggerEvent('qpixel-jobs:impound_accept')
end)

RegisterInterfaceCallback('qpixel-jobs:decline_impound_req', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
	TriggerEvent('qpixel-jobs:imp_decline')
end)
...
AddEventHandler('qpixel-polyzone:enter', function(name)
	if name == "towing_shit" then
		...
		pSignOnOff()
		if not clockedon then
			exports['qpixel-interface']:showInteraction('[E] Sign In')
		else
			exports['qpixel-interface']:showInteraction('[E] Sign Out', 'error')
		end
	end
end)
	...
	Citizen.CreateThread(function()
        while towing_duty do
			...
			if IsControlJustReleased(0, 38) then
				if not clockedon then
					TriggerEvent('qpixel-jobs:tow_trucker_duty')
					exports['qpixel-interface']:hideInteraction()
				elseif clockedon then
					DeleteEntity(pVeh)
					TriggerEvent('qpixel-jobs_tow_trucker_off_duty')
					exports['qpixel-interface']:hideInteraction()
				end
			end
		end
...
AddEventHandler('pv:tow', function()
	...
	if isVehicleTow then
		...
		if not currentlyTowedVehicle then
		    ...
		    local finished = exports["qpixel-taskbar"]:taskBar(15000,"Hooking up vehicle.")
        	if finished == 100 and aDist < 5.0 then
				...
				if targetVehicle ~= 0 then
					...
					local part2 = exports["qpixel-taskbar"]:taskBar(7000,"Towing Vehicle")
		    ...
		    TriggerEvent("animation:tow")
		    TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10.0, 'towtruck', 0.5)
		    local finished = exports["qpixel-taskbar"]:taskBar(7000,"Untowing Vehicle")
        	if finished == 100 and aDist < 2.0 then
                hasVehicleHookedUp = false

        		TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10.0, 'towtruck2', 0.5)
        		local part2 = exports["qpixel-taskbar"]:taskBar(7000,"Unhooking Vehicle")
