...
RegisterNetEvent("qpixel-fishing:updateLocation")
AddEventHandler("qpixel-fishing:updateLocation", function(location)
    fishingLocation = location
end)
...
local function loopSkill(count)
    ...
    while loopCount < count do
        ...
        local finished = exports["qpixel-ui"]:taskBarSkill(math.random(2000, 2500), math.random(7, 12))
...
local function reelInFish()
    ...
    for _, fish in pairs(fishes) do
        if not found and fish.chance > fishChance then
            ...
            if success then
                TriggerEvent('qpixel-fishing:caught_fish')
            else
...
RegisterNetEvent('qpixel-fishing:caught_fish')
AddEventHandler('qpixel-fishing:caught_fish', function()
...
local function sellFish()
    ...
    function processFish(fish, bMarket)
        local qty = exports["qpixel-inventory"]:getQuantity(fish, true)
    ...
    if totalFish > 0 then
        if exports['qpixel-hud']:MoneyBuff() then
            TriggerServerEvent('qpixel-jobs:givePayout', totalFish * pricePerFish * 3, "cash", '[Fishing Job | With Buff]')
        else
            TriggerServerEvent('qpixel-jobs:givePayout', totalFish * pricePerFish, "cash", '[Fishing Job | Without Buff]')
        end
    ...
    if totalBMarketFish > 0 then
        if nightTime then
            local payoutFactor = exports["qpixel-config"]:GetModuleConfig("main").payoutFactor
...
AddEventHandler("qpixel-npcs:ped:sellFish", function()
    sellFish()
end)
...
RegisterNetEvent('qpixel-fishing:clockIn')
AddEventHandler('qpixel-fishing:clockIn', function()
    pClockedIn = true
    TriggerEvent('qpixel-phone:signInFishing')
    ...
    Citizen.CreateThread(function()
        while sign_in_fishing do
			...
			if IsControlJustReleased(0, 38) then
               TriggerEvent('qpixel-fishing:clockIn') 
			end
		end
	end)
end ]]
