...
Citizen.CreateThread(function()
    exports["qpixel-polyzone"]:AddBoxZone("cosmic_picking", vector3(164.48, -238.71, 50.06), 4.6, 4.8,  {
		name="cosmic_picking",
        heading=340,
        minZ=48.26,
        maxZ=52.26
    ...
    exports["qpixel-polyzone"]:AddBoxZone("cosmic_roof_lower", vector3(196.9, -268.35, 50.02), 5, 5.4, {
    ...
    exports["qpixel-polyzone"]:AddBoxZone("cosmic_roof_upper", vector3(184.98, -263.91, 63.15), 5, 7.0, {
    ...
    exports["qpixel-polyzone"]:AddBoxZone("cosmic_inside_upper", vector3(185.6, -236.04, 54.07), 1, 1.4, {
    ...
    exports["qpixel-polyzone"]:AddBoxZone("cosmic_inside_down", vector3(201.78, -241.1, 65.74), 1, 1.2, {
...
AddEventHandler('qpixel-polyzone:enter', function(name)
    if name == "cosmic_picking" then
        local isEmployed = exports["qpixel-business"]:IsEmployedAt("cosmic_cannabis")
        local HasPermission = exports["qpixel-business"]:HasPermission('cosmic_cannabis',"craft_access")
        if isEmployed and HasPermission then
            nearPicking = true
            StartHarvisting()
			exports['qpixel-interface']:showInteraction(("[E] %s"):format("Start Harvisting"))
        end
        ...
        while nearPicking do
            ...
            if isEmployed and HasPermission then
                if IsControlJustReleased(0, 38) then
                    ...
                    local finished = exports['qpixel-taskbar']:taskBar(math.random(45000, 60000), 'Picking Weed')
...
RegisterNetEvent('qpixel-jobs:CraftJointsMenu')
AddEventHandler('qpixel-jobs:CraftJointsMenu', function()
    local isEmployed = exports["qpixel-business"]:IsEmployedAt("cosmic_cannabis")
    local HasPermission = exports["qpixel-business"]:HasPermission('cosmic_cannabis',"craft_access")
    if isEmployed and HasPermission then
        local CCJoint = {
            {
                ...
                key = "LS.CF",
                action = 'qpixel-jobs:ls_cf',
            },
            {
                title = "Alaskan Thunder Fuck",
                description = "Roll 2g Alaskan Thunder Fuck Joint",
                key = "AS.FF",
                action = 'qpixel-jobs:af',
            },
            {
                title = "Chiliad Kush",
                description = "Roll 2g Chiliad Kush Joint",
                key = "CK.J",
                action = 'qpixel-jobs:ck_j',
            },
        }
        exports["qpixel-interface"]:showContextMenu(CCJoint)
    else
        TriggerEvent('DoLongHudText', 'You are not employed at this business', 2)
    end
end)

RegisterInterfaceCallback('qpixel-jobs:ls_cf', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    if exports['qpixel-inventory']:hasEnoughOfItem('weedq', 1) then
        FreezeEntityPosition(GetPlayerPed(-1),true)
        TriggerEvent("animation:PlayAnimation","cokecut")
        local finished = exports['qpixel-taskbar']:taskBar(math.random(20000, 30000), 'Crafting LS Confidential')
        if (finished == 100) then
...
RegisterInterfaceCallback('qpixel-jobs:af', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    if exports['qpixel-inventory']:hasEnoughOfItem('weedq', 1) then
        FreezeEntityPosition(GetPlayerPed(-1),true)
        TriggerEvent("animation:PlayAnimation","cokecut")
        local finished = exports['qpixel-taskbar']:taskBar(math.random(20000, 30000), 'Alaskan Thunder Fuck')
        if (finished == 100) then
            TriggerEvent("inventory:removeItem","weedq", 1)
            Wait(1000)
            FreezeEntityPosition(GetPlayerPed(-1),false)
...
RegisterInterfaceCallback('qpixel-jobs:ck_j', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    if exports['qpixel-inventory']:hasEnoughOfItem('weedq', 1) then
        FreezeEntityPosition(GetPlayerPed(-1),true)
        TriggerEvent("animation:PlayAnimation","cokecut")
        local finished = exports['qpixel-taskbar']:taskBar(math.random(20000, 30000), 'Chiliad Kush')
        if (finished == 100) then
            TriggerEvent("inventory:removeItem","weedq", 1)
            Wait(1000)
            FreezeEntityPosition(GetPlayerPed(-1),false)
...
RegisterNetEvent('qpixel-jobs:EdiblesMenu')
AddEventHandler('qpixel-jobs:EdiblesMenu', function()
    local isEmployed = exports["qpixel-business"]:IsEmployedAt("cosmic_cannabis")
    local HasPermission = exports["qpixel-business"]:HasPermission('cosmic_cannabis',"craft_access")
    if isEmployed and HasPermission then
        local CCE = {
            {
                ...
                key = "CB",
                action = 'qpixel-jobs:brownies',
            },
            {
                title = "Cannabis Absinthe",
                description = "Make some cannabis absinthe",
                key = "CA",
                action = 'qpixel-jobs:absinthe',
            },
            {
                title = "Cannabis Gummies",
                description = "Make a batch of gummies",
                key = "CG",
                action = 'qpixel-jobs:gummies',
            },
            {
                title = "420 Bar",
                description = "Make some chocolate",
                key = "420",
                action = 'qpixel-jobs:420',
            },
        }
        exports["qpixel-interface"]:showContextMenu(CCE)
    else
        TriggerEvent('DoLongHudText', 'You are not employed at this business', 2)
    end
end)

RegisterInterfaceCallback('qpixel-jobs:brownies', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    if exports['qpixel-inventory']:hasEnoughOfItem('weedq', 1) then
        FreezeEntityPosition(GetPlayerPed(-1),true)
        TriggerEvent("animation:PlayAnimation","cokecut")
        local finished = exports['qpixel-taskbar']:taskBar(math.random(10000, 15000), 'Making Cannabis Brownies')
        if (finished == 100) then
            TriggerEvent("inventory:removeItem","weedq", 1)
            Wait(1000)
            FreezeEntityPosition(GetPlayerPed(-1),false)
...
RegisterInterfaceCallback('qpixel-jobs:absinthe', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    if exports['qpixel-inventory']:hasEnoughOfItem('weedq', 1) then
        FreezeEntityPosition(GetPlayerPed(-1),true)
        TriggerEvent("animation:PlayAnimation","cokecut")
        local finished = exports['qpixel-taskbar']:taskBar(math.random(10000, 15000), 'Making Cannabis Absinthe')
        if (finished == 100) then
            TriggerEvent("inventory:removeItem","weedq", 1)
            Wait(1000)
            FreezeEntityPosition(GetPlayerPed(-1),false)
...
RegisterInterfaceCallback('qpixel-jobs:gummies', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    if exports['qpixel-inventory']:hasEnoughOfItem('weedq', 1) then
        FreezeEntityPosition(GetPlayerPed(-1),true)
        TriggerEvent("animation:PlayAnimation","cokecut")
        local finished = exports['qpixel-taskbar']:taskBar(math.random(10000, 15000), 'Making Cannabis Gummies')
        if (finished == 100) then
            TriggerEvent("inventory:removeItem","weedq", 1)
            Wait(1000)
            FreezeEntityPosition(GetPlayerPed(-1),false)
...
RegisterInterfaceCallback('qpixel-jobs:420', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    if exports['qpixel-inventory']:hasEnoughOfItem('weedq', 1) then
        FreezeEntityPosition(GetPlayerPed(-1),true)
        TriggerEvent("animation:PlayAnimation","cokecut")
        local finished = exports['qpixel-taskbar']:taskBar(math.random(10000, 15000), 'Making 420 Bar')
        if (finished == 100) then
            TriggerEvent("inventory:removeItem","weedq", 1)
            Wait(1000)
            FreezeEntityPosition(GetPlayerPed(-1),false)
...
RegisterNetEvent("qpixel-cosmic:tray_1")
AddEventHandler("qpixel-cosmic:tray_1", function()
    TriggerEvent("server-inventory-open", "1", "cosmic-table-1");
end)

RegisterNetEvent("qpixel-cosmic:tray_2")
AddEventHandler("qpixel-cosmic:tray_2", function()
    TriggerEvent("server-inventory-open", "1", "cosmic-table-2");
end)

exports["qpixel-polytarget"]:AddBoxZone("cosmic_cannabis_roll_joint", vector3(164.45, -233.44, 50.06), 1, 2.6, {
    heading=70,
    minZ=46.46,
    maxZ=50.46
})
exports["qpixel-polytarget"]:AddBoxZone("cosmic_cannabis_box_weed", vector3(166.92, -232.68, 50.06), 1, 2.0, {
    heading=340,
    minZ=46.66,
    maxZ=50.66
})

exports["qpixel-interact"]:AddPeekEntryByPolyTarget("cosmic_cannabis_tray_1", {
    {
        event = "qpixel-cosmic:tray_1",
...
exports["qpixel-interact"]:AddPeekEntryByPolyTarget("cosmic_cannabis_tray_2", {
    {
        event = "qpixel-cosmic:tray_2",
 ...
 exports["qpixel-interact"]:AddPeekEntryByPolyTarget("cosmic_counter", {
     {
