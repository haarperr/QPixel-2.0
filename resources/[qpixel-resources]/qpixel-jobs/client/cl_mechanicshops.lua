...
AddEventHandler('qpixel-polyzone:enter', function(name)
    if name == "tuner_shop_craft" then
        ...
        TunerCraft()
        local isEmployed = exports["qpixel-business"]:IsEmployedAt("tuner_shop")
        if isEmployed then
...
AddEventHandler('qpixel-polyzone:enter', function(name)
    if name == "harmony_repairs_craft" then
        Harmony_Craft = true     
        HarmonyCraft()
        local isEmployed = exports["qpixel-business"]:IsEmployedAt("harmony_repairs")
        if isEmployed then
            exports['qpixel-interface']:showInteraction("[E] Craft")
        end
...
AddEventHandler('qpixel-polyzone:enter', function(name)
    if name == "hayes_autos_craft" then
        Hayes_Craft = true     
        HayesCraft()
        local isEmployed = exports["qpixel-business"]:IsEmployedAt("hayes_autos")
        if isEmployed then
            exports['qpixel-interface']:showInteraction("[E] Craft")
        end
...
AddEventHandler('qpixel-polyzone:enter', function(name)
    if name == "ottos_autos_craft" then
        ...
        OttosCraft()
        local isEmployed = exports["qpixel-business"]:IsEmployedAt("ottos_autos")
        if isEmployed then
            exports['qpixel-interface']:showInteraction("[E] Craft")
        end
...
Citizen.CreateThread(function()
    -- Tuner Shop | Start --
    exports["qpixel-polyzone"]:AddBoxZone("tuner_shop_stash", vector3(128.46, -3014.08, 7.04), 2.5, 2.5, {
