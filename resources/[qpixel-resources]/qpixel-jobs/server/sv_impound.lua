RegisterServerEvent('qpixel-impound:select_plate')
AddEventHandler('qpixel-impound:select_plate', function(pPlate)
    local src = source
    ...
    {['@license_plate'] = pPlate}, function(pValues)
        if pValues[1].current_garage == "Full Impound" or pValues[1].current_garage == "Normal Impound" then
            if char.id == pValues[1].cid then
                TriggerClientEvent('qpixel-impound:show_car', src, pValues[1].name, pValues[1].license_plate, false)
            else
...
RegisterServerEvent('qpixel-impound:release_car')
AddEventHandler('qpixel-impound:release_car', function(pPlate)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
...
RegisterNetEvent('qpixel-imp:FullImpound')
AddEventHandler('qpixel-imp:FullImpound', function(plate)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
...
RegisterNetEvent('qpixel-imp:NormalImpound')
AddEventHandler('qpixel-imp:NormalImpound', function(plate)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
...
RegisterNetEvent('qpixel-imp:ScuffImpound')
AddEventHandler('qpixel-imp:ScuffImpound', function(plate)
