...
AddEventHandler('server:GroupPayment', function( gangid, cashamount, sauce)
  ...
  RegisterServerEvent('qpixel-weedbox:payout')
AddEventHandler('qpixel-weedbox:payout', function(money)
    local source = source
        ...
        for i = 1, #Registers[regID] do
            if Registers[regID][i].regID == regID then
                ...
                if (tonumber(user:getCash()) >= tonumber(amount)) then
                    ...
                    local owner = exports["qpixel-base"]:getModule("Player"):GetUser(Registers[regID][i].owner)
...
RegisterServerEvent("qpixel-jobs:cosmic_cannabis:update:pay")
AddEventHandler("qpixel-jobs:cosmic_cannabis:update:pay", function(cid)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
