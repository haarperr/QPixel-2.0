Citizen.CreateThread(function()
    ...
    for idx,polyTarget in ipairs(ChairZones) do
        exports['qpixel-interact']:AddPeekEntryByPolyTarget('qpixel-polychair:chair_' .. tostring(idx) .. "-".. tostring(polyTarget[4].name), {{
            event = "qpixel-polychair:chairSit",
...
function chairSit(chairPosition, cancel)
    ...
    if isSitting then
        ...
        isSitting = false
        TriggerEvent('qpixel-polychair:stopSitting')
    else
        ...
        TriggerEvent('qpixel-polychair:startSitting')
    end
    exports["qpixel-flags"]:SetPedFlag(ped, 'isSittingOnChair', isSitting)
end

AddEventHandler('qpixel-polychair:chairSit', function(pParameters, pEntity, pContext)
    chairSit(pParameters.chairPosition, true)
end)

RegisterNetEvent("qpixel-emotes:sitOnChair")
AddEventHandler("qpixel-emotes:sitOnChair", function(pArgs, pEntity, pContext)
    chairSit(nil, true)
end)
