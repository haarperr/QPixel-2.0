...
Citizen.CreateThread(function()
	...
	-- Register Keybinds
	exports["qpixel-keybinds"]:registerKeyMapping("", "Weapons", "Switch Weapon Mode", "+switchWeaponMode", "-switchWeaponMode")
...
function SwitchWeaponMode()
	...
	if ForceWeaponMode[pWeaponHash] ~= nil then
		if SELECTED_FIRE_RATE == ForceWeaponMode[pWeaponHash] then
			...
			TriggerEvent("qpixel-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
		end
		return
	end
	...
	if selectedRate.label == "Single Fire" then
		TriggerEvent('qpixel-weapons:client:setWeaponFireRate', 30)
	elseif selectedRate.label == "Burst Fire" then
		TriggerEvent('qpixel-weapons:client:setWeaponFireRate', 70)
	elseif selectedRate.label == "Full Auto" then
		TriggerEvent('qpixel-weapons:client:setWeaponFireRate', 100)
	end	
end
...
RegisterNetEvent("qpixel-weapons:client:weaponEquiped")
AddEventHandler("qpixel-weapons:client:weaponEquiped", function(pWeaponHash, newInformation, sqlID, itemToRemove, pArmed)
	if ForceWeaponMode[pWeaponHash] ~= nil then
		SELECTED_FIRE_RATE = ForceWeaponMode[pWeaponHash]
		TriggerEvent("qpixel-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
	end
	...
	TriggerEvent("qpixel-weapons:client:showWeaponFireRate", pArmed)
	TriggerEvent("qpixel-weapons:client:setWeaponFireRate", 33.3 * SELECTED_FIRE_RATE)
end)
