...
CreateThread(function()
  ...
  while true do
    ...
    if RecentlyRiding then
      local BikeArmor = exports["qpixel-inventory"]:hasEnoughOfItem("bikearmor", 1, false,true)
      if BikeArmor then
...
function ProcessHitByCash(pAttacker)
  TriggerServerEvent("qpixel-weapons:attackedByCash", pAttacker)
end
...
AddEventHandler("DamageEvents:EntityDamaged", function(victim, attacker, pWeapon, isMelee)
  ...
  print(pWeapon)
  if ( (pWeapon == CrashHash or pWeapon == RamHash or pWeapon == FallHash) and exports["qpixel-inventory"]:hasEnoughOfItem("bikearmor", 1, false, true) and not DegApplied and RecentlyRiding) then
    DegApplied = true
