...
function createPendingRace(id, options)
    ...
    RPC.execute("qpixel-racing:createPendingRace", id, options)
end
...
function startRace(countdown)
    ...
    for k, v in pairs(pendingRaces) do
        ...
        if v.owner == characterId then
            ...
            RPC.execute("qpixel-racing:startRace", v.id, countdown or v.countdown, v.owner)
            return
        end
    end
end
...
function endRace()
    ...
    if curRace then
        RPC.execute("qpixel-racing:endRace")
        print("ENDING RACE 1")
    else
        RPC.execute("qpixel-racing:leaveRace")
    end
end

function joinRace(id, alias, characterId)
    RPC.execute("qpixel-racing:joinRace", id, alias, characterId)
end
...
function leaveRace()
    ...
    if curRace then
        RPC.execute("qpixel-racing:dnfRace", curRace.id)
        cleanupRace()
    else
        RPC.execute("qpixel-racing:leaveRace")
    end
...
function getAllRaces()
    ...
    local res = RPC.execute("qpixel-racing:getAllRaces")
    races = res.races
...
function getHasRaceUsbAndAlias()
    ...
    local pdRacingUsbItem = exports["qpixel-inventory"]:GetInfoForFirstItemOfName("racingusb3")
