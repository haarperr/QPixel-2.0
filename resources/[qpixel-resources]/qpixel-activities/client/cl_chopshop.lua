...
AddEventHandler("qpixel-polyzone:enter", function(zone)
    if zone == "chop_zone" and GetVehiclePedIsIn(PlayerPedId(), false) == aspectChopCar then
        ...
        aspectStartChopping()
        exports['qpixel-interface']:showInteraction('[E] Start Chopping')
    end
end)
...
function ChopVehicleTyre(vehicle, boneID, tyreIndex)
    ...
    local finished = exports["qpixel-taskbar"]:taskBar(10000,"Slicing Vehicle Tyre")
...
function ChopVehicleDoor(vehicle, boneID, doorIndex)
    ...
    FreezeEntityPosition(GetPlayerPed(-1),true)
    local finished = exports["qpixel-taskbar"]:taskBar(14000,"Messing With Door")
...
function ChopVehicleRemains(vehicle, boneID)
    ...
    FreezeEntityPosition(GetPlayerPed(-1),true)
    local finished = exports["qpixel-taskbar"]:taskBar(25000,"Seeing Whats Left ..")
...
AddEventHandler('aspect-chop:cooldown', function()
    ...
    if ClockedIntoChop then
        TriggerEvent('qpixel-phone:chop_shop_offer')
        CreateChopCar()
    end
end)
...
function ChopVehiclePart(vehicle)
    ...
    if success then
        TriggerServerEvent("qpixel-chopshop:vehicle_loot", bone.type)
...
RegisterNetEvent('qpixel-chopshop:clock-in')
AddEventHandler('qpixel-chopshop:clock-in', function()
    if CoolDownActive then
        TriggerEvent('DoLongHudText', 'You have a active cooldown!', 2)
    elseif ClockedIntoChop then
        TriggerEvent('qpixel-chopshop:clock-out')
    else
        ClockedIntoChop = true
        TriggerEvent('qpixel-phone:signInInfoChopshop')
        Citizen.Wait(math.random(60000, 120000))
        TriggerEvent('qpixel-phone:chop_shop_offer')
    end
end)

RegisterNetEvent('qpixel-chopshop:clock-out')
AddEventHandler('qpixel-chopshop:clock-out', function()
    if ClockedIntoChop then
