Config = {
    ...
    InventoryName = 'qpixel-inventory',
    TargetName = 'q',
    ...
    local bliptable = {}
        ...
        RegisterNetEvent('start-oxy', function()
            if exports['qpixel-inventory']:hasEnoughOfItem('darkmarketdeliveries', 1) and exports['qpixel-inventory']:hasEnoughOfItem('vpnxj', 1) then
                TriggerEvent('qpixel-phone:oxyRunNoti', 'Wait for a job offer !')
                Citizen.Wait(math.random(60000, 120000))
                TriggerEvent('qpixel-phone:oxyrun:offer')
            else
                TriggerEvent('DoLongHudText', 'You need a delivery list and a VPN to work here !', 2)
            end
        end)

        RegisterNetEvent('qpixel-oxy:Accept:Offer')
        AddEventHandler('qpixel-oxy:Accept:Offer', function()
            TriggerServerEvent('started-job')
            TriggerEvent('qpixel-phone:oxyRunNoti', 'Head towards the location on your GPS.')
        end)
        ...
        RegisterNetEvent('generate-ped-loc', function()
            ...
            exports["qpixel-interact"]:AddPeekEntryByModel(GetHashKey('a_m_m_bevhills_01'), {{
        ...
        RegisterNetEvent('collect-package', function()
            if toplampaket < 10 then
                ...
                if not exports['qpixel-inventory']:hasEnoughOfItem('darkmarketpackage', 1) then
                    runStarted = true
                    TriggerEvent("player:receiveItem", "darkmarketpackage", 1, false, nil)
                    toplampaket = toplampaket+1
                    TriggerEvent('qpixel-phone:oxyRunNoti', '('..toplampaket..'/10) Packages Collected.')

                    if toplampaket == 10 then
                        TriggerEvent('qpixel-phone:oxyRunNoti', 'All Packages Collected.')
        ...
        RegisterNetEvent('generate-new-corner-loc', function()
            ...
            jobs = jobs+1
            TriggerEvent('qpixel-phone:oxyRunNoti', 'Drive to the next location.')
        ...
        RegisterNetEvent('handoff-package', function()
            ...
            if exports['qpixel-inventory']:hasEnoughOfItem('darkmarketpackage', 1) then
                if licensePlate == pPlate then
                    ...
                    if handoff < 5 then
                        ...
                        handoff = handoff+1
                        TriggerEvent('qpixel-phone:oxyRunNoti', '('..handoff..'/5) Packages Handed Off.')
                        ...
                        if exports['qpixel-inventory']:hasEnoughOfItem('darkmarketpackage', 1, true) then
                            local rnd = math.random(1, 2)
                            if rnd == 1 then
                                exports["qpixel-dispatch"]:dispatchadd('10-37', "Investigate Suspicious Activity", "102")
                                TriggerEvent("player:receiveItem", "oxy", math.random(1, 5))
                            end
                            TriggerEvent("inventory:removeItem", "darkmarketpackage", 1)
                            TriggerEvent('qpixel-oxyruns:clean')
                            Wait(500)
                            TaskVehicleDriveWander(ped, veh, 20.0, 16777216)
                            SetDriverAggressiveness(ped, 0.0)
                            Wait(15000)
        ...
        AddEventHandler("qpixel-oxyruns:clean", function(pRandomChance)
            local payment = 0
            for typ, conf in pairs(billsCleaningStuff) do
              ...
              if math.random() < randomChance and exports["qpixel-inventory"]:hasEnoughOfItem(typ, totalAmount, false) then
                ...
                if exports['qpixel-hud']:MoneyBuff() then
                    TriggerServerEvent('qpixel-activities:givePayout', payment*2, 'cash', '[Oxy Run | With Buff | Cleaned Bands]')
                else
                    TriggerServerEvent('qpixel-activities:givePayout', payment, 'cash', '[Oxy Run | Without Buff | Cleaned Bands]')
                end
              end
            end
            if payment == 0 then
                TriggerServerEvent('qpixel-activities:givePayout', math.random(150, 350), 'cash', '[Oxy Run | Without Buff | Reg Payout]')
            end
          end)
          ...
          function thread()
            ...
            if Config.TargetName == 'q' then
                ...
                exports["qpixel-polytarget"]:AddCircleZone("oxystart", vector3(-1564.06, -440.33, 37.39), 0.55, {
                    useZ=true,
                })
                
                exports["qpixel-interact"]:AddPeekEntryByPolyTarget("oxystart", {
                    {
                        event = "start-oxy",
                        id = "oxystart1",
                        icon = "circle",
