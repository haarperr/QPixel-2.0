...
Citizen.CreateThread(function()
	local result = RPC.execute('qpixel-houserobbery:enter_property')
					...
					if #(GetEntityCoords(PlayerPedId()) - objCoords) <= items[i].dist and not items[i].robado then
						...
						if IsControlJustPressed(0,38) then
							...
							if items[i].anim == 'cajafuerte' then
								...
								TriggerEvent('qpixel-houserobber:animations',items[i].anim,items[i].delete,i)
							end
						end
					end
				end
...
RegisterNetEvent('qpixel-activities:acceptJob')
AddEventHandler('qpixel-activities:acceptJob', function()
...
RegisterNetEvent('qpixel-houserobbery:getJob')
AddEventHandler('qpixel-houserobbery:getJob', function()
	if not working then
		if exports['qpixel-inventory']:hasEnoughOfItem('vpnxj', 1) then
			if exports['qpixel-inventory']:hasEnoughOfItem('advlockpick', 1) then
				cleanup()
				working = true
				enterCoords = Houses()
                TriggerEvent('qpixel-phone:signInInfoHouseRobbery')
				Citizen.Wait(math.random(1,2) * 60 * 1000)
				TriggerEvent('qpixel-phone:house_rob_offer')
			else
...
RegisterNetEvent('qpixel-houserobbery:make_noise')
AddEventHandler('qpixel-houserobbery:make_noise', function()
	...
	while stealing do		
		if within then	
			...
			if noise > 100 then
				-- dispatch for pd
				exports["qpixel-dispatch"]:dispatchadd('10-31A', "House Robbery", "40")
				Citizen.Wait(300000)
			end
		end
...
RegisterCommand('raid', function()
	...
	if (job == "police" or job == "sheriff" or job == "state" or job == "sheriff" or job == "ranger") then
		TriggerEvent('qpixel-houserobbery:police_enter')
	end
end)


RegisterNetEvent('qpixel-houserobbery:police_enter')
AddEventHandler('qpixel-houserobbery:police_enter', function()
	...
	while working do
		local espera = 1500
			...
			if distancia < 3 and isnight() then
				espera = 5
				exports['qpixel-interface']:showInteraction('[H] To Enter House')
					if IsControlJustReleased(0,74) and distancia < 3 and firstentry then
						RemoveBlip(HouseRobberyLocation)
						if exports['qpixel-inventory']:hasEnoughOfItem('advlockpick', 1) then
							Animation()
							local finished = exports["qpixel-ui"]:taskBarSkill(3500,math.random(100, 200))
							if (finished == 100) then
								local finished2 = exports["qpixel-ui"]:taskBarSkill(1000,math.random(200,400))
								if (finished2 == 100) then
									local finished3 = exports["qpixel-ui"]:taskBarSkill(2000,math.random(50,100))
									if (finished3 == 100) then
										local finished4 = exports["qpixel-ui"]:taskBarSkill(1250,math.random(200,400))
										if (finished4 == 100) then
											local finished5 = exports["qpixel-ui"]:taskBarSkill(5000,math.random(200,400))
...
function EnterHouse(police)
	if not stealing then
		...
		stealing = true
		if not police then
			TriggerEvent('qpixel-houserobbery:make_noise')
		end
	else
...
RegisterNetEvent('qpixel-houserobber:animations')
AddEventHandler('qpixel-houserobber:animations', function(anim, delete, i)
	local ped = PlayerPedId()
	...
	if anim == 'tv' then
		...
		local StolenTV = exports['qpixel-taskbar']:taskBar(10000, 'Grabbing TV')
	...
	elseif anim == 'telescopio' then
		...
		FreezeEntityPosition(ped,true)
		TaskPlayAnim(ped, "mp_common_heist", 'use_terminal_loop', 2.0, 2.0, -1, 1, 0, true, true, true)
		local StolenTelescope = exports['qpixel-taskbar']:taskBar(2000, 'Grabbing Telescope')
	...
	elseif anim == 'pintura' then
		FreezeEntityPosition(ped,true)
		TaskPlayAnim(ped, "mp_common_heist", 'use_terminal_loop', 2.0, 2.0, -1, 1, 0, true, true, true)
		local StolenArt = exports['qpixel-taskbar']:taskBar(2000, 'Grabbing Art')
	...
	elseif anim == 'normal' then
		FreezeEntityPosition(ped,true)	
		AnimBolso()
		TriggerEvent('qpixel-houserobbery:common_items')
		if delete then
			DeleteEntity(items[i].objeto)
		end
	...
	elseif anim == 'laptop' then
		...
		FreezeEntityPosition(ped,true)
		TaskPlayAnim(ped, "mp_common_heist", 'use_terminal_loop', 2.0, 2.0, -1, 1, 0, true, true, true)
		local StolenMicrowave = exports['qpixel-taskbar']:taskBar(5000, 'Grabbing Microwave')
...
RegisterNetEvent("qpixel-houserobbery:common_items")
AddEventHandler("qpixel-houserobbery:common_items", function()
...
RegisterNetEvent('qpixel-houserobberies:sell_tv')
AddEventHandler('qpixel-houserobberies:sell_tv', function()
    if exports['qpixel-inventory']:hasEnoughOfItem('stolentv', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['qpixel-taskbar']:taskBar(5000, 'Selling Stolen TV.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolentv', 1)
           if exports['qpixel-hud']:MoneyBuff() then
                TriggerServerEvent('qpixel-activities:givePayout', math.random(250, 325), 'cash', '[House Robbery Saless | With Buff]')
            else
				TriggerServerEvent('qpixel-activities:givePayout', math.random(100, 300), 'cash', '[House Robbery Saless | Without Buff]')
            end
        end
    else
...
RegisterNetEvent('qpixel-houserobberies:sell_telescope')
AddEventHandler('qpixel-houserobberies:sell_telescope', function()
    if exports['qpixel-inventory']:hasEnoughOfItem('stolentelescope', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['qpixel-taskbar']:taskBar(5000, 'Selling Stolen Telescope.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
...
RegisterNetEvent('qpixel-houserobberies:sell_art')
AddEventHandler('qpixel-houserobberies:sell_art', function()
    if exports['qpixel-inventory']:hasEnoughOfItem('stolenart', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['qpixel-taskbar']:taskBar(5000, 'Selling Stolen Art.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolenart', 1)
           if exports['qpixel-hud']:MoneyBuff() then
				TriggerServerEvent('qpixel-activities:givePayout', math.random(225, 315), 'cash', '[House Robbery Saless | With Buff]')
			else
				TriggerServerEvent('qpixel-activities:givePayout', math.random(100, 300), 'cash', '[House Robbery Saless | Without Buff]')
        	end
   		end
...
RegisterNetEvent('qpixel-houserobberies:sell_laptop')
AddEventHandler('qpixel-houserobberies:sell_laptop', function()
    if exports['qpixel-inventory']:hasEnoughOfItem('stolenlaptop', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['qpixel-taskbar']:taskBar(5000, 'Selling Stolen Laptop.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolenlaptop', 1)
           if exports['qpixel-hud']:MoneyBuff() then
				TriggerServerEvent('qpixel-activities:givePayout', math.random(75, 126), 'cash', '[House Robbery Saless | With Buff]')
			else
				TriggerServerEvent('qpixel-activities:givePayout', math.random(50, 100), 'cash', '[House Robbery Saless | Without Buff]')
        	end
   		end
    else
...
RegisterNetEvent('qpixel-houserobberies:sell_microwave')
AddEventHandler('qpixel-houserobberies:sell_microwave', function()
    if exports['qpixel-inventory']:hasEnoughOfItem('stolenmicrowave', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['qpixel-taskbar']:taskBar(5000, 'Selling Stolen Microwave.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolenmicrowave', 1)
           if exports['qpixel-hud']:MoneyBuff() then
				TriggerServerEvent('qpixel-activities:givePayout', math.random(125, 200), 'cash', '[House Robbery Saless | With Buff]')
			else
				TriggerServerEvent('qpixel-activities:givePayout', math.random(75, 125), 'cash', '[House Robbery Saless | Without Buff]')
        	end
   		end
    else
...
RegisterNetEvent('qpixel-houserobberies:sell_nokia')
AddEventHandler('qpixel-houserobberies:sell_nokia', function()
    if exports['qpixel-inventory']:hasEnoughOfItem('stolennokia', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['qpixel-taskbar']:taskBar(5000, 'Selling Stolen Nokia.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolennokia', 1)
           if exports['qpixel-hud']:MoneyBuff() then
				TriggerServerEvent('qpixel-activities:givePayout', math.random(75, 100), 'cash', '[House Robbery Saless | With Buff]')
			else
				TriggerServerEvent('qpixel-activities:givePayout', math.random(25, 50), 'cash', '[House Robbery Saless | Without Buff]')
            end
        end
    else
...
RegisterNetEvent('qpixel-houserobberies:sell_iphone')
AddEventHandler('qpixel-houserobberies:sell_iphone', function()
    if exports['qpixel-inventory']:hasEnoughOfItem('stoleniphone', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['qpixel-taskbar']:taskBar(5000, 'Selling Stolen iPhone.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
...
RegisterNetEvent('qpixel-houserobberies:sell_gameboy')
AddEventHandler('qpixel-houserobberies:sell_gameboy', function()
    if exports['qpixel-inventory']:hasEnoughOfItem('stolengameboy', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['qpixel-taskbar']:taskBar(5000, 'Selling Stolen Game Boy.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolengameboy', 1)
           if exports['qpixel-hud']:MoneyBuff() then
			TriggerServerEvent('qpixel-activities:givePayout', math.random(75, 80), 'cash', '[House Robbery Saless | With Buff]')
		else
			TriggerServerEvent('qpixel-activities:givePayout', math.random(26, 50), 'cash', '[House Robbery Saless | Without Buff]')
		end
        end
    else
...
RegisterNetEvent('qpixel-houserobberies:sell_casio_watch')
AddEventHandler('qpixel-houserobberies:sell_casio_watch', function()
    if exports['qpixel-inventory']:hasEnoughOfItem('stolencasiowatch', 1) then
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['qpixel-taskbar']:taskBar(5000, 'Selling Stolen Casio Watch.')
        if finished == 100 then
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerEvent('inventory:removeItem', 'stolencasiowatch', 1)
			if exports['qpixel-hud']:MoneyBuff() then
				TriggerServerEvent('qpixel-activities:givePayout', math.random(126, 137), 'cash', '[House Robbery Saless | With Buff]')
			else
				TriggerServerEvent('qpixel-activities:givePayout', math.random(25, 75), 'cash', '[House Robbery Saless | Without Buff]')
			end
        end
    else
...
RegisterNetEvent('qpixel-houserobberies:sell_goods')
AddEventHandler('qpixel-houserobberies:sell_goods', function()

	local HouseRobSellData = {
		{
            title = "Sell Stolen TV",
            description = "",
            action = 'qpixel-houserobberies:sell_television',
        },
        {
            title = "Sell Stolen Telescope",
            description = "",
            action = 'qpixel-houserobberies:sell_ts',
        },
        {
            title = "Sell Stolen Art",
            description = "",
            action = 'qpixel-houserobberies:sell_art',
        },
        {
            title = "Sell Stolen Laptop",
            description = "",
            action = 'qpixel-houserobberies:sell_lt',
        },
        {
            title = "Sell Stolen Microwave",
            description = "",
            action = 'qpixel-houserobberies:sell_mw',
        },
        {
            title = "Sell Stolen Nokia",
            description = "",
            action = 'qpixel-houserobberies:sell_nk',
        },
        {
            title = "Sell Stolen iPhone",
            description = "",
            action = 'qpixel-houserobberies:sell_phone',
        },
        {
            title = "Sell Stolen Gameboy",
            description = "",
            action = 'qpixel-houserobberies:sell_gb',
        },
        {
            title = "Sell Stolen Casio Watch",
            description = "",
            action = 'qpixel-houserobberies:sell_watch',
        },
    }
    exports["qpixel-interface"]:showContextMenu(HouseRobSellData)
end)


RegisterInterfaceCallback('qpixel-houserobberies:sell_art', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('qpixel-houserobberies:sell_art')
end)

RegisterInterfaceCallback('qpixel-houserobberies:sell_television', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('qpixel-houserobberies:sell_tv')
end)

RegisterInterfaceCallback('qpixel-houserobberies:sell_ts', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('qpixel-houserobberies:sell_telescope')
end)

RegisterInterfaceCallback('qpixel-houserobberies:sell_lt', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('qpixel-houserobberies:sell_laptop')
end)

RegisterInterfaceCallback('qpixel-houserobberies:sell_mw', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('qpixel-houserobberies:sell_microwave')
end)

RegisterInterfaceCallback('qpixel-houserobberies:sell_nk', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('qpixel-houserobberies:sell_nokia')
end)

RegisterInterfaceCallback('qpixel-houserobberies:sell_phone', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('qpixel-houserobberies:sell_iphone')
end)

RegisterInterfaceCallback('qpixel-houserobberies:sell_gb', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('qpixel-houserobberies:sell_gameboy')
end)

RegisterInterfaceCallback('qpixel-houserobberies:sell_watch', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('qpixel-houserobberies:sell_casio_watch')
end)
