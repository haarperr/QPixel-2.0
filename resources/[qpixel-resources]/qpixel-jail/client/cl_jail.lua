...
function RoleplayStats()
    ...
    if exports["qpixel-inventory"]:hasEnoughOfItem("shitlockpick",1,false) then
        totalroleplay = totalroleplay + 10
    end

    if exports["qpixel-inventory"]:hasEnoughOfItem("jailfood",1,false) then
        totalroleplay = totalroleplay + 10
    end

    if exports["qpixel-inventory"]:hasEnoughOfItem("methbag",1,false) then
        totalroleplay = totalroleplay + 15
    end

    if exports["qpixel-inventory"]:hasEnoughOfItem("assphone",1,false) then
        totalroleplay = totalroleplay + 15
    ...
    if exports["qpixel-inventory"]:hasEnoughOfItem("slushy",1,false) then
        totalroleplay = totalroleplay + 10
    end
...
function InmateHasAll()
    if exports["qpixel-inventory"]:hasEnoughOfItem("slushy",1,false)
        and
        exports["qpixel-inventory"]:hasEnoughOfItem("-1810795771",1,false)
        and
        exports["qpixel-inventory"]:hasEnoughOfItem("methbag",1,false)
        and
        exports["qpixel-inventory"]:hasEnoughOfItem("assphone",1,false)
        and
        exports["qpixel-inventory"]:hasEnoughOfItem("slushy",1,false)
    then
        return true
    else
        return false
...
RegisterNetEvent('qpixel-jail:checkTime')
AddEventHandler('qpixel-jail:checkTime', function()
...
RegisterNetEvent('qpixel-jailmenu')
AddEventHandler('qpixel-jailmenu', function()

	local Jailmenu = {
		{
            title = "List Lawyers",
            description = "Check how many lawyers are in the city!",
            action = "qpixel-jail:lawyerspog",
        },
        {
            title = "Jail Time",
            description = "Check your jail time",
            action = "qpixel-jail:checktime2",
        },
        {
            ...
            children = {
				{
                    title = "Yes",
                    action = "qpixel-jail:swapchar",
                },
				{
                    title = "No",
                    action = "qpixel-jail:swap_char_no",
                },
            },
        },
    }
    exports["qpixel-interface"]:showContextMenu(Jailmenu)
end)

RegisterInterfaceCallback('qpixel-jail:swap_char_no', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
end)

RegisterInterfaceCallback('qpixel-jail:swapchar', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('swappingCharsLoop')
end)

RegisterInterfaceCallback('qpixel-jail:checktime2', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('qpixel-jail:checkTime')
end)

RegisterInterfaceCallback('qpixel-jail:lawyerspog', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('yellowPages:retrieveLawyersOnline')
end)

exports["qpixel-polytarget"]:AddCircleZone("jail_interactions", vector3(1828.76, 2579.78, 46.51), 0.3, {
    useZ = true
})

exports["qpixel-interact"]:AddPeekEntryByPolyTarget("jail_interactions", {
    {
        event = "qpixel-jailmenu",
        id = "jail_interact_3",
        icon = "circle",
        label = "Jail Services",
        parameters = {},
