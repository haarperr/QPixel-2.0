...
RegisterCommand('unjail', function(source, args)
    ...
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local target = exports["qpixel-base"]:getModule("Player"):GetUser(args[1])
    ...
    if user:getVar("job") == 'police' or user:getVar("job") == 'state' or user:getVar("job") == 'police' or user:getVar("job") == 'doc' or user:getVar("job") == 'judge' then
        if args[1] and exports["qpixel-base"]:getModule("Player"):GetUser(tonumber(args[1])) then
...
RegisterServerEvent('qpixel-jail:pJailedLog')
AddEventHandler('qpixel-jail:pJailedLog', function(months, jailed_player)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local user2 = exports["qpixel-base"]:getModule("Player"):GetUser(jailed_player)
...
RegisterServerEvent('qpixel-jail:pBilledLog')
AddEventHandler('qpixel-jail:pBilledLog', function(pFineAmount, fined_player)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local user2 = exports["qpixel-base"]:getModule("Player"):GetUser(fined_player)
    local charInfo = user:getCurrentCharacter()
    local charInfo2 = user2:getCurrentCharacter()
