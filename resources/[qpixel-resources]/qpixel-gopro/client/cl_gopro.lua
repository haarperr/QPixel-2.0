...
RegisterNetEvent("qpixel-gopros:updateRegisteredCams")
AddEventHandler("qpixel-gopros:updateRegisteredCams", function(pDashcams)
  activeDashcams = pDashcams
  exports["qpixel-interface"]:sendAppEvent("gopros", { dashcams = activeDashcams })
end)
AddEventHandler("playerSpawned", function()
  Wait(5550)
  RPC.execute("qpixel-gopros:playerSpawnedSendCams")
end)

RegisterCommand("qpixel-gopros:addUserToCamera", function()
  RPC.execute("qpixel-gopros:addUserToCamera",1,868)
  
  end)
  

--[[
RegisterUICallback("qpixel-interface:goproChangeSelectedPov", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = 'done' }})
  local activateGoPro = activateGoPro(tonumber(data.netId))
  Wait(600)
  exports["qpixel-interface"]:sendAppEvent("gopros", { switchingViews = false })
end)
]]
...
AddEventHandler("qpixel-interface:application-closed", function(name)
  ...
  DoScreenFadeOut(400)
  Wait(400)
  TriggerEvent("qpixel-casino:inVRHeadset", false)
  deactivateGoPro()
  ...
  exports["qpixel-interface"]:sendAppEvent("hud", { display = true })
  DoScreenFadeIn(1000)
end)
          ...
          if IsControlJustReleased(0, 38) then
              ...
              if destinationSelected then
                ...
                exports['qpixel-interface']:openApplication('textbox', {
                  callbackUrl = 'qpixel-interface:goProRegisterSecCam',
                  key = 1,
                  items = {
                    {
                      icon = "pencil-alt",
                ...
                local camPos = exports["qpixel-selector"]:getCurrentSelection()
                ...
                Wait(100)
                exports["qpixel-interface"]:showInteraction(("[E] %s"):format("View Here"))
                destinationSelected = true
                listenForKeypress()
                exports["qpixel-selector"]:startSelecting(1, 1, function()
                  return false
                end)
              end
          end
...
AddEventHandler("qpixel-inventory:itemUsed", function(item)
  ...
  local result = exports["qpixel-objects"]:PlaceObject(
    `prop_spycam`,
    {
      collision = false,
      groundSnap = false,
  ...
  local prompt = exports["qpixel-interface"]:OpenInputMenu({
    {
      name = 'name',
      label = 'Security Camera Stream Name',
      icon = 'pencil-alt'
  ...
  local success = RPC.execute("qpixel-gopros:addSecCamera", prompt.name, result[2].coords, result[2].rotation)
  if success then
...
local function activateSecurityCameraById(pId)
  local cam = RPC.execute("qpixel-gopros:getSecurityCameraById", pId)
  ...
  exports["qpixel-interface"]:sendAppEvent("hud", { display = false })
  DoScreenFadeIn(1000)
end
...
--[[
RegisterUICallback("qpixel-interface:gopros:getSecurityCameras", function(data, cb)
  local cid = exports["isPed"]:isPed("cid")
  local cams = RPC.execute("qpixel-gopros:getSecurityCameraByCid", cid)
  cb({ data = cams, meta = { ok = true, message = "done" } })
end)

RegisterUICallback("qpixel-interface:gopros:viewCameraById", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  Wait(500)
  exports["qpixel-interface"]:closeApplication("phone")
...
Citizen.CreateThread(function()
  exports["qpixel-keybinds"]:registerKeyMapping("", "Camera", "Look Up", "+securityCamLookUp", "-securityCamLookUp", "PAGEUP")
  exports["qpixel-keybinds"]:registerKeyMapping("", "Camera", "Look Down", "+securityCamLookDown", "-securityCamLookDown", "PAGEDOWN")
  exports["qpixel-keybinds"]:registerKeyMapping("", "Camera", "Look Left", "+securityCamLookLeft", "-securityCamLookLeft", "HOME")
  exports["qpixel-keybinds"]:registerKeyMapping("", "Camera", "Look Right", "+securityCamLookRight", "-securityCamLookRight", "END")
  exports["qpixel-keybinds"]:registerKeyMapping("", "Camera", "Turn Off", "+securityCamOff", "-securityCamOff", "ESCAPE")
  ...
  exports["qpixel-interact"]:AddPeekEntryByModel({ `prop_spycam` }, {
    {
      id = "cam_blur_action",
      event = "qpixel-gopro:blurCamera",
      icon = "fingerprint",
      label = "Smudge",
    },
...
AddEventHandler('qpixel-gopro:blurCamera', function(pArgs, pEntity, pContext)
  ...
  exports["qpixel-objects"]:UpdateObject(object.id, { blurred = true })
end)

AddEventHandler('qpixel-objects:objectCreated', function(pObject, pEntity)
  if pObject.data.model == `prop_spycam` then
    SetEntityCollision(pEntity, false, false)
  end
end)
--[[
RegisterUICallback("qpixel-interface:gopros:addUserToCamera", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  RPC.execute("qpixel-gopros:addUserToCamera", data.camera, data.cid)
end)
