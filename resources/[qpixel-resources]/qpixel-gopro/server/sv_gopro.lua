RPC.register("qpixel-gopros:addSecCamera", function(pSource, pName, pCoords, pRotation)
...
RPC.register("qpixel-gopros:getSecurityCameraById", function(pSource, pCameraID)
    local result = MySQL.query.await([[
...
RPC.register("qpixel-gopros:getSecurityCameraByStateID", function(pSource, pCid)
...
RPC.register("qpixel-gopros:getSecurityCameraAccessByCameraID", function(pSource, pCamera)
...
RPC.register("qpixel-gopros:addUserToCamera", function(pSource, pCamera, pCid)
    local camera = MySQL.query.await([[
        SELECT * FROM _cameras WHERE id = ?
    ]], {pCamera})

    print(pCid)
    local pCid = exports['qpixel-phone']:getCIDFromPhoneNumber(pCid)
    print(pCid)
...
RPC.register("qpixel-gopros:removeUserFromCamera", function(pSource, pCamera, pCid)
    local camera = MySQL.query.await([[
        SELECT * FROM _cameras WHERE id = ?
    ]], {pCamera})
