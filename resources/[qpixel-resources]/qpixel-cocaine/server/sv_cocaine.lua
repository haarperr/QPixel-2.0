RPC.register('heists:cocaine_paynow', function()
    local src = source
    if exports['qpixel-base']:GetModule('GetBalance')(src) >= 100000 then
        exports['qpixel-base']:GetModule('RemoveBank')(src, 100000)
    return true
    else
        return false
    end
...
RPC.register('heists:cocaine_start_vehicle', function()
    local src = source
    TriggerClientEvent('qpixel-heists:doVehicle', src, "dinghy",2004.4, 4007.85, 29.22)
...
RPC.register('heists:cocaine_dump_vehicle', function(x,y,z)
    local src = source
    TriggerClientEvent('qpixel-heists:doVehicle', src, "sultan",x,y,z)
