...
RegisterNetEvent('qpixel-items:SetAmmo')
AddEventHandler('qpixel-items:SetAmmo', function(sentammoTable)
...
function actionBarDown()
  ...
  TriggerServerEvent("qpixel-financials:cash:get", GetPlayerServerId(PlayerId()))
end
...
Citizen.CreateThread(function()
	exports["qpixel-keybinds"]:registerKeyMapping("", "Player", "Action Bar", "+actionBar", "-actionBar", "TAB")
	RegisterCommand('+actionBar', actionBarDown, false)
	RegisterCommand('-actionBar', actionBarUp, false)

	exports["qpixel-keybinds"]:registerKeyMapping("", "Player", "Wink", "+playerWink", "-playerWink")
	RegisterCommand('+playerWink', playerWink, false)
	RegisterCommand('-playerWink', function() end, false)

	exports["qpixel-keybinds"]:registerKeyMapping("", "Player", "Passive Mode", "+passiveMode", "-passiveMode")
...
AddEventHandler('qpixel-actionbar:hotreload', function()
...
Citizen.CreateThread( function()
	...
	while true do
      ...
      if weapon == "-37975472" then
        TriggerEvent("qpixel-weapons:threwSmokeGrenade")
        if lastEquippedItemToRemove then
          if exports["qpixel-inventory"]:hasEnoughOfItem(lastEquippedItemToRemove,1,false) then
        ...
        lastEquippedItemToRemove = nil
			...
			if throwableWeapons[weapon] and exports["qpixel-inventory"]:hasEnoughOfItem(weapon,1,false) then
		...
		if IsPedPlantingBomb(ped) then
			if exports["qpixel-inventory"]:hasEnoughOfItem("741814745",1,false) then
...
AddEventHandler("qpixel-voice:focus:set", function(pState)
  focusTaken = pState
end)
...
AddEventHandler('equipWeaponID', function(hash,newInformation,sqlID,itemToRemove)
	--GiveAmmoNow()
	if not exports["qpixel-propattach"]:canPullWeaponHoldingEntity() then return end
  ...
  lastEquippedItemToRemove = itemToRemove
	...
	if armed then
		...
		holster1h()		
		TriggerEvent('qpixel-hud:armed', 0)
	else
		armed = true
	...
	TriggerEvent("qpixel-weapons:client:weaponEquiped", hash, newInformation, sqlID, itemToRemove, armed)
	TriggerServerEvent("qpixel-weapons:weaponEquiped", hash, newInformation, sqlID, itemToRemove, armed)
...
function updateAmmoTable(newammo,ammoType)
	...
	TriggerServerEvent("qpixel-weapons:updateAmmo",newammo,ammoType,ammoTable)
end
...
RegisterNetEvent('qpixel-item:CheckClientAmmo')
AddEventHandler('qpixel-item:CheckClientAmmo', function(weapons)
	local ped = PlayerPedId()
...
function AttachmentCheck(weaponhash)
	...
	if exports["qpixel-inventory"]:hasEnoughOfItem("weapon_silencer_assault", 1, false, true) then
	...
	if exports["qpixel-inventory"]:hasEnoughOfItem("weapon_silencer_pistol", 1, false, true) then
	...
	if exports["qpixel-inventory"]:hasEnoughOfItem("weapon_oil_silencer", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 1532150734 )
	end

	if exports["qpixel-inventory"]:hasEnoughOfItem("weapon_scope", 1, false, true) then
	...
	if exports["qpixel-inventory"]:hasEnoughOfItem("weapon_uzi_extended", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0xE40F1CD2 )	
	end
	if exports["qpixel-inventory"]:hasEnoughOfItem("weapon_uzi_foldstock", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x8FBBD54C )		
	end
	if exports["qpixel-inventory"]:hasEnoughOfItem("weapon_uzi_woodstock", 1, false, true) then
...
RegisterNetEvent("qpixel-weapons:hitPlayerWithCash", function(pTarget)
  TriggerServerEvent("qpixel-weapons:processGiveCashAmount", pTarget, lastEquippedInfo.amount, lastEquippedInfo.id)
end)
