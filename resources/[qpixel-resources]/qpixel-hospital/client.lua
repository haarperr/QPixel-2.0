...
RegisterNetEvent("qpixel-hospitalization:checkin")
AddEventHandler("qpixel-hospitalization:checkin", function(allowed)
	if allowed then
		loadAnimDict('anim@narcotics@trash')
		TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',1.0, 1.0, -1, 1, 0, 0, 0, 0)
		local finished = exports["qpixel-taskbar"]:taskBar(2500,"Checking Credentials")
...
RegisterNetEvent("qpixel-hospitalization:page")
AddEventHandler("qpixel-hospitalization:page", function()
	if exports["isPed"]:isPed("countems") >= 1 then 
		loadAnimDict('anim@narcotics@trash')
		TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',1.0, 1.0, -1, 1, 0, 0, 0, 0)
		local finished = exports["qpixel-taskbar"]:taskBar(2500,"Paging a Doctor")
		if finished == 100 then
...
Citizen.CreateThread(function()
exports["qpixel-polytarget"]:AddCircleZone("pillbox_checkin", vector3(307.59, -595.3, 43.08), 0.35, {
...
AddEventHandler('qpixel-polyzone:enter', function(name)
    if name == "pillbox_ems_equipment" then
        ...
        EMSEquipment()
        if exports['isPed']:isPed('myjob') == "ems" then
            exports['qpixel-interface']:showInteraction("[E] Equipment")
        end
    end
end)
