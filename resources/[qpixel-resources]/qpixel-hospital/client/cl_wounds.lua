...
function CheckDamage(ped, bone, weapon)
    ...
    if Config.Bones[bone] ~= nil then
        if not BodyParts[Config.Bones[bone]].isDamaged then
            ...
            TriggerServerEvent('qpixel-hospital:server:SyncInjuries', {
                limbs = BodyParts,
...
RegisterNetEvent('qpixel-hospital:client:SyncBleed')
AddEventHandler('qpixel-hospital:client:SyncBleed', function(bleedStatus)
    isBleeding = tonumber(bleedStatus)
    DoBleedAlert()
end)

RegisterNetEvent('qpixel-hospital:client:FieldTreatLimbs')
AddEventHandler('qpixel-hospital:client:FieldTreatLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 1
    end
...
RegisterNetEvent('qpixel-hospital:client:FieldTreatBleed')
AddEventHandler('qpixel-hospital:client:FieldTreatBleed', function()
    if isBleeding > 1 then
...
RegisterNetEvent('qpixel-hospital:client:ReduceBleed')
AddEventHandler('qpixel-hospital:client:ReduceBleed', function()
    if isBleeding > 0 then
        isBleeding = tonumber(isBleeding) - 1
...
RegisterNetEvent('qpixel-hospital:client:ResetLimbs')
AddEventHandler('qpixel-hospital:client:ResetLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 0
    end
...
RegisterNetEvent('qpixel-hospital:client:RemoveBleed')
AddEventHandler('qpixel-hospital:client:RemoveBleed', function()
    isBleeding = 0
...
AddEventHandler('baseevents:onPlayerKilled', function(killedBy, data)
    TriggerEvent('qpixel-hospital:client:ResetLimbs')
    TriggerEvent('qpixel-hospital:client:RemoveBleed')
end)
