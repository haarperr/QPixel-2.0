...
RegisterInterfaceCallback("qpixel-signin:handler", function(data, cb)
  ...
  if eventData.sign_in then
    if eventData.job == "police" then
      ...
      TriggerServerEvent("qpixel-signin:duty", eventData.job)
    end
...
RegisterInterfaceCallback("qpixel-signin:spawnVehicle", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  local eventData = data.key
...
AddEventHandler("qpixel-signin:peekAction", function(pArgs, pEntity, pContext)
  ...
  print(MenuData[name])
  exports["qpixel-interface"]:showContextMenu(MenuData[name])
end)
