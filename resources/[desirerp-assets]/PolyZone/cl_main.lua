...
RegisterNetEvent('desirerp-base:PolyZoneUpdate')
AddEventHandler('desirerp-base:PolyZoneUpdate', function()
    local combo = ComboZone:Create({}, {name = "desirerp-polyzone", debugPoly = false})
    combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
        if enteredZones then
            for i = 1, #enteredZones do
                TriggerEvent('desirerp-polyzone:entered:'.. enteredZones[i].data.cComboZone, enteredZones[i].name, enteredZones[i].data)
                print('desirerp-polyzone:ssadasdsadsda2')
            end
        end
        if leftZones then
            for i = 1, #leftZones do
                TriggerEvent('desirerp-polyzone:exited:'.. leftZones[i].data.cComboZone, leftZones[i].name)
                print('desirerp-polyzone:stop2')
            end
        end
    end)
...
AddEventHandler("onResourceStart", function(resourceName)
    ...
    combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
        if enteredZones then
            for i = 1, #enteredZones do
                TriggerEvent('desirerp-polyzone:entered:'.. enteredZones[i].data.cComboZone, enteredZones[i].name, enteredZones[i].data)
                print('desirerp-polyzone:wedasdsadasdsadsadsa')
            end
        end
        if leftZones then
            for i = 1, #leftZones do
                TriggerEvent('desirerp-polyzone:exited:'.. leftZones[i].data.cComboZone, leftZones[i].name)
                print('desirerp-polyzone:stop')
            end
        end
