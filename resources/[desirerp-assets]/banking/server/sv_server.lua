...
AddEventHandler('bank:givemecash', function(sender, reciever, amount)
  local user = exports["desirerp-base"]:getModule("Player"):GetUser(sender)
...
AddEventHandler('bank:transfer', function(receiver, amount, cid, statement)
  ...
  local player = exports["desirerp-base"]:getModule("Player"):GetUser(tonumber(receiver))
...
AddEventHandler("bank:getLogs", function()
  local srcID = source
  local user = exports["desirerp-base"]:getModule("Player"):GetUser(srcID)
  local char = user:getCurrentCharacter()
