...
function getPedAppearance(ped)
    return {
        ...
        headBlend = exports["qpixel-clothes"]:getPedHeadBlend(ped),
        faceFeatures = exports["qpixel-clothes"]:getPedFaceFeatures(ped),
        headOverlays = exports["qpixel-clothes"]:getPedHeadOverlays(ped),
        components = getPedComponents(ped),
        props = exports["qpixel-clothes"]:getPedProps(ped),
        hair = getPedHair(ped),
...
function setPedAppearance(player, data)
    ...
    Citizen.CreateThread(function()    
        ...
        if data.faceFeatures then
            exports["qpixel-clothes"]:setPedFaceFeaturesJS(GetPlayerPed(-1), data.faceFeatures)
        end

        if data.headBlend then
            exports["qpixel-clothes"]:setPedHeadBlend(GetPlayerPed(-1), data.headBlend)
        end

        if data.props then
            exports["qpixel-clothes"]:setPedProps(GetPlayerPed(-1), data.props)
        end

        if data.headOverlays then
            exports["qpixel-clothes"]:setPedHeadOverlays(GetPlayerPed(-1), data.headOverlays)
        end
    ...
    local playerWalkstyle = RPC.execute("qpixel-clothes:getPlayerWalkstyle")
