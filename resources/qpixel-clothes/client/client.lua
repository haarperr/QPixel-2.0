...
RegisterNUICallback("closeNui", function(data)
    ...
    exports["qpixel-clothes"]:closeCamera()
...
--     QBCore.Functions.TriggerCallback('qpixel-clothes:getCosts', function(pass)
--         if pass then
--             print('geçti')
--         else
--             print('geçemedi')
...
function OpenShops(type)
    ...
    exports["qpixel-clothes"]:setCamera(2)
...
RegisterNUICallback("changeClothe", function(data, cb)
    ...
    elseif data.type == "fade" then
        ...
        local charGender = "m"
        local playerSex = RPC.execute("qpixel-clothes:getPlayerGender")
        if playerSex == 0 then
            charGender = "m"
        else
            charGender = "f"
...
RegisterNetEvent('qpixel-clothes:loadSkin')
AddEventHandler('qpixel-clothes:loadSkin', function(ped, data)
    setPedAppearance(ped, data)
end)

RegisterNetEvent("qpixel-clothes:openClothing", OpenShops)
