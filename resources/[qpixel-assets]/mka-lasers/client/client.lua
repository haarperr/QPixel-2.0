...
local function calculateCurrentPoint(fromPoint, toPoint, deltaTime, travelTimeBetweenTargets)
  local qpixeldDirection = toPoint - fromPoint
  local qpixeldDirectionDist = #qpixeldDirection
  local percentOfTravelTime = deltaTime / (travelTimeBetweenTargets * 1000)
  local distance = math.min(qpixeldDirectionDist * percentOfTravelTime, qpixeldDirectionDist)
  return fromPoint + (norm(qpixeldDirection) * distance)
end
