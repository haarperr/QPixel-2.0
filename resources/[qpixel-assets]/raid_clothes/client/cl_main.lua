...
function Save(save, close)
    ...
    if save then
        ...
        if not startingMenu or passedClothing then
            ...
            TriggerEvent("qpixel-spawn:finishedClothing","Finished")
        ...
        TriggerEvent("qpixel-spawn:finishedClothing","Old")
        LoadPed(oldPed)
    end
    ...
    TriggerEvent('qpixel-weapons:getAmmo')
...
RegisterInterfaceCallback("qpixel-ui:raid_clothes:addOutfitPrompt", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    Wait(1) --wait to fix ui bug?
    exports['qpixel-interface']:openApplication('textbox', {
        callbackUrl = 'qpixel-ui:raid_clothes:addOutfit',
        key = 1,
        items = {
            {
                icon = "pencil-alt",
...
RegisterInterfaceCallback("qpixel-ui:raid_clothes:addOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterInterfaceCallback("qpixel-ui:raid_clothes:changeOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
    TriggerEvent('hotel:outfit', { true, data.key }, 3)
    exports["qpixel-ui"]:hideContextMenu()
end)

RegisterInterfaceCallback("qpixel-ui:raid_clothes:deleteOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
...
AddEventHandler('raid_clothes:ListOutfits', function(skincheck)
    ...
    for i = 1, #skincheck do
        ...
        menuData[#menuData + 1] = {
            ...
            key = slot,
            children = {
                { title = "Change Outfit", action = "qpixel-ui:raid_clothes:changeOutfit", key = slot},
                { title = "Delete Outfit", action = "qpixel-ui:raid_clothes:deleteOutfit", key = slot},
            }
        }
    end
    if #menuData >= 0 then
        if #menuData < 20 then
            ...
            menuData[#menuData + 1] = {
                ...
                description = '',
                key = emptySlot,
                action = "qpixel-ui:raid_clothes:addOutfitPrompt"
            }
        end
...
AddEventHandler("qpixel-polyzone:enter", function(zone, data)
    ...
    if currentZone then
        exports['qpixel-interface']:showInteraction(("[M] %s"):format(currentZone.text))
...
RegisterNetEvent("qpixel-context:closeglobal")
AddEventHandler("qpixel-context:closeglobal", function()
    ...
    local data = {
        {
            ...
            description = "",
            key = true,
            action = "qpixel-ui:raid_clothes:CustomaddOutfitPrompt"
        },
    }
...
RegisterInterfaceCallback("qpixel-ui:raid_clothes:CustomaddOutfitPrompt", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = 'done' } })
    Wait(1) --wait to fix ui bug?
    exports['qpixel-ui']:openApplication('textbox', {
        callbackUrl = 'qpixel-ui:raid_clothes:addCustomOutfit',
        key = data.key,
        items = {
            {
                icon = "pencil-alt",
...
RegisterInterfaceCallback("qpixel-ui:raid_clothes:addCustomOutfit", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterCommand("h1", function(source, args, rawCommand)
    if exports['qpixel-inventory']:hasEnoughOfItem('hat', 1) then
...
RegisterCommand("m1", function(source, args, rawCommand)
    if exports['qpixel-inventory']:hasEnoughOfItem('mask', 1) then
...
AddEventHandler("qpixel-inventory:itemUsed", function(pItem, pInfo)
  storedpItem = pItem
  storedpInfo = pInfo
