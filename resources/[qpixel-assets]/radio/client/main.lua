...
AddEventHandler('ChannelSet3', function()
  ...
  exports["qpixel-voice"]:setVoiceProperty("radioEnabled", true)
  exports['qpixel-voice']:addPlayerToRadio(3)
  PlaySound(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
...
AddEventHandler('ChannelSet4', function()
  ...
  exports["qpixel-voice"]:setVoiceProperty("radioEnabled", true)
  exports['qpixel-voice']:addPlayerToRadio(4)
  PlaySound(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
...
AddEventHandler('ChannelSet5', function()
  ...
  exports["qpixel-voice"]:setVoiceProperty("radioEnabled", true)
  exports['qpixel-voice']:addPlayerToRadio(5)
  PlaySound(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
...
function openGui()
  ...
  elseif job == "ems" then
    Emergency = true
  elseif exports["qpixel-inventory"]:hasEnoughOfItem("scanner",1,false) then
    Emergency = true
  end
...
local function handleConnectionEvent(pChannel)
  ...
  if newChannel < 1.0 then
    ...
    pRadioChannel = true
    exports["qpixel-voice"]:setVoiceProperty("radioEnabled", true)
    exports['qpixel-voice']:addPlayerToRadio(newChannel)
  end
end
...
function hasRadio()
    if exports["qpixel-inventory"]:hasEnoughOfItem("radio", 1, false, true) or exports["qpixel-inventory"]:hasEnoughOfItem("civradio", 1, false, true)then
      return true
    else
      return false
    end
...
RegisterNUICallback('volumeUp', function(data, cb)
  ...
  TriggerEvent('qpixel-voice:SetRadioVolume', radioVolume)
...
AddEventHandler('qpixel-inventory:itemCheck', function (item, state, quantity)
    if item ~= "civradio" and item ~= "radio" and item ~="scanner" then return end
    if state or quantity > 0 then return end
    exports["qpixel-voice"]:removePlayerFromRadio(false)
    exports["qpixel-ui"]:sendAppEvent("radio", { value = 0, state = false })
