...
RegisterNetEvent('qpixel-base:PolyZoneUpdate')
AddEventHandler('qpixel-base:PolyZoneUpdate', function()
    local combo = ComboZone:Create({}, {name = "qpixel-polyzone", debugPoly = false})
    combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
        if enteredZones then
            for i = 1, #enteredZones do
                TriggerEvent('qpixel-polyzone:entered:'.. enteredZones[i].data.cComboZone, enteredZones[i].name, enteredZones[i].data)
                print('qpixel-polyzone:ssadasdsadsda2')
            end
        end
        if leftZones then
            for i = 1, #leftZones do
                TriggerEvent('qpixel-polyzone:exited:'.. leftZones[i].data.cComboZone, leftZones[i].name)
                print('qpixel-polyzone:stop2')
            end
        end
    end)
...
AddEventHandler("onResourceStart", function(resourceName)
    ...
    combo:onPlayerInOutExhaustive(function(isPointInside, point, insideZones, enteredZones, leftZones)
        if enteredZones then
            for i = 1, #enteredZones do
                TriggerEvent('qpixel-polyzone:entered:'.. enteredZones[i].data.cComboZone, enteredZones[i].name, enteredZones[i].data)
                print('qpixel-polyzone:wedasdsadasdsadsadsa')
            end
        end
        if leftZones then
            for i = 1, #leftZones do
                TriggerEvent('qpixel-polyzone:exited:'.. leftZones[i].data.cComboZone, leftZones[i].name)
                print('qpixel-polyzone:stop')
            end
        end
