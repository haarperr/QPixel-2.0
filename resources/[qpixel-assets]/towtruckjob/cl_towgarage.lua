...
-- 		local finished = exports["qpixel-taskbar"]:taskBar(20000,"Changing plates", true)
...
AddEventHandler('towgarage:craft', function(res,amount)
	...
	if rank >= 4 or rank2 > 1 then -- Eddie wants rank 4 for crafting
		...
		local finished = exports["qpixel-taskbar"]:taskBar(1000,"Completing Task",false,true)

		if finished ~= 100 then
			return
		end
		if res == 1 and rank > 1 then
			local hasitems = exports["qpixel-inventory"]:hasEnoughOfItem("electronics",100*amount,true)
			if hasitems then
		...
		elseif res == 2 then
			...
			local hasitems = exports["qpixel-inventory"]:hasEnoughOfItem("copper",3*amount,true)
			if hasitems then
		...
		elseif res == 3 then
			...
			local hasitems = exports["qpixel-inventory"]:hasEnoughOfItem("aluminium",2*amount,true)
			if hasitems then
		...
		elseif res == 4 then
			...
			local hasitems = exports["qpixel-inventory"]:hasEnoughOfItem("steel",2*amount,true)
			if hasitems then
				TriggerEvent("inventory:removeItem","steel", 2*amount)
				TriggerEvent("player:receiveItem","lockpick",amount)
			end
		elseif res == 5 then
			local hasitems = exports["qpixel-inventory"]:hasEnoughOfItem("electronics",3*amount,true)
			if hasitems then
		...
		elseif res == 6 and rank > 1 then
			local hasitems = exports["qpixel-inventory"]:hasEnoughOfItem("rubber",10*amount,true)
			if hasitems then
...
AddEventHandler('towgarage:triggermenu', function(degradation,eHealth,bHealth)
	...
	if targetVehicle ~= nil  and targetVehicle ~= 0 then
				...
				children = {
					{
						...
						key = "brakes",
						action = "qpixel-vehicles:fixPart"
					},
					{
						title = "Axle",
		...
		exports["qpixel-interface"]:showContextMenu(Internals)
	end
end)

RegisterInterfaceCallback('qpixel-vehicles:fixPart', function(data, cb)
...
AddEventHandler('towgarage:TriggerRepairs', function(degname,amount)
	...
	if not notfucked then
		...
		local finished = exports["qpixel-taskbar"]:taskBar(15000,"Repairing")
