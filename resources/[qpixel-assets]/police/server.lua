...
AddEventHandler('police:targetCheckBank', function(target)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(target)
...
AddEventHandler('police:jailGranted', function(args)
    ...
    local target = exports["qpixel-base"]:getModule("Player"):GetUser(player)
    local character = target:getCurrentCharacter()
    local playerName = character.first_name .. ' ' .. character.last_name

    local pdunit = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    if not pdunit:getVar("job") == "police" then
        local steamid, name = pdunit:getVar("name"), pdunit:getVar("steamid")

        exports["qpixel-log"]:AddLog("Exploiter", pdunit, "User Attempted to jail player while not on pd", {
            target = playerName,
            reason = reason,
            cid = cid,
    ...
    exports["qpixel-base"]:getModule("JobManager"):SetJob(target, "unemployed", true)
...
AddEventHandler("police:deletecrimes", function(target)
    ...
    if not pdunit:getVar("job" == "police") then
        ...
        exports["qpixel-log"]:AddLog("Exploiter", pdunit, "User Attempted delete crimes while not on duty", {
            target = target,
            src = src
        })
        DropPlayer(src, "")
...
AddEventHandler('police:dnaAsk', function(t)
	local src = source
	local user2 = exports["qpixel-base"]:getModule("Player"):GetUser(t)
...
AddEventHandler("police:targetCheckInventory", function(target, status)
    local src = source
    local user2 = exports["qpixel-base"]:getModule("Player"):GetUser(target)
...
AddEventHandler('Gimmeurfuknmoney', function(target)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local targetply = exports["qpixel-base"]:getModule("Player"):GetUser(target)
...
AddEventHandler('police:SeizeCash', function(target)
    ...
    local userz = exports["qpixel-base"]:getModule("Player"):GetUser(target)
...
RegisterCommand('bill', function(source, args)
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(tonumber(args[1]))
    local character = user:getCurrentCharacter()
    local user2 = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local character2 = user2:getCurrentCharacter()
    exports.ghmattimysql:execute('SELECT job FROM jobs_whitelist WHERE cid = ?', {character2.id}, function(result)
        if result[1].job == 'police' then
            ...
            TriggerServerEvent('qpixel-financials:business_money', args[2], 'police', 'add')
        end
    end)
end)
    ...
    if jobType == "police" or jobType == "ems" or jobType == "doctor" or jobType == "therapist" or jobType == "doc" then
        exports.ghmattimysql:execute("SELECT rank from jobs_whitelist WHERE job = @job AND cid = @cid", {
            ...
            if (result[1]) then
                ...
                exports["qpixel-log"]:AddLog("HC Added Job", "Character: " .. char.id .. " Changed Char: " .. arg ..
    ...
    if jobType == "police" or jobType == "ems" or jobType == "doctor" or jobType == "therapist" or jobType == "doc" then
        exports.ghmattimysql:execute("SELECT rank from jobs_whitelist WHERE job = @job AND cid = @cid", {
            ...
            if (result[1]) then
                ...
                exports["qpixel-log"]:AddLog("HC Removed Job", "Character: " .. char.id .. " Changed Char: " .. arg ..
...
-- 	local commands = exports["qpixel-base"]:getModule("Commands")
...
function jailUser(player, reason, cid, time, src)
    local jailer = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    if not jailer then
        return false
    end
    ...
    if not jailer:getVar("job") == "police" and not jailer:getVar("job") == "doc" then
        ...
        exports["qpixel-log"]:AddLog("Exploiter", jailer,
    ...
    TriggerEvent("qpixel-police:playerJailed", character)
...
AddEventHandler("911", function(args, pStreetName)
    ...
    local connect = {
        {
            ["color"] = color,
            ["title"] = "** qpixel [Police] | 911 Call **",
            ["description"] = "Steam Name:"..pName.. " \n Steam ID: "..identifiers[1].. " \n 911 | " .. char.first_name .. " " .. char.last_name .. " \n Message: " .. tostring(message),
        }
    }
    PerformHttpRequest("https://discord.com/api/webhooks/1012082593790951525/jOaujaZf4fXM5XVf8hiVBfylIGS65mGODaGY3G7xvmOMo6KHkyhWjd6xvNDyeCMfQ4OR", function(err, text, headers) end, 'POST', json.encode({username = "qpixel | 911 Call", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
    ...
    local users = exports["qpixel-base"]:getModule("Player"):GetUsers()

    local emergencyPlayers = {}

    for k, v in pairs(users) do
        local user = exports["qpixel-base"]:getModule("Player"):GetUser(v)
        local job = user:getVar("job")
...
AddEventHandler("311", function(args)
    ...
    TriggerClientEvent("chatMessage", src,
        ...
        local connect = {
            {
                ["color"] = color,
                ["title"] = "** qpixel [Police] | 311 Call **",
                ["description"] = "Steam Name:"..pName.. " \n Steam ID: "..identifiers[1].. " \n 911 | " .. char.first_name .. " " .. char.last_name .. " \n Message: " .. tostring(message),
            }
        }
        PerformHttpRequest("https://discord.com/api/webhooks/1012082637910847600/cKpN2i_Sn8PNYulkz54mPViM_SawPRIVmzrUUfKXXdSY6OeGUvR9n970_1_y2ixT36Tt", function(err, text, headers) end, 'POST', json.encode({username = "qpixel | 311 Call", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
...
AddEventHandler("police:targetCheckWeapons", function(playerId,items)
    local src = source
    local char = exports["qpixel-base"]:getModule("Player"):GetUser(playerId)
...
-- 	end
-- 	local caller = tostring(char.first_name) .. " " .. tostring(char.last_name)
-- 	exports["qpixel-log"]:AddLog("911 Chat", user, caller .. ": " .. message, {})
...
RegisterCommand('pstate', function(source, args)
    local src = source
    local users = exports["qpixel-base"]:getModule("Player"):GetUser(src)
...
RegisterServerEvent('qpixel-police:hire_pd')
AddEventHandler('qpixel-police:hire_pd', function(pCID, pRank, pCallsign, pDept)
...
RegisterServerEvent('qpixel-police:sv:open_hire_menu')
AddEventHandler('qpixel-police:sv:open_hire_menu', function()
    local src = source
    local user = exports["qpixel-base"]:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
...
local JSON = json.decode(LoadResourceFile("qpixel-hud", "/config/pursuitModes.json"))
