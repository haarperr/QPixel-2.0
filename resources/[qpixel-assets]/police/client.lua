...
Citizen.CreateThread(function()
    exports["qpixel-keybinds"]:registerKeyMapping("", "Player", "Tackle", "+plyTackel", "-plyTackel")
...
Citizen.CreateThread(function()
    while true do
        ...
        TriggerServerEvent("qpixel-base:updatecoords", coords.x, coords.y, coords.z)
        Wait(13000)
    end
end)
    ...
    while true do
        ...
        if recentsearch == 0 then
            ...
            if endDistance < 0.7 or endDistance2 < 0.7 then
                ...
                currentWeapons = exports["qpixel-inventory"]:GetCurrentWeapons()
...
AddEventHandler("police:rob", function()
    ...
    if not exports["isPed"]:isPed("dead") then
        ...
        local finished = exports["qpixel-taskbar"]:taskBar(3500, "Robbing", false, true)
...
AddEventHandler('KneelHU', function()
    ...
    if (DoesEntityExist(player) and not IsEntityDead(player)) then
        ...
        local finished = exports["qpixel-taskbar"]:taskBar(2500, "Surrendering")
...
AddEventHandler('police:wshotgun', function()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
            ...
            local finished = exports["qpixel-taskbar"]:taskBar(2000, "Unracking Shotgun")
            if finished == 100 then
...
AddEventHandler('police:wrifle', function()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
            ...
            loadAnimDict('anim@narcotics@trash')
            TaskPlayAnim(PlayerPedId(), 'anim@narcotics@trash', 'drop_front', 0.9, -8, 1900, 49, 3.0, 0, 0, 0)
            local finished = exports["qpixel-taskbar"]:taskBar(2000, "Unracking Rifle")
            if finished == 100 then
...
AddEventHandler('police:woxy', function()
    ...
    if vehFront > 0 then
        ...
        local finished = exports["qpixel-taskbar"]:taskBar(4000, "Grabbing Scuba Gear")
        if finished == 100 then
            loadAnimDict('anim@narcotics@trash')
            TaskPlayAnim(PlayerPedId(), 'anim@narcotics@trash', 'drop_front', 0.9, -8, 1900, 49, 3.0, 0, 0, 0)
...
AddEventHandler('giveRadioToPed', function()
    ...
    if (not exports["qpixel-inventory"]:hasEnoughOfItem("radio", 1, false)) then
...
AddEventHandler('swatcopLoadout', function()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
            ...
            local finished = exports["qpixel-taskbar"]:taskBar(2500, "Changing to Swat")
            if finished == 100 then
                ChangeToSkinNoUpdatePolice("s_m_y_swat_01")
            end
            ClearPedTasks(PlayerPedId())
...
AddEventHandler('police:fingerPrintPlayer', function()
    local finished = exports["qpixel-taskbar"]:taskBar(8000,"DNA Testing")
    if finished == 100 then
        t, distance = GetClosestPlayer()
        if(distance ~= -1 and distance < 5) then
...
AddEventHandler('police:gsr', function(t)
    ...
    local finished = exports["qpixel-taskbar"]:taskBar(15000, "GSR Testing")
    if finished == 100 then
        t, distance = GetClosestPlayer()
        if (distance ~= -1 and distance < 7) then
...
-- local finished = 0
-- if not isDead then
-- finished = exports["qpixel-taskbarskill"]:taskBar(1200,7)
-- end
...
AddEventHandler('police:getArrested2', function(cuffer)
    ...
    if not isDead and cuffAttemptCount < 4 then
        ...
        finished = exports["qpixel-ui"]:taskBarSkill(cuffAttemptTbl[cuffAttemptCount][1],
...
AddEventHandler('police:jailing', function(args)
    ...
    TriggerServerEvent('qpixel-jail:pJailedLog', tonumber(args[2]), tonumber(args[1]))
end)
...
AddEventHandler('qpixel-polyzone:enter', function(pZone)
    if pZone == "evidence_zone" then
        inEvidenceZone = true
        exports['qpixel-interface']:showInteraction('/evidence [LOCKER ID]')
    end
end)

RegisterNetEvent('qpixel-polyzone:exit')
AddEventHandler('qpixel-polyzone:exit', function(zoneName)
...
AddEventHandler('police:tenThirteenA', function()
    exports["qpixel-dispatch"]:policedead('10-13A', "Officer Down URGENT", "630", true)
end)

RegisterNetEvent('police:tenThirteenB')
AddEventHandler('police:tenThirteenB', function()
    exports["qpixel-dispatch"]:policedead('10-13B', "Officer Down", "630", false)
end)

RegisterNetEvent("police:tenForteenA")
AddEventHandler("police:tenForteenA", function()
    exports["qpixel-dispatch"]:policedead('10-14A', "Medic Down URGENT", "630", true)
end)

RegisterNetEvent("police:tenForteenB")
AddEventHandler("police:tenForteenB", function()
    local pos = GetEntityCoords(PlayerPedId(), true)
    exports["qpixel-dispatch"]:policedead('10-14B', "Medic Down", "630", false)
end)
    ...
    if t ~= nil and t ~= -1 then
        if (distance ~= -1 and distance < 5) then
            ...
            if myjob ~= "ems" and myjob ~= "doctor" then
                local bandages = exports["qpixel-inventory"]:getQuantity("bandage")
                if bandages == 0 then
                    return
                else
...
AddEventHandler("ems:stomachpump", function()
    ...
    if t ~= nil and t ~= -1 then
        if (distance ~= -1 and distance < 5) then
            local finished = exports["qpixel-taskbar"]:taskBar(10000, "Inserting stomach pump 🤢", false, true)
...
RegisterCommand('911', function(source, args)
    TriggerEvent('qpixel-dispatch:911')
...
AddEventHandler('FlipVehicle', function()
    ...
    local finished = exports["qpixel-taskbar"]:taskBar(18000, "Flipping Vehicle Over", false, true)
...
AddEventHandler("police:dorifto", function()
    if (IsPedInAnyVehicle(PlayerPedId(), false)) then
        ...
        if Driver == PlayerPedId() and IsVehicleModel(veh, defaultHash) or IsVehicleModel(veh, defaultHash2) or
            ...
            local finished = exports["qpixel-taskbar"]:taskBar(3000, "Toggling Drift Mode", false, true)
...
AddEventHandler('police:frisk', function()
    t, distance = GetClosestPlayer()
    if (distance ~= -1 and distance < 5) then
        local items = exports["qpixel-inventory"]:getFullItemList()
        TriggerServerEvent("police:targetCheckWeapons", GetPlayerServerId(t),items)
        ExecuteCommand("e search")
        local finished = exports["qpixel-taskbar"]:taskBar(3000, "Frisking")
    else
        TriggerEvent("DoLongHudText", "No player near you!", 2)
    end
end)
...
RegisterInterfaceCallback('police:hireNewPersonel', function(data, cb)
    ...
    if dept == 1 then
        TriggerServerEvent('qpixel-police:hire_pd', data.values.cid, data.values.rank, data.values.callsign, 'police')
    elseif dept == 2 then
        TriggerServerEvent('qpixel-police:hire_pd', data.values.cid, data.values.rank, data.values.callsign, 'sheriff')
    elseif dept == 3 then
        TriggerServerEvent('qpixel-police:hire_pd', data.values.cid, data.values.rank, data.values.callsign, 'state')
    end
end)
...
AddEventHandler('police:hire', function()
    TriggerServerEvent('qpixel-police:sv:open_hire_menu')
end)

exports["qpixel-polytarget"]:AddCircleZone("qpixel_police_hire",  vector3(463.28, -984.29, 30.72), 0.55, {
    useZ = true
})

exports["qpixel-interact"]:AddPeekEntryByPolyTarget("qpixel_police_hire", {{
    event = "police:hire",
    id = "qpixel_police_hire",
    icon = "id-badge",
    label = "Hire Personel!",
    parameters = {},
}}, {
...
AddEventHandler('police:BillLog', function(pAmt, pPlayer)
    TriggerServerEvent('qpixel-jail:pBilledLog', pAmt, pPlayer)
end)
...
AddEventHandler('qpixel-polyzone:enter', function(name)
    if name == "qpixel_police_swat_armory" then
        if exports['isPed']:isPed('myjob') == "police" then
            pInSwatArmory = true
            exports["qpixel-interface"]:showInteraction("[E] Armory")
            SwatArmory()
        end
    end
end)
