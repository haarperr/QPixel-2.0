...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if isNear then
            TriggerServerEvent('desirerp-paleto:getGetDoorStateSV')
...
RegisterNetEvent("desirerp-paleto:getDoorCheckCL")
AddEventHandler("desirerp-paleto:getDoorCheckCL", function(result)
...
RegisterNetEvent("desirerp-paleto:resetDoorState")
AddEventHandler("desirerp-paleto:resetDoorState", function(name)
    Freeze[name] = 0
end)

RegisterNetEvent("desirerp-paleto:lootup_c")
AddEventHandler("desirerp-paleto:lootup_c", function(var, var2)
    LootCheck[var][var2] = true
end)

RegisterNetEvent("desirerp-paleto:outcome")
AddEventHandler("desirerp-paleto:outcome", function(oc, arg)
    for i = 1, #Check, 1 do
        Check[i] = false
    end
    ...
    if oc then
        Check[arg] = true
        TriggerEvent("desirerp-paleto:startheist", Paleto.Banks[arg], arg)
    elseif not oc then
        TriggerEvent("DoLongHudText", arg, 2)
    end
end)

RegisterNetEvent("desirerp-paleto:startLoot_c")
AddEventHandler("desirerp-paleto:startLoot_c", function(data, name)
    currentname = name
            ...
            while true do
                ...
                if dst < 40 then
                    ...
                    if LootCheck[name].Stop or (LootCheck[name].Loot) then
                        ...
                        if initiator then
                            TriggerEvent("desirerp-paleto:reset", name, data)
                            return
                        end
                        return
                    end
...
RegisterNetEvent("desirerp-paleto:stopHeist_c")
AddEventHandler("desirerp-paleto:stopHeist_c", function(name)
    LootCheck[name].Stop = true
end)

-- MAIN DOOR UPDATE --

AddEventHandler("desirerp-paleto:freezeDoors", function()
    Citizen.CreateThread(function()
...
RegisterNetEvent("desirerp-paleto:ClosePaletoDoor")
AddEventHandler("desirerp-paleto:ClosePaletoDoor", function()
    local count = 0
    local VaultDoor = GetClosestObjectOfType(-104.8747253418,6472.1274414062,31.621948242188, 25.0, `v_ilev_cbankvauldoor01`, 0, 0, 0)
    repeat
        ...
        Citizen.Wait(10000)
    until count == 900
    TriggerServerEvent('desirerp-paleto:getGetDoorStateSVSV', false)
end)


-- RegisterNetEvent("desirerp-paleto:toggleVault")
...
--             TriggerServerEvent("desirerp-paleto:updateVaultState", key, Doors[key][1].state)
--         elseif state then
--             local obj = GetClosestObjectOfType(Paleto.Banks[key].doors.startloc.x, Paleto.Banks[key].doors.startloc.y, Paleto.Banks[key].doors.startloc.z, 2.0, GetHashKey(Paleto.vaultdoor), false, false, false)
--             local count = 0
...
RegisterNetEvent("desirerp-paleto:OpenPaletoDoor")
AddEventHandler("desirerp-paleto:OpenPaletoDoor", function()
    TriggerServerEvent('desirerp-paleto:getGetDoorStateSVSV', true)
    local count = 0
    local VaultDoor = GetClosestObjectOfType(-104.8747253418, 6472.1274414062, 31.621948242188, 25.0, `v_ilev_cbankvauldoor01`, 0, 0, 0)
    repeat
...
AddEventHandler("desirerp-paleto:reset", function(name, data)
    ...
    TriggerServerEvent("desirerp-paleto:closeDoor")
    -- TriggerServerEvent("desirerp-paleto:toggleVault", name, true)
    TriggerEvent("desirerp-paleto:cleanUp", data, name)
end)


AddEventHandler("desirerp-paleto:startheist", function(data, name)
    disableinput = true
    currentname = name
  --  currentcoords = vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z)
    initiator = true
    ...
    TriggerServerEvent('desirerp-paleto:openDoor')
    TriggerServerEvent("desirerp-paleto:setCooldown", name)
    --TriggerServerEvent("desirerp-paleto:toggleVault", name, false)
    startdstcheck = true
    currentname = name
    SpawnTrolleys(data, name)
end)

-- AddEventHandler("desirerp-paleto:cleanUp", function(data, name)
--     Citizen.Wait(10000)
...
function SpawnTrolleys(data, name)
    ...
    TriggerServerEvent("desirerp-paleto:startLoot", data, name, missionplayers)
    done = false
end
        ...
        if startdstcheck then
            if initiator then
                ...
                if (GetDistanceBetweenCoords(playercoord, currentcoords, true)) > 20 then
                    ...
                    startdstcheck = false
                    TriggerServerEvent("desirerp-paleto:stopHeist", currentname)
                end
            end
            Citizen.Wait(1)
        else
...
RegisterNetEvent('desirerp-paleto:darkmail')
AddEventHandler('desirerp-paleto:darkmail', function()
    Citizen.Wait(1000)
...
RegisterNetEvent('desirerp-paleto:UseBlueLapTop')
AddEventHandler('desirerp-paleto:UseBlueLapTop', function(item)
    local ply = PlayerPedId()
    local plyCoords = GetEntityCoords(ply)
    local p = promise:new()

    Paleto.Banks, doors = RPC.execute("desirerp-paleto:getBanks")
	TriggerEvent("desirerp-paleto:freezeDoors")
                    ...
                    exports['hacking']:OpenHackingGame(function(Success)
                        if Success then
                            ...
                            Citizen.SetTimeout(timeout, function()
                                TriggerServerEvent("desirerp-paleto:startcheck", k)
                            end)
...
AddEventHandler('paletogate:doorburn', function()
	TriggerServerEvent('desirerp-doors:change-lock-state', 498, false)
end)

RegisterNetEvent('paletogate2:doorburn') -- Doorlock
AddEventHandler('paletogate2:doorburn', function()
	TriggerServerEvent('desirerp-doors:change-lock-state', 517, false)
end)
