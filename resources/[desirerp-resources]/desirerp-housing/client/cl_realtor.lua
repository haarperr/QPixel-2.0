...
RegisterNetEvent("desirerp-housing:enterBUILDINGCMON", function()
    InteractionPressed()
end)
...
RegisterCommand("desirerp-housing:getGarages", function()
...
function getIsPropLock()
    ...
    local pp =  RPC.execute("desirerp-housing:isHousingLockedServer", _propertyID)
...
AddEventHandler('desirerp-housing:phone:check', function(type)
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
    ...
    local isThisPropertyOwned = RPC.execute("desirerp-housing:isThisPropertyOwned", propertyId)
    ...
    if type == "edit" then
        ...
        TriggerEvent('desirerp-phone:editingHouse',id,street,price)
    else
        TriggerEvent('desirerp-phone:sentHouse',nil,id,street,price,category)
...
function enterEdit(pPropertyId)
    ...
    if playerInRangeOfProperty(propertyID) then
        ...
        TriggerEvent("desirerp-housing:edit")
    else
...
function setInventory()
    ...
    if Housing.currentlyInsideBuilding then
        ...
        local buildingVector = exports["desirerp-build"]:getModule("func").currentBuildingVector()
...
function exitingBuilding()
    ...
    if id then
        TriggerEvent("desirerp-editor:destroyName", id)
    end
...
function Housing.func.loadInteractions(model,overWrite)
    ...
    local isBuiltCoords = exports["desirerp-build"]:getModule("func").addInteractionPoints(model,interactionPoints,overWrite)
end
...
AddEventHandler("desirerp-housing:sell", function()
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
        TriggerEvent("DoLongHudText", propertyId, 2)
        return
...
AddEventHandler("desirerp-housing:sellTo", function(pParams)
    local input = exports["desirerp-ui"]:showInput({
		{
            icon = "id-card",
            label = "PayPal ID",
            name = "cid",
	...
	if input["cid"] then
		...
		TriggerServerEvent("desirerp-housing:sell", cid, pParams.propertyId)
	end
end)
...
RegisterCommand('rent', function()
    TriggerEvent('desirerp-housing:rent')
end)

RegisterNetEvent("desirerp-housing:rent", function()
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
        TriggerEvent("DoLongHudText", propertyId, 2)
        return
...
RegisterNetEvent("desirerp-housing:rent2", function(pParams)
    local complete, info = RPC.execute("desirerp-housing:rent", pParams.propertyId, pParams.total, pParams.tax)
...
RegisterNetEvent("desirerp-housing:buyed", function(pParams)
    print('hello?')
    local complete, info = RPC.execute("desirerp-housing:buy", pParams.propertyId, pParams.total, pParams.tax, pParams.seller)
...
RegisterNetEvent("desirerp-housing:edit", function()
end)

RegisterNetEvent("desirerp-housing:editOptions", function(pParams)
...
AddEventHandler("housing:frontdoor", function()
    DoScreenFadeOut(1)
    exports["desirerp-build"]:getModule("func").exitCurrentRoom(Housing.info[Housing.currentHousingInteractions.id]["pos"])
    exitingBuilding()
    DoScreenFadeIn(1900)
end)
...
AddEventHandler("housing:internalBackdoor", function()
    ...
    if max.canHaveBackDoor then
        ...
        DoScreenFadeOut(1)
        exports["desirerp-build"]:getModule("func").exitCurrentRoom(Housing.currentHousingInteractions.backdoor_coordinates.external)
        exitingBuilding()
        DoScreenFadeIn(1900)
    end
end)
...
AddEventHandler("housing:kickFromHouse", function(propertyID)
    ...
    if id == propertyID then
        DoScreenFadeOut(1)
        exports["desirerp-build"]:getModule("func").exitCurrentRoom(Housing.info[id]["pos"])
        exitingBuilding()
        DoScreenFadeIn(1900)
    end
end)

RegisterNetEvent("desirerp-housing:blips")
AddEventHandler("desirerp-housing:blips", function()
    Housing.Blips = not Housing.Blips

    local houses = RPC.execute("desirerp-housing:owned")
...
RegisterNetEvent('desirerp-housing:logShit')
AddEventHandler('desirerp-housing:logShit', function(hID, Price)
    TriggerServerEvent('desirerp-housing:buyLog', hID, Price)
end)