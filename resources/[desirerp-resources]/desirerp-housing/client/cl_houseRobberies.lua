...
function attemptToLockPickHouse(skipPicking)
    ...
    if not skipPicking then
        if not Housing.housingBeingRobbedClient[propertyID] then
            ...
            local finished = exports["desirerp-taskbar"]:taskBar(math.random(5000, 10000), "Breaking in")
...
function alarm(clearAlarm)
    if Housing.alarm then
        local alarmOffset = exports["desirerp-build"]:getModule("func").getAlarm()
...
function displayPickup(force,itemName)
    ...
    if Housing.currentlyDisplayingPickup == false and not force then
        ...
        if type(itemName) == "string" then
            exports["desirerp-textui"]:showInteraction("[E] Take Object")
        else
            exports["desirerp-textui"]:showInteraction("[E] Look for some belongings.")
        end
    end
end
...
function interactRob()
    ...
    if type(item) == "string" then
        local weight = exports["desirerp-inventory"]:getCurrentWeight()
    ...
    if type(item) == "string" then
        ...
        local finished = exports["desirerp-taskbar"]:taskBar(timer,"Securing Item")
        ClearPedTasks(PlayerPedId())
        ...
        TriggerEvent("animation:PlayAnimation","search")
        local finished = exports["desirerp-taskbar"]:taskBar(15000,"Procurando")
        ClearPedTasks(PlayerPedId())
        if finished ~= 100 then
            return
        end
    ...
    if finished then
        if type(item) == "string" then
            ...
            if euemeusmanosodiamosoartmike >= 90 then
                TriggerServerEvent("desirerp-heists:complete", math.random(30, 50))
...
RegisterNetEvent("desirerp-weathersync:currentTime", function(pHour, pMinute)
...
RegisterNetEvent("desirerp-housing:alarmFail")
AddEventHandler("desirerp-housing:alarmFail", function()
...
RegisterNetEvent("desirerp-housing:alarmEnter")
AddEventHandler("desirerp-housing:alarmEnter", function()
    insideGame = true
    local successevrp
...
AddEventHandler("housing:exitFrontDoor", function()
    ...
    TriggerServerEvent("desirerp-housing:leftHouse", Housing.currentHousingInteractions.id)
...
RegisterNetEvent("desirerp-housing:alarm")
AddEventHandler("desirerp-housing:alarm", function(pHouseId)
    if pHouseId == Housing.currentlyRobInside then
        local gameTimer = GetGameTimer() + 30000


        while true do
            local alarmOffset = exports["desirerp-build"]:getModule("func").currentBuildingVector()
...
RegisterNetEvent("desirerp-housing:beingRobbed", function(pPropertyId)
