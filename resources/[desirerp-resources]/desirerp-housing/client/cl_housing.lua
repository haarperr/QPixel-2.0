...
function Housing.func.enterBuilding(propertyID,enterOverride,counterPart)
    ...
    local isBuiltCoords, objects = exports["desirerp-build"]:getModule("func").buildRoom(model,spawnBuildingLocation,false,Housing.destroyedObjects,enterOverride,true)
    if counterPart and Housing.staticObjectRobPoints == nil then
        Housing.staticObjectRobPoints = exports["desirerp-build"]:getModule("func").getRobLocationsForObjects(model,spawnBuildingLocation,Housing.housingRobTargets.static)
    ...
    if isBuiltCoords then
        ...
        TriggerServerEvent("desirerp-housing:enterHouse", propertyID)

        if not counterPart then
            TriggerEvent("desirerp-editor:buildName", propertyID, objects)
        end
    else
        Housing.currentHousingInteractions = nil
        Housing.currentlyInsideBuilding = false
...
function unlock(propertyID)
    ...
    if Housing.currentHousingLocks[propertyID] == nil then
        ...
        --     TriggerEvent(
        --         "desirerp-phone:addnotification",
...
RegisterNetEvent("desirerp-housing:loadHousingClient")
AddEventHandler("desirerp-housing:loadHousingClient", function()
    gatherPlayerInfo()
end)

RegisterNetEvent("desirerp-housing:reset")
AddEventHandler("desirerp-housing:reset", function()
    -- cl_housing
    Housing.currentOwned = {}
    Housing.currentKeys = {}
    Housing.currentlyEditing = false
...
RegisterNetEvent("desirerp-housing:refresh")
AddEventHandler("desirerp-housing:refresh", function()
    gatherPlayerInfo()
end)
