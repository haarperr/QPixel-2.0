...
RegisterNetEvent("desirerp-housing:sell")
AddEventHandler("desirerp-housing:sell", function(pCid, pPropertyId)
    local src = source
    ...
    print('here')
    TriggerClientEvent("desirerp-housing:buy", pCid, pPropertyId, seller)
end)
...
AddEventHandler("CheckFurniture", function(pData, pPropertyId)
    ...
    if housingEditing[pPropertyId] ~= nil then
        ...
        TriggerClientEvent("desirerp-editor:loadEditor", src, pData)
    end
end)
...
RPC.register("desirerp-housing:buy", function(src, pPropertyId, pTotal, pTax, pSeller)
    local user = exports['desirerp-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    ...
    TriggerClientEvent("desirerp-housing:loadHousingClient",src)
    TriggerClientEvent('desirerp-housing:logShit', src, pPropertyId, pTotal)
    return true, "Purchase Successful" 
end)

RPC.register("desirerp-housing:rent", function(src, pPropertyId, pTotal, pTax)
    local user = exports['desirerp-base']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
    ...
    --local success, message = exports["desirerp-financials"]:transaction(accountId, groupBank, pTotal, comment, cid, 5)
    --if not success then
    --    return false, message
    --end

    --exports["desirerp-financials"]:addTax("Propertys", pTax)
...
RPC.register("updateCurrentSelected", function(src, pPropertyId, pInformation, pOrigin)
    ...
    --exports["desirerp-vehicles"]:setGarage(name, data, nil, false)
...
RPC.register("desirerp-housing:isHousingLockedServer", function(pSrc, pHid)
    print(pHid)
...
RPC.register("desirerp-housing:owned", function(src)
...
RegisterNetEvent("desirerp-housing:getGarages", function()
    local src = source
    ...
    for i, v in ipairs(houses) do
        ...
        TriggerClientEvent("menu:send:desirerp-housing:garages", src,json.decode(garagedata),v.hid)
    end
end)