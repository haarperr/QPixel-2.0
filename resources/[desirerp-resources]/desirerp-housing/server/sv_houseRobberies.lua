...
RegisterNetEvent("desirerp-base:sessionStarted", function()
    local src = source
    TriggerClientEvent("desirerp-housing:beingRobbed", src, Housing.housingBeingRobbed)
end)
...
RPC.register("housing:robbery:pickedLock", function(src, pHouseId)
    ...
    TriggerClientEvent("desirerp-housing:beingRobbed", -1, pHouseId)
...
RPC.register("property:alarmDisabled", function(src, pHouseId, pFail)
    ...
    for k, v in pairs(Housing.Players) do
        if v == pHouseId then
            ...
            if pFail then
                TriggerClientEvent("desirerp-housing:alarm", src, pHouseId)
            end
        end
    end
