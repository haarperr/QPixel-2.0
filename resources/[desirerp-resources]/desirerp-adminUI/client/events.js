...
function toggleNUI(){
    ...
    SendNuiMessage(JSON.stringify({
        action: 'setKeyBinds',
        selectionMenu: global.exports["desirerp-keybinds"].getKeyMapping("+enableSelectionMenu"),
        selectionKey: global.exports["desirerp-keybinds"].getKeyMapping("+adminSelect")
    }))
}
function exitNUI()
{
...
on('__cfx_nui:closeMenu', (data, cb) => {
    ...
    SetNuiFocus(false,false)
    SetNuiFocusKeepInput(false)
    emit('desirerp:admin:updateUI')


    if(data.toSelect){
        global.exports["desirerp-admin"].enterSelection();
    }
...
on("onResourceStart", (resourceName) => {
    if(GetCurrentResourceName() != 'desirerp-admin') {
      return;
    }
...
on('__cfx_nui:insideCommand', (data, cb) => {
    ...
    if(data.inside){
        ...
        emit('desirerp-binds:should-execute',false)
        ...
        emit('desirerp-binds:should-execute',true)
        if(blockingActions){
...
global.exports["desirerp-keybinds"].registerKeyMapping("", "zzAdmin", "Selection Menu", "+enableSelectionMenu", "-enableSelectionMenu", "")
