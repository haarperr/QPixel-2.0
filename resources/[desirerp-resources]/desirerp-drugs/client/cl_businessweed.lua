RegisterNetEvent("desirerp-weedstore:startpicking")
AddEventHandler("desirerp-weedstore:startpicking", function()
    local rank = exports["isPed"]:GroupRank("weed_store")
    if rank > 0 then  
        if exports['desirerp-inventory']:hasEnoughOfItem("purifiedwater", 1) and exports['desirerp-inventory']:hasEnoughOfItem("plantpot", 1) and exports['desirerp-inventory']:hasEnoughOfItem("fertilizer", 1) then
            ...
            local finished = exports['desirerp-taskbar']:taskBar(20000, 'Harvesting')
...
RegisterNetEvent("desirerp-weedstore:startpacking")
AddEventHandler("desirerp-weedstore:startpacking", function()
    local rank = exports["isPed"]:GroupRank("weed_store")
    if rank > 0 then  
        if exports['desirerp-inventory']:hasEnoughOfItem("smallbud", 3) and exports['desirerp-inventory']:hasEnoughOfItem("plastic", 1) then
            local dict = 'missfinale_c2ig_11'
            LoadDict(dict)
            FreezeEntityPosition(GetPlayerPed(-1),true)
            TaskPlayAnim(GetPlayerPed(-1), dict, "pushcar_offcliff_f", 3.0, -8, -1, 63, 0, 0, 0, 0 )
            SetEntityHeading(GetPlayerPed(-1), 186.32456970215)
            local finished = exports['desirerp-taskbar']:taskBar(20000, 'Packaging')
            if (finished == 100) then
...
AddEventHandler("weedstore:register", function(registerID)
    ...
    if myJob == "weed_store" then
        local order = exports["desirerp-applications"]:KeyboardInput({
