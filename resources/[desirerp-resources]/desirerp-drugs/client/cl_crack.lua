...
AddEventHandler("crack:start", function()
	...
	if exports["desirerp-inventory"]:hasEnoughOfItem("bakingsoda",5,false) and exports["desirerp-inventory"]:hasEnoughOfItem("chloroform",2,false) and exports["desirerp-inventory"]:hasEnoughOfItem("dye",3,false) and ongoing == false then
		TriggerEvent("animation:crack")
		FreezeEntityPosition(PlayerPedId(),true)
		SetEntityHeading(PlayerPedId(), 357.96401977539)
		local finished = exports["desirerp-taskbar"]:taskBar(10000,"Mixing This Shit!")
		if (finished == 100) then
			...
			if math.random(1,30) == 18  then
				...
				TriggerEvent("desirerp-dispatch:methexplosion")
				DaBabyBoom()
			else
				TriggerEvent("inventory:removeItem","bakingsoda", 5)
				TriggerEvent("inventory:removeItem","chloroform", 2)
...
AddEventHandler("crack:sell", function()
	if exports["desirerp-inventory"]:hasEnoughOfItem("1gcrack",1,false) then
...
AddEventHandler("stopmixing", function()
	if mixing == true then
      ...
      exports["desirerp-polytarget"]:AddBoxZone("crack_start", vector3(1092.93, -3195.72, -38.99), 1.2, 2.6, {
    heading=0
  })