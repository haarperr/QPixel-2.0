...
RegisterNetEvent('desirerp-doors:initial-lock-state')
AddEventHandler('desirerp-doors:initial-lock-state', function(pDoors)
    doors = pDoors
...
RegisterNetEvent('desirerp-doors:change-lock-state')
AddEventHandler('desirerp-doors:change-lock-state', function(pDoorId, pDoorLockState, pDoorForceUnlock)
...
local function listenForKeypress()
    ...
    Citizen.CreateThread(function()
        ...
        if not hasAccess and currentDoorLockState then
            exports["desirerp-interface"]:showInteraction('Locked', 'error')
        end
        ...
        while listening do
            ...
            if currentDoorLockState ~= newLockState then
                if #(GetOffsetFromEntityGivenWorldCoords(PlayerPedId(), currentDoorCoords)) <= 1.2 then
                    ...
                    if hasAccess then
                        exports["desirerp-interface"]:showInteraction(("[E] %s"):format(newLockState and 'Locked' or 'Unlocked'), newLockState and 'error' or 'success')
                    else
                        exports["desirerp-interface"]:showInteraction((newLockState and 'Locked' or 'Unlocked'), newLockState and 'error' or 'success')
                    end
                else
                    idle = 100
                end
            ...
            if currentDoorId ~= nil and hasAccess and IsControlJustReleased(0, 38) and #(GetOffsetFromEntityGivenWorldCoords(PlayerPedId(), currentDoorCoords)) <= 1.2 then
                ...
                TriggerServerEvent("desirerp-doors:change-lock-state", currentDoorId, not currentDoorLockState)
            end

            Wait(idle)
        end

        exports["desirerp-interface"]:hideInteraction((not hasAccess or newLockState) and 'error' or 'success')
    end)
end
...
AddEventHandler("desirerp-doors:doorKeyFob", function()
    ...
    if currentZone ~= nil and bollards[currentZone].inside then
        ...
        local entity = exports['desirerp-target']:GetEntityPlayerIsLookingAt(10.0, 2.0, 16)
    ...
    TriggerServerEvent("desirerp-doors:change-lock-state", doorId, isBollard and (not isLocked and 6 or 0) or (not isLocked))
end)

Citizen.CreateThread(function()
    exports["desirerp-polyzone"]:AddBoxZone("mrpd_bollards_01", vector3(411.66, -1027.95, 29.24), 7.8, 23.4, {
        heading=0,
        minZ=28.14,
        maxZ=32.14
    })
    exports["desirerp-polyzone"]:AddBoxZone("mrpd_bollards_02", vector3(411.66, -1020.09, 29.34), 7.8, 23.4, {
        heading=0,
        minZ=28.14,
        maxZ=32.14
    })
    TriggerServerEvent("desirerp-doors:request-lock-state")
end)

RegisterCommand("addnewdoor", function()
    local pEntity = exports['desirerp-target']:GetEntityPlayerIsLookingAt(10.0, 2.0, 16)
    TriggerServerEvent("desirerp-door:add", GetEntityCoords(pEntity), GetEntityModel(pEntity))
end, false)

RegisterNetEvent("desirerp-doors:add")
AddEventHandler("desirerp-doors:add", function(pArgs, pEntity, pContext)
...
RegisterNetEvent("desirerp-doors:detCoord")
AddEventHandler("desirerp-doors:detCoord", function ()
  ...
  Citizen.CreateThread(function()
    ...
    ClearPedTasks(ped) 
    DeleteObject(bomba)
    TriggerServerEvent("desirerp-doors:change-lock-state", targetDoor, 0)
