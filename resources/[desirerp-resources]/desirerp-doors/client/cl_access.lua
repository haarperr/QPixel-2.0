...
AddEventHandler('desirerp-jobs:jobChanged', function(currentJob, previousJob)
...
function hasSecuredAccess(pId, pType)
    ...
    if secured.access.item ~= nil then
        ...
        for i, v in pairs(secured.access.item) do
            if exports["desirerp-inventory"]:hasEnoughOfItem(i, 1, false) then
                return true
            end
        end
    end
    
    local mypasses = RPC.execute("desirerp-business:business:GetEmploymentInformation")
    for i=1, #mypasses do
    if secured.access.business and secured.access.business[mypasses[i]["businessid"]] == true then
    local hasPermission = RPC.execute("desirerp-business:business:hasPermission", mypasses[i]["businessid"], "key_access")
    if hasPermission then
        if secured.access.permission then
            local hasPermission = RPC.execute("desirerp-business:business:hasPermission", mypasses[i]["businessid"], secured.access.permission[1])
            if hasPermission then
            accessCheckCache[pType][pId] = true
                return true
            end
