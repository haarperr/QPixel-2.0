...
RegisterNetEvent('desirerp-doors:request-lock-state')
AddEventHandler('desirerp-doors:request-lock-state', function()
    local src = source
    TriggerClientEvent('desirerp-doors:initial-lock-state', src, doors)
end)

RegisterNetEvent('desirerp-doors:change-lock-state')
AddEventHandler('desirerp-doors:change-lock-state', function(pDoorId, pDoorLockState)
    if doors[pDoorId] then
        doors[pDoorId].lock = pDoorLockState
        TriggerClientEvent('desirerp-doors:change-lock-state', -1, pDoorId, pDoorLockState)
    end
end)
...
-- RegisterNetEvent("desirerp-doors:save-config")
-- AddEventHandler("desirerp-doors:save-config", function(pDoorData)
