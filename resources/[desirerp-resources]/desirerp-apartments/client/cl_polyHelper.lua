...
function polyHelperEnter(apartmentLocNumber,apartmentNumber)
    ...
    listening = true
    exports["desirerp-interface"]:showInteraction(getInteractionMessage(apartmentLocNumber,apartmentNumber))
...
local function showApartmentMenu(apartmentLocNumber,apartmentNumber)
    ...
    for k,v in pairs(Apart.currentRoomLocks[apartmentLocNumber]) do
        if v == false then
            unlockedApartments[#unlockedApartments + 1] = { title = "Apartment # "..k, action = "desirerp-interface:apartmentsContext", key = {"EnterUnlocked",apartmentLocNumber,k} }
        end 
    end
    ...
    if Apart.defaultInfo[1] == apartmentLocNumber and Apart.defaultInfo[2] == apartmentNumber then
        data[#data+1] = {
            ...
            action = "desirerp-interface:apartmentsContext"
        }
    end
...
RegisterInterfaceCallback("desirerp-interface:apartmentsContext", function(data, cb)
    ...
    if action == "locks" then
        ...
        exports["desirerp-interface"]:showInteraction(getInteractionMessage(globalLocNumber,globalNumber))
    end
    ...
    if action == "lockdownCid" then
        ...
        local prompt = exports['desirerp-interface']:OpenInputMenu(elements, function(values)
