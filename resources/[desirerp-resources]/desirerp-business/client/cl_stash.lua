...
RegisterCommand("creationmenu", function()
	...
	local hasPerm = RPC.execute("desirerp-business:hasPermission", "ace_industries", "craft_access")
	
	if employed and hasPerm then
		TriggerEvent("desirerp-business:stash:creationMenu")
	end
end)
...
RegisterInterfaceCallback("desirerp-business:stash:enteredAuthCode", function(data, cb)
...
function getPDAuthCode()
  ...
  exports["desirerp-interface"]:openApplication("textbox", {
    callbackUrl = "desirerp-business:stash:enteredAuthCode",
    key = "",
    items = {
...
function openInventory(type, data)
  ...
  if type == "stash" then
    local success = RPC.execute("desirerp-business:hasPermission", biz, "stash_access")
  ...
  elseif type == "craft" then
    local success = RPC.execute("desirerp-business:hasPermission", biz, "craft_access")
    if not success and isPD() then
      success = getPDAuthCode()
    end
    if success then
...
function enterPoly(name, data)
  ...
  exports["desirerp-interface"]:showInteraction("[E] " .. name)
end
...
AddEventHandler("desirerp-polyzone:exit", function(name, data)
  if name == "business_stash" or name == "business_craft" then
...
local creationMenu = {
  key = 1,
  show = true,
  callbackUrl = "desirerp-business:client:createStash",
  items = { }
}

Citizen.CreateThread(function()
  exports["desirerp-interact"]:AddPeekEntryByModel(businessStashProps, {
    {
      id = "business_stash",
      event = "desirerp-business:client:openStash",
      icon = "archive",
      label = "Open Storage",
    },
  }, 
  { 
    ...
    isEnabled = function(pEntity)
      local objInfo = exports["desirerp-objects"]:GetObjectByEntity(pEntity)
      return objInfo ~= nil
    end
  })
  exports["desirerp-interact"]:AddPeekEntryByModel(businessStashProps, {
    {
      ...
      event = "desirerp-business:client:removeStash",
      icon = "trash",
      label = "Remove Storage",
    },
    {
      id = "business_stash_update",
      event = "desirerp-business:client:changeStashModel",
      icon = "archive",
...
RegisterNetEvent("desirerp-business:stash:creationMenu")
AddEventHandler("desirerp-business:stash:creationMenu", function ()
  ...
  exports["desirerp-interface"]:openApplication("textbox", creationMenu)
end)

AddEventHandler("desirerp-business:client:removeStash", function (pData, pEntity)
  local objInfo = exports["desirerp-objects"]:GetObjectByEntity(pEntity)
  if objInfo == nil then return end

	TriggerEvent("animation:PlayAnimation", "hammering")

  local finished = exports["desirerp-taskbar"]:taskBar(15000, "Destroying Storage")
  ClearPedTasks(PlayerPedId())
  if finished ~= 100 then return end

  local deleted = exports["desirerp-objects"]:DelObject(objInfo.id)
  if not deleted then
...
AddEventHandler("desirerp-business:client:openStash", function (pData, pEntity)
  local objInfo = exports["desirerp-objects"]:GetObjectByEntity(pEntity)
  if objInfo == nil then return end
  ...
  local success = RPC.execute("desirerp-business:hasPermission", businessCode, "stash_access")
  if not success and isPD() then
    success = getPDAuthCode()
  end
...
AddEventHandler("desirerp-business:client:changeStashModel", function (pData, pEntity)
  local objInfo = exports["desirerp-objects"]:GetObjectByEntity(pEntity)
  if objInfo == nil then return end
  ...
  local menuData = {
    key = { id = objInfo.id },
    show = true,
    callbackUrl = "desirerp-business:client:updateStashModel",
    items = {
      { icon = "object-ungroup", _type = "select", label = "New Unit Model", name = "stashProp", options = unitOptions },
    }
  }
  exports["desirerp-interface"]:openApplication("textbox", menuData)
end)

RegisterInterfaceCallback("desirerp-business:client:updateStashModel", function(pData, pCb) 
  ...
  local finished = exports["desirerp-taskbar"]:taskBar(15000, "Changing Storage")
  ClearPedTasks(PlayerPedId())
  if finished ~= 100 then return end

  local updated = exports["desirerp-objects"]:UpdateObject(pData.key.id, stashProp)
end)

RegisterInterfaceCallback("desirerp-business:client:createStash", function(pData, pCb) 
  pCb({ data = {}, meta = { ok = true, message = "done" } })
  exports["desirerp-interface"]:closeApplication("textbox")
  ...
  local result = exports["desirerp-objects"]:PlaceAndSaveObject(
    stashProp, 
    metaData, 
    { groundSnap = true }
  )
