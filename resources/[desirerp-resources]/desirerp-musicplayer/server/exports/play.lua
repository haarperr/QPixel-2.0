function PlayUrl(source, name_, url_, volume_, loop_)
    TriggerClientEvent("desirerp-musicplayer:stateSound", source, "play", {
        soundId = name_,
        url = url_,
        volume = volume_,
        loop = loop_
...
function PlayUrlPos(source, name_, url_, volume_, pos, loop_)
    TriggerClientEvent("desirerp-musicplayer:stateSound", source, "playpos", {
        soundId = name_,
        position = pos,
        url = url_,
        volume = volume_,
...
function TextToSpeech(source, name_, lang, text, volume_, loop_)
    TriggerClientEvent("desirerp-musicplayer:stateSound", source, "texttospeech", {
        soundId = name_,
        url = text,
        lang = lang,
        volume = volume_,
...
function TextToSpeechPos(source, name_, lang, text, volume_, pos, loop_)
    TriggerClientEvent("desirerp-musicplayer:stateSound", source, "texttospeechpos", {
        soundId = name_,
        lang = lang,
        position = pos,
        url = text,
...
RegisterServerEvent("vanilla:request:song:sv", function(url)
    ...
    TriggerClientEvent("desirerp-musicplayer:stateSound", -1, "distance", data)
end)
...
RegisterServerEvent("vanilla:change:volume", function(volume)
    ...
    TriggerClientEvent("desirerp-musicplayer:stateSound", -1, "volume", data)
end)
