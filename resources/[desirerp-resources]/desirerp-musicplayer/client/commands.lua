...
RegisterCommand("streamermode", function(source, args, rawCommand)
    ...
    if disableMusic then
        TriggerEvent('chat:addMessage', { args = { "^1[desirerp-musicplayer]", config.Messages["streamer_on"] } })
        ...
        TriggerEvent('chat:addMessage', { args = { "^1[desirerp-musicplayer]", config.Messages["streamer_off"] } })
    end
end, false)