RegisterNUICallback("data_status", function(data)
    if soundInfo[data.id] ~= nil then
        if data.type == "finished" then
            ...
            TriggerEvent("desirerp-musicplayer:songStopPlaying", data.id)
        end
...
RegisterNetEvent("desirerp-musicplayer:stateSound")
AddEventHandler("desirerp-musicplayer:stateSound", function(state, data)
