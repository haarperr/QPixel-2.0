    ...
    if(howler){
        for (var soundName in soundList)
        {
            ...
            if(sound.loaded() == false){
                ...
                $.post('https://desirerp-musicplayer/events', JSON.stringify(
                {
                    type: "onPlay",
                    id: sound.getName(),
                }));
                ...
                $.post('https://desirerp-musicplayer/data_status', JSON.stringify(
                {
                    time: time,
                    type: "maxDuration",
                    id: sound.getName(),
...
function isLooped(divId){
	for (var soundName in soundList)
	{
        ...
        if(sound.getDivId() === divId && sound.isLoop()){
            ...
            $.post('https://desirerp-musicplayer/data_status', JSON.stringify({ type: "finished",id: soundName }));
            $.post('https://desirerp-musicplayer/events', JSON.stringify(
            {
                type: "onEnd",
                id: sound.getName(),
