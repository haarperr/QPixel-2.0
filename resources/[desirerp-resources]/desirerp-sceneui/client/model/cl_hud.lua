...
--
local cmdsSet = {}
RegisterNUICallback("desirerp-ui:hudSetPreferences", function(data, cb)
    ...
    if data["hud.presets"] then
      ...
      for _, p in pairs(data["hud.presets"]) do
        ...
        if not cmdsSet[idx] then
          ...
          exports["desirerp-keybinds"]:registerKeyMapping("", "HUD", "Enable " .. s, "+hud_enable_" .. s, "-hud_enable_" .. s)
          RegisterCommand('+hud_enable_' .. s, function()
            exports["desirerp-ui"]:sendAppEvent("preferences", eventOptions)
          end, false)
    ...
    TriggerEvent("desirerp-preferences:setPreferences", data)
end)
...
AddEventHandler("desirerp-ui:watch", function()
...
AddEventHandler('desirerp-inventory:itemCheck', function (item, hasItem, quantity)
...
RegisterNetEvent("desirerp:admin:currentDebug")
AddEventHandler("desirerp:admin:currentDebug", function(debugToggle)
...
AddEventHandler('desirerp-vehicles:pursuitMode', function (pEnabled, pMode, pModes)
...
AddEventHandler('desirerp-jail:attachedCollar', function (pAttached)
...
RegisterNetEvent('desirerp-boosting:client:setBoostCompletions')
AddEventHandler('desirerp-boosting:client:setBoostCompletions', function (pAmount)
...
RegisterNetEvent('desirerp-weapons:client:setWeaponFireRate')
AddEventHandler('desirerp-weapons:client:setWeaponFireRate', function (pAmount)
    weaponFireRate = pAmount
    setHudValue("weaponFireRate", weaponFireRate)
end)

RegisterNetEvent('desirerp-weapons:client:showWeaponFireRate')
AddEventHandler('desirerp-weapons:client:showWeaponFireRate', function (pToggle)
    showWeaponFireRate = pToggle
    setHudValue("showWeaponFireRate", pToggle)
end)

RegisterNetEvent('desirerp-ui:hud:forceShowMinimap', function(pForce)
...
function roundedRadar()
    ...
    Citizen.CreateThread(function()
        ...
        -- for k,v in pairs(exports["desirerp-base"]:getModule("Blips")) do
...
function getFuel()
    fuel = exports["desirerp-vehicles"]:CurrentFuel() or 0
end
    ...
    Citizen.CreateThread(function()
        while engineOn do
            ...
            if GetPedInVehicleSeat(veh, -1) == GetPed() then
                harnessDurability = exports["desirerp-vehicles"]:GetVehicleMetadata(veh, 'harness')
            end

            local degradation = exports["desirerp-vehicles"]:GetVehicleDegradation(veh)
...
AddEventHandler('desirerp-ui:setGaze', function (pEntity, pDelay)
    GazeEntity = pEntity
    GazeDelay = pDelay
end)
...
Citizen.CreateThread(function()
  while true do
    ...
    if toggleParachuteOff and pState == -1 then
      ...
      exports["desirerp-ui"]:sendAppEvent("hud", { hasParachute = false })
...
AddEventHandler("hud:equipParachute", function()
  hasParachute = true
  exports["desirerp-ui"]:sendAppEvent("hud", { hasParachute = true })
end)

AddEventHandler("desirerp-island:onIsland", function(pState)
    onIsland = pState
end)
