...
AddEventHandler('desirerp-config:configReady', function()
    loadPublicData()
end)

function loadPublicData()
    local showWatermark = exports['desirerp-config']:GetMiscConfig('hud.watermark')
    local showroomPurchase = exports['desirerp-config']:GetMiscConfig('showrooms.catalog.purchase')
    local skipMdwProfileCheck = exports['desirerp-config']:GetMiscConfig('mdw.profiles.skip')
    local serverCode = exports["desirerp-config"]:GetServerCode()
...
function sendCharacterData()
    Citizen.CreateThread(function()
        ...
        SendUIMessage({ source = "desirerp-nui", app = "character", data = character });
        
        local endpointData = exports["desirerp-fiber"]:GetGameDetails()
        SendUIMessage({ source = "desirerp-nui", app = "game", data = endpointData });

        Citizen.Wait(5000)

        TriggerEvent('desirerp-ui:phoneReady')
        TriggerServerEvent('desirerp-ui:phoneReady')
    end)
end
...
-- AddEventHandler("desirerp-ui:hud:values", function(values)
--   -- sendAppEvent("hud", {
--   --     food = values["hunger"],
