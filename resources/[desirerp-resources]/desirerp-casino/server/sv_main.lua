...
RegisterNetEvent("desirerp-casino:casinoPurchaseChips",function(qty)
    local src = source
...
RegisterNetEvent("desirerp-casino:withdraw",function(moneytype,qty)
    local src = source
	local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local quantity = tonumber(qty)
...
RegisterNetEvent("desirerp-casino:purchageMembership",function(cardtype, cid)
    local src = source
	local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
...
AddEventHandler('desirerp-luckywheel:getLucky', function()
    local src = source
    if not isRoll then
        isRoll = true
