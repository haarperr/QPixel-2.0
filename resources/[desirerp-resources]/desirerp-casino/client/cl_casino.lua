...
Citizen.CreateThread(function()
  -- local isPublic = exports['desirerp-config']:IsPublic()
...
function drawCarForWins()
  ...
  if carOnShowAppearence then
    --exports['desirerp-vehicles']:SetVehicleAppearance(spinningCar, carOnShowAppearence)
  end
  if carOnShowMods then
    --exports['desirerp-vehicles']:SetVehicleMods(spinningCar, carOnShowMods)
  end
...
AddEventHandler("desirerp-casino:elevatorEnterCasino", function()
  enterCasino(true)
end)
AddEventHandler("desirerp-casino:elevatorExitCasino", function()
  enterCasino(false)
end)
...
--   -- RPC.execute("desirerp-bugs:playerLogAction", {
...
function enterCasino(pIsInCasino)
  ...
  --     -- end
  --     if inCasino then
  --       local pedCoords = RPC.execute("desirerp-casino:getSpawnedPedCoords", true)
  ...
  if not inCasino then
    ...
    --RPC.execute("desirerp-casino:getSpawnedPedCoords", false)
    TriggerEvent("desirerp-casino:casinoExitedEvent")
    TriggerServerEvent('desirerp-infinity:scopes:casino', false)
  ...
  -- handlePedCoordsBaby(pedCoords)
  doInitStuff()
  TriggerEvent("desirerp-casino:casinoEnteredEvent")
  TriggerServerEvent('desirerp-infinity:scopes:casino', true)
end

Citizen.CreateThread(function()
  --enterCasino(true)
  Wait(10000)
 -- RPC.execute("desirerp-casino:getCurrentInteriorSetName")
end)
    ...
    while inCasino do
      ...
      if inCasino and postGateTriggered then
        if hasMembership() then
          --RPC.execute("desirerp-casino:generateRaffleTicket")
        end
      end
    end
...
function doSomeTextureReplacesBruv(apply)
  if not apply then
    ...
    exports["desirerp-lib"]:releaseDui(wheelDui.id)
    wheelDui = nil
    return
  end
  if wheelDui == nil then
    wheelDui = exports["desirerp-lib"]:getDui(wheelUrl, 1024, 1024)
    ...
    exports["desirerp-lib"]:changeDuiUrl(wheelDui.id, wheelUrl)
...
function showThe6StrImage(apply)
  if not apply then
    ...
    exports["desirerp-lib"]:releaseDui(tvDui.id)
    tvDui = nil
    return
  end
  if tvDui == nil then
    tvDui = exports["desirerp-lib"]:getDui(tvUri, 512, 256)
    AddReplaceTexture('vw_prop_vw_cinema_tv_01', 'script_rt_tvscreen', tvDui.dictionary, tvDui.texture)
  else
    exports["desirerp-lib"]:changeDuiUrl(tvDui.id, tvUri)
  end
end
...
function handlePedCoordsBaby(pPedCoords)
 ...
 -- RPC.execute("desirerp-casino:handoffPedData", myPeds)
  Citizen.CreateThread(function()
    while inCasino do
  ...
  local data = {
    {
        ...
        action = "desirerp-ui:purchaseChipsAction",
        key = "purchase"
    },
    -- {
...
-- chips
RegisterInterfaceCallback("desirerp-ui:casinoPurchaseChipsAmount", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  local amount = data[1].value
  TriggerServerEvent("desirerp-casino:casinoPurchaseChips",amount)
  exports['desirerp-ui']:closeApplication('textbox')
end)

RegisterInterfaceCallback("desirerp-ui:transferChipsAmount", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  local amount = data[1].value
...
RegisterInterfaceCallback("desirerp-ui:purchaseChipsAction", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  exports["desirerp-ui"]:hideContextMenu(data)
  ...
  if action == "purchase" then
    ...
    exports['desirerp-ui']:openApplication('textbox', {
      callbackUrl = 'desirerp-ui:casinoPurchaseChipsAmount',
      key = 1,
  ...
  elseif action == "purchase:dirty" then
    ...
    if exports["desirerp-inventory"]:hasEnoughOfItem("markedbills", 20, false, true) then
      TriggerEvent("inventory:removeItem", "markedbills", 20)
      payment = payment + (250 * 20) -- $5k / $250 per
    end
    if exports["desirerp-inventory"]:hasEnoughOfItem("rollcash", 5, false, true) then
      TriggerEvent("inventory:removeItem", "rollcash", 5)
      payment = payment + (30 * 5) -- $150 / $30 per
    end
    if exports["desirerp-inventory"]:hasEnoughOfItem("band", 5, false, true) then
      TriggerEvent("inventory:removeItem", "band", 5)
      payment = payment + (300 * 5) -- $1500, / $300 per
    end
    --RPC.execute("desirerp-casino:purchaseChipsDirty", payment)
  elseif action == "withdraw:cash" then
    local chipsAmount = exports["desirerp-inventory"]:getQuantity("redchip")
    TriggerServerEvent("desirerp-casino:withdraw","cash",chipsAmount)
  elseif action == "withdraw:bank" then
    TriggerServerEvent("desirerp-casino:withdraw","bank",chipsAmount)
  elseif action == "transfer" then
    Wait(100)
    exports['desirerp-ui']:openApplication('textbox', {
      callbackUrl = 'desirerp-ui:transferChipsAmount',
      key = 1,
...
AddEventHandler("desirerp-casino:purchaseMembership", function()
  ...
  -- end
  exports['desirerp-ui']:openApplication('textbox', {
    callbackUrl = 'desirerp-ui:casinoGetMembership',
    key = 1,
...
RegisterInterfaceCallback("desirerp-ui:casinoGetMembership", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  exports['desirerp-ui']:closeApplication('textbox')
  TriggerServerEvent("desirerp-casino:purchageMembership","normal", data[1].value)
end)

-- AddEventHandler("desirerp-casino:purchaseMembershipCard", function()
--   local characterId = exports["isPed"]:isPed("cid")
--   --RPC.execute("desirerp-casino:purchaseMembershipCard", characterId)
-- end)

AddEventHandler("desirerp-casino:getLoyaltyCard", function()
  local characterId = exports["isPed"]:isPed("cid")
  local jobAccess = RPC.execute("IsEmployedAtBusiness", { character = { id = characterId }, business = { id = "casino"} })
  if not jobAccess then
    TriggerEvent("DoLongHudText", "Please talk to a member of floor staff", 2)
...
AddEventHandler("desirerp-casino:purchaseVipMembership", function()
  local characterId = exports["isPed"]:isPed("cid")
  local jobAccess = RPC.execute("IsEmployedAtBusiness", { character = { id = characterId }, business = { id = "casino"} })
  if not jobAccess then
    TriggerEvent("DoLongHudText", "Please talk to a member of floor staff", 2)
  ...
  exports['desirerp-ui']:openApplication('textbox', {
    callbackUrl = 'desirerp-ui:casinoGetVipMembership',
    key = 1,
...
RegisterInterfaceCallback("desirerp-ui:casinoGetVipMembership", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  exports['desirerp-ui']:closeApplication('textbox')
  TriggerServerEvent("desirerp-casino:purchageMembership","vip", data[1].value)
end)

AddEventHandler("desirerp-casino:purchaseDrinks", function(data)
  TriggerEvent("server-inventory-open", "46", "Shop")
end)

function hasMembership(hrOnly)
  if not exports['desirerp-inventory']:hasEnoughOfItem("casinomember", 1, false, true) and not exports['desirerp-inventory']:hasEnoughOfItem("casinovip", 1, false, true) then
    return false
  else
    return true
  end
...
AddEventHandler("desirerp-casino:inVRHeadset", function(pInVRHeadset)
  inVRHeadset = pInVRHeadset
end)

RegisterInterfaceCallback("desirerp-ui:casinoCheckSpentWheelAmount", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  local stateId = data.values.state_id
  --RPC.execute("desirerp-casino:checkSpentAmount", stateId)
  exports['desirerp-ui']:closeApplication('textbox')
end)
AddEventHandler("desirerp-casino:wheel:checkSpentAmount", function()
  ...
  exports['desirerp-ui']:openApplication('textbox', {
    callbackUrl = 'desirerp-ui:casinoCheckSpentWheelAmount',
    key = 1,
    header = "Amount Spend",
    items = {
      {
...
-- end, false)
-- Citizen.CreateThread(function()
--   exports["desirerp-keybinds"]:registerKeyMapping("", "Casino", "Add Coords", "+addCasinoCoords", "-addCasinoCoords")
-- end)

-- Citizen.CreateThread(function()
--   for _, v in pairs(exports["desirerp-casino"]:getPedCoordsC()) do
