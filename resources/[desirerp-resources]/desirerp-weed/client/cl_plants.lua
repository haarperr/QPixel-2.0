            ...
            if hit then
                ...
                for matHash,matType in pairs(MaterialHashes) do
                    if materialHash == matHash then
                        ...
                        local finished = exports["desirerp-taskbar"]:taskBar(3000, _L('weed-plants-plant-seed', 'Planting Seed'), false, true, false, false, nil, 5.0, PlayerPedId())
                        ClearPedTasks(PlayerPedId())
                        if finished == 100 then
                            RPC.execute("desirerp-weed:plantSeed", endCoords, typeMod)
...
RegisterNetEvent('desirerp-weed:checkPlant')
AddEventHandler("desirerp-weed:checkPlant", function(pContext, pEntity)
    local plantId = getPlantByPos(pContext["polyZoneCoordinates"])
    if not plantId then return end
    showPlantMenu(plantId)
end)

RegisterNetEvent('desirerp-weed:addWater')
AddEventHandler('desirerp-weed:addWater', function (pPlantId)
    playPourAnimation()
    local finished = exports["desirerp-taskbar"]:taskBar(2000, "Adding Water", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local success = RPC.execute("desirerp-weed:addWater", pPlantId)
        if not success then
...
RegisterNetEvent('desirerp-weed:addFertilizer')
AddEventHandler('desirerp-weed:addFertilizer', function (pPlantId, key)
    playPourAnimation()
    local finished = exports["desirerp-taskbar"]:taskBar(2000, _L('weed-plants-add-fertilizer', 'Adding Fertilizer'), false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local success = RPC.execute("desirerp-weed:addFertilizer", {["id"] = pPlantId, ["type"] = key})
        if not success then
...
RegisterNetEvent('desirerp-weed:addMaleSeed')
AddEventHandler('desirerp-weed:addMaleSeed', function (pPlantId)
    TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_GARDENER_PLANT", 0, true)
    local finished = exports["desirerp-taskbar"]:taskBar(3000, "Adding Male Seed", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        RPC.execute("desirerp-weed:addMaleSeed", pPlantId)
        TriggerEvent("inventory:removeItem", "maleseed", 1)
    end
    showPlantMenu(pPlantId)
end)

RegisterNetEvent('desirerp-weed:removePlant')
AddEventHandler('desirerp-weed:removePlant', function ( pPlantId)
    TriggerEvent("animation:PlayAnimation","layspike")
    local finished = exports["desirerp-taskbar"]:taskBar(3000, _L('weed-plants-remove', 'Removing'), false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local getFertilizer = getPlantGrowthPercent(getPlantById( pPlantId)) > 20.0
        local success = RPC.execute("desirerp-weed:removePlant", pPlantId, getFertilizer)
        if not success then
...
RegisterNetEvent('desirerp-weed:pickPlant')
AddEventHandler("desirerp-weed:pickPlant", function(pContext, pEntity)
    ...
    local plyWeight = exports["desirerp-inventory"]:getCurrentWeight()
    ...
    TriggerEvent("animation:PlayAnimation","layspike")
    local finished = exports["desirerp-taskbar"]:taskBar(10000, _L('weed-plants-harvest', 'Harvesting'), false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        RPC.execute("desirerp-weed:harvestPlant", plantId)
    end
end)

AddEventHandler("desirerp-polyzone:enter", function(zone, data)
    if zone == "desirerp-weed:weed_zone" then
        inZone = inZone + 1
        if inZone == 1 then
            RPC.execute("desirerp-weed:getPlants")
        end
    end
end)

AddEventHandler("desirerp-polyzone:exit", function(zone, data)
    if zone == "desirerp-weed:weed_zone" then
        inZone = inZone - 1
...
RegisterNetEvent('desirerp-weed:trigger_zone')
AddEventHandler("desirerp-weed:trigger_zone", function (type, pData)
    --Update all plants
    if type == 1 then
...
function showPlantMenu(pPlantId)
    ...
    if growth < PlantConfig.HarvestPercent then
        context[#context+1] = {
            ...
            args = { pPlantId },
            event = 'desirerp-weed:addWater',
            disabled = not exports["desirerp-inventory"]:hasEnoughOfItem("water", 1, false)
        }

        context[#context+1] = {
            title = "Add Fertilizer",
            event = 'desirerp-weed:addFertilizerMenu',
            args = { pPlantId },
            disabled = not exports["desirerp-inventory"]:hasEnoughOfItem("fertilizer", 1, false)
        }
    end
    ...
    if getStageFromPercent(growth) < 3 and plant.gender == 0 then
        context[#context+1] = {
            title = "Add Male Seed",
            args = { pPlantId },
            event = "desirerp-weed:addMaleSeed",
            description = "Make the plant preggies",
            disabled = not exports["desirerp-inventory"]:hasEnoughOfItem("maleseed", 1, false)
        }
    end
    ...
    if growth >= 95 or myjob == "police" or myjob == "judge" then
        context[#context+1] = {
            title = "Destroy Plant",
            args = { pPlantId },
            event = "desirerp-weed:removePlant"
        }
    end
...
RegisterNetEvent('desirerp-weed:addFertilizerMenu')
AddEventHandler("desirerp-weed:addFertilizerMenu", function (pPlantId)
    local context = {}

    context[#context+1] = {
        title = "Add Fertilizer (N)",
        args = { pPlantId, "n" },
        event = 'desirerp-weed:addFertilizer',
    }
