    ...
    data = {
        {
            ...
            icon = "handshake",
            event = "desirerp-weed:startCornering",
            parameters = {}
        }
    },
    options = {
        ...
        isEnabled = function(pEntity, pContext)
            local hasWeed = exports["desirerp-inventory"]:hasEnoughOfItem("weedq", 1, false, true)
        --    local isCorneringAlready = exports["desirerp-meth"]:IsCornering()
    ...
    data = {
        {
            ...
            event = "desirerp-weed:stopCornering",
            parameters = {}
        }
    },
    options = {
    ...
    data = {
        {
            ...
            icon = 'comment-dollar',
            event = 'desirerp-weed:cornerSale',
            parameters = {},
        }
    },
    options = {
    ...
    data = {
        {
            ...
            event = 'desirerp-weed:prepareBaggies',
            parameters = {},
        }
    },
    options = {
        distance = { radius = 5.0 },
        isEnabled = function(pEntity, pContext)
            local hasWeed = exports["desirerp-inventory"]:hasEnoughOfItem("weedpackage", 1, false, true)
            return hasWeed
        end
    }
}
...
Citizen.CreateThread(function()
    exports["desirerp-interact"]:AddPeekEntryByEntityType(InteractCorner.group, InteractCorner.data, InteractCorner.options)
    exports["desirerp-interact"]:AddPeekEntryByEntityType(InteractCornerVehicle.group, InteractCornerVehicle.data, InteractCornerVehicle.options)
    exports["desirerp-interact"]:AddPeekEntryByEntityType(InteractStartCorner.group, InteractStartCorner.data, InteractStartCorner.options)
    exports["desirerp-interact"]:AddPeekEntryByEntityType(InteractStopCorner.group, InteractStopCorner.data, InteractStopCorner.options)
end)

AddEventHandler("desirerp-weed:startCornering", function(pContext, pEntity)
    ...
    local canCorner, message, engMessage = RPC.execute("desirerp-weed:startCorner", CurrentCornerCoords)
    ...
    Citizen.CreateThread(function()
        ...
        while CurrentlyCornering do
            ...
            if not foundPed then
                ...
                RPC.execute("desirerp-weed:cornerPed", coords, NetworkGetNetworkIdFromEntity(foundPed), NetworkGetNetworkIdFromEntity(CurrentCornerVehicle))
            end
...
AddEventHandler('desirerp-weed:cornerSale', function(pContext, pEntity)
    ...
    local hasBaggies = exports['desirerp-inventory']:hasEnoughOfItem('weedq', 1, true)
    ...
    RPC.execute('desirerp-weed:cornerSyncHandoff', CurrentCornerCoords, NetworkGetNetworkIdFromEntity(pEntity))
    ...
    local didSale = RPC.execute('desirerp-weed:cornerSale', CurrentCornerCoords, NetworkGetNetworkIdFromEntity(pEntity), CurrentCornerZone)
    if didSale then
      PlayAmbientSpeech1(pEntity, 'Chat_State', 'Speech_Params_Force')
      local rndAmt = math.random(1, 3)
      if rndAmt == 1 and exports['desirerp-inventory']:hasEnoughOfItem('weedq', 1) then
          TriggerEvent('money:clean')
          TriggerEvent('inventory:removeItem', 'weedq', 1)
        elseif rndAmt == 2 and exports['desirerp-inventory']:hasEnoughOfItem('weedq', 3) then
          TriggerEvent('money:clean')
          TriggerEvent('money:clean')
          TriggerEvent('money:clean')
          TriggerEvent('inventory:removeItem', 'weedq', 1)
          TriggerEvent('inventory:removeItem', 'weedq', 1)
          TriggerEvent('inventory:removeItem', 'weedq', 1)
        elseif rndAmt == 3 and exports['desirerp-inventory']:hasEnoughOfItem('weedq', 2) then
          TriggerEvent('money:clean')
          TriggerEvent('money:clean')
          TriggerEvent('inventory:removeItem', 'weedq', 1)
          TriggerEvent('inventory:removeItem', 'weedq', 1)
    ...
    if didSale and math.random() < 0.05 then
      TriggerEvent('desirerp-mdt:drugsale')
    end
  end)

AddEventHandler("desirerp-weed:prepareBaggies", function(pContext, pEntity)
    if CornerConfig.RequireScales then
        local hasScale = exports["desirerp-inventory"]:hasEnoughOfItem("qualityscales", 1, false) or exports["desirerp-inventory"]:hasEnoughOfItem("smallscales", 1, false)
    ...
    local hasBaggies = exports["desirerp-inventory"]:hasEnoughOfItem("emptybaggies", CornerConfig.BaggiesPerBrick / 4, false)
    if not hasBaggies then
    ...
    local finished = exports["desirerp-taskbar"]:taskBar(30000, _L('weed-corner-prepare', 'Preparing'), false, true, false, false, nil, 5.0, pEntity)
    if finished == 100 then
        local weedpackages = exports["desirerp-inventory"]:getItemsOfType("weedpackage", 1, true)
        if #weedpackages == 0 then return end
        local package = weedpackages[1]
        local info = json.decode(package.information)
        RPC.execute("desirerp-weed:prepareBaggies", info)
    end
end)

AddEventHandler("desirerp-weed:stopCornering", function(pContext, pEntity)
    TriggerEvent("DoLongHudText", _L('weed-corner-not-selling', 'No longer selling...'))
    stopCornering()
end)

RegisterNetEvent("desirerp-weed:addCorneredPed")
AddEventHandler("desirerp-weed:addCorneredPed", function(pPed)
    local ped = NetworkGetEntityFromNetworkId(pPed)
    ActivePeds[ped] = false
end)

RegisterNetEvent("desirerp-weed:cornerSyncHandoff")
AddEventHandler("desirerp-weed:cornerSyncHandoff", function(pPed)
    local ped = NetworkGetEntityFromNetworkId(pPed)
...
RegisterNetEvent("desirerp-weed:cornerPed")
AddEventHandler("desirerp-weed:cornerPed", function(pPed, pCornerCoords, pVehicle)
    local ped = NetworkGetEntityFromNetworkId(pPed)
    ActivePeds[ped] = true
    UsedPeds[ped] = true
    if NetworkHasControlOfEntity(ped) then
...
RegisterNetEvent("desirerp-weed:cleanCornerPeds")
AddEventHandler("desirerp-weed:cleanCornerPeds", function()
    UsedPeds = {}
end)
...
function stopCornering()
    ...
    RPC.execute("desirerp-weed:stopCorner")
end
