...
RegisterNetEvent('desirerp-jobs:tow_trucker_duty')
AddEventHandler('desirerp-jobs:tow_trucker_duty', function()
	if not clockedon then
		...
		clockedon = true
		TriggerEvent('desirerp-phone:signInInfoTowing')
		TriggerServerEvent('desirerp-duty:tow_trucker')
		TriggerEvent('desirerp-jobs:give_tow_job')
	else
		TriggerEvent('DoLongHudText', 'You\'re already clocked in.', 2)
	end
end)

RegisterNetEvent('desirerp-jobs_tow_trucker_off_duty')
AddEventHandler('desirerp-jobs_tow_trucker_off_duty', function()
	if clockedon then
...
RegisterNetEvent('desirerp-jobs:towing_accept')
AddEventHandler('desirerp-jobs:towing_accept', function()
	GiveTowJobWanker()
end)
...
function GiveTowJobWanker()
	...
	local vehicle = exports['desirerp-vehicles']:GetVehicleTable()
			...
			Citizen.CreateThread(function()
				while Aspect.Looking do
					...
					if #(vector3(Aspect.Location.x, Aspect.Location.y, Aspect.Location.z) - GetEntityCoords(PlayerPedId())) <= 320 then
						...
						setimpoundveh = plate
                        TriggerEvent('desirerp-fuel:setFuel', GetVehiclePedIsIn(PlayerPedId()), 100)
...
RegisterNetEvent('desirerp-jobs:give_tow_job')
AddEventHandler('desirerp-jobs:give_tow_job', function()
	...
	if clockedon then
		TriggerEvent('desirerp-phone:towing_offer')
	end
end)
...
Citizen.CreateThread(function()
    exports["desirerp-polyzone"]:AddBoxZone("desire_impound_location", vector3(-236.51, -1172.86, 37.6), 20, 27.6, {
        name="desire_impound_location",
        heading=270,
		-- debugPoly=true,
		minZ=22.2,
		maxZ=26.2
...
AddEventHandler('desirerp-polyzone:enter', function(name)
    if name == "desire_impound_location" then
        DesirePolyImp = true     
    end
end)
...
RegisterNetEvent('desirerp-jobs:impound_confirm_shit_idk_anymore')
AddEventHandler('desirerp-jobs:impound_confirm_shit_idk_anymore', function()
	if DesirePolyImp and CamImp then
		TriggerEvent('desirerp-jobs:towing_imp')
	end
end)

RegisterNetEvent('desirerp-jobs:impound_accept')
AddEventHandler('desirerp-jobs:impound_accept', function()
	...
	if DoesEntityExist(vehicle) and NetworkHasControlOfEntity(vehicle) then
		...
		if licensePlate == setimpoundveh then
			...
			if exports['desirerp-hud']:MoneyBuff() then
				TriggerServerEvent('desirerp-jobs:givePayout', math.random(1076, 1554), "cash", '[24/7 Delivery Job | With Buff]')
			else
				TriggerServerEvent('desirerp-jobs:givePayout', math.random(652, 886), "cash", '[24/7 Delivery Job | Without Buff]')
			end
			TriggerEvent('desirerp-jobs:give_tow_job')
...
RegisterNetEvent('desirerp-jobs:imp_decline')
AddEventHandler('desirerp-jobs:imp_decline', function()
...
RegisterNetEvent('desirerp-jobs:towing_imp')
AddEventHandler('desirerp-jobs:towing_imp', function()
	...
	if DoesEntityExist(veh) and NetworkHasControlOfEntity(veh) then
		...
		if licensePlate == setimpoundveh then
			if CanImp then
				TriggerEvent('desirerp-jobs_tow_menu')
			end
		else
...
RegisterNetEvent('desirerp-jobs_tow_menu')
AddEventHandler('desirerp-jobs_tow_menu', function()
	...
	local TowMenu = {
		...
		{
			title = "Accept",
			action = "desirerp-jobs:accept_impound_req",
		},
		{
			title = "Decline",
			action = "desirerp-jobs:decline_impound_req",
		},
	}
	exports["desirerp-interface"]:showContextMenu(TowMenu)
end)

RegisterInterfaceCallback('desirerp-jobs:accept_impound_req', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
	TriggerEvent('desirerp-jobs:impound_accept')
end)

RegisterInterfaceCallback('desirerp-jobs:decline_impound_req', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
	TriggerEvent('desirerp-jobs:imp_decline')
end)
...
exports["desirerp-polyzone"]:AddBoxZone("towing_shit", vector3(-191.63, -1166.46, 23.67), 2, 2, {
	name="towing_shit",
	heading=0,
	--debugPoly=true,
	minZ=21.27,
...
AddEventHandler('desirerp-polyzone:enter', function(name)
	if name == "towing_shit" then
		...
		pSignOnOff()
		if not clockedon then
			exports['desirerp-interface']:showInteraction('[E] Sign In')
		else
			exports['desirerp-interface']:showInteraction('[E] Sign Out', 'error')
		end
	end
end)
	...
	Citizen.CreateThread(function()
        while towing_duty do
			...
			if IsControlJustReleased(0, 38) then
				if not clockedon then
					TriggerEvent('desirerp-jobs:tow_trucker_duty')
					exports['desirerp-interface']:hideInteraction()
				elseif clockedon then
					DeleteEntity(pVeh)
					TriggerEvent('desirerp-jobs_tow_trucker_off_duty')
					exports['desirerp-interface']:hideInteraction()
				end
			end
		end
...
AddEventHandler('pv:tow', function()
	...
	if isVehicleTow then
		...
		if not currentlyTowedVehicle then
		    ...
		    local finished = exports["desirerp-taskbar"]:taskBar(15000,"Hooking up vehicle.")
        	if finished == 100 and aDist < 5.0 then
				...
				if targetVehicle ~= 0 then
					...
					local part2 = exports["desirerp-taskbar"]:taskBar(7000,"Towing Vehicle")
		    ...
		    TriggerEvent("animation:tow")
		    TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10.0, 'towtruck', 0.5)
		    local finished = exports["desirerp-taskbar"]:taskBar(7000,"Untowing Vehicle")
        	if finished == 100 and aDist < 2.0 then
                hasVehicleHookedUp = false

        		TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10.0, 'towtruck2', 0.5)
        		local part2 = exports["desirerp-taskbar"]:taskBar(7000,"Unhooking Vehicle")
