...
Citizen.CreateThread(function()
  while true do
    ...
    if dst < 40 and jobSpawned == false then
      TriggerEvent('desirerp-hunting:spawnJobPed',dumbass, 225.87)
      jobSpawned = true
      refreshPed = true
    end
    if dst >= 41  then
...
RegisterNetEvent('desirerp-hunting:spawnAnimal')
AddEventHandler('desirerp-hunting:spawnAnimal', function()
...
RegisterNetEvent('desirerp-hunting:skinAnimal')
AddEventHandler('desirerp-hunting:skinAnimal', function()
  if exports["desirerp-inventory"]:hasEnoughOfItem("huntingknife", 1) then
    if DoesEntityExist(baitAnimal) then
        LoadAnimDict('amb@medic@standing@kneel@base')
        LoadAnimDict('anim@gangops@facility@servers@bodysearch@')
        TaskPlayAnim(PlayerPedId(), "amb@medic@standing@kneel@base" ,"base" ,8.0, -8.0, -1, 1, 0, false, false, false )
        TaskPlayAnim(PlayerPedId(), "anim@gangops@facility@servers@bodysearch@" ,"player_search" ,8.0, -8.0, -1, 48, 0, false, false, false )
        local finished = exports["desirerp-taskbar"]:taskBar(5000,"Skinning")
        if finished == 100 then
          ClearPedTasksImmediately(PlayerPedId())
          DeleteEntity(baitAnimal)
          TriggerServerEvent('desirerp-hunting:skinReward')
        end
    end
  else
...
RegisterNetEvent('desirerp-hunting:usedBait')
AddEventHandler('desirerp-hunting:usedBait', function()
  for k,v in pairs (HuntingZones) do
    if IsEntityInZone(GetPlayerPed(-1),v) then
      if cooldown <= 0 then
        LoadAnimDict('amb@medic@standing@kneel@base')
        TaskPlayAnim(PlayerPedId(), "amb@medic@standing@kneel@base" ,"base" ,8.0, -8.0, -1, 1, 0, false, false, false )
        local finished = exports["desirerp-taskbar"]:taskBar(5000,"Placing Bait",false,false,playerVeh)
        Citizen.Wait(100)
        cooldown = useBaitCooldown * 10000
        ClearPedTasksImmediately(PlayerPedId())
        used = true
        usedBait()
        TriggerEvent('desirerp-hunting:spawnAnimal')
        TriggerServerEvent('desirerp-hunting:removeBait')
        baitCooldown()
      end
    end
  end
...
local function processScope(freeAiming)
    if not isFreeAiming and freeAiming then
        isFreeAiming = true
        exports["desirerp-ui"]:sendAppEvent("sniper-scope", { show = true })
    elseif isFreeAiming and not freeAiming then
        isFreeAiming = false
        exports["desirerp-ui"]:sendAppEvent("sniper-scope", { show = false })
    end
end
...
local function sellAnimals()
  ...
  for _, carcass in pairs(carcasses) do
      local qty = exports["desirerp-inventory"]:getQuantity(carcass.name)
...
AddEventHandler("desirerp-polyzone:enter", function(name)
  ...
  exports['desirerp-interface']:showInteraction("[E] Sell Animal Carcass")
  listenForKeypress()
end)
...
Citizen.CreateThread(function()
  exports["desirerp-polyzone"]:AddCircleZone("huntingsales", vector3(12.28, -1299.32, 29.29), 1.3, {
      useZ = true,
      debugPoly= false,
  })
end)