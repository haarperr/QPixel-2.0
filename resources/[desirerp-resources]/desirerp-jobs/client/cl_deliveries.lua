...
RegisterNetEvent('desirerp-jobs:deliverySignIn')
AddEventHandler('desirerp-jobs:deliverySignIn', function()
    if not inJob then
        TriggerEvent('desirerp-phone:signInInfoDelivery')
        print('[24/7 DELIVERIES] AWAITING JOB')
        inJob = true
        Citizen.Wait(math.random(60000, 120000))
        print('[24/7 DELIVERIES] SENDING JOB')
        TriggerEvent('desirerp-phone:delivery_offer')
    end
end)

RegisterNetEvent('desirerp-jobs:deliveries:getCar')
AddEventHandler('desirerp-jobs:deliveries:getCar', function()
    local model = "benson"
    RequestModel(model)
    ...
    TriggerEvent('desirerp-jobs:getDeliveryJob')
end)

RegisterNetEvent('desirerp-jobs:deleteDeliveryVehicle')
AddEventHandler('desirerp-jobs:deleteDeliveryVehicle', function()
    DeleteEntity(pVeh)
    pVeh = nil
end)

RegisterNetEvent('desirerp-jobs:getDeliveryJob')
AddEventHandler('desirerp-jobs:getDeliveryJob', function()
...
Citizen.CreateThread(function()
    exports["desirerp-polyzone"]:AddBoxZone("delivery_store_1", vector3(-48.17, -1751.95, 29.42), 12.6, 15, {
    ...
    exports["desirerp-polyzone"]:AddBoxZone("delivery_store_2", vector3(-712.37, -911.54, 19.01), 15, 10.2, {
    ...
    exports["desirerp-polyzone"]:AddBoxZone("delivery_store_3", vector3(378.01, 329.12, 103.45), 12, 11.0, {
    ...
    exports["desirerp-polyzone"]:AddBoxZone("delivery_store_4", vector3(-1224.38, -905.82, 12.31), 7, 7, {
    ...
    exports["desirerp-polyzone"]:AddBoxZone("delivery_store_5", vector3(-2968.34, 390.36, 15.04), 7, 7, {
...
AddEventHandler('desirerp-polyzone:enter', function(name)
    if name == "delivery_store_1" then
        ...
        if onWay1 and hasGoods then
            pDropGoods()
            exports['desirerp-interface']:showInteraction('[E] Drop Goods [Delivery Job]')
            canDropGoods = true     
        end
	...
	Citizen.CreateThread(function()
        while delivery_store_1 or delivery_store_2 or delivery_store_3 or delivery_store_4 or delivery_store_5 do
			...
			if IsControlJustReleased(0, 38) and hasGoods then
                ...
                exports['desirerp-interface']:hideInteraction()
                TriggerEvent('desirerp-jobs:deliveryReturnToDepo')
			end
		end
	end)
end

RegisterNetEvent('desirerp-jobs:deliveryReturnToDepo')
AddEventHandler('desirerp-jobs:deliveryReturnToDepo', function()
    pDeliveryJob = false
    onWay1 = false
    onWay2 = false
    onWay3 = false
        ...
        local aDist = GetDistanceBetweenCoords(setCoords, coordA)
            if aDist < 15.0 then
                ...
                if exports['desirerp-hud']:MoneyBuff() then
                    TriggerServerEvent('desirerp-jobs:givePayout', math.random(650, 727), "cash", '[24/7 Delivery Job | With Buff]')
                else
                    TriggerServerEvent('desirerp-jobs:givePayout', math.random(500, 678), "cash", '[24/7 Delivery Job | Without Buff]')
                end
                TriggerEvent('desirerp-jobs:deleteDeliveryVehicle')
            end
        end
    end)
end)
...
exports["desirerp-polytarget"]:AddCircleZone("delivery_start_stop",  vector3(919.82, -1256.64, 25.51), 0.8, {
    useZ = true
})

exports["desirerp-interact"]:AddPeekEntryByPolyTarget("delivery_start_stop", {{
    event = "desirerp-jobs:deliverySignIn",
...
-- Ped Shit -- (IGNORE THIS IS FOR DESIRERP-ACTIVITIES HOUSE ROBBERIES)

exports["desirerp-polytarget"]:AddCircleZone("start_house_robbery",  vector3(206.81, -1851.14, 27.76), 0.85, {
    useZ = true
})

exports["desirerp-interact"]:AddPeekEntryByPolyTarget("start_house_robbery", {{
    event = "desirerp-houserobbery:getJob",
