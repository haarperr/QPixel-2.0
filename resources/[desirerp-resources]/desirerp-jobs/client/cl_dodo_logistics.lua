...
RegisterNetEvent('desirerp-jobs:passed')
AddEventHandler('desirerp-jobs:passed', function()
    pisInDodoJob = true
    ...
    TriggerEvent('desirerp-phone:goToLocation')
end)

RegisterNetEvent('desirerp-jobs:startDodoLogistics')
AddEventHandler('desirerp-jobs:startDodoLogistics', function()
    TriggerEvent('desirerp-phone:signInDodo')
    ...
    TriggerEvent('desirerp-phone:dodoJobOffer')
end)

RegisterNetEvent('desirerp-jobs:dodoLogisticsPayout')
AddEventHandler('desirerp-jobs:dodoLogisticsPayout', function()
    if pOnWayDodo == true then
        if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("pounder"))  then
            if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("pounder")) then
                TriggerServerEvent('desirerp-jobs:dodoLogisticsPayout')
    ...
    while true do
        ...
        if isToDropLocation == true then
            ...
            if GetDistanceBetweenCoords(px,py,pz, GetEntityCoords(GetPlayerPed(-1),true)) < 2.5 then
                if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("pounder")) then
                    exports['desirerp-interface']:showInteraction('[E] Drop Off Package')
                    if IsControlJustPressed(1,38) then
                        TriggerEvent('desirerp-phone:goBackDepo')
                        exports['desirerp-interface']:hideInteraction()
                            ...
                            local aDist = GetDistanceBetweenCoords(setCoords, coordA)
                                if aDist < 15.0 then
                                    ...
                                    if exports['desirerp-hud']:MoneyBuff() then
                                        TriggerServerEvent('desirerp-jobs:givePayout', math.random(878, 927), "bank", '[Dodo Logistics | With Buff]')
                                    else
                                        TriggerServerEvent('desirerp-jobs:givePayout', math.random(650, 778), "bank", '[Dodo Logistics | Without Buff]')
                                    end
                                end
                            end
                        end)
...
RegisterNetEvent("desirerp-jobs:spawn-truck")
AddEventHandler("desirerp-jobs:spawn-truck", function()
	Citizen.CreateThread(function()
        ...
        TriggerEvent('desirerp-jobs:passed')
...
exports["desirerp-polytarget"]:AddCircleZone("dodo_sign_in", vector3(-437.91, -2795.5, 7.39), 0.6, {
    useZ=true,
})

exports["desirerp-interact"]:AddPeekEntryByPolyTarget("dodo_sign_in", {{
    event = "desirerp-jobs:startDodoLogistics",
    id = "dodo_sign_in",
    icon = "circle",
    label = "Sign In",
    parameters = {},
