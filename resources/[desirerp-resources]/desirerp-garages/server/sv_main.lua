...
RegisterServerEvent("desirerp-garages:resetpdgarage")
AddEventHandler("desirerp-garages:resetpdgarage",function ()
    ResetPDGarages()
end)



RegisterServerEvent("desirerp-garages:resetcartoimpound")
AddEventHandler("desirerp-garages:resetcartoimpound",function (plate)
...
RPC.register("desirerp-garages:select",function(pSource,garname)
	local src = source
	if garname.param == "Impound Lot" then
		exports.ghmattimysql:execute('SELECT * FROM characters_cars WHERE vehicle_state = @state', {['@state'] = "Out"}, function(vehicles)
			for k, v in ipairs(vehicles) do
				TriggerClientEvent("desirerp-garages:resetcartoimpound", src, v.license_plate)
			end
		end)
	end
	...
	exports.ghmattimysql:execute('SELECT * FROM characters_cars WHERE cid = @cid AND current_garage = @garage ', { ['@cid'] = char.id, ["garage"] = garname.param }, function(result)
		if (result and result[1]) then
			for ind, value in pairs(result) do
				...
				TriggerClientEvent('desirerp-context:sendMenu', src, {
					{
						id = value.id,
						header = value.name,
...
RPC.register("desirerp-garages:selectShared",function(pSource,garname)
    local src = source
	local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
				...
				TriggerClientEvent('desirerp-context:sendMenu', src, {
					{
						...
						txt = 'Plate: ' .. value.license_plate,
						params = {
							event = "desirerp-garages:takeout2",
							args = value.id
						}
					},
				})
...
RPC.register("desirerp-garages:attempt:sv",function(pSource,pdata)
	local src = source
	local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
...
RPC.register("desirerp-garages:attempt:sv2",function(pSource,pdata)
	local src = source
	local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
		...
		for ind, value in pairs(result) do
			TriggerClientEvent('desirerp-context:sendMenu', src, {
				...
				},
				{
					...
					txt = 'Spawn Selected Vehicle',
					params = {
						event = "desirerp-garages:attempt:spawn2",
						args = result
					}
				},
				{
...
RPC.register("desirerp-garages:spawned:get",function(pSource,valid)
	local src = source
	local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
	...
	exports.ghmattimysql:execute('SELECT * FROM characters_cars WHERE cid = @cid AND id = @id', { ['@cid'] = char.id, ['@id'] = valid.param }, function(result)
		if result[1].vehicle_state == "In" then
			TriggerClientEvent("desirerp-garages:attempt:spawn", src, result, true)
		else
...
RPC.register("desirerp-garages:spawned:getShared",function(pSource,valid)
	local src = source
	local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
	...
	exports.ghmattimysql:execute('SELECT * FROM characters_cars WHERE id = @id', { ['@id'] = valid.param }, function(result)
		if result[1].vehicle_state == "In" then
			TriggerClientEvent("desirerp-garages:attempt:spawn2", src, result, true)
		else
			TriggerClientEvent('DoLongHudText', src, 'Vehicle is not in the garage. Check in your phone maybe.', 2)
		end
    end)
