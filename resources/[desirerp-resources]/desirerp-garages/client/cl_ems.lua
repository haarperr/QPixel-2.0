...
Citizen.CreateThread(function()
    exports["desirerp-polyzone"]:AddBoxZone("ems_buy", vector3(301.07, -574.73, 43.26), 1.4, 1, {
        name="ems_buy",
        heading=18,
        --debugPoly=true,
        minZ=40.46,
        ...
        while nearBuy2 do
            ...
            if job == 'ems' then
                if IsControlJustReleased(0, 38) then
                    TriggerEvent('desirerp-garages:openBuyMenu2')
                end
            end
        end
    end)
...
RegisterNetEvent('desirerp-garages:openBuyMenu2')
AddEventHandler('desirerp-garages:openBuyMenu2', function()
    TriggerEvent('desirerp-context:sendMenu', {
		{
			...
			params = {
				event = "desirerp-garages:PurchasedSpeedo"
			}
		},
        --[[{ WIP
			id = 2,
			...
			txt = "Purchase for $5000",
			params = {
				event = "desirerp-garages:PurchasedBamb"
			}
		},--]]
        {
			id = 2,
			...
			params = {
				event = "desirerp-garages:PurchasedBexp"
			}
		},
        {
			id = 3,
			...
			params = {
				event = "desirerp-garages:PurchasedEMSPRO"
			}
		},
...
RegisterNetEvent('desirerp-garages:PurchasedSpeedo')
AddEventHandler('desirerp-garages:PurchasedSpeedo', function()
    if exports["isPed"]:isPed("mycash") >= 5000 then
        --TriggerServerEvent('bankingremoveMoney')
        TriggerServerEvent('yorktax:speedo')
        TriggerEvent('desirerp-garages:PurchasedVeh2', 'Speedo', 'emsnspeedo', '0')
    else
        TriggerEvent('DoLongHudText', "You do not have enough money!", 2)
    end
end)

RegisterNetEvent('desirerp-garages:PurchasedBamb')
AddEventHandler('desirerp-garages:PurchasedBamb', function()
    if exports["isPed"]:isPed("mycash") >= 5000 then
        --TriggerServerEvent('bankingremoveMoney', 5000)
        TriggerServerEvent('yorktax:bamb')
        TriggerEvent('desirerp-garages:PurchasedVeh2', 'Ambulance', 'bamb', '0')
    else
        TriggerEvent('DoLongHudText', "You do not have enough money!", 2)
    end
end)

RegisterNetEvent('desirerp-garages:PurchasedBexp')
AddEventHandler('desirerp-garages:PurchasedBexp', function()
    if exports["isPed"]:isPed("mycash") >= 15000 then
        --TriggerServerEvent('bankingremoveMoney', 5000)
        TriggerServerEvent('yorktax:bexp')
        TriggerEvent('desirerp-garages:PurchasedVeh2', 'Explorer', 'bexp', '0')
    else
        TriggerEvent('DoLongHudText', "You do not have enough money!", 2)
    end
end)   

RegisterNetEvent('desirerp-garages:PurchasedEMSPRO')
AddEventHandler('desirerp-garages:PurchasedEMSPRO', function()
    if exports["isPed"]:isPed("mycash") >= 35000 then
        --TriggerServerEvent('bankingremoveMoney', 5000)
        TriggerServerEvent('yorktax:emspro')
        TriggerEvent('desirerp-garages:PurchasedVeh2', 'Sierra', 'emspro', '0')
    else
        TriggerEvent('DoLongHudText', "You do not have enough money!", 2)
    end
end)  

RegisterNetEvent('desirerp-garages:PurchasedVeh2')
AddEventHandler('desirerp-garages:PurchasedVeh2', function(name, veh, price)
    ...
    TriggerServerEvent('desirerp-garages:FinalizedPur2', plate, name, vehicle, price)
    Citizen.Wait(100)
    exports['desirerp-interface']:hideInteraction()
end)
