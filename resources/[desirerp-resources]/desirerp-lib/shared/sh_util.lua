if not IsDuplicityVersion() then
    ...
    function SetCustomNuiFocus(hasKeyboard, hasMouse)
        ...
        TriggerEvent("desirerp-base:focus", HasNuiFocus, hasKeyboard, hasMouse)
    end
    ...
    function TaskItem(item, animDict, animName, typeAnim, speed, prop, message, timer, movement, weapon, remove)
        if not LocalPlayer.state.doge then
            TriggerEvent('desirerp-lib:forceAnimation', animDict, animName, typeAnim, speed, prop)
        end
        local finished = exports["desirerp-taskbar"]:TaskBar(message, timer, movement, weapon)
        if finished == 100 then
            if exports["desirerp-inventory"]:HasItem(item) then
                if remove then
                    TriggerEvent("inventory:destroy", item)
                end
                TriggerEvent('desirerp-lib:forceAnimation')
                return true
            else
    ...
    function GetNearbyPeds(X, Y, Z, Radius)
        ...
        if tonumber(X) and tonumber(Y) and tonumber(Z) then
            if tonumber(Radius) then
                for Ped in exports["desirerp-admin"]:EnumeratePeds() do
    ...
    function SkillCircleLooped(count)
        ...
        while loop < count do
            loop = loop + 1
            local finished = exports["desirerp-skillcircle"]:SkillCircle(math.random(13, 60))
    ...
    function SpeedSkillCircle()
        local finished = exports["desirerp-skillcircle"]:SkillCircle(math.random(4, 7))
        if not finished then
            return false
        end
        return true
    ...
    function SkillCircleLooped2(sent)
        ...
        for i = 1, #speeds do
            local finished = exports["desirerp-skillcircle"]:SkillCircle(speeds[i])
            if not finished then
                return false
            end
            Wait(200)
    ...
    function SpawnVehicle(model, spawn, heading)
        ...
        TriggerEvent('desirerp-vmod:addKeys', _vehicle)
...
function encodeAccountId(plain)
    if type(plain) == 'number' then
        return math.floor(exports["desirerp-financials"]:EncodeId(plain))
    end
end

function decodeAccountId(encoded)
    if type(encoded) == 'number' then
        return exports["desirerp-financials"]:DecodeId(encoded)
    end
end
