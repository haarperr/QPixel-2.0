...
function GetEntityContext(pEntity, pEntityType, pEntityModel)
    ...
    if context.type == 1 then
        context.flags = exports["desirerp-flags"]:GetPedFlags(pEntity)
    ...
    elseif context.type == 2 then
        context.flags = exports["desirerp-flags"]:GetVehicleFlags(pEntity)
    elseif context.type == 3 then
        context.flags = exports["desirerp-flags"]:GetObjectFlags(pEntity)
    end
...
function GetPedContext(pEntity, pContext)
    ...
    if pContext.flags["isJobEmployer"] then
        pContext.job = exports["desirerp-jobs"]:GetNPCJobData(npcId)
    end
end

--[[function isDisabled()
    return exports["desirerp-base"]:getVar("dead") or
        exports["desirerp-base"]:getVar("handcuffed")
end]]

function hasPhone()
    return exports["desirerp-inventory"]:hasEnoughOfItem("mobilephone", 1, false) or
...
function hasKeys(pEntity)
    ...
    local hasKeys = exports["desirerp-vehicles"]:HasVehicleKey(pEntity)
...
function hasRepairItems(type)
    ...
    for i, v in pairs(items) do
        if exports["desirerp-inventory"]:hasEnoughOfItem(v, 1, false) then
            return true
        end
    end
...
AddEventHandler("desirerp-polyzone:enter", function(zone, data)
    if zone == "desirerp-jobs:impound:dropOff" then IsImpoundDropOff = true end
...
RegisterNetEvent("desirerp-jobs:jobChanged")
AddEventHandler("desirerp-jobs:jobChanged", function(pJobId)
    CurrentJob = pJobId
