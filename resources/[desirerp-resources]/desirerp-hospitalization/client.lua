...
AddEventHandler("desirerp-hospitalization:checkin", function()
	if exports["isPed"]:isPed("countems") <= 0 or exports["isPed"]:isPed("myJob") == 'ems'  then
		...
		local finished = exports["desirerp-taskbar"]:taskBar(1700,"Checking Credentials")
...
AddEventHandler("desirerp-hospitalization:page", function()
	loadAnimDict('anim@narcotics@trash')
	TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',1.0, 1.0, -1, 1, 0, 0, 0, 0)
	local finished = exports["desirerp-taskbar"]:taskBar(1700,"Paging a Doctor")
	if finished == 100 then
...
AddEventHandler("client:bed",function()
  ...
  if DoesEntityExist(objFound) then
	...
	exports['desirerp-textui']:showInteraction("[E] Leave Bed")
...
function findBed(fadein)
	...
	if not inbed then
		...
		inbed = false
		injurycount = 0
		TriggerEvent('desirerp-hospital:client:ResetLimbs')
		TriggerEvent('desirerp-hospital:client:RemoveBleed')
