...
RegisterInterfaceCallback("desirerp-signin:handler", function(data, cb)
  ...
  if eventData.sign_in then
    if eventData.job == "police" then
      ...
      TriggerServerEvent("desirerp-signin:duty", eventData.job)
    end
...
RegisterInterfaceCallback("desirerp-signin:spawnVehicle", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  local eventData = data.key
...
AddEventHandler("desirerp-signin:peekAction", function(pArgs, pEntity, pContext)
  ...
  print(MenuData[name])
  exports["desirerp-interface"]:showContextMenu(MenuData[name])
end)
