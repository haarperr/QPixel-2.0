RPC.register("desirerp-gopros:addSecCamera", function(pSource, pName, pCoords, pRotation)
...
RPC.register("desirerp-gopros:getSecurityCameraById", function(pSource, pCameraID)
    local result = MySQL.query.await([[
...
RPC.register("desirerp-gopros:getSecurityCameraByStateID", function(pSource, pCid)
...
RPC.register("desirerp-gopros:getSecurityCameraAccessByCameraID", function(pSource, pCamera)
...
RPC.register("desirerp-gopros:addUserToCamera", function(pSource, pCamera, pCid)
    local camera = MySQL.query.await([[
        SELECT * FROM _cameras WHERE id = ?
    ]], {pCamera})

    print(pCid)
    local pCid = exports['desirerp-phone']:getCIDFromPhoneNumber(pCid)
    print(pCid)
...
RPC.register("desirerp-gopros:removeUserFromCamera", function(pSource, pCamera, pCid)
    local camera = MySQL.query.await([[
        SELECT * FROM _cameras WHERE id = ?
    ]], {pCamera})
