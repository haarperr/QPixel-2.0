...
RegisterNetEvent("desirerp-gopros:updateRegisteredCams")
AddEventHandler("desirerp-gopros:updateRegisteredCams", function(pDashcams)
  activeDashcams = pDashcams
  exports["desirerp-interface"]:sendAppEvent("gopros", { dashcams = activeDashcams })
end)
AddEventHandler("playerSpawned", function()
  Wait(5550)
  RPC.execute("desirerp-gopros:playerSpawnedSendCams")
end)

RegisterCommand("desirerp-gopros:addUserToCamera", function()
  RPC.execute("desirerp-gopros:addUserToCamera",1,868)
  
  end)
  

--[[
RegisterUICallback("desirerp-interface:goproChangeSelectedPov", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = 'done' }})
  local activateGoPro = activateGoPro(tonumber(data.netId))
  Wait(600)
  exports["desirerp-interface"]:sendAppEvent("gopros", { switchingViews = false })
end)
]]

AddEventHandler("desirerp-interface:application-closed", function(name)
  if name ~= "gopros" then return end
  DoScreenFadeOut(400)
  Wait(400)
  TriggerEvent("desirerp-casino:inVRHeadset", false)
  deactivateGoPro()
  ...
  exports["desirerp-interface"]:sendAppEvent("hud", { display = true })
  DoScreenFadeIn(1000)
end)
          ...
          if IsControlJustReleased(0, 38) then
              ...
              if destinationSelected then
                ...
                exports['desirerp-interface']:openApplication('textbox', {
                  callbackUrl = 'desirerp-interface:goProRegisterSecCam',
                  key = 1,
                  items = {
                    {
                      icon = "pencil-alt",
                ...
                local camPos = exports["desirerp-selector"]:getCurrentSelection()
                ...
                Wait(100)
                exports["desirerp-interface"]:showInteraction(("[E] %s"):format("View Here"))
                destinationSelected = true
                listenForKeypress()
                exports["desirerp-selector"]:startSelecting(1, 1, function()
                  return false
                end)
              end
          end
...
AddEventHandler("desirerp-inventory:itemUsed", function(item)
  ...
  local result = exports["desirerp-objects"]:PlaceObject(
    `prop_spycam`,
    {
      collision = false,
      groundSnap = false,
  ...
  local prompt = exports["desirerp-interface"]:OpenInputMenu({
    {
      name = 'name',
      label = 'Security Camera Stream Name',
      icon = 'pencil-alt'
  ...
  local success = RPC.execute("desirerp-gopros:addSecCamera", prompt.name, result[2].coords, result[2].rotation)
  if success then
...
local function activateSecurityCameraById(pId)
  local cam = RPC.execute("desirerp-gopros:getSecurityCameraById", pId)
  ...
  exports["desirerp-interface"]:sendAppEvent("hud", { display = false })
  DoScreenFadeIn(1000)
end
...
--[[
RegisterUICallback("desirerp-interface:gopros:getSecurityCameras", function(data, cb)
  local cid = exports["isPed"]:isPed("cid")
  local cams = RPC.execute("desirerp-gopros:getSecurityCameraByCid", cid)
  cb({ data = cams, meta = { ok = true, message = "done" } })
end)

RegisterUICallback("desirerp-interface:gopros:viewCameraById", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  Wait(500)
  exports["desirerp-interface"]:closeApplication("phone")
...
Citizen.CreateThread(function()
  exports["desirerp-keybinds"]:registerKeyMapping("", "Camera", "Look Up", "+securityCamLookUp", "-securityCamLookUp", "PAGEUP")
  exports["desirerp-keybinds"]:registerKeyMapping("", "Camera", "Look Down", "+securityCamLookDown", "-securityCamLookDown", "PAGEDOWN")
  exports["desirerp-keybinds"]:registerKeyMapping("", "Camera", "Look Left", "+securityCamLookLeft", "-securityCamLookLeft", "HOME")
  exports["desirerp-keybinds"]:registerKeyMapping("", "Camera", "Look Right", "+securityCamLookRight", "-securityCamLookRight", "END")
  exports["desirerp-keybinds"]:registerKeyMapping("", "Camera", "Turn Off", "+securityCamOff", "-securityCamOff", "ESCAPE")
  ...
  exports["desirerp-interact"]:AddPeekEntryByModel({ `prop_spycam` }, {
    {
      id = "cam_blur_action",
      event = "desirerp-gopro:blurCamera",
      icon = "fingerprint",
      label = "Smudge",
    },
...
AddEventHandler('desirerp-gopro:blurCamera', function(pArgs, pEntity, pContext)
  ...
  local function loopSkill(count)
    ...
    while loopCount < count do
        ...
        local finished = exports["desirerp-interface"]:taskBarSkill(math.random(1400, 2000), math.random(7, 12))
  ...
  exports["desirerp-objects"]:UpdateObject(object.id, { blurred = true })
end)

AddEventHandler('desirerp-objects:objectCreated', function(pObject, pEntity)
  if pObject.data.model == `prop_spycam` then
    SetEntityCollision(pEntity, false, false)
  end
end)
--[[
RegisterUICallback("desirerp-interface:gopros:addUserToCamera", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  RPC.execute("desirerp-gopros:addUserToCamera", data.camera, data.cid)
end)
