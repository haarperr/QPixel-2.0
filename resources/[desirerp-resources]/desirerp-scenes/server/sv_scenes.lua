...
RegisterNetEvent("desirerp-scenes:getScenes")
AddEventHandler("desirerp-scenes:getScenes", function()
    local src = source
    local data = Await(SQL.execute("SELECT * FROM __scenes", {}))

    if not data then
        TriggerClientEvent("desirerp-scenes:allScenes", src, {})
        return
    end
    ...
    TriggerClientEvent("desirerp-scenes:allScenes", src, scenes)
end)

RegisterNetEvent("desirerp-scenes:addScene")
AddEventHandler("desirerp-scenes:addScene", function(pData)
    local src = source
    ...
    TriggerClientEvent("desirerp-scenes:refreshScenes", -1, data[1].id, data[1])
end)

RegisterNetEvent("desirerp-scenes:deleteScene")
AddEventHandler("desirerp-scenes:deleteScene", function(pCoords)
    local data = Await(SQL.execute("SELECT * FROM __scenes", {}))
    for k,v in pairs(data) do
        ...
        if #(acCoord - pCoords) < 2.0 then
            ...
            if delete then
                TriggerClientEvent("desirerp-scenes:deleteScene", -1, v.id)
                return
            end
        end
    end
