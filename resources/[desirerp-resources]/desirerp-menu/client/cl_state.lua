...
AddEventHandler('desirerp-polyzone:entered:garages', polyZoneEnter)
AddEventHandler('desirerp-polyzone:exited:garages', polyZoneExit)
...
RegisterNetEvent("menu:send:desirerp-housing:garages", function(pTable, pHid, pHeading)
        exports["desirerp-polyzone"]:AddBoxZone("housingGarage-"..pHid, pTable, 10, 10, {
            heading = pHeading,
            minZ = -25,
            maxZ = 100,
          })
...
AddEventHandler('desirerp-polyzone:enter', function(name)
    ...
    for k, v in pairs (sharedGarages) do
        if name == v then
            local garageData = exports["desirerp-garages"]:getGarageData(name)
