...
function updateCarTable(model,price,name)
	for i=1, #carTable do
		...
		TriggerServerEvent("desirerp-jacksons_autos:CarTable",carTable)
	end
end

local myspawnedvehs = {}

RegisterNetEvent("desirerp-jacksons_autos:testdrive")
AddEventHandler("desirerp-jacksons_autos:testdrive", function()
	if exports["desirerp-business"]:IsEmployedAt('jacksons_autos') or #(vector3(-775.9081, -1032.289, 13.719951) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("desirerp-jacksons_autos:EnableBuy")
AddEventHandler("desirerp-jacksons_autos:EnableBuy", function()
	if #(vector3(-775.4795, -1032.456, 14.129936) - GetEntityCoords(PlayerPedId())) > 50.0 and exports["desirerp-business"]:IsEmployedAt('jacksons_autos') then
		return
	end
	...
	TriggerServerEvent("desirerp-jacksons_autos:BuyEnabledSV",vehplate)
end)

RegisterNetEvent("desirerp-jacksons_autos:BuyEnabledCL")
AddEventHandler("desirerp-jacksons_autos:BuyEnabledCL", function(addplate)
	buyPlate[addplate] = true
	Citizen.Wait(60000)
	buyPlate[addplate] = nil
end)

RegisterNetEvent("desirerp-jacksons:CommissionPrice")
AddEventHandler("desirerp-jacksons:CommissionPrice", function(newAmount)
	if eexports["desirerp-business"]:IsEmployedAt('jacksons_autos') or #(vector3(-775.4795, -1032.456, 14.129936) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("desirerp-jacksons_autos:ReturnCarTable")
AddEventHandler("desirerp-jacksons_autos:ReturnCarTable", function(newTable)
	carTable = newTable
	DespawnSaleVehicles()
	SpawnSaleVehicles()
end)
...
function AttemptBuy(tableid)
	...
	TriggerServerEvent('desirerp-jacksons_autos:ChechMoney',name, model, price)
...
function DrawPrices()
	for i = 1, #pSpawnLocations do
		if #(vector3(pSpawnLocations[i]["x"],pSpawnLocations[i]["y"],pSpawnLocations[i]["z"]) - GetEntityCoords(PlayerPedId())) < 1.5 then
			...
			if exports["desirerp-business"]:IsEmployedAt('jacksons_autos') then
...
function SpawnSaleVehicles()
	if not hasspawned then
		TriggerServerEvent("desirerp-jacksons_autos:RequestCarTable")
		Citizen.Wait(1500)
	end
...
function CloseCreator(name, veh, price, plate)
	Citizen.CreateThread(function()
		...
		if not boughtcar then
			...
			TriggerServerEvent('desirerp-jacksons_autos:BuyVehicle', plate, vehname, vehicle, price, VehicleProps)
			TriggerServerEvent('desirerp-cardealer:log', exports['isPed']:isPed('cid'), vehname, price, plate, 'Jacksons Autos')
			TriggerServerEvent('desirerp-financials:business_money', price / 10, 'jacksons_autos', 'add')
			exports['desirerp-interface']:hideInteraction()
			DespawnSaleVehicles()
			SpawnSaleVehicles()
		end
...
RegisterNetEvent("desirerp-jacksons_autos:FailedPurchase")
AddEventHandler("desirerp-jacksons_autos:FailedPurchase", function()
...
function ButtonSelected(button)
	...
	elseif this == "b-class" or this == "s-class" then
		if ownerMenu then
			...
			TriggerServerEvent('desirerp-jacksons_autos:ChechMoney',button.name, button.model, button.costs)
		end
	end
end
	...
	while true do
		...
		if vehshop.opened then
			...
			for i,button in pairs(menu.buttons) do
				...
				if exports["desirerp-business"]:IsEmployedAt('jacksons_autos') and i >= vehshop.menu.from and i <= vehshop.menu.to then
...
RegisterCommand('commission', function(source, args, raw)
	if exports["desirerp-business"]:IsEmployedAt('jacksons_autos') then
		...
		if tonumber(amount) > 0 and tonumber(amount) <= 60 then
			TriggerEvent('desirerp-jacksons_autos:CommissionPrice', amount)
		else
...
RegisterCommand('testdrive', function(source, args, raw)
	if exports["desirerp-business"]:IsEmployedAt('jacksons_autos') then
		TriggerEvent('desirerp-jacksons_autos:testdrive')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)
...
RegisterCommand('enableBuy', function(source, args, raw)
	if exports["desirerp-business"]:IsEmployedAt('jacksons_autos') then
		TriggerEvent('desirerp-jacksons_autos:EnableBuy')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)