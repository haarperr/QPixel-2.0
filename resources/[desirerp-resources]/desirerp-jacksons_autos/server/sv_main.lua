...
RegisterServerEvent('desirerp-jacksons_autos:CarTable')
AddEventHandler('desirerp-jacksons_autos:CarTable', function(table)
    if table ~= nil then
        carTable = table
        TriggerClientEvent('desirerp-jacksons_autos:ReturnCarTable', -1, carTable)
        for i=1, #carTable do
...
RegisterServerEvent('desirerp-jacksons_autos:FinaceEnabledSV')
AddEventHandler('desirerp-jacksons_autos:FinaceEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('desirerp-jacksons_autos:FinaceEnabledCL', -1, plate)
    end
end)

RegisterServerEvent('desirerp-jacksons_autos:BuyEnabledSV')
AddEventHandler('desirerp-jacksons_autos:BuyEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('desirerp-jacksons_autos:BuyEnabledCL', -1, plate)
    end
end)

-- return table
-- TODO (return db table)
RegisterServerEvent('desirerp-jacksons_autos:RequestCarTable')
AddEventHandler('desirerp-jacksons_autos:RequestCarTable', function()
    local user = source
    exports.oxmysql:execute("SELECT * FROM vehicle_display", {}, function(display)
        ...
        TriggerClientEvent('desirerp-jacksons_autos:ReturnCarTable', user, carTable)
    end)
end)

-- Check if player has enough money
RegisterServerEvent('desirerp-jacksons_autos:ChechMoney')
AddEventHandler('desirerp-jacksons_autos:ChechMoney', function(name, model, price)
    local src = source
    ...
    elseif tonumber(cash) <= price then
        ...
        TriggerClientEvent('desirerp-jacksons_autos:FailedPurchase', src)
    end
end)

RegisterServerEvent('desirerp-jacksons_autos:BuyVehicle')
AddEventHandler('desirerp-jacksons_autos:BuyVehicle', function(plate, name, vehicle, price, personalvehicle)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
