...
RegisterServerEvent('desirerp-boosting:removeGNE')
AddEventHandler('desirerp-boosting:removeGNE', function(amount, cid)
    exports.oxmysql:execute('SELECT gne FROM boosting_users WHERE identifier = ?', {cid}, function(result)
...
RegisterServerEvent('desirerp-boosting:pay_gne')
AddEventHandler('desirerp-boosting:pay_gne', function()
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
    local gnes = GetGNE(cid)
