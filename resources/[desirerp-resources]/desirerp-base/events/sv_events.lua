...
RegisterServerEvent("desirerp-events:listenEvent")
AddEventHandler("desirerp-events:listenEvent", function(id, name, args)
    local src = source
    ...
    DPX.Events.Registered[name].f(DPX.Events.Registered[name].mod, args, src, function(data)
        TriggerClientEvent("desirerp-events:listenEvent", src, id, data)
    end)
end)
