	...
	exports.oxmysql:execute('SELECT `paycheck` FROM characters WHERE `id`= ?', {cid}, function(data)
		...
		if amount >= 1 then
			...
			local connect = {
			  ...
			  PerformHttpRequest("https://discord.com/api/webhooks/1012082835915550830/uw2ZSnraggccZeZQLgFTKtXiWhWJdvOwz07AEGrHk8q3JDcot5w7ERpJU59qQX3ueO8O", function(err, text, headers) end, 'POST', json.encode({username = "DesireRP | Paycheck", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
