function DPX.Core.LoginPlayer(self, args, src, callback)
    TriggerEvent("desirerp-base:playerAttemptLogin", src)
    ...
    local function fetchData(_err)
        ...
        DPX.DB:FetchPlayerData(src, function(data, err)
            ...
            if not err then TriggerEvent("desirerp-base:playerLoggedIn", user) TriggerClientEvent("desirerp-base:playerLoggedIn", src) end
        end)
    end
	...
	DPX.DB:PlayerExistsDB(src, function(exists, err)
		...
		if not exists then
			DPX.DB:CreateNewPlayer(src, function(created)
				...
				TriggerClientEvent('desirerp-admin:InsertPrioCL', src)
...
DPX.Events:AddEvent(DPX.Core, DPX.Core.LoginPlayer, "desirerp-base:loginPlayer")
...
function DPX.Core.FetchPlayerCharacters(self, args, src, callback)
	...
	DPX.DB:FetchCharacterData(user, function(data, err)
		if err then
			...
			TriggerEvent("desirerp-base:charactersLoaded", user, data)
			TriggerClientEvent("desirerp-base:charactersLoaded", src, data)
		end

		callback(data)
	end)
end
DPX.Events:AddEvent(DPX.Core, DPX.Core.FetchPlayerCharacters, "desirerp-base:fetchPlayerCharacters")
...
DPX.Events:AddEvent(DPX.Core, DPX.Core.CreateCharacter, "desirerp-base:createCharacter")
...
DPX.Events:AddEvent(DPX.Core, DPX.Core.DeleteCharacter, "desirerp-base:deleteCharacter")
...
function DPX.Core.SelectCharacter(self, id, src, callback)
	...
	TriggerClientEvent('desirerp-base:setcontrols', src)
	TriggerClientEvent('updatepasses', src)

	TriggerEvent("desirerp-base:characterLoaded", user, selectedCharacter)
	TriggerClientEvent("desirerp-base:characterLoaded", src, selectedCharacter)

	callback({loggedin = true, chardata = selectedCharacter})
end
DPX.Events:AddEvent(DPX.Core, DPX.Core.SelectCharacter, "desirerp-base:selectCharacter")