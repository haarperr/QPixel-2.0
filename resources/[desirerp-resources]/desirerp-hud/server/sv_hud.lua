...
AddEventHandler('carfill:checkmoney', function(costs)
	...
	if exports['desirerp-base']:getModule('GetCash')(src) >= costs then
		TriggerClientEvent("RefuelCarServerReturn", src)
		exports['desirerp-base']:getModule('RemoveCash')(src, costs)
	else
		moneyleft = costs - exports['desirerp-base']:getModule('GetCash')(src)
		TriggerClientEvent('DoLongHudText', src, "Requires $" .. costs)
	end
end)

RegisterServerEvent('desirerp-hud:UpdateStress_SV')
AddEventHandler('desirerp-hud:UpdateStress_SV',function(amount)
	local src = source
