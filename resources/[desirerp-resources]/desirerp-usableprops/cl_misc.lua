...
local function benchObjectHandler(pBenchType, pItem)
    ...
    local id = exports['desirerp-objects']:PlaceAndSaveObject(
...
local itemHandlers = {
    ...
    ['pailshovel'] = function(info)
        ...
        local finished = exports["desirerp-taskbar"]:taskBar(15000, "Building Sandcastle", false, true, false, false, nil, 5.0, ped)
        ClearPedTasks(ped)
    ...
    ['letterbox'] = function(info)
        ...
        local properties = exports['desirerp-housing']:getHousingData()
        ...
        local name
        if elements then
            local prompt2 = exports['desirerp-ui']:OpenInputMenu(elements)
            if not prompt2 then
                return false
            end
            ...
            if mailType == 'property' then
                local housingTable = exports['desirerp-housing']:retriveHousingTable()
            ...
            elseif mailType == 'business' then
                local zones = exports['desirerp-polyzone']:GetZones(plyCoords)
                if not zones then
        ...
        local results = exports['desirerp-objects']:PlaceObject(
            model,
            {
                forceGroundSnap = true,
                zOffset = -0.05,
        ...
        local objId = RPC.execute('desirerp-usableprops:placeLetterbox', model, results[2].coords, results[2].rotation, mailType, foundInfo, name)
        if objId then
...
AddEventHandler('desirerp-inventory:itemUsed', function(item, info)
...
local function getMailboxInfo(pEntity)
    local object = exports['desirerp-objects']:GetObjectByEntity(pEntity)
    if not object then
        return false
    end
...
local function isPD()
    local job = exports["desirerp-base"]:getModule("LocalPlayer"):getVar("job")
...
-- Citizen.CreateThread(function()
--     exports["desirerp-interact"]:AddPeekEntryByModel({ `prop_sign_road_01a` }, {{
...
--     exports["desirerp-interact"]:AddPeekEntryByModel({ 2108567945, -1940238623 }, {{
--         event = "desirerp-usableprops:payParkingMeter",
--         id = "desirerp-usableprops:parking_pay",
--         icon = "dollar-sign",
...
--     exports["desirerp-interact"]:AddPeekEntryByModel({ `prop_skid_tent_01`, `prop_skid_tent_03` }, {{
--         event = "desirerp-usableprops:pickupTent",
--         id = "desirerp-usableprops:pickup_tent",
--         icon = "arrow-up",
--         label = "pickup tent",
--         parameters = {},
--     }}, {
--         distance = { radius = 2.0 },
--         isEnabled = function(pEntity)
--           return exports['desirerp-objects']:GetObjectByEntity(pEntity) ~= nil
--         end,
--     })

--     exports["desirerp-interact"]:AddPeekEntryByModel({ `prop_bench_04`, `prop_ld_bench01`, `prop_snow_bench_01` }, {{
--         event = "desirerp-usableprops:pickupBench",
--         id = "desirerp-usableprops:pickup_bench",
--         icon = "arrow-up",
--         label = "pickup bench",
--         parameters = {},
--     }}, {
...
--     exports["desirerp-interact"]:AddPeekEntryByModel({ `prop_letterbox_01`, `prop_letterbox_02`, `prop_letterbox_03`, `prop_letterbox_04`, `prop_postbox_01a` }, {{
--         event = "desirerp-usableprops:openMail",
--         id = "desirerp-usableprops:open_mailbox",
--         icon = "envelope",
--         label = "deposit mail",
--         parameters = {},
--     }}, {
...
--     exports["desirerp-interact"]:AddPeekEntryByModel({ `prop_letterbox_01`, `prop_letterbox_02`, `prop_letterbox_03`, `prop_postbox_01a` }, {{
--         event = "desirerp-usableprops:checkMail",
--         id = "desirerp-usableprops:check_mailbox",
--         icon = "envelope-open",
--         label = "check mail",
--         parameters = {},
--     }}, {
...
--             return (mailType == 'business' and (exports['desirerp-business']:HasPermission(info, 'stash_access') or isPD())) or
--             (mailType == 'property' and exports['desirerp-housing']:hasPropertyAccess(info, true)) or
--             (mailType == 'mrpd' and isPD()) or
--             (mailType == 'pillbox' and (job == 'ems' or job == 'doctor'))
--         end,
--     })

--     exports["desirerp-interact"]:AddPeekEntryByModel({ `prop_letterbox_01`, `prop_letterbox_02`, `prop_letterbox_03` }, {{
--         event = "desirerp-usableprops:removeMailbox",
--         id = "desirerp-usableprops:remove_mailbox",
--         icon = "trash-alt",
...
--         isEnabled = function(pEntity)
--             local info, mailType = getMailboxInfo(pEntity)
--             return mailType == 'business' and (exports['desirerp-business']:HasPermission(info, 'stash_access')) or
--             (mailType == 'property' and exports['desirerp-housing']:hasPropertyAccess(info, false))
--         end,
--     })

-- end)

AddEventHandler("desirerp-usableprops:stealStopSign", function(pArgs, pEntity, pContext)
    ...
    local finished = exports["desirerp-taskbar"]:taskBar(15000, "Stealing", false, true, false, false, nil, 5.0, ped)
    ClearPedTasks(ped)
    if finished == 100 then
...
RegisterNetEvent("desirerp-usableprops:deleteEntity", DeleteEntityAtCoords)
...
AddEventHandler("desirerp-usableprops:payParkingMeter", function(args, entity, context)
    PaidMeters[entity] = true
    local success = RPC.execute('desirerp-usableprops:payParkingMeter', args.price)
    if success then
        local pos = GetEntityCoords(entity)
        TriggerServerEvent("desirerp-scenes:addScene", {
...
AddEventHandler("desirerp-usableprops:pickupTent", function(args, entity, context)
    local tent = exports['desirerp-objects']:GetObjectByEntity(entity)
    if tent then
        local success = exports['desirerp-objects']:DeleteObject(tent.id)
        if success then
...
AddEventHandler("desirerp-usableprops:pickupBench", function(args, entity, context)
    local bench = exports['desirerp-objects']:GetObjectByEntity(entity)
    if bench then
        local success = exports['desirerp-objects']:DeleteObject(bench.id)
        if success then
...
local closeData = {}
AddEventHandler("desirerp-usableprops:openMail", function(args, entity, context)
    local info, mailType, object = getMailboxInfo(entity)
    waitForClose = RPC.execute('desirerp-usableprops:openMailbox', info, mailType, object.id)
...
AddEventHandler("desirerp-usableprops:checkMail", function(args, entity, context)
    if isPD() and mailType == 'business' then
        local prompt = exports['desirerp-ui']:OpenInputMenu({
    ...
    local info, mailType, object = getMailboxInfo(entity)
    RPC.execute('desirerp-usableprops:checkMailbox', info, mailType, object.id)
end)

AddEventHandler('desirerp-inventory:closed', function(pName)
    if waitForClose and pName == 'mailtemp:' .. closeData.mailType .. ':' .. closeData.objId then
        RPC.execute('desirerp-usableprops:closeMailbox', closeData.info, closeData.mailType, closeData.objId)
        waitForClose = false
        closeData = {}
    end
end)

AddEventHandler('desirerp-usableprops:removeMailbox', function(args, entity, context)
    TriggerEvent("animation:PlayAnimation","mechanic3")
    local finished = exports['desirerp-taskbar']:taskBar(15000, "Removing mailbox", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local obj = exports['desirerp-objects']:GetObjectByEntity(entity)
        if not obj then
            return
        end
        exports["desirerp-objects"]:DeleteObject(obj.id)
    end
end)

AddEventHandler('desirerp-objects:objectCreated', function(pObject, handle)
    if pObject.data.metadata.mail then
        FreezeEntityPosition(handle, false)
    end
end)

AddEventHandler('desirerp-spawn:characterSpawned', function(pCharacterId)
    RPC.execute('desirerp-usableprops:checkMailAlerts', pCharacterId)
end)
