...
RegisterServerEvent("desirerp-vehicles:checkrepo")
AddEventHandler("desirerp-vehicles:checkrepo", function(plate)
    local src = source
...
-- RegisterServerEvent("desirerp-vehicles:repo")
-- AddEventHandler("desirerp-vehicles:repo", function(plate)
--     local src = source
-- 	local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
--                         ['@repoed'] = "1"
--                     })
--                     TriggerClientEvent("desirerp-vehicles:repo:success", src)
...
-- RegisterServerEvent("desirerp-vehicles:judgerepo")
-- AddEventHandler("desirerp-vehicles:judgerepo", function(plate)
--     local src = source
-- 	local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
-- RegisterServerEvent("desirerp-vehicles:judgerepodelete")
-- AddEventHandler("desirerp-vehicles:judgerepodelete", function(plate)
--     local src = source
-- 	local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
--     exports.oxmysql:execute("SELECT `license_plate`, `repoed` FROM `characters_cars` WHERE license_plate = ?", {char.id}, function(data)
...
-- RegisterServerEvent("desirerp-vehicles:repo2")
-- AddEventHandler("desirerp-vehicles:repo2", function(plate)
--     local src = source
-- 	local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
--                     ['@vehicle_state'] = "In"
--                 })
--                 TriggerClientEvent("desirerp-vehicles:repo:success2", src)
--             else
--                 TriggerClientEvent("desirerp-vehicles:repo:success2", src)
--             end
--         end
...
-- RegisterServerEvent("desirerp-vehicles:release:vehicle")
-- AddEventHandler("desirerp-vehicles:release:vehicle", function(ServerID, plate)
...
--         ['repoed'] = "0"
--     })
--     exports['desirerp-exports']:removeCash(SrcID, 500)
...
RegisterCommand('towbill', function(source, args)
	...
	exports.oxmysql:execute('SELECT * FROM jobs_whitelist WHERE cid = ?', {character2.id}, function(result)
		if result[1].job == 'towunion' then
			...
			TriggerClientEvent("desirerp-vehicles:towBill", src, tonumber(args[2]))
		else
		end
	end)
end)