...
local function explodeVehicle(pEntity)
  exports['desirerp-sync']:SyncedExecution('NetworkExplodeVehicle', pEntity, 1, 0, 0)
  TriggerServerEvent('desirerp-miscscripts:carbombs:removeBomb', NetworkGetNetworkIdFromEntity(pEntity))
end
...
local function checkForCarBomb(pEntity) 
  ...
  local progress = exports['desirerp-taskbar']:taskBar(25000, 'Searching for car bomb...', true)
  ClearPedTasks(PlayerPedId())

  if progress ~= 100 then return end

  local metaData = exports['desirerp-vehicles']:GetVehicleMetadata(pEntity)
  local carBombMeta = metaData.carBombData or false
  local remoteBomb = DPX.Procedures.execute('desirerp-usableprops:hasPhoneBomb', NetworkGetNetworkIdFromEntity(pEntity))

  if remoteBomb and remoteBomb.planted then
    local number = remoteBomb.bomb.number
    TriggerServerEvent('desirerp-usableprops:defusePhoneBomb', number)
    TriggerEvent('desirerp-police:client:showTextPopup', { show = true, text = 'Cell Phone Found, Number: ' .. number })
  ...
  if carBombMeta and carBombMeta.hasCarBomb then
    TriggerServerEvent('desirerp-miscscripts:carbombs:foundBomb', NetworkGetNetworkIdFromEntity(pEntity), carBombMeta)
...
RegisterCommand("test1", function()
  ...
  TriggerEvent("desirerp-bomb:test", 'car_bomb')

end, false)

AddEventHandler('desirerp-bomb:test', function (name, info)
  local items = {
    ['car_bomb'] = true,
    ['bombmirror'] = true,
  }
  ...
  local prompt = exports['desirerp-interface']:OpenInputMenu(elements)
  ...
  local progress = exports['desirerp-taskbar']:taskBar(25000, 'Planting car bomb...', true)
  ...
  local added = DPX.Procedures.execute('desirerp-miscscripts:carbombs:addCarBomb', netId, minSpeed, ticksBeforeExplode, ticksForRemoval, gridSize, coloredSquares, timeToComplete)
  TriggerEvent('inventory:removeItem', 'car_bomb', 1)
  
  return TriggerEvent('DoLongHudText', 'Successfully added car bomb to vehicle', 1)
end)

AddEventHandler('desirerp-miscscripts:carBombs:removeBomb', function (pData, pEntity)
  local metaData = exports['desirerp-vehicles']:GetVehicleMetadata(pEntity)
  local carBombMeta = metaData.carBombData or false
  ...
  if carBombMeta and carBombMeta.hasCarBomb then
    ...
    exports['desirerp-ui']:openApplication('memorygame', {
      gameFinishedEndpoint = 'desirerp-miscscripts:carBombs:completeHacking',
...
--[[ RegisterUICallback('desirerp-vehicles:carBombs:completeHacking', function(data, cb)
  ...
  TriggerServerEvent('desirerp-miscscripts:carbombs:removeBomb', netId)
  ...
  Citizen.Wait(2000)
  exports['desirerp-ui']:closeApplication('memorygame')
end) ]]
