...
CreateThread(function()
  ...
  while true do
    ...
    if RecentlyRiding then
      local BikeArmor = exports["desirerp-inventory"]:hasEnoughOfItem("bikearmor", 1, false,true)
      if BikeArmor then
...
function ProcessHitByCash(pAttacker)
  TriggerServerEvent("desirerp-weapons:attackedByCash", pAttacker)
end
...
AddEventHandler("DamageEvents:EntityDamaged", function(victim, attacker, pWeapon, isMelee)
  ...
  print(pWeapon)
  if ( (pWeapon == CrashHash or pWeapon == RamHash or pWeapon == FallHash) and exports["desirerp-inventory"]:hasEnoughOfItem("bikearmor", 1, false, true) and not DegApplied and RecentlyRiding) then
    DegApplied = true
