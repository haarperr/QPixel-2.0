...
AddEventHandler('bank:openbank', function()
  ...
  open = not open
  bankanimation()
  local finished = exports["desirerp-taskbar"]:taskBar(1500,"Opening Banking")
...
RegisterNUICallback('bankWithdraw', function(data, cb)
  ...
  if success then
    --print("success")
    TriggerServerEvent('desirerp-financials:withdraw_log', data.amount, data.comment)
    local accounts, cash, name = RPC.execute("GetAccounts")
    local transactions = RPC.execute("GetTransactionLogs", data.id)
    SendReactMessage('bankData', {
      type = banktype,
...
RegisterNUICallback('bankDeposit', function(data, cb)
  ...
  if success then
    --print("success")
    TriggerServerEvent('desirerp-financials:deposit_log', data.amount, data.comment)
    local accounts, cash, name = RPC.execute("GetAccounts")
    local transactions = RPC.execute("GetTransactionLogs", data.id)
    SendReactMessage('bankData', {
      type = banktype,
