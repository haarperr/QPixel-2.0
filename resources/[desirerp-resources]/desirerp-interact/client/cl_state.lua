...
function hasJobPermission(pJob, pPermission)
  ...
  local hasJobPermission = function()
    if pPermission == "employee" then
      ...
      return RPC.execute('desirerp-business:hasPermission', pJob, pPermission)
    end
  end
...
RegisterNetEvent("desirerp-interact:bustVehicleOwnerShipCache", function(pEntity)
...
function getVehicleOwner(pEntity)
  ...
  local vin = exports['desirerp-vehicles']:GetVehicleIdentifier(pEntity)
  local vehicleOwner = RPC.execute("desirerp-ottosauto:getVehicleOwner", vin)
...
AddEventHandler("desirerp-fishing:currentlyRentedBoat", function(handle)
