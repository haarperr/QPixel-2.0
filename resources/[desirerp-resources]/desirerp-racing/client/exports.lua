...
function createPendingRace(id, options)
    ...
    RPC.execute("desirerp-racing:createPendingRace", id, options)
end
...
function startRace(countdown)
    ...
    for k, v in pairs(pendingRaces) do
        ...
        if v.owner == characterId then
            ...
            RPC.execute("desirerp-racing:startRace", v.id, countdown or v.countdown, v.owner)
            return
        end
    end
end
...
function endRace()
    ...
    if curRace then
        RPC.execute("desirerp-racing:endRace")
        print("ENDING RACE 1")
    else
        RPC.execute("desirerp-racing:leaveRace")
    end
end

function joinRace(id, alias, characterId)
    RPC.execute("desirerp-racing:joinRace", id, alias, characterId)
end
...
function leaveRace()
    ...
    if curRace then
        RPC.execute("desirerp-racing:dnfRace", curRace.id)
        cleanupRace()
    else
        RPC.execute("desirerp-racing:leaveRace")
    end
...
function getAllRaces()
    ...
    local res = RPC.execute("desirerp-racing:getAllRaces")
    races = res.races
...
function getHasRaceUsbAndAlias()
    ...
    local pdRacingUsbItem = exports["desirerp-inventory"]:GetInfoForFirstItemOfName("racingusb3")
