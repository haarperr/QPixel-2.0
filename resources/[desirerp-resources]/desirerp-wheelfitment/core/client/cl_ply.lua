DecorRegister("desirerp-wheelfitment_applied", 2)
DecorRegister("desirerp-wheelfitment_w_width", 1)
DecorRegister("desirerp-wheelfitment_w_fl", 1)
DecorRegister("desirerp-wheelfitment_w_fr", 1)
DecorRegister("desirerp-wheelfitment_w_rl", 1)
DecorRegister("desirerp-wheelfitment_w_rr", 1)
--[[
cl_ply.lua
]] -- #[Local Variables]#--
...
function SyncWheelFitment()
    if isPlyWhitelisted then
        ...
        if didPlyAdjustFitments then
            if not DecorExistOn(plyVeh, "desirerp-wheelfitment_applied") then
                DecorSetBool(plyVeh, "desirerp-wheelfitment_applied", true)
            end

            DecorSetFloat(plyVeh, "desirerp-wheelfitment_w_width", roundNum(GetVehicleWheelWidth(plyVeh), 2))
            DecorSetFloat(plyVeh, "desirerp-wheelfitment_w_fl", roundNum(GetVehicleWheelXOffset(plyVeh, 0), 2))
            DecorSetFloat(plyVeh, "desirerp-wheelfitment_w_fr", roundNum(GetVehicleWheelXOffset(plyVeh, 1), 2))
            DecorSetFloat(plyVeh, "desirerp-wheelfitment_w_rl", roundNum(GetVehicleWheelXOffset(plyVeh, 2), 2))
            DecorSetFloat(plyVeh, "desirerp-wheelfitment_w_rr", roundNum(GetVehicleWheelXOffset(plyVeh, 3), 2))
            ...
            TriggerServerEvent("desirerp-wheelfitment_sv:saveWheelfitment", plate, fitment)
        ...
        RPC.execute("desirerp-wheelfitment_sv:setIsWheelFitmentInUse", false)
    end
end
...
function checkVehicleFitment()
    ...
    for _, veh in ipairs(vehicles) do
        ...
        if isNear(plyPos, GetEntityCoords(veh), 150) then
            if DecorExistOn(veh, "desirerp-wheelfitment_applied") then
                vehiclesToCheckFitment[#vehiclesToCheckFitment + 1] = {
                    vehicle = veh,
                    w_width = DecorGetFloat(veh, "desirerp-wheelfitment_w_width"),
                    w_fl = DecorGetFloat(veh, "desirerp-wheelfitment_w_fl"),
                    w_fr = DecorGetFloat(veh, "desirerp-wheelfitment_w_fr"),
                    w_rl = DecorGetFloat(veh, "desirerp-wheelfitment_w_rl"),
                    w_rr = DecorGetFloat(veh, "desirerp-wheelfitment_w_rr")
                }
            end
        end
    end
...
Citizen.CreateThread(function()
    isWheelFitmentInUse = RPC.execute("desirerp-wheelfitment_sv:getIsWheelFitmentInUse")
    isPlyWhitelisted = RPC.execute("desirerp-wheelfitment_sv:checkIfWhitelisted")

    exports["desirerp-polyzone"]:AddBoxZone("desirerp-wheelfitment:zone1", vector3(136.0, -3030.36, 6.52), 7.8, 4.2, {
        name="desirerp-wheelfitment:zone1",
        heading=1,
        --debugPoly=true
      })
end)
...
AddEventHandler("desirerp-polyzone:enter", function(zone, data)
    if zone ~= "desirerp-wheelfitment:zone1" then return end
    Citizen.CreateThread(function()
        local plyPed = PlayerPedId()
        inZone = true
        while inZone do
                ...
                if not isMenuShowing then
                    ...
                    if isNear(plyPos, cfg_wheelFitmentPos, 2.0) then
                        ...
                        if IsControlJustReleased(1, 38) then -- Key: E
                            ...
                            RPC.execute("desirerp-wheelfitment_sv:setIsWheelFitmentInUse", true)
...
RegisterNetEvent('desirerp-fitment:devaleks',function()
    devwheelMenu ()
end)


AddEventHandler("desirerp-polyzone:exit", function(zone)
    if zone ~= "desirerp-wheelfitment:zone1" then
        return
    end
    inZone = false
end)
...
-- #[Event Handlers]#--
RegisterNetEvent("desirerp-wheelfitment_cl:applySavedWheelFitment")
AddEventHandler("desirerp-wheelfitment_cl:applySavedWheelFitment", function(plyVeh, VehicleWheel)
    ...
    DecorSetFloat(plyVeh, "desirerp-wheelfitment_w_width", wheelFitments.width)
    DecorSetFloat(plyVeh, "desirerp-wheelfitment_w_fl", wheelFitments.fl)
    DecorSetFloat(plyVeh, "desirerp-wheelfitment_w_fr", wheelFitments.fr)
    DecorSetFloat(plyVeh, "desirerp-wheelfitment_w_rl", wheelFitments.rl)
    DecorSetFloat(plyVeh, "desirerp-wheelfitment_w_rr", wheelFitments.rr)

    performVehicleCheck = true
    checkVehicleFitment()
end)

RegisterNetEvent("desirerp-wheelfitment_cl:forceMenuClose")
AddEventHandler("desirerp-wheelfitment_cl:forceMenuClose", function()
    if isMenuShowing then
        local plyPed = PlayerPedId()
        local plyVeh = GetVehiclePedIsIn(plyPed, false)
...
RegisterCommand("leavefitment", function()
    TriggerEvent("desirerp-wheelfitment_cl:forceMenuClose")
end)