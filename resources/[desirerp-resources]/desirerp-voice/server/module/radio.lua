...
function removePlayerFromRadio(source, radioChannel)
	...
	for player, _ in pairs(radioData[radioChannel]) do
		TriggerClientEvent('desirerp-voice:removePlayerFromRadio', player, source)
	end
...
function addPlayerToRadio(source, radioChannel)
	...
	radioData[radioChannel] = radioData[radioChannel] or {}
	for player, _ in pairs(radioData[radioChannel]) do
		TriggerClientEvent('desirerp-voice:addPlayerToRadio', player, source)
	end
	voiceData[source] = voiceData[source] or defaultTable(source)

	voiceData[source].radio = radioChannel
	radioData[radioChannel][source] = false
	TriggerClientEvent('desirerp-voice:syncRadioData', source, radioData[radioChannel])
end
...
function setPlayerRadio(source, radioChannel)
	...
	if GetInvokingResource() then
		...
		-- changed
		TriggerClientEvent('desirerp-voice:clSetPlayerRadio', source, radioChannel)
	end
	voiceData[source] = voiceData[source] or defaultTable(source)
...
RegisterNetEvent('desirerp-voice:setPlayerRadio', function(radioChannel)
	...
	if radioTbl then
		...
		for player, _ in pairs(radioTbl) do
			if player ~= source then
				TriggerClientEvent('desirerp-voice:setTalkingOnRadio', player, source, talking)
