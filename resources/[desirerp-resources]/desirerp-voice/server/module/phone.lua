...
function removePlayerFromCall(source, callChannel)
    ...
    for player, _ in pairs(callData[callChannel]) do
        TriggerClientEvent('desirerp-voice:removePlayerFromCall', player, source)
    end
...
function addPlayerToCall(source, callChannel)
    ...
    for player, _ in pairs(callData[callChannel]) do
        ...
        if player ~= source then
            TriggerClientEvent('desirerp-voice:addPlayerToCall', player, source)
        end
    end
    callData[callChannel][source] = false
    voiceData[source] = voiceData[source] or defaultTable(source)
    voiceData[source].call = callChannel
    TriggerClientEvent('desirerp-voice:syncCallData', source, callData[callChannel])
end
...
function setPlayerCall(source, callChannel)
    ...
    if GetInvokingResource() then
        ...
        -- changed
        TriggerClientEvent('desirerp-voice:clSetPlayerCall', source, callChannel)
    end
    voiceData[source] = voiceData[source] or defaultTable(source)
...
RegisterNetEvent('desirerp-voice:setPlayerCall', function(callChannel)
    ...
    if callTbl then
        ...
        for player, _ in pairs(callTbl) do
            if player ~= source then
                ...
                TriggerClientEvent('desirerp-voice:setTalkingOnCall', player, source, talking)
            end
        end
    else
...
RegisterNetEvent('desirerp-voice:setTalkingOnCall', setTalkingOnCall)