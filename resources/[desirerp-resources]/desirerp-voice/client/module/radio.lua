...
RegisterNetEvent('desirerp-voice:syncRadioData', syncRadioData)
...
RegisterNetEvent('desirerp-voice:setTalkingOnRadio', setTalkingOnRadio)
...
function addPlayerToRadio(plySource)
	TriggerEvent('desirerp-voice:getRadioChannel')
...
RegisterNetEvent('desirerp-voice:addPlayerToRadio', addPlayerToRadio)
...
RegisterNetEvent('desirerp-voice:removePlayerFromRadio', removePlayerFromRadio)
...
function setRadioChannel(channel)
	...
	TriggerServerEvent('desirerp-voice:setPlayerRadio', channel)
	...
	if not exports['ragdoll']:GetDeathStatus() then 
		if not radioPressed and radioEnabled then
			if radioChannel > 0 then
				...
				playerTargets(radioData, NetworkIsPlayerTalking(PlayerId()) and callData or {})
				TriggerServerEvent('desirerp-voice:setTalkingOnRadio', true)
				radioPressed = true
				playMicClicks(true)
				...
				Citizen.CreateThread(function()
					TriggerEvent('desirerp-hud:transmitting', radioPressed)
					TriggerEvent("desirerp-voice:radioActive", true)
...
RegisterCommand('-radiotalk', function()
	...
	if radioChannel > 0 or radioEnabled and radioPressed then
		...
		playerTargets(NetworkIsPlayerTalking(PlayerId()) and callData or {})
		TriggerEvent("desirerp-voice:radioActive", false)
		TriggerEvent('desirerp-hud:transmitting', radioPressed)
		playMicClicks(false)
		if GetConvarInt('voice_enableRadioAnim', 1) == 1 then
			StopAnimTask(PlayerPedId(), "random@arrests", "generic_radio_enter", -4.0)
		end
		TriggerServerEvent('desirerp-voice:setTalkingOnRadio', false)
	end
end, false)
...
RegisterNetEvent('desirerp-voice:clSetPlayerRadio', syncRadio)

RegisterNetEvent("desirerp-voice:getRadioChannel")
AddEventHandler("desirerp-voice:getRadioChannel", function()
	TriggerServerEvent('desirerp-dispatch:GrabRadioChannel', radioChannel)
end)