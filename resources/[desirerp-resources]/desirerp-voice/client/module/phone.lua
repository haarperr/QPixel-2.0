	...
	Citizen.CreateThread(function()
		...
		while callChannel ~= 0 do
			...
			if NetworkIsPlayerTalking(PlayerId()) and not changed then
				...
				TriggerServerEvent('desirerp-voice:setTalkingOnCall', true)
			elseif changed and NetworkIsPlayerTalking(PlayerId()) ~= 1 then
				changed = false
				MumbleClearVoiceTargetPlayers(1)
				TriggerServerEvent('desirerp-voice:setTalkingOnCall', false)
			end
			Wait(0)
		end
	end)
...
RegisterNetEvent('desirerp-voice:syncCallData', function(callTable, channel)
	callData = callTable
...
RegisterNetEvent('desirerp-voice:setTalkingOnCall', function(tgt, enabled)
	if tgt ~= playerServerId then
...
RegisterNetEvent('desirerp-voice:addPlayerToCall', function(plySource)
	callData[plySource] = false
end)

RegisterNetEvent('desirerp-voice:removePlayerFromCall', function(plySource)
...
function setCallChannel(channel)
	...
	TriggerServerEvent('desirerp-voice:setPlayerCall', channel)
	callChannel = channel
...
RegisterNetEvent('desirerp-voice:clSetPlayerCall', function(_callChannel)
	if GetConvarInt('voice_enablePhones', 1) ~= 1 then return end
