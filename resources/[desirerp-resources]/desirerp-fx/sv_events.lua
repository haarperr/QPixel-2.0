...
RegisterServerEvent("desirerp-fx:setSteamId")
AddEventHandler("desirerp-fx:setSteamId", function()
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
    local steamId = user:getVar("steamid")
    TriggerClientEvent("desirerp-fix:setSteamIdClient", src, steamId)
end)

RegisterServerEvent("desirerp-fx:smoke:grenade")
AddEventHandler("desirerp-fx:smoke:grenade", function(pCoords)
...
RegisterNetEvent("desirerp-fx:chain:blingDiamonds", function(pCoords, pType, pSize, pStrength, pScale)
