...
$(document).ready(function() {
    $('.save-settings').click(() => {
        ...
        $.post(
            'https://desirerp-inventory/UpdateSettings',
            JSON.stringify({
    ...
    window.addEventListener('message', function(event) {
        ...
        if (item.response == 'openGui') {
            ...
            if (itemList[item.id]) {
                $.post(
                    'https://desirerp-inventory/GiveItem',
                    JSON.stringify([
                        item.id,
                        item.amount,
...
function invStack(
    ...
    $.post('https://desirerp-inventory/stack', JSON.stringify(arr));
}
...
function invMove(
    ...
    $.post('https://desirerp-inventory/move', JSON.stringify(arr));
}
...
function invSwap(targetSlot, targetInventory, originSlot, originInventory, itemid1, metainformation1, itemid2, metainformation2) {
    ...
    $.post('https://desirerp-inventory/swap', JSON.stringify(arr));
}

function removeCraftItems(itemid, moveAmount, CraftArrayID) {
    let arr = itemList[itemid].craft[CraftArrayID];
    let amount = moveAmount;
    $.post('https://desirerp-inventory/removeCraftItems', JSON.stringify([arr, amount]));
}
...
function BuildDrop(brokenSlots) {
    $.post(
        'https://desirerp-inventory/dropIncorrectItems',
        JSON.stringify({
            slots: brokenSlots,
        }),
    );
...
function UpdateSetWeights(secondaryName) {
    ...
    $.post(
        'https://desirerp-inventory/Weight',
        JSON.stringify({
...
function DragToggle(fromSlot, isUsing, mouseEvent) {
    ...
    if (fromSlot) {
        ...
        if (occupiedslot && !isDragging) {
            ...
            // $.post('https://desirerp-inventory/SlotInuse', JSON.stringify(parseInt(draggingid.replace(/\D/g, ''))));
...
function DropItem(slot, amountDropped) {
    ...
    // $.post('https://desirerp-inventory/dropitem', JSON.stringify({
...
function ErrorMove() {
    // $.post('https://desirerp-inventory/move:fail', JSON.stringify({}));
}

function SuccessMove() {
    // $.post('https://desirerp-inventory/move:success', JSON.stringify({}));
}
...
function CompileStacks(
    ...
    $.post(
        'https://desirerp-inventory/SlotJustUsed',
        JSON.stringify({
...
function SwapStacks(targetSlot, originSlot, inv1, inv2, itemid1, metainformation1, itemid2, metainformation2) {
    // $.post('https://desirerp-inventory/swapstack', JSON.stringify({
    //   slot1: slot1,
    //   slot2: slot2,
    //   inv1: inv1,
    //   inv2: inv2,
...
function InsertItem(targetSlot, originSlot, inv1, inv2, itemid1, iteminfo1, itemid2, iteminfo2) {
    ...
    $.post(
        'https://desirerp-inventory/insert-item',
        JSON.stringify({
            originInventory,
            targetInventory,
            originSlot,
...
function closeInv(pIsItemUsed = false) {
    ...
    $.post(
        'https://desirerp-inventory/ServerCloseInventory',
        JSON.stringify({
    ...
    $.post(
        'https://desirerp-inventory/Close',
        JSON.stringify({
...
function CheckCraftFail(itemid, moveAmount) {
    ...
    for (let ArrayX = 0; ArrayX < requirementsArray.length; ArrayX++) {
        ...
        if (!itemcheck && !weightcheck) {
            ...
            $.post('https://desirerp-inventory/craftProgression', JSON.stringify({
...
function useitem() {
    ...
    if (currentInventory == 1) {
        ...
        $.post('https://desirerp-inventory/invuse', JSON.stringify(arr));
