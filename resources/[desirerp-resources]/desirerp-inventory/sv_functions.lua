...
function getQuantity(pServerId, pItemId)
    local user, amount = exports["desirerp-base"]:getModule("Player"):GetUser(pServerId), 0
    ...
    if user then
        ...
        if checkQuality then
            decayDate = exports['desirerp-inventory']:GetDecayDate(pItemId)
...
function removeItem(pServerId, pItemId, pAmount, pShouldNotify)
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(pServerId)
