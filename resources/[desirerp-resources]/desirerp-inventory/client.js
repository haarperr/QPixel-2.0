...
on('desirerp-base:playerSpawned', async (broughtData) => {
...
//  $.post("http://desirerp-inventory/SlotJustUsed", JSON.stringify({target: targetSlot, origin: originSlot, itemid: itemidsent }));
let recentused = [];
...
on('inventory:open_hidden', (penis, vehicleFound) => {
    let vehId = exports['desirerp-vehicles'].GetVehicleIdentifier(vehicleFound)
...
on('inventory-open-request', async () => {
  ...
  //  const apartmentFloor = exports["desirerp-apartments"].getModule("func").getApartment()
    let targetid = 0;
    cid = exports.isPed.isPed("cid");
    ...
    const currentTarget = exports['desirerp-target'].GetCurrentEntity()
    ...
    if (isInVehicle) {
       ...
       // let serverCode = exports["desirerp-config"].GetServerCode();
...
on('desirerp-items:SetAmmo', (sentammoTable) => {
    if (sentammoTable) {
...
on('__cfx_nui:ServerCloseInventory', (data, cb) => {
    ...
    if (data.name != 'none') {
        ...
        emit('desirerp-inventory:closed', data.name);
    }
    cb({});
});
...
on('__cfx_nui:craftProgression', (data, cb) => {
    cb("ok");
    emit("desirerp-inventory:craftProgression", data);
    emitNet("desirerp-inventory:craftProgression", data);
});
...
on('__cfx_nui:move', (data, cb) => {
    if (global.exports['desirerp-taskbar'].pInInv()) {
        emit('DoLongHudText', 'Nice try exploiting mate.', 2)
        emitNet('desirerp-inventory:exploiterKick')
    } else {
...
function SetCustomNuiFocus(hasKeyboard, hasMouse) {
    ...
    //   if (HasNuiFocus === true) {
    //   	emit("desirerp-voice:focus:set", HasNuiFocus, hasKeyboard, hasMouse);
    //   } else {
    // 	  setTimeout(() => {if (HasNuiFocus !== true) emit("desirerp-voice:focus:set", false, false, false);}, 1000)
    //   }
}
...
exports('setPlayerWeight', (cid, weight) => {
    ...
    emitNet('desirerp-inventory:server:weightChange', cid, weight);
