...
RegisterNUICallback('btnTwatter', function()
    ...
    local twattermessages = RPC.execute("desirerp-phone:getTwatters")
    -- if not getTwat then
...
RegisterNUICallback('newTwatSubmit', function(data, cb)
    ...
    TriggerServerEvent('desirerp-phone:twatSendNotification', handle, data, data.time)
...
RegisterNUICallback("desirerp-phone:getTwatter", function(data, cb)
  cb({ data = currentTwats, meta = { ok = success, message = (not success and currentTwats or 'done') } })
end)



RegisterNetEvent('desirerp-phone:insertTwat')
AddEventHandler('desirerp-phone:insertTwat', function(name,msg,time,attach)
  table.insert(currentTwats, {
    handle = name,
    message = msg,
    attachment = attach,
