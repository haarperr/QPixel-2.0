...
RegisterNetEvent("desirerp-phone:addToCurrentBanks", function(data)
    local addedBank = {
    ...
    RPC.execute("desirerp-phone:addToCurrentBanks", addedBank)
end)
...
RegisterNUICallback('getCurrentBanks', function(data, cb)
    local banks = RPC.execute('desirerp-phone:getCurrentBanks')
...
RegisterNUICallback('onPlayerJoinBankBusters', function(data, cb)
    RPC.execute("desirerp-phone:addPlayerToGroup", tonumber(data.targetCID), data.groupID, tonumber(data.senderID))
end)
...
RegisterNUICallback('addBankbusterGroup', function(data, cb)
    RPC.execute("desirerp-phone:requestPlayerToGroup", tonumber(data.cid), data.groupID)
end)

RegisterNUICallback('createBankbusterGroup', function(data, cb)
    RPC.execute("desirerp-phone:createGroup")
end)
    ...
    for k,v in pairs(currentGroups) do
        for j,s in pairs(v.members) do
            if tonumber(s.cid) == tonumber(data.cid) and s.leader then
                RPC.execute("desirerp-phone:deleteGroup", data.groupID)
            elseif tonumber(s.cid) == tonumber(data.cid) then
                RPC.execute("desirerp-phone:removePlayerFromGroup", tonumber(data.cid), data.groupID)
            end
        end
    end
end)