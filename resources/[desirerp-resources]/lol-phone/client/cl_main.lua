...
RegisterNUICallback('sendThePM', function(data, cb)
  ...
  RPC.execute("desirerp-phone:sendThePM", thePM)
end)

RegisterCommand("fuckmeintheasdasvf", function()
  RPC.execute("desirerp-phone:sendThePM", 'f')
end)
...
function openGui(isBurner, cid)
  ...
  if hasPhone() then
    ...
    SendNUIMessage({
      ...
      hasDevice = device, 
      hasDecrypt = decrypt, 
      pNum = RPC.execute("desirerp-phone:getPhoneNumber"),
...
RegisterNetEvent("phone:sendThePM", function(name, msg)
  if exports["desirerp-inventory"]:hasEnoughOfItem("thepmdongle",1,false,true) then
...
RegisterNUICallback('jobcenter-setgps', function(data)	
  ...
  if data.item == "oxy" then	
    TriggerServerEvent("desirerp-deliveries:JobCenter")	
