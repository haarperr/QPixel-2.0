...
AddEventHandler('Tweet', function(handle, data, time)
  ...
  TriggerClientEvent('desirerp-phone:insertTwat', -1, handle,data.text,time,data.attachment)
  -- TriggerClientEvent('Client:UpdateTweet1', src, newtwat,allTweet)
 
end)

RegisterNetEvent('desirerp-phone:twatSendNotification')
AddEventHandler('desirerp-phone:twatSendNotification', function(handle, data, time)

  local newtwat = { ['handle'] = handle, ['message'] = data, ['time'] = time}
  TriggerClientEvent('desirerp-phone:TwatNotify', -1, newtwat)
end)

RegisterNetEvent('desirerp-phone:openGetTweets')
AddEventHandler('desirerp-phone:openGetTweets', function()
  local src = source
  exports.oxmysql:execute('SELECT * FROM (SELECT * FROM tweets ORDER BY `time` DESC LIMIT 50) sub ORDER BY time ASC', {}, function(tweets) -- Get most recent 100 tweets
...
RPC.register('desirerp-phone:getTwatters', function(source)
...
RPC.register("desirerp-phone:twitterhandle", function(pSource)
  local src = source
  local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
