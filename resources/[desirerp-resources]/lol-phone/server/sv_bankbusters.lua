...
RPC.register("desirerp-phone:addToCurrentBanks", function(pSource, pBank)
...
RPC.register("desirerp-phone:getCurrentBanks", function(pSource)
    return currentBanks
end)

RPC.register("desirerp-phone:requestPlayerToGroup", function(pSource, pCid, pGroupID)
...
RPC.register("desirerp-phone:addPlayerToGroup", function(pSource, pCid, pGroupID, pSenderID)
...
RPC.register('desirerp-phone:removePlayerFromGroup', function(pSource, pCid, pGroupID)
...
RPC.register("desirerp-phone:createGroup", function(pSource)
...
RPC.register('desirerp-phone:deleteGroup', function(pSource, pGroupID)
    local jsonDecodedGroups = json.decode(groupTable)
    table.remove(jsonDecodedGroups, pGroupID)
    groupTable = json.encode(jsonDecodedGroups)
end)

RPC.register('desirerp-phone:getMyGroup', function(pSource, pCid)
    local jsonDecodedGroups = json.decode(groupTable)
    for k,v in pairs(jsonDecodedGroups) do
        local jsonDecodedGroupMembers = json.decode(v.members)
        for j,s in pairs(jsonDecodedGroupMembers) do
