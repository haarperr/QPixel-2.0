		...
		Citizen.Wait(1)
			if(Vdist(methlocation["x"],methlocation["y"],methlocation["z"], pos.x, pos.y, pos.z) < 10.0)then
				...
				if(Vdist(methlocation["x"],methlocation["y"],methlocation["z"], pos.x, pos.y, pos.z) < 5.0)then
					...
					local finished = exports["desirerp-taskbar"]:taskBar(60000,"Searching...")
...
RegisterNetEvent('desirerp-shops:exchangeGold')
AddEventHandler('desirerp-shops:exchangeGold', function()
	if exports["desirerp-inventory"]:hasEnoughOfItem("goldbar",70,true) then
		local finished = exports["desirerp-taskbar"]:taskBar(120000,"Trading Bars")
		if finished == 100 then
			if exports["desirerp-inventory"]:hasEnoughOfItem("goldbar",70,true) then
				TriggerServerEvent('desirerp-banking:addMoney', 35000)
...
AddEventHandler('slushy:general', function()
	...
	local finished = exports ["desirerp-ui"]:taskBarSkill(18000,math.random(10,20))
	if (finished == 100) then
		ExecuteCommand("e handshake")
		local finished = exports ["desirerp-ui"]:taskBarSkill(18000,math.random(10,20))
		if (finished == 100) then
...
AddEventHandler('lockpickshit:general', function()
	local finished = exports["desirerp-taskbar"]:taskBar(60000,"What dis????")
	if (finished == 100) then
...
AddEventHandler("illegal:crafting", function()
	...
	local finished = exports['desirerp-taskbar']:taskBar(12000, 'Inspecting Table')
	if (finished == 100) then
...
-- 	local finished = exports['desirerp-taskbar']:taskBar(20000, 'Smelting')
-- 	if (finished == 100) then
...
RegisterNetEvent("desirerp-shops:redCounter")
AddEventHandler("desirerp-shops:redCounter", function()
    TriggerEvent("server-inventory-open", "1", "redcircle_counter");
end)

RegisterNetEvent("desirerp-shops:redCounter2")
AddEventHandler("desirerp-shops:redCounter2", function()
...
AddEventHandler('weapon:general', function()
	TriggerServerEvent('desirerp-shops:WeaponShopStatus')
end)
...
AddEventHandler("hunting:level1", function()
	if exports["desirerp-inventory"]:getQuantity("coyotepelt") >= 1 then
		playerAnim()
		local finished = exports["desirerp-taskbar"]:taskBar(4000,"Selling Coyotepelt",true,false,playerVeh)
		if finished == 100 then
			if exports["desirerp-inventory"]:getQuantity("coyotepelt") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'coyotepelt', 1)
				TriggerServerEvent('desirerp-banking:addMoney', math.random(80, 150))
			else
...
AddEventHandler("hunting:level2", function()
	if exports["desirerp-inventory"]:getQuantity("buck") >= 1 then
		playerAnim()
		local finished = exports["desirerp-taskbar"]:taskBar(4000,"Selling Buck",true,false,playerVeh)
		if finished == 100 then
			if exports["desirerp-inventory"]:getQuantity("buck") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'buck', 1)
				TriggerServerEvent('desirerp-banking:addMoney', math.random(160, 200))
			else
...
AddEventHandler("hunting:level3", function()
	if exports["desirerp-inventory"]:getQuantity("boar") >= 1 then
		playerAnim()
		local finished = exports["desirerp-taskbar"]:taskBar(4000,"Selling Boar",true,false,playerVeh)
		if finished == 100 then
			if exports["desirerp-inventory"]:getQuantity("boar") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'boar', 1)
				TriggerServerEvent('desirerp-banking:addMoney', math.random(350, 500))
			else
...
--     exports['desirerp-menuinteract']:AddButton("Sell Coyotepelt", "Wow you suck", "hunting:level1", 'example button 1', "jobmenu")
--     exports['desirerp-menuinteract']:AddButton("Sell Buck", "Wow you kinda okay", "hunting:level2", 'example button 2', "jobmenu")
--     exports['desirerp-menuinteract']:AddButton("Sell Boar", "Wow you good", "hunting:level3", 'example button 3', "jobmenu")
--     exports['desirerp-menuinteract']:SubMenu("Hunting Menu" , "Click to select sell!" , "jobmenu")
-- end
