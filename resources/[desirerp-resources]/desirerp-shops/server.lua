...
AddEventHandler("itemMoneyCheck", function(itemType,askingPrice,location)
	...
	if (user:getCash() >= askingPrice) then
		...
		if askingPrice > 0 then
			TriggerClientEvent('desirerp-notification:client:Alert', src, {style = 'info', duration = 3000, message = "Purchased"})
		end
		TriggerClientEvent("player:receiveItem",src,itemType,1)

	else
		TriggerClientEvent('desirerp-notification:client:Alert', src, {style = 'error', duration = 3000, message = "Not enough money!"})
	end
end)
...
AddEventHandler("shop:useVM:server", function(locatoion)
	...
	if (tonumber(pCash) >= 20) then
		...
		TriggerClientEvent('desirerp-notification:client:Alert', src, {style = 'error', duration = 3000, message = "You need 20$"})
	end
end)
...
AddEventHandler("take100", function(tgtsent)
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(tgtsent)
    local char = user:getCurrentCharacter()
...
RegisterServerEvent("desirerp-shops:WeaponShopStatus")
AddEventHandler("desirerp-shops:WeaponShopStatus", function()
	local src = source
	local user = exports["desirerp-base"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
