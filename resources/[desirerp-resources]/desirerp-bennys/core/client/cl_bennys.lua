...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("desirerp-bennys:attemptPurchase",cheap, type, upgradeLevel)
...
function RepairVehicle()
    ...
    ExitBennys()
    local finished = exports["desirerp-taskbar"]:taskBar(10000 + (missingBodyHealth / 50), "Repairing vehicle...", false, true)
...
function ExitBennys()
    ...
    TriggerServerEvent("desirerp-bennys:removeFromInUse", currentBennys)
    if currentBennys:find('^devbennys') then
        --
    else
        exports["desirerp-interface"]:showInteraction("Bennys")
    end
    currentBennys = nil
end
...
--#[Event Handlers]#--
RegisterNetEvent("desirerp-bennys:purchaseSuccessful")
AddEventHandler("desirerp-bennys:purchaseSuccessful", function()
    isPurchaseSuccessful = true
    attemptingPurchase = false
end)

RegisterNetEvent("desirerp-bennys:purchaseFailed")
AddEventHandler("desirerp-bennys:purchaseFailed", function()
...
AddEventHandler("bennys:enter", function()
    ...
    --[[local isBennysInUse = RPC.execute("desirerp-bennys:checkIfUsed", currentBennys)
    ...
    ::hasAccess::
    TriggerServerEvent("desirerp-bennys:addToInUse", currentBennys)
    print(currentBennys)
    ...
    elseif currentBennys == "bennyshayes" then
        if exports['desirerp-business']:IsEmployedAt('hayes_autos') then
            enterLocation(bennys, repair)
        end
    ...
    elseif currentBennys == "bennysharmony" then
        if exports['desirerp-business']:IsEmployedAt('harmony_repairs') then
            enterLocation(bennys, repair)
        end
    elseif currentBennys == "ottosbay1" then
        if exports['desirerp-business']:IsEmployedAt('ottos_autos') then
            enterLocation(bennys, repair)
        end
...
RegisterNetEvent("desirerp-bennys:repairVeh")
AddEventHandler('desirerp-bennys:repairVeh', function()
    ...
    if car ~= 0 then
        ...
        local finished = exports["desirerp-taskbar"]:taskBar(4000,"Repairing Engine",false,false,playerVeh)
        Citizen.Wait(1000)
        local finished = exports["desirerp-taskbar"]:taskBar(3000,"Repairing Body",false,false,playerVeh)
        if (finished == 100) then
...
RegisterNetEvent('desirerp-public-bennys')
AddEventHandler('desirerp-public-bennys', function()
    TriggerServerEvent('desirerp-bennys:payPublicBennys')
end)

Citizen.CreateThread(function()
    exports["desirerp-polyzone"]:AddBoxZone("pubbennys", vector3(-33.2, -1053.37, 28.4), 8.0, 9.4, {
        heading=340,
        minZ=25.72,
        maxZ=29.52
    })
