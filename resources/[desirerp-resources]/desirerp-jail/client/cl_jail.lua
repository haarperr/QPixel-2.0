...
function drink()
    CledesirerpedSecondaryTask(PlayerPedId())
...
function RoleplayStats()
    ...
    if exports["desirerp-inventory"]:hasEnoughOfItem("shitlockpick",1,false) then
        totalroleplay = totalroleplay + 10
    end

    if exports["desirerp-inventory"]:hasEnoughOfItem("jailfood",1,false) then
        totalroleplay = totalroleplay + 10
    end

    if exports["desirerp-inventory"]:hasEnoughOfItem("methbag",1,false) then
        totalroleplay = totalroleplay + 15
    end

    if exports["desirerp-inventory"]:hasEnoughOfItem("assphone",1,false) then
        totalroleplay = totalroleplay + 15
    ...
    if exports["desirerp-inventory"]:hasEnoughOfItem("slushy",1,false) then
        totalroleplay = totalroleplay + 10
    end
...
function InmateHasAll()
    if exports["desirerp-inventory"]:hasEnoughOfItem("slushy",1,false)
        and
        exports["desirerp-inventory"]:hasEnoughOfItem("-1810795771",1,false)
        and
        exports["desirerp-inventory"]:hasEnoughOfItem("methbag",1,false)
        and
        exports["desirerp-inventory"]:hasEnoughOfItem("assphone",1,false)
        and
        exports["desirerp-inventory"]:hasEnoughOfItem("slushy",1,false)
    then
        return true
    else
        return false
...
RegisterNetEvent('desirerp-jail:checkTime')
AddEventHandler('desirerp-jail:checkTime', function()
...
RegisterNetEvent('desirerp-jailmenu')
AddEventHandler('desirerp-jailmenu', function()

	local Jailmenu = {
		{
            title = "List Lawyers",
            description = "Check how many lawyers are in the city!",
            action = "desirerp-jail:lawyerspog",
        },
        {
            title = "Jail Time",
            description = "Check your jail time",
            action = "desirerp-jail:checktime2",
        },
        {
            ...
            children = {
				{
                    title = "Yes",
                    action = "desirerp-jail:swapchar",
                },
				{
                    title = "No",
                    action = "desirerp-jail:swap_char_no",
                },
            },
        },
    }
    exports["desirerp-interface"]:showContextMenu(Jailmenu)
end)

RegisterInterfaceCallback('desirerp-jail:swap_char_no', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
end)

RegisterInterfaceCallback('desirerp-jail:swapchar', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('swappingCharsLoop')
end)

RegisterInterfaceCallback('desirerp-jail:checktime2', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('desirerp-jail:checkTime')
end)

RegisterInterfaceCallback('desirerp-jail:lawyerspog', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('yellowPages:retrieveLawyersOnline')
end)

exports["desirerp-polytarget"]:AddCircleZone("jail_interactions", vector3(1828.76, 2579.78, 46.51), 0.3, {
    useZ = true
})

exports["desirerp-interact"]:AddPeekEntryByPolyTarget("jail_interactions", {
    {
        event = "desirerp-jailmenu",
        id = "jail_interact_3",
        icon = "circle",
        label = "Jail Services",
        parameters = {},
