Citizen.CreateThread(function()
    ...
    for idx,polyTarget in ipairs(ChairZones) do
        exports['desirerp-interact']:AddPeekEntryByPolyTarget('desirerp-polychair:chair_' .. tostring(idx) .. "-".. tostring(polyTarget[4].name), {{
            event = "desirerp-polychair:chairSit",
...
function chairSit(chairPosition, cancel)
    ...
    if isSitting then
        ...
        isSitting = false
        TriggerEvent('desirerp-polychair:stopSitting')
    else
        ...
        TriggerEvent('desirerp-polychair:startSitting')
    end
    exports["desirerp-flags"]:SetPedFlag(ped, 'isSittingOnChair', isSitting)
end

AddEventHandler('desirerp-polychair:chairSit', function(pParameters, pEntity, pContext)
    chairSit(pParameters.chairPosition, true)
end)

RegisterNetEvent("desirerp-emotes:sitOnChair")
AddEventHandler("desirerp-emotes:sitOnChair", function(pArgs, pEntity, pContext)
    chairSit(nil, true)
end)
