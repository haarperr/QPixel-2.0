...
RegisterInterfaceCallback('desirerp-fuel:pay_bank', function(data, cb, pParams)
	cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterNetEvent('desirerp-fuel:OfferAccepted', function()
	...
	if not paid then
		paid = true
		canPayBill = RPC.execute('desirerp-fuel:PurchaseSuccessful', FuelCost)
	end
...
RegisterInterfaceCallback('desirerp-fuel:RefuelVehicle', function(data, cb)
	...
	-- exports['desirerp-taskbar']:Progress({
	-- 	duration = timer,
	-- 	label = "Refueling",
	-- }, function(cancelled)
