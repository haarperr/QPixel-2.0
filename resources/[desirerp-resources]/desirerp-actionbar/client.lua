...
function actionBarDown()
  ...
  exports["desirerp-ui"]:sendAppEvent("hud", {
    displayAllForce = true,
    displayAllForceVehicle = GetVehiclePedIsIn(PlayerPedId()) ~= 0,
	displayRadioChannel = true,
  })
  TriggerServerEvent("desirerp-financials:cash:get", GetPlayerServerId(PlayerId()))
end
...
Citizen.CreateThread(function()
	exports["desirerp-keybinds"]:registerKeyMapping("", "Player", "Action Bar", "+actionBar", "-actionBar", "TAB")
	RegisterCommand('+actionBar', actionBarDown, false)
	RegisterCommand('-actionBar', actionBarUp, false)

	exports["desirerp-keybinds"]:registerKeyMapping("", "Player", "Wink", "+playerWink", "-playerWink")
	RegisterCommand('+playerWink', playerWink, false)
	RegisterCommand('-playerWink', function() end, false)

	exports["desirerp-keybinds"]:registerKeyMapping("", "Player", "Passive Mode", "+passiveMode", "-passiveMode")
...
AddEventHandler('desirerp-actionbar:hotreload', function()
...
Citizen.CreateThread( function()
	...
	while true do
      ...
      if weapon == "-37975472" then
        TriggerEvent("desirerp-weapons:threwSmokeGrenade")
        if lastEquippedItemToRemove then
          if exports["desirerp-inventory"]:hasEnoughOfItem(lastEquippedItemToRemove,1,false) then
        ...
        lastEquippedItemToRemove = nil
			...
			if throwableWeapons[weapon] and exports["desirerp-inventory"]:hasEnoughOfItem(weapon,1,false) then
		...
		if IsPedPlantingBomb(ped) then
			if exports["desirerp-inventory"]:hasEnoughOfItem("741814745",1,false) then
...
AddEventHandler("desirerp-voice:focus:set", function(pState)
  focusTaken = pState
end)
...
AddEventHandler('equipWeaponID', function(hash,newInformation,sqlID,itemToRemove)
	--GiveAmmoNow()
	if not exports["desirerp-propattach"]:canPullWeaponHoldingEntity() then return end
  ...
  lastEquippedItemToRemove = itemToRemove
	...
	if armed then
		...
		holster1h()		
		TriggerEvent('desirerp-hud:armed', 0)
	else
		armed = true
	...
	TriggerEvent("desirerp-weapons:client:weaponEquiped", hash, newInformation, sqlID, itemToRemove, armed)
	TriggerServerEvent("desirerp-weapons:weaponEquiped", hash, newInformation, sqlID, itemToRemove, armed)
...
function updateAmmoTable(newammo,ammoType)
	...
	TriggerServerEvent("desirerp-weapons:updateAmmo",newammo,ammoType,ammoTable)
end
...
RegisterNetEvent('desirerp-item:CheckClientAmmo')
AddEventHandler('desirerp-item:CheckClientAmmo', function(weapons)
	local ped = PlayerPedId()
...
function AttachmentCheck(weaponhash)
	...
	if exports["desirerp-inventory"]:hasEnoughOfItem("weapon_silencer_assault", 1, false, true) then
	...
	if exports["desirerp-inventory"]:hasEnoughOfItem("weapon_silencer_pistol", 1, false, true) then
	...
	if exports["desirerp-inventory"]:hasEnoughOfItem("weapon_oil_silencer", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 1532150734 )
	end

	if exports["desirerp-inventory"]:hasEnoughOfItem("weapon_scope", 1, false, true) then
	...
	if exports["desirerp-inventory"]:hasEnoughOfItem("weapon_uzi_extended", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0xE40F1CD2 )	
	end
	if exports["desirerp-inventory"]:hasEnoughOfItem("weapon_uzi_foldstock", 1, false, true) then
		GiveWeaponComponentToPed( PlayerPedId(), weaponhash, 0x8FBBD54C )		
	end
	if exports["desirerp-inventory"]:hasEnoughOfItem("weapon_uzi_woodstock", 1, false, true) then
...
RegisterNetEvent("desirerp-weapons:hitPlayerWithCash", function(pTarget)
  TriggerServerEvent("desirerp-weapons:processGiveCashAmount", pTarget, lastEquippedInfo.amount, lastEquippedInfo.id)
end)
