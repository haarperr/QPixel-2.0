...
function DPX.Jobs.CanBecomeJob(self, user, job, callback)
    ...
    if not hexId or not characterId or not src then callback(false, "Id's don't exist") return end
        ...
        TriggerEvent("desirerp-jobmanager:attemptBecomeJob", src, characterId, function(allowed, reason)
...
function DPX.Jobs.SetJob(self, user, job, notify, callback)
    ...
    DPX.Jobs:CanBecomeJob(user, job, function(allowed, reason, rank) 
        ...
        TriggerClientEvent("desirerp-jobmanager:playerBecameJob", src, job, name, false)
        TriggerClientEvent("desirerp-jobmanager:playerBecomeEvent", src, job, name, notify)
...
AddEventHandler("desirerp-base:exportsReady", function()
    exports["desirerp-base"]:addModule("JobManager", DPX.Jobs)
end)
                ...
                for src,data in pairs(tbl) do
                    ...
                    if user then
                        ...
                        if tonumber(curTime) == tonumber(data.lastPayCheck) or tonumber(data.lastPayCheck) >= 480 then
                            ...
                            --exports["desirerp-log"]:AddLog("Job Pay", user, "User recieved paycheck, amount: " .. tostring(payCheck))
                        else
...
RPC.register("desirerp-jobmanager:jobCount", function(pSource, pJob)
