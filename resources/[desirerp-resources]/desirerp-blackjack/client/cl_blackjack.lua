...
function dwStatusHud(pValues)
  if dwRefreshChipCount then
    ...
    dwCurrentChipCount = RPC.execute("desirerp-casino:getCurrentChipCount")
  end
...
Citizen.CreateThread(function()
  --Wait(10000)
  dealerPedSeeds = RPC.execute("desirerp-blackjack:getDealerPedSeeds")
    ...
    while true do 
        if not sittingAtBlackjackTable then
            if closestChair ~= nil and closestChairDist < 1.6 then
                if not timeoutHowToBlackjack then
                    if not uiIntShown then
                        ...
                        if exports['desirerp-inventory']:hasEnoughOfItem("casinomember", 1, false, true) and not isHighRoller then
                            background = "success"
                            canPlayAtCurrentTable = true
                        elseif exports['desirerp-inventory']:hasEnoughOfItem("casinovip", 1, false, true) then
                            background = "success"
                            canPlayAtCurrentTable = true
                        else
                            background = "error"
                        ...
                        exports["desirerp-interface"]:showInteraction(
            ...
            elseif uiIntShown then
                uiIntShown = false
                local background = "info"
                if exports['desirerp-inventory']:hasEnoughOfItem("casinomember", 1, false, true) or exports['desirerp-inventory']:hasEnoughOfItem("casinovip", 1, false, true) then
                    background = "success"
                else
                    background = "error"
                end
                exports["desirerp-interface"]:hideInteraction(background)
            end
        elseif uiIntShown then
            uiIntShown = false
            local background = "info"
...
RegisterInterfaceCallback("desirerp-interface:blackjackHitStand", function(data, cb)
  ...
  elseif action == "double" then
    local dwUseChipsResult = RPC.execute("desirerp-casino:useChips", currentBetAmount)
  ...
  elseif action == "surrender" then
    ...
    -- RPC.execute("desirerp-casino:winChips", currentBetAmount / 2)
...
Citizen.CreateThread(function()
    while true do
        ...
        --         data[#data + 1] = {
        --           action = "desirerp-interface:blackjackHitStand",
...
Citizen.CreateThread(function()
    while true do
        if drawTimerBar and not timerBarHasBeenDrawn then
            ...
            exports["desirerp-interface"]:sendAppEvent("hud", {
...
RegisterInterfaceCallback("desirerp-interface:blackJackBet", function(data, cb)
...
AddEventHandler("desirerp-interface:application-closed", function(name, data)
...
function dwWaitForBet(act)
  ...
  if waitingForBetState then
    ...
    if sittingAtBlackjackTable then
      ...
      local data = {}
      data[#data + 1] = {
        action = "desirerp-interface:blackJackBet",
        title = "Place bet",
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    -- RPC.execute("desirerp-casino:winChips", currentBetAmount)
    dwRefreshChipCount = true
    if closeToCasino then
        blackjackGameInProgress = false
        dealerPed = getDealerFromTableId(tableId)
...
AddEventHandler("Blackjack:blackjackWin",function(tableId, dwHitBlackjack)
    ...
    -- RPC.execute("desirerp-casino:winChips", (currentBetAmount * 2) + extra)
    dwRefreshChipCount = true
    if closeToCasino then
        blackjackGameInProgress = false
        dealerPed = getDealerFromTableId(tableId)
