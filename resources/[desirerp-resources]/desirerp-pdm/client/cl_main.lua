...
function updateCarTable(model,price,name)
	for i=1, #carTable do
		...
		TriggerServerEvent("desirerp-pdm:CarTablePDM",carTable)
	end
end

local myspawnedvehs = {}

RegisterNetEvent("desirerp-pdm:testdrive")
AddEventHandler("desirerp-pdm:testdrive", function()
...
RegisterNetEvent("desirerp-pdm:EnableBuy")
AddEventHandler("desirerp-pdm:EnableBuy", function()
	if #(vector3(-30.31887, -1089.345, 26.04948) - GetEntityCoords(PlayerPedId())) > 50.0 and exports["desirerp-business"]:IsEmployedAt("pdm") then
		return
	end
	...
	TriggerServerEvent("desirerp-pdm:BuyEnabledSV",vehplate)
end)

RegisterNetEvent("desirerp-pdm:BuyEnabledCL")
AddEventHandler("desirerp-pdm:BuyEnabledCL", function(addplate)
...
RegisterNetEvent("desirerp-pdm:ReturnPDMTTable")
AddEventHandler("desirerp-pdm:ReturnPDMTTable", function(newTable)
	carTable = newTable
	DespawnSaleVehicles()
	SpawnSaleVehicles()
end)
...
function AttemptBuy(tableid)
	...
	TriggerServerEvent('desirerp-pdm:ChechMoney',name, model, price)
...
function SpawnSaleVehicles()
	if not hasspawned then
		TriggerServerEvent("desirerp-pdm:RequestPDMTTable")
		Citizen.Wait(1500)
	end
...
function CloseCreator(name, veh, price, plate)
	Citizen.CreateThread(function()
		...
		if not boughtcar then
			...
			TriggerServerEvent('desirerp-pdm:BuyVehicle', plate, vehname, vehicle, price, VehicleProps)
			TriggerServerEvent('desirerp-cardealer:log', exports['isPed']:isPed('cid'), vehname, price, plate, 'PDM')
			TriggerServerEvent('desirerp-financials:business_money', price / 10, 'pdm', 'add')
			exports['desirerp-interface']:hideInteraction()
			DespawnSaleVehicles()
			SpawnSaleVehicles()
		end
...
RegisterNetEvent("desirerp-pdm:FailedPurchase")
AddEventHandler("desirerp-pdm:FailedPurchase", function()
...
function ButtonSelected(button)
	...
	elseif this == "jobvehicles" or this == "a-class" or this == "b-class" or this == "s-class" or this == "pdm-tuners" or this == "c-class" or this == "casino" or this == "d-class" or this == "offroad" or this == "suvs" or this == "vans" or this == "industrial" or this == "cycles" or this == "motorcycles" then
		if ownerMenu then
			...
			TriggerServerEvent('desirerp-pdm:ChechMoney',button.name, button.model, button.costs)
		end
	end
end
	...
	while true do
		...
		if vehshop.opened then
			...
			for i,button in pairs(menu.buttons) do
				...
				if exports["desirerp-business"]:IsEmployedAt("pdm") and i >= vehshop.menu.from and i <= vehshop.menu.to then
