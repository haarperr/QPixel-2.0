RegisterNetEvent('desirerp-phone:transferWenmo', function(num,amount,comm)
    local src = source
    local zcid = getIdentifierByPhoneNumber(num)
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(tonumber(src))
...
RPC.register('desirerp-phone:getWenmo', function(source)
    local src = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(tonumber(src))
