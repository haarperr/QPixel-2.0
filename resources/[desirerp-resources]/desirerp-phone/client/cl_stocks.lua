...
RegisterNUICallback('buying-darkMarket', function()
  SendNUIMessage({
    openSection = "buydarkMarket", materials = exports['desirerp-phone']:getBuying(), blueprintid = RPC.execute("desirerp-phone:blueprintid")
  })
end)
...
RegisterNUICallback('stocksTradeToPlayer', function(data, cb)
  ...
  if data.identifier == "Shungite" then
      status = RPC.execute("desirerp-phone:buyCrypto",1,data.amount)
    if status then
  ...
  elseif data.identifier == "Guinea" then
    status = RPC.execute("desirerp-phone:buyCrypto",2,data.amount)
    if status then
  ...
  elseif data.identifier == "DVD Prime" then
    status = RPC.execute("desirerp-phone:buyCrypto",3,data.amount)
    if status then
...
RegisterNUICallback('exchangeCrypto', function(e)
  ...
  if playerid == characterId then
    RPC.execute("desirerp-phone:exchangeCrypto",popo,amount)
  else
    RPC.execute("desirerp-phone:transferCrypto",popo,amount,playerid)
  end
...
RegisterNUICallback('buyCrypto', function(e)
  ...
  elseif e.stockID == "DVD Prime" then
    ...
    RPC.execute("desirerp-phone:buyCrypto",popo,amount)
    sendStocksToPhone()
end)
...
function sendStocksToPhone(isRefresh)
  ...
  if isRefresh then
    ...
    serverstockvalues = RPC.execute("desirerp-phone:incomingCrypto")
  else
    serverstockvalues = RPC.execute("desirerp-phone:incomingCrypto")
  end

  Shungitelol = RPC.execute("desirerp-phone:GetShungite")
  Tgblol = RPC.execute("desirerp-phone:GetTgb")
  Dvdlol = RPC.execute("desirerp-phone:GetDvd")
...
RegisterNUICallback('purchase-darkMarket', function(data)
    ...
    local colorLaptop = exports['desirerp-phone']:getBuying()
    local time = math.random(60000,120000)

    if not orderItem then
      if data.item == "vpn" then
        result = RPC.execute('desirerp-phone:purchaseDarkMarket',1,20)
        print(result)
          if result then
            itemDM[#itemDM+1] = {
              itemDM = "vpnxj"
      ...
      elseif data.item == "deliverylist" then
        result = RPC.execute('desirerp-phone:purchaseDarkMarket',1,10)
          if result then
            itemDM[#itemDM+1] = {
        ...
        elseif data.item == "choplist" then
          result = RPC.execute('desirerp-phone:purchaseDarkMarket',2,20)
            if result then
              itemDM[#itemDM+1] = {
                itemDM = "choplist"
              }
      ...
      elseif data.item == "racingusb2" then
        result = RPC.execute('desirerp-phone:purchaseDarkMarket',2,50)
          if result then
            itemDM[#itemDM+1] = {
              itemDM = "racingusb2"
            }
      ...
      elseif data.item == "boostlaptop" then
        -- result = RPC.execute('desirerp-phone:purchaseDarkMarket',1,100)
        -- if result then
      ...
      elseif data.item == "laptopred" then
        result = RPC.execute('desirerp-phone:purchaseDarkMarket',1,65)
        if result then
          itemDM[#itemDM+1] = {
            itemDM = "heistlaptop4"
          }
      ...
      elseif data.item == "laptopgreen" then
        result = RPC.execute('desirerp-phone:purchaseDarkMarket',1,50)
          if result then
            itemDM[#itemDM+1] = {
              itemDM = "heistlaptop3"
            }
...
RegisterNetEvent('desirerp-phone:takeDMarket')
AddEventHandler('desirerp-phone:takeDMarket', function()
  if orderItem then
