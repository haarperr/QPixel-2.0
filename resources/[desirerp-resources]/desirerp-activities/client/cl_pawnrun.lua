	...
	while tasking do
		...
		if dstcheck < 2.0 and pedCreated then
			...
			if IsControlJustReleased(0,38) then
				...
				local finished = exports["desirerp-taskbar"]:taskBar(2000,"Making An Exchange")
...
Citizen.CreateThread(function()
	exports["desirerp-polyzone"]:AddBoxZone("rolex_run_start", vector3(182.79, -1319.47, 29.32), 2, 2, {
...
AddEventHandler('desirerp-polyzone:enter', function(name)
    if name == "rolex_run_start" then
		...
		if not RolexRun then
			exports["desirerp-interface"]:showInteraction("[E] Start Run ($500)")
		else 
			exports["desirerp-interface"]:showInteraction("Finish Your Run")
		end
    end
end)
...
function RolexLocation()
	Citizen.CreateThread(function()
        while RolexSpot do
            Citizen.Wait(5)
			if IsControlJustReleased(0, 38) then
				if exports['desirerp-inventory']:hasEnoughOfItem('rolexwatch', 1) or exports['desirerp-inventory']:hasEnoughOfItem('stolenpsp', 1) or exports['desirerp-inventory']:hasEnoughOfItem('stolens8', 1) or exports['desirerp-inventory']:hasEnoughOfItem('stolenBrokenGoods', 1) or exports['desirerp-inventory']:hasEnoughOfItem('stolen2ctchain', 1) or exports['desirerp-inventory']:hasEnoughOfItem('stolenraybans', 1) or exports['desirerp-inventory']:hasEnoughOfItem('stolencasiowatch', 1) then
					if not RolexRun then
...
function DoDropOff(requestMoney)
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
			...
			if RolexRun then
		        if exports["desirerp-inventory"]:hasEnoughOfItem("rolexwatch",1) then     
                TriggerEvent("inventory:removeItem","rolexwatch", 1) 
                if exports['desirerp-hud']:MoneyBuff() then
                    TriggerServerEvent('desirerp-activities:pawnPayout', math.random(200, 275), '[Pawn Run | With Buff] | Rolex')
                else
                    TriggerServerEvent('desirerp-activities:pawnPayout', math.random(150, 175), '[Pawn Run | Without Buff] | Rolex')
                end

				elseif exports["desirerp-inventory"]:hasEnoughOfItem("stolenpsp",1) then     
					TriggerEvent("inventory:removeItem","stolenpsp", 1)   
                    if exports['desirerp-hud']:MoneyBuff() then
                        TriggerServerEvent('desirerp-activities:pawnPayout', math.random(350, 400), '[Pawn Run | With Buff] | Stolen PSP')
                    else
                        TriggerServerEvent('desirerp-activities:pawnPayout', math.random(250, 300), '[Pawn Run | Without Buff] | Stolen PSP')
                    end           

				elseif exports["desirerp-inventory"]:hasEnoughOfItem("stolens8",1) then     
					TriggerEvent("inventory:removeItem","stolens8", 1)   
                    if exports['desirerp-hud']:MoneyBuff() then
                        TriggerServerEvent('desirerp-activities:pawnPayout', math.random(250, 275), '[Pawn Run | With Buff] | Stolen S8')
                    else
                        TriggerServerEvent('desirerp-activities:pawnPayout', math.random(175, 225), '[Pawn Run | Without Buff] | Stolen S8')
                    end           

				elseif exports["desirerp-inventory"]:hasEnoughOfItem("stolenBrokenGoods",1) then     
					TriggerEvent("inventory:removeItem","stolenBrokenGoods", 1)   
                    if exports['desirerp-hud']:MoneyBuff() then
                        TriggerServerEvent('desirerp-activities:pawnPayout', math.random(350, 425), '[Pawn Run | With Buff] | Broken Goods')
                    else
                        TriggerServerEvent('desirerp-activities:pawnPayout', math.random(250, 325), '[Pawn Run | Without Buff] | Broken Goods')
                    end                 

				elseif exports["desirerp-inventory"]:hasEnoughOfItem("stolen2ctchain",1) then     
					TriggerEvent("inventory:removeItem","stolen2ctchain", 1)   
					TriggerServerEvent('stolen2ctchain:cash:payment')     
                    if exports['desirerp-hud']:MoneyBuff() then
                        TriggerServerEvent('desirerp-activities:pawnPayout', math.random(200, 275), '[Pawn Run | With Buff] | Stolen 2CT Chain')
                    else
                        TriggerServerEvent('desirerp-activities:pawnPayout', math.random(150, 175), '[Pawn Run | Without Buff] | Stolen 2CT Chain')
                    end                   

				elseif exports["desirerp-inventory"]:hasEnoughOfItem("stolenraybans",1) then     
					TriggerEvent("inventory:removeItem","stolenraybans", 1)   
                    if exports['desirerp-hud']:MoneyBuff() then
                        TriggerServerEvent('desirerp-activities:pawnPayout', math.random(125, 200), '[Pawn Run | With Buff] | Stolen Raybans')
                    else
                        TriggerServerEvent('desirerp-activities:pawnPayout', math.random(50, 100), '[Pawn Run | Without Buff] | Stolen Raybans')
                    end

				elseif exports["desirerp-inventory"]:hasEnoughOfItem("stolencasiowatch",1) then     
					TriggerEvent("inventory:removeItem","stolencasiowatch", 1)   
                    if exports['desirerp-hud']:MoneyBuff() then
                        TriggerServerEvent('desirerp-activities:pawnPayout', math.random(150, 250), '[Pawn Run | With Buff] | Stolen Casio')
                    else
                        TriggerServerEvent('desirerp-activities:pawnPayout', math.random(100, 125), '[Pawn Run | Without Buff] | Stolen Casio')
                    end   
                    
                elseif exports["desirerp-inventory"]:hasEnoughOfItem("goldbar",1) then     
					TriggerEvent("inventory:removeItem","goldbar", 1)   
                    if exports['desirerp-hud']:MoneyBuff() then
                        TriggerServerEvent('desirerp-activities:pawnPayout', math.random(3000, 5000), '[Pawn Run | With Buff] | Gold Bar')
                    else
                        TriggerServerEvent('desirerp-activities:pawnPayout', math.random(1500, 2500), '[Pawn Run | Without Buff] | Gold Bar')
                    end   

                elseif exports["desirerp-inventory"]:hasEnoughOfItem("valuablegoods",1) then     
					TriggerEvent("inventory:removeItem","valuablegoods", 1)   
                    if exports['desirerp-hud']:MoneyBuff() then
                        TriggerServerEvent('desirerp-activities:pawnPayout', math.random(1500, 2500), '[Pawn Run | With Buff] | Valuable Goods')
                    else
                        TriggerServerEvent('desirerp-activities:pawnPayout', math.random(750, 1000), '[Pawn Run | Without Buff] | Valuable Goods')
                    end   

                elseif exports["desirerp-inventory"]:hasEnoughOfItem("goldcoin",1) then     
					TriggerEvent("inventory:removeItem","goldcoin", 1)   
                    if exports['desirerp-hud']:MoneyBuff() then
                        TriggerServerEvent('desirerp-activities:pawnPayout', math.random(1500, 2500), '[Pawn Run | With Buff] | Gold Coin')
                    else
                        TriggerServerEvent('desirerp-activities:pawnPayout', math.random(750, 1000), '[Pawn Run | Without Buff] | Gold Coin')
                    end   
