...
Citizen.CreateThread(function()
    exports["desirerp-polyzone"]:AddBoxZone("chop_zone", vector3(1359.1, -2097.81, 52.0), 15, 15, {
    heading=40,
    --debugPoly=true,
    minZ=51.0,
    maxZ=55.0
...
AddEventHandler("desirerp-polyzone:enter", function(zone)
    if zone == "chop_zone" and GetVehiclePedIsIn(PlayerPedId(), false) == aspectChopCar then
        ...
        aspectStartChopping()
        exports['desirerp-interface']:showInteraction('[E] Start Chopping')
    end
end)
...
function ChopVehicleTyre(vehicle, boneID, tyreIndex)
    ...
    local finished = exports["desirerp-taskbar"]:taskBar(10000,"Slicing Vehicle Tyre")
...
function ChopVehicleDoor(vehicle, boneID, doorIndex)
    ...
    FreezeEntityPosition(GetPlayerPed(-1),true)
    local finished = exports["desirerp-taskbar"]:taskBar(14000,"Messing With Door")
...
function ChopVehicleRemains(vehicle, boneID)
    ...
    FreezeEntityPosition(GetPlayerPed(-1),true)
    local finished = exports["desirerp-taskbar"]:taskBar(25000,"Seeing Whats Left ..")
...
AddEventHandler('aspect-chop:cooldown', function()
    ...
    if ClockedIntoChop then
        TriggerEvent('desirerp-phone:chop_shop_offer')
        CreateChopCar()
    end
end)
...
function ChopVehiclePart(vehicle)
    ...
    if success then
        TriggerServerEvent("desirerp-chopshop:vehicle_loot", bone.type)
...
RegisterNetEvent('desirerp-chopshop:clock-in')
AddEventHandler('desirerp-chopshop:clock-in', function()
    if CoolDownActive then
        TriggerEvent('DoLongHudText', 'You have a active cooldown!', 2)
    elseif ClockedIntoChop then
        TriggerEvent('desirerp-chopshop:clock-out')
    else
        ClockedIntoChop = true
        TriggerEvent('desirerp-phone:signInInfoChopshop')
        Citizen.Wait(math.random(60000, 120000))
        TriggerEvent('desirerp-phone:chop_shop_offer')
    end
end)

RegisterNetEvent('desirerp-chopshop:clock-out')
AddEventHandler('desirerp-chopshop:clock-out', function()
    if ClockedIntoChop then
