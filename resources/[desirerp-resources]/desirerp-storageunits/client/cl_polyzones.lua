...
Citizen.CreateThread(function()
    local storage_units = RPC.execute("desirerp-storageunits:get")
    for i = 1, #storage_units do
   
        exports["desirerp-polyzone"]:AddBoxZone(storage_units[i].storage_name, vector3(storage_units[i].coords.x, storage_units[i].coords.y, storage_units[i].coords.z), storage_units[i].coords.width, storage_units[i].coords.height, {
...
AddEventHandler('desirerp-polyzone:enter', function(name)
    for i = 1, #storage_units_data do
        if (name == storage_units_data[i].storage_name) then
            ...
            TriggerEvent('desirerp-textui:ShowUI', 'show', "[E] Open storage unit "..currently_visiting_unit) 
        end
    end
end)
...
AddEventHandler('desirerp-polyzone:exit', function(name)
    for i = 1, #storage_units_data do
        if (name == storage_units_data[i].storage_name) then
            near_storage_unit = false
            currently_visiting_unit = 0
            TriggerEvent('desirerp-textui:HideUI')
        end
    end
end)
