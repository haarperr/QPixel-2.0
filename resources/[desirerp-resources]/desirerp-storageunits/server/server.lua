RPC.register("desirerp-storageunits:get", function()
    return storage_units
end)


RegisterServerEvent("desirerp-storageunits:purchase", function(args)
    local pSrc = source
...
RPC.register("desirerp-storageunits:occupied", function(storage_unit_id)
    local pSrc = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(pSrc)
        ...
        if result[1] ~= nil then
            ...
            local menu_data = {
                ...
                },
                {
                    ...
                    params = {
                        event = "desirerp-storageunits:attempt:inventory",
                        args = {
                            id = storage_unit_id,
                        }
                    }
            ...
            
                {
                    ...
                    params = {
                        event = "desirerp-storageunits:attempt:access",
                        args = {
                            id = storage_unit_id,
            ...
            if tonumber(storage_owner) == char_id then
                TriggerClientEvent('desirerp-context:sendMenu', pSrc, menu_data)  
            else
                for i=1, #keys do
            ...
            local menu_data = {
                ...
                },
                {
                    ...
                    params = {
                        event = "desirerp-storageunits:attempt:purchase",
                        args = {
                            id = storage_unit_id,
                            price = 70000
                        }
...
RPC.register("desirerp-storageunits:give:access", function(storage_unit_id, given_cid)
    local pSrc = source
...
RPC.register("desirerp-storageunits:remove:access", function(storage_unit_id, given_cid, given_name)
    local pSrc = source
...
RegisterServerEvent("desirerp-storageunit:view", function(storage_unit_id)
    local pSrc = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(pSrc)
    exports["ghmattimysql"]:execute("SELECT `given_access` FROM storage_units WHERE id = @id", {['id'] = storage_unit_id}, function(result)
                            ...
                            TriggerClientEvent('desirerp-context:sendMenu', pSrc, {
                                {
                                    ...
                                    txt = "CID: " ..result[i].id,
                                    params = {
                                        event = "desirerp-storageunit:attempt:remove",
                                        args = {
                                            id = storage_unit_id,
...
RPC.register("desirerp-storageunits:attenpt:transfer", function(storage_unit_id, transfer_cid)
    local pSrc = source
    local user = exports["desirerp-base"]:getModule("Player"):GetUser(pSrc)
    local char_id = user:getCurrentCharacter().id
