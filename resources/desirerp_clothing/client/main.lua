local CLOTHES = {
	...
	face_features = {"Nose_Width","Nose_Peak_Hight","Nose_Peak_Lenght","Nose_Bone_High","Nose_Peak_Lowering","Nose_Bone_Twist","EyeBrown_High","EyeBrown_Forward","Cheeks_Bone_High","Cheeks_Bone_Width","Cheeks_Width","Eyes_Openning","Lips_Thickness","Jaw_Bone_Width","Jaw_Bone_Back_Lenght","Chdesirerp_Bone_Lowering","Chdesirerp_Bone_Lenght","Chdesirerp_Bone_Width","Chdesirerp_Hole","Neck_Thikness"},
	  ...
	  for i=1, #self.ClothingShops do
		...
		local shopPos = shop['coords']
			  ...
			  if not (shop['blip'] == false) then
				...
				--   exports['impartialrp']:createBlip({
	...
	Save = function(self, save)
	  ...
	  if save then
		...
		TriggerServerEvent("desirerp_clothing:insert_character_current", data)
  
		if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
			TriggerServerEvent("desirerp_clothing:insert_character_face", data)
			TriggerServerEvent("desirerp_clothing:set_tats", self.CurrentTats)
			regmodels = true
		end
	  else
	...
	ToggleNui = function(self, open, menu, save, updateStatus)
	  ...
	  --exports['desirerp_hud']:toggleNui(not self.isOpen))
	  ...
	  if canHandleNewPlayer then
		...
		--TriggerServerEvent('impartialrp:updateBucket', 0)
		DoScreenFadeOut(500)
	  end
	...
	SetDefault = function(self, isFirstSpawn, armor, stress, thirst, hunger)
	  ...
	  TriggerServerEvent('impartial:updateBucket', PlayerPedId() + 1)
	...
	SetClothes = function(self, data, doesExist, isFirstSpawn, armor, stress, thirst, hunger)  
	  ...
	  TriggerServerEvent("desirerp_clothing:fromclient:get_character_face", model)
	  TriggerServerEvent("desirerp_clothing:retrieve_tats")
	...
	CustomPed = function(self, sentType)
	  ...
	  if not sentType then 
		--exports['desirerp_notifications']:SendAlert('error', 'You did not specify a ped.', 5000)
	  ...
	  if canPass then
		...
		if not Info then 
			TriggerEvent('DoLongHudText', 'This custom ped does not exist.', 2)
		  	--exports['desirerp_notifications']:SendAlert('error', 'This custom ped does not exist.', 5000)
		  return 
		end
			...
			if Info['job'] == job['name'] then 
			  self.SetModelOC(self, sentType) 
				...
				TriggerEvent('DoLongHudText', 'You lack permission to use this ped.', 2)
			  	--exports['desirerp_notifications']:SendAlert('error', 'You lack permission to use this ped.', 5000)
			end
		  else
			local plyCid = exports['isPed']:isPed('cid')
		...
		--exports['desirerp_notifications']:SendAlert('error', 'Not near a valid wardrobe!', 5000)
  ...
  RegisterNetEvent("desirerp_clothing:setclothes", function(...)
	CLOTHES:SetClothes(...)
  end)

  RegisterNetEvent("desirerp_clothing:settattoos", function(playerTattoosList)
	CLOTHES:SetTattoos(playerTattoosList)
	CLOTHES:SetTats(CLOTHES:GetTats()) 
  end)
  
  RegisterNetEvent('desirerp_clothing:setpedfeatures', function(data)
	CLOTHES:SetPedFeatures(data)
  end)
  
  RegisterNetEvent('desirerp_clothing:outfits', function(pAction, pId, pName)
  
	  if pAction == 1 then 
	  TriggerServerEvent("desirerp_clothing:set_outfit", pId, pName, CLOTHES:GetCurrentPed()) -- Update/Add
	  elseif pAction == 2 then 
	  TriggerServerEvent("desirerp_clothing:remove_outfit", pId) -- Delete
	  elseif pAction == 3 then 
	  TriggerEvent('imp-sounds:PlayOnOne','Clothes1', 0.6) 
	  TriggerServerEvent("desirerp_clothing:get_outfit", pId) -- Set
	  elseif pAction == 4 then 
	  TriggerServerEvent("desirerp_clothing:rename_outfit", pId, pName, CLOTHES:GetCurrentPed()) -- Rename outfit
	  end
...
--   end, false)
  ...
  end) -- exports['desirerp_clothing']:isPedMale()
  
  AddEventHandler('desirerp_clothing:openMenu', function()
	CLOTHES:OpenMenu()
  end)
