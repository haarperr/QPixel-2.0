...
RegisterCommand("outfits", function(source, args, rawCommand)
    if exports['desirerp_clothing']:IsInsideClothing() then
        TriggerEvent('desirerp_clothing:outfits', 4)
    else
        TriggerEvent('DoLongHudText', "You are not near a wardrobe", 2)
    end
end, false)  

RegisterNetEvent('desirerp_clothing:outfits')
AddEventHandler('desirerp_clothing:outfits', function(pAction, pId, pName)
    if pAction == 1 then
        TriggerServerEvent("desirerp_clothing:set_outfit",pId, pName, GetCurrentPed())
    elseif pAction == 2 then
        TriggerServerEvent("desirerp_clothing:remove_outfit",pId)
    elseif pAction == 3 then 
        --TriggerEvent("hud:saveCurrentMeta")
        TriggerEvent('item:deleteClothesDna')
        TriggerEvent('InteractSound_CL:PlayOnOne','Clothes1', 0.6)
        TriggerServerEvent("desirerp_clothing:get_outfit", pId)
    else
        TriggerEvent("hud:saveCurrentMeta")
...
RegisterNetEvent('desirerp_clothing:ListOutfits')
AddEventHandler('desirerp_clothing:ListOutfits', function(skincheck)
    local menuData = {}
    local takenSlots = {}
...
RegisterNetEvent("desirerp_clothing:setclothes")
AddEventHandler("desirerp_clothing:setclothes", function(data,alreadyExist)
    player = PlayerPedId()
...
RegisterNetEvent("desirerp_clothing:setpedfeatures")
AddEventHandler("desirerp_clothing:setpedfeatures", function(data)
    player = PlayerPedId()
    if data == false then
...
RegisterInterfaceCallback("qpixel-ui:clothes:addOutfit", function(data, cb)
    ...
    print('HI')
    TriggerServerEvent("desirerp_clothing:set_outfit", outfitSlot, outfitName, GetCurrentPed())
end)