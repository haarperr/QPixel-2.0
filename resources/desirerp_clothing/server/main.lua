...
RegisterNetEvent("desirerp_clothing:insert_character_current")
...
RegisterNetEvent("desirerp_clothing:insert_character_face")
...
AddEventHandler("desirerp_clothing:fromclient:get_character_face",function(model)
	...
	exports.oxmysql:fetch("SELECT hairColor, headBlend, headOverlay, headStructure FROM character_face WHERE cid = :cid LIMIT 1", { cid = cid}, function(data)
			if data[1] ~= nil then
					...
					if model == 1885233650 or model == -1667301416 then TriggerClientEvent("desirerp_clothing:setpedfeatures", source, temp_data) end
			else
					TriggerClientEvent("desirerp_clothing:setpedfeatures", source, false)
			end
	end)
end)
			...
			if data[1] ~= nil then
					...
					exports.oxmysql:fetch("SELECT model FROM character_current WHERE cid = :cid LIMIT 1", { cid = cid}, function(data)
							...
							if model == 1885233650 or model == -1667301416 then
									TriggerClientEvent("desirerp_clothing:setpedfeatures", src, temp_data)
							end
					end)
			else
					TriggerClientEvent("desirerp_clothing:setpedfeatures", src, false)
			end
	end)
end)

RegisterNetEvent("desirerp_clothing:get_character_current")
AddEventHandler("desirerp_clothing:get_character_current",function(pSrc, cid, firstSpawn, kvpArmour, kvpStress, kvpThirst, kvpHunger)
	...
	exports.oxmysql:fetch("SELECT model, drawables, props, drawtextures, proptextures FROM character_current WHERE cid = :cid LIMIT 1", { cid = cid}, function(skin)
			if (skin ~= nil and skin[1] ~= nil) then
					...
					TriggerClientEvent("desirerp_clothing:setclothes", src, temp_data, 0, firstSpawn, kvpArmour, kvpStress, kvpThirst, kvpHunger)
			else
					TriggerClientEvent('desirerp_clothing:setclothes',src,{},nil, firstSpawn, kvpArmour, kvpStress, kvpThirst, kvpHunger)
			end
	end)
end)
...
AddEventHandler("desirerp_clothing:retrieve_tats",function(pSrc, cid)
	...
	exports.oxmysql:fetch("SELECT tattoos FROM playersTattoos WHERE identifier = :cid LIMIT 1", { cid = cid}, function(result)
		if #result == 0 then
			...
			TriggerClientEvent("desirerp_clothing:settattoos", src, {})
		elseif result[1]['tattoos'] ~= "null" then
			TriggerClientEvent("desirerp_clothing:settattoos", src, json.decode(result[1].tattoos))
		else
			TriggerClientEvent("desirerp_clothing:settattoos", src, {})
		end
	end)
...
AddEventHandler("clothing:checkIfNew", function(kvpArmour, kvpStress, kvpThirst, kvpHunger)
	...
	TriggerEvent('impartialrp:getplayerfromid', src, function(player)
		if player then
			...
			checkExistenceClothes(cid, function(exists)
				if exists then
					TriggerEvent("desirerp_clothing:get_character_current", src, player.cid, true, kvpArmour, kvpStress, kvpThirst, kvpHunger)
				else
					TriggerClientEvent('desirerp_clothing:setclothes',src,{},nil,true)
				end
			end)
		end
	end)
...
RegisterNetEvent('desirerp_clothing:chargePlayer')
AddEventHandler('desirerp_clothing:chargePlayer', function(price)
	...
	TriggerClientEvent('desirerp_notifications:client:SendAlert', source, { type = 'inform', text = 'You were charged $'..price..' for the items purchased.', length = 5000 })
end)